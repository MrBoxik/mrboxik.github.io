"use strict";

// Configure this with your deployed Cloudflare Worker URL (no token in client code).
const IMPROVE_UPLOAD_ENDPOINT = "https://broad-star-66c2.mrtnhliza.workers.dev/";
const IMPROVE_UPLOAD_TIMEOUT_MS = 45000;
const IMPROVE_UPLOAD_MAX_FILE_BYTES = 50 * 1024 * 1024;
const IMPROVE_UPLOAD_UNEXPECTED_MAX_FILE_BYTES = 50 * 1024 * 1024;
const IMPROVE_UPLOAD_UNEXPECTED_TOTAL_LIMIT_BYTES = 1024 * 1024 * 1024;
const IMPROVE_UPLOAD_UNEXPECTED_MAX_WHEN_TOTAL_EXCEEDED = 3;

const BASE_MAPS = [
  { code: "US_01", name: "Michigan" },
  { code: "US_02", name: "Alaska" },
  { code: "RU_02", name: "Taymyr" },
];

const SEASON_REGION_MAP = {
  1: { code: "RU_03", label: "Season 1: Search & Recover (Kola Peninsula)" },
  2: { code: "US_04", label: "Season 2: Explore & Expand (Yukon)" },
  3: { code: "US_03", label: "Season 3: Locate & Deliver (Wisconsin)" },
  4: { code: "RU_04", label: "Season 4: New Frontiers (Amur)" },
  5: { code: "RU_05", label: "Season 5: Build & Dispatch (Don)" },
  6: { code: "US_06", label: "Season 6: Haul & Hustle (Maine)" },
  7: { code: "US_07", label: "Season 7: Compete & Conquer (Tennessee)" },
  8: { code: "RU_08", label: "Season 8: Grand Harvest (Glades)" },
  9: { code: "US_09", label: "Season 9: Renew & Rebuild (Ontario)" },
  10: { code: "US_10", label: "Season 10: Fix & Connect (British Columbia)" },
  11: { code: "US_11", label: "Season 11: Lights & Cameras (Scandinavia)" },
  12: { code: "US_12", label: "Season 12: Public Energy (North Carolina)" },
  13: { code: "RU_13", label: "Season 13: Dig & Drill (Almaty)" },
  14: { code: "US_14", label: "Season 14: Reap & Sow (Austria)" },
  15: { code: "US_15", label: "Season 15: Oil & Dirt (Quebec)" },
  16: { code: "US_16", label: "Season 16: High Voltage (Washington)" },
  17: { code: "RU_17", label: "Season 17: Repair & Rescue (Zurdania)" },
  18: { code: "US_18", label: "Season 18: Patch & Power (Chiapas)" },
};

const SEASON_ID_MAP = Object.fromEntries(
  Object.entries(SEASON_REGION_MAP).map(([season, data]) => [Number(season), data.code]),
);

const REGION_LONG_NAME_MAP = (() => {
  const out = {};
  for (const map of BASE_MAPS) {
    out[map.code.toUpperCase()] = map.name;
  }
  for (const season of Object.keys(SEASON_REGION_MAP).map(Number).sort((a, b) => a - b)) {
    const entry = SEASON_REGION_MAP[season];
    if (!entry) {
      continue;
    }
    out[String(entry.code || "").toUpperCase()] = entry.label || entry.code;
  }
  out.TRIALS = "Trials";
  return out;
})();

const REGION_ORDER = Object.keys(REGION_LONG_NAME_MAP);

const RANK_XP_REQUIREMENTS = {
  1: 0,
  2: 700,
  3: 1700,
  4: 2900,
  5: 4100,
  6: 5400,
  7: 6900,
  8: 8500,
  9: 10100,
  10: 11800,
  11: 13700,
  12: 15700,
  13: 17800,
  14: 20100,
  15: 22500,
  16: 25000,
  17: 27500,
  18: 30100,
  19: 32700,
  20: 35500,
  21: 38300,
  22: 41300,
  23: 44300,
  24: 47500,
  25: 50700,
  26: 54100,
  27: 57500,
  28: 61100,
  29: 64900,
  30: 69000,
};

const MONEY_MIN = -2147483647;
const MONEY_MAX = 2147483647;
const SAVE_VERSION_EXPECTED = {
  objVersion: 9,
  birthVersion: 9,
  cfg_version: 1,
};
const DEFAULT_RECOVERY_PRICE = [0, 0, 2500, 5000, 8000, 5000, 2000];
const DEFAULT_FULL_REPAIR_PRICE = [0, 0, 1500, 2500, 5000, 2500, 1500];
const DEFAULT_SETTINGS_DICT = {
  ADDON_AVAILABILITY: 1,
  CONTEST_ATTEMPTS: 0,
  STARTING_MONEY: 0,
  REPAIR_POINTS_AMOUNT: 0,
  TRUCK_SELLING: 0,
  MAP_MARKER: 0,
  TRAILER_AVAILABILITY: 1,
  RECOVERY: 0,
  TIME_SETTINGS: 0,
  STARTING_RANK: 0,
  GARAGE_REPAIRE: 0,
  TYRE_AVAILABILITY: 1,
  REPAIR_POINTS_COST: 0,
  TRUCK_AVAILABILITY: 3,
  REGION_TRAVELLING: 0,
  VEHICLE_STORAGE: 0,
  LOADING: 0,
  FUEL_PRICE: 1,
  STARTING_RULES: 0,
  INTENAL_ADDON_AVAILABILITY: 1,
  TASKS_CONTESTS: 0,
  GARAGE_REFUEL: 0,
  TRAILER_STORE_AVAILBILITY: 0,
  DLC_VEHICLES: 1,
  TELEPORTATION: 0,
  CONTRACTS: 0,
  TRAILER_PRICING: 0,
  TRUCK_PRICING: 0,
  TRAILER_SELLING: 0,
  VEHICLE_DAMAGE: 0,
  ADDON_PRICING: 0,
  REGIONAL_REPAIR: 0,
};
const DEFAULT_DEPLOY_PRICE = { Region: 3500, Map: 1000 };
const DEFAULT_AUTOLOAD_PRICE = 150;
const EXTRA_SAFE_DEFAULTS = {
  upgradesGiverUnlocks: {
    level_us_18_01: {
      US_18_01_UPG_02: 2,
      US_18_01_UPG_01: 2,
      US_18_01_UPG_03: 2,
    },
    level_us_18_02: {
      US_18_02_UPG_01: 2,
      US_18_02_UPG_03: 2,
      US_18_02_UPG_02: 2,
    },
  },
  watchpointsUnlocks: {
    level_us_18_01: {
      US_18_01_WATCHTOWER_04: true,
      US_18_01_WATCHTOWER_05: true,
      US_18_01_WATCHTOWER_03: true,
      US_18_01_WATCHTOWER_02: true,
      US_18_01_WATCHTOWER_01: true,
    },
    level_us_18_02: {
      US_18_02_WATCHPOINT_05: true,
      US_18_02_WATCHPOINT_04: true,
      US_18_02_WATCHPOINT_03: true,
      US_18_02_WATCHPOINT_02: true,
      US_18_02_WATCHPOINT_01: true,
    },
  },
  discoveredTrucksDefaults: {
    level_us_18_01: { current: 0, all: 0 },
    level_us_18_02: { current: 0, all: 0 },
  },
};

function mergeDefaultMap(base, extras) {
  const out = {};
  const baseMap = base && typeof base === "object" && !Array.isArray(base) ? base : {};
  const extraMap = extras && typeof extras === "object" && !Array.isArray(extras) ? extras : {};
  for (const [key, value] of Object.entries(baseMap)) {
    out[key] = value && typeof value === "object" && !Array.isArray(value) ? { ...value } : value;
  }
  for (const [key, value] of Object.entries(extraMap)) {
    const existing = out[key] && typeof out[key] === "object" && !Array.isArray(out[key]) ? out[key] : {};
    out[key] = value && typeof value === "object" && !Array.isArray(value)
      ? { ...existing, ...value }
      : value;
  }
  return out;
}

const SAFE_DEFAULTS = (() => {
  const source =
    typeof window !== "undefined" && window.SR_EDITOR_DEFAULTS && typeof window.SR_EDITOR_DEFAULTS === "object"
      ? window.SR_EDITOR_DEFAULTS
      : {};
  const sourceUpgrades =
    source.upgradesGiverUnlocks && typeof source.upgradesGiverUnlocks === "object"
      ? source.upgradesGiverUnlocks
      : {};
  const sourceWatchpoints =
    source.watchpointsUnlocks && typeof source.watchpointsUnlocks === "object"
      ? source.watchpointsUnlocks
      : {};
  const sourceDiscoveries =
    source.discoveredTrucksDefaults && typeof source.discoveredTrucksDefaults === "object"
      ? source.discoveredTrucksDefaults
      : {};
  return {
    upgradesGiverUnlocks: mergeDefaultMap(sourceUpgrades, EXTRA_SAFE_DEFAULTS.upgradesGiverUnlocks),
    watchpointsUnlocks: mergeDefaultMap(sourceWatchpoints, EXTRA_SAFE_DEFAULTS.watchpointsUnlocks),
    discoveredTrucksDefaults: mergeDefaultMap(sourceDiscoveries, EXTRA_SAFE_DEFAULTS.discoveredTrucksDefaults),
  };
})();

const TRIALS_LIST = [
  ["Ride-on King", "TRIAL_01_01_SCOUTING_CNT"],
  ["Lost in wilderness", "TRIAL_01_02_TRUCK_TSK"],
  ["Snowbound Valley", "TRIAL_02_01_DELIVERING"],
  ["Zalukodes", "TRIAL_02_02_SEARCH_CNT"],
  ["Northern Thread", "TRIAL_03_01_SCOUTING_CNT"],
  ["Wolves' Bog", "TRIAL_03_03_SCOUTING_CNT"],
  ["The Slope", "TRIAL_04_02_TSK"],
  ["Escape from Tretyakov", "TRIAL_04_01_SCOUTING_CNT"],
  ["Aftermath", "TRIAL_05_01_TSK"],
  ["Tumannaya Pass", "TRIAL_03_02_DELIVERY_CNT"],
];

const PROS_ENTITLEMENTS = [
  ["ProsRegistrationReward", "Mammoth Ornament & Stickers"],
  ["ProsRoadcraftReward", "Voron-AE4380 skin + 3 stickers"],
];

const VISITED_LEVELS_DEFAULTS = [
  "level_ru_02_01_crop",
  "level_ru_02_02",
  "level_ru_02_03",
  "level_ru_02_04",
  "level_ru_03_01",
  "level_ru_03_02",
  "level_ru_04_01",
  "level_ru_04_02",
  "level_ru_04_03",
  "level_ru_04_04",
  "level_ru_05_01",
  "level_ru_05_02",
  "level_ru_08_01",
  "level_ru_08_02",
  "level_ru_08_03",
  "level_ru_08_04",
  "level_ru_13_01",
  "level_ru_17_01",
  "level_ru_17_02",
  "level_us_01_01",
  "level_us_01_02",
  "level_us_01_03",
  "level_us_01_04_new",
  "level_us_02_01",
  "level_us_02_02_new",
  "level_us_02_03_new",
  "level_us_02_04_new",
  "level_us_03_01",
  "level_us_03_02",
  "level_us_04_01",
  "level_us_04_02",
  "level_us_06_01",
  "level_us_06_02",
  "level_us_07_01",
  "level_us_09_01",
  "level_us_09_02",
  "level_us_10_01",
  "level_us_10_02",
  "level_us_11_01",
  "level_us_11_02",
  "level_us_12_01",
  "level_us_12_02",
  "level_us_12_03",
  "level_us_12_04",
  "level_us_14_01",
  "level_us_14_02",
  "level_us_15_01",
  "level_us_15_02",
  "level_us_16_01",
  "level_us_16_02",
  "level_us_16_03",
  "level_us_18_01",
  "level_us_18_02",
];

const LEVEL_GARAGE_STATUSES_DEFAULTS = {
  level_us_12_02: 1,
  level_ru_03_01: 2,
  level_ru_04_01: 2,
  level_ru_08_03: 1,
  level_us_04_01: 2,
  level_us_03_01: 2,
  level_ru_05_01: 2,
  level_us_02_01: 2,
  level_ru_02_04: 0,
  level_us_01_02: 1,
  level_us_11_02: 0,
  level_us_14_01: 2,
  level_ru_03_02: 1,
  level_us_09_02: 0,
  level_ru_02_01_crop: 0,
  level_ru_02_02: 2,
  level_ru_17_01: 2,
  level_us_01_01: 2,
  level_ru_08_04: 1,
  level_us_15_01: 2,
  level_us_02_03_new: 1,
  level_us_01_03: 0,
  level_us_12_03: 1,
  level_us_14_02: 0,
  level_us_16_01: 0,
  level_ru_08_02: 0,
  level_us_16_03: 0,
  level_ru_05_02: 0,
  level_us_12_04: 0,
  level_us_10_01: 2,
  level_ru_17_02: 0,
  level_us_06_01: 2,
  level_us_02_02_new: 0,
  level_us_04_02: 1,
  level_us_16_02: 2,
  level_us_10_02: 1,
  level_us_01_04_new: 0,
  level_us_06_02: 0,
  level_ru_04_04: 1,
  level_us_02_04_new: 0,
  level_ru_02_03: 1,
  level_ru_04_02: 1,
  level_us_03_02: 1,
  level_us_15_02: 0,
  level_us_11_01: 2,
  level_ru_04_03: 0,
  level_ru_08_01: 2,
  level_us_07_01: 2,
  level_ru_13_01: 2,
  level_us_12_01: 2,
  level_us_09_01: 2,
  level_us_18_01: 1,
  level_us_18_02: 0,
};

const REGION_LEVELS = (() => {
  const out = {};
  for (const levelId of VISITED_LEVELS_DEFAULTS) {
    const m = /^level_([a-z]{2}_\d{2})/i.exec(levelId);
    if (!m) {
      continue;
    }
    const code = m[1].toUpperCase();
    if (!out[code]) {
      out[code] = [];
    }
    out[code].push(levelId);
  }
  return out;
})();

const RULE_RANDOM_LABEL = "random";
const RULE_RANDOM_VALUE = "__RULE_RANDOM__";

const RULE_DEFINITIONS = [
  {
    label: "Game difficulty",
    key: "gameDifficultyMode",
    options: [
      { label: "Normal", value: 0 },
      { label: "Hard", value: 1 },
      { label: "New Game+", value: 2 },
    ],
  },
  {
    label: "Truck availability",
    key: "truckAvailability",
    options: [
      { label: "default", value: 1 },
      { label: "all trucks available from start", value: 0 },
      { label: "5-15 trucks in each garage", value: 3 },
      { label: "store unlocks at rank 10", value: 2 },
      { label: "store unlocks at rank 20", value: 2 },
      { label: "store unlocks at rank 30", value: 2 },
      { label: "store is locked", value: 4 },
    ],
  },
  {
    label: "Truck pricing",
    key: "truckPricingFactor",
    options: [
      { label: "default", value: 1 },
      { label: "free", value: 0 },
      { label: "2x", value: 2 },
      { label: "4x", value: 4 },
      { label: "6x", value: 6 },
    ],
  },
  {
    label: "Truck selling price",
    key: "truckSellingFactor",
    options: [
      { label: "normal price", value: 1 },
      { label: "50%", value: 0.5 },
      { label: "30%", value: 0.3 },
      { label: "10%", value: 0.1 },
      { label: "cant be sold", value: -1 },
    ],
  },
  {
    label: "DLC vehicles availability",
    key: "needToAddDlcTrucks",
    options: [
      { label: "available", value: true },
      { label: "unavailable", value: false },
    ],
  },
  {
    label: "Vehicle storage slots",
    key: "vehicleStorageSlots",
    options: [
      { label: "default", value: 0 },
      { label: "only 3", value: 3 },
      { label: "only 5", value: 5 },
      { label: "only 10", value: 10 },
      { label: "only scouts", value: -1 },
    ],
  },
  {
    label: "External addon availability",
    key: "externalAddonAvailability",
    options: [
      { label: "default", value: 0 },
      { label: "all addons unlocked", value: 1 },
    ],
  },
  {
    label: "Internal addon availability",
    key: "internalAddonAvailability",
    options: [
      { label: "default", value: 0 },
      { label: "all internal addons unlocked", value: 1 },
      { label: "10-50 per garage", value: 2 },
      { label: "30-100 per garage", value: 2 },
      { label: "50-150 per garage", value: 2 },
      { label: "0-100 per garage", value: 2 },
    ],
  },
  {
    label: "Tire availability",
    key: "tyreAvailability",
    options: [
      { label: "default", value: 1 },
      { label: "all tires available", value: 0 },
      { label: "highway and allroad", value: 2 },
      { label: "highway, allroad, offroad", value: 3 },
      { label: "no mud tires", value: 4 },
      { label: "no chained tires", value: 5 },
    ],
  },
  {
    label: "Vehicle addon pricing",
    key: "addonPricingFactor",
    options: [
      { label: "default", value: 1 },
      { label: "free", value: 0 },
      { label: "2x", value: 2 },
      { label: "4x", value: 4 },
      { label: "6x", value: 6 },
    ],
  },
  {
    label: "Addon selling price",
    key: "addonSellingFactor",
    options: [
      { label: "normal", value: 1.0 },
      { label: "10%", value: 0.1 },
      { label: "30%", value: 0.3 },
      { label: "50%", value: 0.5 },
      { label: "no refunds", value: 0 },
    ],
  },
  {
    label: "Trailer store availability",
    key: "trailerStoreAviability",
    options: [
      { label: "default", value: 0 },
      { label: "one random store per region", value: 1 },
    ],
  },
  {
    label: "Trailer availability",
    key: "trailerAvailability",
    options: [
      { label: "default", value: 0 },
      { label: "all trailers available", value: 1 },
    ],
  },
  {
    label: "Trailer pricing",
    key: "trailerPricingFactor",
    options: [
      { label: "normal price", value: 1 },
      { label: "free", value: 0 },
      { label: "2x", value: 2 },
      { label: "4x", value: 4 },
      { label: "6x", value: 6 },
    ],
  },
  {
    label: "Trailer selling price",
    key: "trailerSellingFactor",
    options: [
      { label: "normal price", value: 1 },
      { label: "50%", value: 0.5 },
      { label: "30%", value: 0.3 },
      { label: "10%", value: 0.1 },
      { label: "cant be sold", value: -1 },
    ],
  },
  {
    label: "Fuel price",
    key: "fuelPriceFactor",
    options: [
      { label: "default", value: 1 },
      { label: "free", value: 0 },
      { label: "2x", value: 2 },
      { label: "4x", value: 4 },
      { label: "6x", value: 6 },
    ],
  },
  {
    label: "Garage repair price",
    key: "garageRepairePriceFactor",
    options: [
      { label: "default", value: 0 },
      { label: "paid", value: 1 },
      { label: "2x", value: 2 },
      { label: "4x", value: 4 },
      { label: "6x", value: 6 },
    ],
  },
  {
    label: "Garage refuelling",
    key: "isGarageRefuelAvailable",
    options: [
      { label: "available", value: true },
      { label: "unavailable", value: false },
    ],
  },
  {
    label: "Repair points cost",
    key: "repairPointsCostFactor",
    options: [
      { label: "default", value: 1 },
      { label: "paid", value: 1 },
      { label: "2x", value: 2 },
      { label: "4x", value: 4 },
      { label: "6x", value: 6 },
    ],
  },
  {
    label: "Repair points required",
    key: "repairPointsRequiredFactor",
    options: [
      { label: "default", value: 1 },
      { label: "2x more effective", value: 0.5 },
      { label: "2x", value: 2 },
      { label: "4x", value: 4 },
      { label: "6x", value: 6 },
    ],
  },
  {
    label: "Vehicle repair regional rules",
    key: "regionRepaireMoneyFactor",
    options: [
      { label: "default", value: 1 },
      { label: "2x outside home region", value: 2 },
      { label: "3x outside home region", value: 3 },
      { label: "4x outside home region", value: 4 },
    ],
  },
  {
    label: "Vehicle damage",
    key: "vehicleDamageFactor",
    options: [
      { label: "default", value: 1 },
      { label: "no damage", value: 0 },
      { label: "2x", value: 2 },
      { label: "3x", value: 3 },
      { label: "5x", value: 5 },
    ],
  },
  {
    label: "Recovery price",
    key: "recoveryPriceFactor",
    options: [
      { label: "default", value: 0 },
      { label: "paid", value: 1 },
      { label: "2x", value: 2 },
      { label: "4x", value: 4 },
      { label: "6x", value: 6 },
      { label: "unavailable", value: -1 },
    ],
  },
  {
    label: "Automatic cargo loading",
    key: "loadingPriceFactor",
    options: [
      { label: "default", value: 0 },
      { label: "paid", value: 1 },
      { label: "2x", value: 2 },
      { label: "4x", value: 4 },
      { label: "6x", value: 6 },
    ],
  },
  {
    label: "Truck switching price (minimap)",
    key: "teleportationPrice",
    options: [
      { label: "free", value: 0 },
      { label: "500", value: 500 },
      { label: "1000", value: 1000 },
      { label: "2000", value: 2000 },
      { label: "5000", value: 5000 },
    ],
  },
  {
    label: "Region traveling price",
    key: "regionTravellingPriceFactor",
    options: [
      { label: "default", value: 0 },
      { label: "paid", value: 1 },
      { label: "2x", value: 2 },
      { label: "4x", value: 4 },
      { label: "6x", value: 6 },
    ],
  },
  {
    label: "Task and contest payouts",
    key: "tasksAndContestsPayoutsFactor",
    options: [
      { label: "normal", value: 1 },
      { label: "50%", value: 0.5 },
      { label: "150%", value: 1.5 },
      { label: "200%", value: 2 },
      { label: "300%", value: 3 },
    ],
  },
  {
    label: "Contracts payouts",
    key: "contractsPayoutsFactor",
    options: [
      { label: "normal", value: 1 },
      { label: "50%", value: 0.5 },
      { label: "150%", value: 1.5 },
      { label: "200%", value: 2 },
      { label: "300%", value: 3 },
    ],
  },
  {
    label: "Max contest attempts",
    key: "maxContestAttempts",
    options: [
      { label: "default", value: 0 },
      { label: "1 attempt", value: 1 },
      { label: "3 attempts", value: 3 },
      { label: "5 attempts", value: 5 },
      { label: "gold time only", value: -1 },
    ],
  },
  {
    label: "Map marker style",
    key: "isMapMarkerAsInHardMode",
    options: [
      { label: "default", value: false },
      { label: "hard mode", value: true },
    ],
  },
];

const INTERNAL_ADDON_AMOUNT_BY_LABEL = {
  "10-50 per garage": 30,
  "30-100 per garage": 65,
  "50-150 per garage": 100,
  "0-100 per garage": 50,
};

const RULE_NGP_DICT_META = {
  truckAvailability: {
    ngpKey: "TRUCK_AVAILABILITY",
    labelToState: {
      default: 0,
      "all trucks available from start": 1,
      "5-15 trucks in each garage": 2,
      "store unlocks at rank 10": 3,
      "store unlocks at rank 20": 4,
      "store unlocks at rank 30": 5,
      "store is locked": 6,
    },
  },
  truckPricingFactor: { ngpKey: "TRUCK_PRICING", labelToState: { default: 0, free: 1, "2x": 2, "4x": 3, "6x": 4 } },
  truckSellingFactor: { ngpKey: "TRUCK_SELLING", labelToState: { "normal price": 0, "50%": 1, "30%": 2, "10%": 3, "cant be sold": 4 } },
  needToAddDlcTrucks: { ngpKey: "DLC_VEHICLES", labelToState: { available: 0, unavailable: 1 } },
  vehicleStorageSlots: { ngpKey: "VEHICLE_STORAGE", labelToState: { default: 0, "only 3": 1, "only 5": 2, "only 10": 3, "only scouts": 4 } },
  externalAddonAvailability: { ngpKey: "ADDON_AVAILABILITY", labelToState: { default: 0, "all addons unlocked": 1 } },
  internalAddonAvailability: {
    ngpKey: "INTENAL_ADDON_AVAILABILITY",
    labelToState: {
      default: 0,
      "all internal addons unlocked": 1,
      "10-50 per garage": 2,
      "30-100 per garage": 3,
      "50-150 per garage": 4,
      "0-100 per garage": 5,
    },
  },
  tyreAvailability: {
    ngpKey: "TYRE_AVAILABILITY",
    labelToState: {
      default: 0,
      "all tires available": 1,
      "highway and allroad": 2,
      "highway, allroad, offroad": 3,
      "no mud tires": 4,
      "no chained tires": 5,
    },
  },
  trailerStoreAviability: { ngpKey: "TRAILER_STORE_AVAILBILITY", labelToState: { default: 0, "one random store per region": 1 } },
  trailerAvailability: { ngpKey: "TRAILER_AVAILABILITY", labelToState: { default: 0, "all trailers available": 1 } },
  trailerPricingFactor: { ngpKey: "TRAILER_PRICING", labelToState: { "normal price": 0, free: 1, "2x": 2, "4x": 3, "6x": 4 } },
  trailerSellingFactor: { ngpKey: "TRAILER_SELLING", labelToState: { "normal price": 0, "50%": 1, "30%": 2, "10%": 3, "cant be sold": 4 } },
  fuelPriceFactor: { ngpKey: "FUEL_PRICE", labelToState: { default: 0, free: 1, "2x": 2, "4x": 3, "6x": 4 } },
  garageRepairePriceFactor: { ngpKey: "GARAGE_REPAIRE", labelToState: { default: 0, paid: 1, "2x": 2, "4x": 3, "6x": 4 } },
  isGarageRefuelAvailable: { ngpKey: "GARAGE_REFUEL", labelToState: { available: 0, unavailable: 1 } },
  repairPointsCostFactor: { ngpKey: "REPAIR_POINTS_COST", labelToState: { default: 0, paid: 1, "2x": 2, "4x": 3, "6x": 4 } },
  repairPointsRequiredFactor: { ngpKey: "REPAIR_POINTS_AMOUNT", labelToState: { default: 0, "2x more effective": 1, "2x": 2, "4x": 3, "6x": 4 } },
  regionRepaireMoneyFactor: {
    ngpKey: "REGIONAL_REPAIR",
    labelToState: {
      default: 0,
      "2x outside home region": 1,
      "3x outside home region": 2,
      "4x outside home region": 3,
    },
  },
  vehicleDamageFactor: { ngpKey: "VEHICLE_DAMAGE", labelToState: { default: 0, "no damage": 1, "2x": 2, "3x": 3, "5x": 4 } },
  recoveryPriceFactor: { ngpKey: "RECOVERY", labelToState: { default: 0, paid: 1, "2x": 2, "4x": 3, "6x": 4, unavailable: 5 } },
  loadingPriceFactor: { ngpKey: "LOADING", labelToState: { default: 0, paid: 1, "2x": 2, "4x": 3, "6x": 4 } },
  teleportationPrice: { ngpKey: "TELEPORTATION", labelToState: { free: 0, "500": 1, "1000": 2, "2000": 3, "5000": 4 } },
  regionTravellingPriceFactor: { ngpKey: "REGION_TRAVELLING", labelToState: { default: 0, paid: 1, "2x": 2, "4x": 3, "6x": 4 } },
  tasksAndContestsPayoutsFactor: { ngpKey: "TASKS_CONTESTS", labelToState: { normal: 0, "50%": 1, "150%": 2, "200%": 3, "300%": 4 } },
  contractsPayoutsFactor: { ngpKey: "CONTRACTS", labelToState: { normal: 0, "50%": 1, "150%": 2, "200%": 3, "300%": 4 } },
  maxContestAttempts: { ngpKey: "CONTEST_ATTEMPTS", labelToState: { default: 0, "1 attempt": 1, "3 attempts": 2, "5 attempts": 3, "gold time only": 4 } },
  isMapMarkerAsInHardMode: { ngpKey: "MAP_MARKER", labelToState: { default: 0, "hard mode": 1 } },
  addonPricingFactor: { ngpKey: "ADDON_PRICING", labelToState: { default: 0, free: 1, "2x": 2, "4x": 3, "6x": 4 } },
};

const OBJECTIVES_CATALOG_CSV_URL = "./assets/objectives_catalog.csv";
const OBJECTIVES_CATALOG_SCRIPT_URL = "./assets/objectives_catalog.js?v=20260630-1";
const EMBEDDED_OBJECTIVES_CSV = "key,displayName,category,region,region_name,type,cargo_needed,experience,money,descriptionText,Source\nUS_01_01_DROWNED_TRUCK_01_TSK,Drowned Highway Truck,_TASKS,US_01,Michigan,truckDelivery,,190,1250,Hey! Trouble at the farm. We've got trucks wedged deep in sludge and we could use a strong arm to haul 'em out. You'll need a serious engine for this one.,TASKS\nUS_01_01_DROWNED_TRUCK_02_TSK,Drowned Scout Truck,_TASKS,US_01,Michigan,truckDelivery,,180,1100,Hey! Trouble at the farm. We've got trucks wedged deep in sludge and we could use a strong arm to haul 'em out. You'll need a serious engine for this one.,TASKS\nUS_01_01_DROWNED_TRUCK_03_TSK,Drowned Heavy Truck,_TASKS,US_01,Michigan,truckDelivery,,320,3000,Hey! Trouble at the farm. We've got trucks wedged deep in sludge and we could use a strong arm to haul 'em out. You'll need a serious engine for this one.,TASKS\nUS_01_01_DRILLING_RECOVERY_OBJ,Not A Drill,_CONTRACTS,US_01,Michigan,truckDelivery,2\u00d7 Metal Planks; 1\u00d7 Big Drills,740,7450,Excuse the mess. We're still clearing up after the flood. The water dislodged our rigs and destroyed one of the drills. We need new parts and construction materials to get things moving again.,CONTRACTS\nUS_01_01_FACTORY_RECOVERY_OBJ,Pipe Dream,_CONTRACTS,US_01,Michigan,truckDelivery,2\u00d7 Bricks; 1\u00d7 Metal Planks,450,3850,A local oil equipment factory got hammered by the flood. That place is vital to drilling operations in the area so we need to get it back up and running ASAP. Start by getting them some fuel and construction materials.,CONTRACTS\nUS_01_01_TOWN_STORAGE_OBJ,Steel River Supplies,_CONTRACTS,US_01,Michigan,truckDelivery,2\u00d7 Crate Large; 2\u00d7 Wooden Planks,260,2800,\"Things are pretty rough around here right now. We've got food, water and fuel shortages and people are starting to worry. If we don't deal with this soon we're going to have panic on our hands. We need your help.\",CONTRACTS\nUS_01_01_FARM_DELIVERY_OBJ,Farming Tools,_CONTRACTS,US_01,Michigan,truckDelivery,,130,1050,\"A local farm by the name of \"\"Matthew & Sons\"\" has contacted us to see if we can help them out with a shipment of tools for repairs. You should be able to find the shipment somewhere near the garage.\",CONTRACTS\nUS_01_01_SUPPLIES_FOR_FARMERS_OBJ,Wet Harvest,_CONTRACTS,US_01,Michigan,cargoDelivery,2\u00d7 Bricks; 2\u00d7 Wooden Planks; 1\u00d7 Metal Planks,420,3800,A nearby farm was hit pretty bad by the flood. They could really use some basic construction materials to get themselves patched up. We don't want to end up with food shortages.,CONTRACTS\nUS_01_01_BUILD_A_BRIDGE_OBJ,Old Bridge Reconstruction,_CONTRACTS,US_01,Michigan,cargoDelivery,1\u00d7 Metal Planks; 2\u00d7 Wooden Planks,350,2100,Great! Now you have a powerful vehicle for transporting heavier loads. The only problem is that you're cut off from the mainland. Rebuild the bridge so you can reach the garage and get a real contract.,CONTRACTS\nUS_01_01_LOST_CONTAINERS_OBJ,Lost Container,_CONTRACTS,US_01,Michigan,cargoDelivery,1\u00d7 Service Spare Parts Special,210,1300,Listen up. We've got a missing container that's carrying operational supplies and we're struggling to get by without it. It can't have gotten far. Get out there and find it and we'll be mighty grateful.,CONTRACTS\nUS_01_01_EXPLORE_GARAGE_OBJ,Get To The Garage,_CONTRACTS,US_01,Michigan,exploration,,200,850,The old and rusty vehicle garage has seen things. It was abandoned at the beginning of the flood and may be used as a base of local operations now.,CONTRACTS\nUS_01_01_EXPLORING_TRUCK_OBJ,First Truck,_CONTRACTS,US_01,Michigan,exploration,,200,650,\"There should be an old truck nearby the local Lumbermill. Was left over by the logistical company used to operate here. Rusty, but still capable. Discover its location and use it to handle the bridge repairs.\",CONTRACTS\nUS_01_01_EXPLORING_WATCHTOWER_OBJ,The Watchtower,_CONTRACTS,US_01,Michigan,exploration,,100,250,\"Welcome to Steel River Township! There's plenty of work around here, but you'll need a real truck first. There's a new ride with your name on it in a parking lot nearby. First, get to the watchtower to explore the area.\",CONTRACTS\nUS_01_01_MISSED_OILTANK_TSK,Missing Oil Tank,_TASKS,US_01,Michigan,truckDelivery,,280,2400,\"Got a spot of trouble here, pal. We have an oil tank trailer that's missing. Pretty sure it got washed off the road somewhere and is probably stuck in a swamp by now. How 'bout lending a hand?\",TASKS\nUS_01_01_MOTEL_NEEDS_TSK,Motel Woes,_TASKS,US_01,Michigan,truckDelivery,,210,1850,I know this town ain't exactly a tourist trap but we do what we can. The local motel is in bad shape and it's down to us to help 'em out. We've got a shipment of equipment for them. Can you deliver it?,TASKS\nUS_01_01_STUCK_TRAILER_TSK,Stuck Trailer,_TASKS,US_01,Michigan,truckDelivery,,240,1600,\"It looks like the first wave of relief workers that rolled up here were a bunch of idiots. They managed to lose a whole trailer! Go find it and bring it back, will you?\",TASKS\nUS_01_01_LOST_CARGO_TSK,Missing Machinery,_TASKS,US_01,Michigan,cargoDelivery,4\u00d7 Service Spare Parts Special,280,3200,A shipment of important machinery to a local factory was... interrupted... by an unfortunate incident with the previous driver. The responsibility is all yours now. I'd take a crane if I were you.,TASKS\nUS_01_01_MOUNTAIN_BRIDGE_TSK,Mountain Bridge,_TASKS,US_01,Michigan,cargoDelivery,1\u00d7 Metal Planks; 2\u00d7 Blocks,260,1850,These rockslides are becoming the bane of my life. We need to create alternate routes around the area to avoid the slides or operations are going to grind to a halt. A mountain bridge would solve this.,TASKS\nUS_01_01_ROAD_BLOCKAGE_TSK,Road Block,_TASKS,US_01,Michigan,cargoDelivery,2\u00d7 Service Spare Parts,170,1650,\"Rocks? Again? Seriously? Come on! Do me a favour and handle this, will you?\",TASKS\nUS_01_01_BOATMAN_TOOLS_DELIVERY_TSK,Riverside Repair,_TASKS,US_01,Michigan,cargoDelivery,2\u00d7 Service Spare Parts; 2\u00d7 Wooden Planks,180,1600,Steel River here. Our river fleet was badly damaged and the only person who can help us is all out of equipment. Can you supply our guy with the tools he needs to patch us up?,TASKS\nUS_01_01_LANDSLIDE_TSK,Rockslide,_TASKS,US_01,Michigan,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Service Spare Parts,180,1600,We've got a rockslide problem. They keep knocking out our infrastructure but we don't have the resources to stop them. We'd sure be grateful if you could take a look.,TASKS\nUS_01_01_FALLEN_POWER_LINES_TSK,Fallen Powerline,_TASKS,US_01,Michigan,cargoDelivery,1\u00d7 Metal Planks; 2\u00d7 Blocks,200,1550,We can't get a darn thing done round here! The power lines are down and all we get from the town are excuses! Well? What are you waiting for? Do something about it!,TASKS\nUS_01_01_WOODEN_BRIDGE_TSK,Wooden Bridge,_TASKS,US_01,Michigan,cargoDelivery,2\u00d7 Wooden Planks,120,1400,\"The river is swollen and our vehicles are having a nightmare trying to cross to the drilling site. We need a simple, robust bridge to stop our trucks getting waterlogged. Get to it.\",TASKS\nUS_01_01_SWAMP_EXPLORATION_TSK,Drainage,_TASKS,US_01,Michigan,exploration,,180,3050,It's time to drain the swamps. We installed some simple pumps out here in the 70's. They may be a bit old and rusty but they should be functional. See if you can find them.,TASKS\nUS_01_01_KING_OF_HILLS_TSK,King Of The Hill,_TASKS,US_01,Michigan,exploration,,160,2800,You sure have been busy lately! Why not take some time out to enjoy the local country. I've marked a couple of locations on your map that offer fantastic views. Be sure to check them out.,TASKS\nUS_01_01_LOCAL_ENTERTAINMENT_TSK,Local Entertainment,_TASKS,US_01,Michigan,exploration,,130,2200,\"The workers in a nearby factory have a little game. There are multiple barrels hidden in some hard to reach places and you have to knock 'em down. So far, no one has managed all of them. Think you can do it?\",TASKS\nUS_01_01_THE_PLACE_BEYOND_THE_SPRUCES_TSK,The Place Beyond The Spruces,_TASKS,US_01,Michigan,exploration,,60,1050,\"There's a place in the mountains, not far away, that has the most stunning views. I like to relax up there from time to time. Why don't you take a look? Just try not to destroy everything getting up there.\",TASKS\nUS_01_01_FOOD_DELIVERY_CNT,Food Delivery,_CONTESTS,US_01,Michigan,cargoDelivery,2\u00d7 Crate Large,100,1050,Dyson Diesel's regular food shipment has been delayed for days. If we don't get some grub over to those oilmen soon the rumbling in their bellies will be louder than the drilling!,CONTESTS\nUS_01_01_WOODEN_ORDER_CNT,Pinewood Express,_CONTESTS,US_01,Michigan,cargoDelivery,2\u00d7 Wooden Planks; 1\u00d7 Wooden Planks; 1\u00d7 Wooden Planks,110,950,The local farms are in desperate need of fresh wood to repair their damaged structures. The requests are constant. Can you get in there and help. They pay well.,CONTESTS\nUS_01_01_METEO_DATA_CNT,Meteorology Data,_CONTESTS,US_01,Michigan,exploration,,70,1100,Keeping up with this darn river is driving us crazy. We need advanced warning so we can be ready for a spill. There's a couple meteorology sensors nearby. Real tough to reach. You think you can make it?,CONTESTS\nUS_01_02_CLEAR_ROCKS_01_TSK,The Long Way,_TASKS,US_01,Michigan,cargoDelivery,2\u00d7 Service Spare Parts,160,1500,We've got a couple of nasty rockslides clogging up the river. Husky Forwarding won't take responsibility because it's in the river and not on the road. We could really use your help with this.,TASKS\nUS_01_02_RESOURCES_FOR_WINTER_OBJ,Winter Stores,_CONTRACTS,US_01,Michigan,truckDelivery,2\u00d7 Vehicles Spare Parts,420,5000,Winter's closing in. Soon it'll be impossible to get supplies into the base. The rig'll keep drilling no matter what but a fat lot of good it'll do if we run out of food and equipment. Help us out?,CONTRACTS\nUS_01_02_MATERIALS_ORDER_OBJ,Materials Order,_CONTRACTS,US_01,Michigan,cargoDelivery,2\u00d7 Concrete Slab; 2\u00d7 Crate Large; 1\u00d7 Metal Planks,730,5000,Husky Forwarding here. We just got a massive material request from several local facilities in the Smithville Dam area. They're working round the clock to try and recover from the flood and could really use a hand.,CONTRACTS\nUS_01_02_FARM_ORDER_OBJ,The Essentials,_CONTRACTS,US_01,Michigan,cargoDelivery,2\u00d7 Bricks; 2\u00d7 Wooden Planks; 2\u00d7 Barrels,440,4100,The local farm in Smithville Dam area is in dire shape. They need basic materials for repairs plus some fuel to power their generators. The road leading up there is completely destroyed so proceed with caution.,CONTRACTS\nUS_01_02_WORK_FOR_OLD_SWEAT_OBJ,Work For Old Sweat,_CONTRACTS,US_01,Michigan,cargoDelivery,1\u00d7 Concrete Slab; 2\u00d7 Service Spare Parts,380,3450,The local Quarry is still having trouble. The main road that connects them to the town is completely shot and we can't get vehicles in there. Get them what they need and we'll be sure to reward you.,CONTRACTS\nUS_01_02_FUEL_ORDER_OBJ,Fuel Order,_CONTRACTS,US_01,Michigan,cargoDelivery,2\u00d7 Container Small,280,2900,\"Dyson Diesel here. Now that the quarry is back in operation, we need to clear out all the old equipment and move it to the warehouse. Right now, we barely have enough room to move. Please make this a priority.\",CONTRACTS\nUS_01_02_BRICKS_DELIVERY_TSK,Tools Delivery,_TASKS,US_01,Michigan,truckDelivery,,270,2450,One of our warehouses needs to optimise their loading hub. Right now they lack the tools and equipment to do it. Get 'em what they need and we'll make sure you're compensated for your trouble.,TASKS\nUS_01_02_LOST_TRAILER_TSK,Lost Trailer,_TASKS,US_01,Michigan,truckDelivery,,180,1900,How everybody keeps managing to lose their stuff I do not know! We've got another renegade trailer on the loose. Get out there and find it will ya?,TASKS\nUS_01_02_UNLUCKY_FISHERMAN_TSK,Unlucky Fisherman,_TASKS,US_01,Michigan,truckDelivery,,250,1850,\"They say fishing is supposed to be relaxing, unless you manage to lose your truck! Go help that poor sap drag his rig out of the river, will ya? I'm sick of hearing him get weepy over the phone.\",TASKS\nUS_01_02_RIVER_CROSSING_TSK,Across The River,_TASKS,US_01,Michigan,truckDelivery,,150,1350,\"So, we need a trailer delivered... to the middle of a river. Why? Well... I can't tell you. Are you in or not?\",TASKS\nUS_01_02_TRUCK_RESTORATION_TSK,Fixer Upper,_TASKS,US_01,Michigan,truckDelivery,,140,1200,There's an old broken down truck nearby that needs some serious TLC. If you're up for a bit of elbow grease then it's all yours. I'm sure you'll find a use for it.,TASKS\nUS_01_02_CLEAN_THE_RIVER_EAST_TSK,Threatening Accident,_TASKS,US_01,Michigan,cargoDelivery,4\u00d7 Barrels Oil,340,3500,Has the whole town gone looney? A driver managed to drop a whole cargo of fuel into the river! I've since fired him but I've got a bunch of barrels floating around the river that need hauling out.,TASKS\nUS_01_02_CLEAN_THE_RIVER_WEST_TSK,Unfortunate Event,_TASKS,US_01,Michigan,cargoDelivery,3\u00d7 Barrels Oil,310,3100,\"Dyson Diesel here. We've got a situation. We lost a couple of barrels of fuel in the river. I'd appreciate it if you'd handle it... quietly. I don't need any troublesome environmental enquiries, understand?\",TASKS\nUS_01_02_SOLID_FOUNDATION_TSK,Solid Foundation,_TASKS,US_01,Michigan,cargoDelivery,1\u00d7 Concrete Slab,400,3000,The life of a recluse seems to be catching on. A guy wants to build a house in the middle of nowhere and he's got a shipment of concrete slabs he wants to use for the foundation. Can you haul 'em up there?,TASKS\nUS_01_02_LOST_BAGS_TSK,Lost Bags,_TASKS,US_01,Michigan,cargoDelivery,4\u00d7 Bags,330,2900,Dyson Diesel here. One of our contractors has lost a number of construction materials down the river. We need them back.,TASKS\nUS_01_02_FOOD_FOR_WORKERS_TSK,Hungry Workers,_TASKS,US_01,Michigan,cargoDelivery,2\u00d7 Crate Large,240,2400,\"Apparently, Dyson Diesel management has prioritized materials and equipment deliveries above food and water. The workers are living off basic rations and morale is not looking good. Can you pick up the slack?\",TASKS\nUS_01_02_WOODEN_BRIDGE_TSK,Smithville Bridge,_TASKS,US_01,Michigan,cargoDelivery,2\u00d7 Wooden Planks,200,2200,\"We've decided to construct a bridge that will cut out the degraded road and make our route to the warehouse shorter and safer. We need you to get wooden planks out there, fast.\",TASKS\nUS_01_02_DRILL_FOR_OUTCAST_TSK,Well Well Well,_TASKS,US_01,Michigan,cargoDelivery,1\u00d7 Vehicles Spare Parts,210,2100,Old Bill Anderson lives way out in the woods. He's kind of an oddball but we try to help him out when we can. Will you help to deliver everything he needs?,TASKS\nUS_01_02_BARRELS_DELIVERY_TSK,More Fuel,_TASKS,US_01,Michigan,cargoDelivery,2\u00d7 Barrels,180,1900,The local farmers have been pulling long hours to get their properties cleaned up. They're making progress but they're burning through fuel. Can you haul another tank over to their private facility?,TASKS\nUS_01_02_FALLEN_ROCKS_TSK,Rock On,_TASKS,US_01,Michigan,cargoDelivery,2\u00d7 Service Spare Parts,170,1300,We've got another rockslide that's putting a highway out of action. Can you get a shipment over there to help get it cleared out?,TASKS\nUS_01_02_REPAIR_THE_TRUCK_TSK,Repair The Truck,_TASKS,US_01,Michigan,exploration,,180,1900,If you're looking for some extra cash I've got a nice little job for you. I saw a broken truck near an abandoned house. I'd sure be grateful if you could repair it.,TASKS\nUS_01_02_FIND_THE_ANTENNA_TOWER_TSK,Find The Antenna Tower,_TASKS,US_01,Michigan,exploration,,80,1400,Our archives show the existence of an antenna tower in the area. It hasn't been used in twenty years but we want to restore it and put it to good use. Can you take a look around and see if you can find it?,TASKS\nUS_01_02_MICHIGAN_TRIAL_TSK,Michigan Trial,_TASKS,US_01,Michigan,exploration,,80,1350,There's an old trail close by here that the locals use for sport. It's pretty rugged. You have to reach all of the flags on the course. Think you can beat it?,TASKS\nUS_01_02_FLOODED_HOUSE_CNT,Going Under,_CONTESTS,US_01,Michigan,cargoDelivery,2\u00d7 Blocks,150,1250,My darn uncle is so stubborn! Everyone told him not to scrimp on the foundations but he's such a penny pincher. Now his house is half floating away! You've got to help him!,CONTESTS\nUS_01_02_HOUSE_RENOVATION_CNT,New Nest,_CONTESTS,US_01,Michigan,cargoDelivery,2\u00d7 Bricks,110,1050,My house was destroyed in the flood. I managed to rescue most of the sentimental stuff but we're going to have to build a new place to live. Can you get us the equipment and materials we need to get started?,CONTESTS\nUS_01_02_FARMERS_NEEDS_CNT,Heavy Burden,_CONTESTS,US_01,Michigan,cargoDelivery,3\u00d7 Service Spare Parts,110,900,\"As part of our 'Help in the Community' operation, we're supplying a shipment of tools to a local farm to help them recover from the flood. Make sure it reaches them in one piece.\",CONTESTS\nUS_01_03_POWER_WIRES_1_CONTRACT_OBJ,Power Lines,_CONTRACTS,US_01,Michigan,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Metal Planks; 1\u00d7 Metal Planks,610,4400,That weather sure ain't cutting us any slack. The last storm tore up a bunch of power lines which means we have people sitting in the dark until they're fixed. Any chance you can help?,CONTRACTS\nUS_01_03_DROPPED_VEHICLE_SEARCHING_TSK_01,The Huntsman And The Car,_TASKS,US_01,Michigan,truckDelivery,,140,1100,\"Heard about Jim, the local farmer? Went out hunting last weekend and managed to get his car stuck real good in the swamp. Ain't no way we're getting the thing out on our own. Maybe you could give it a try?\",TASKS\nUS_01_03_SWAMP_CROSSING_01_TSK,Tough Negotiations,_TASKS,US_01,Michigan,cargoDelivery,2\u00d7 Wooden Planks,100,1000,\"A certain company made the town an attractive offer, the basis of which has absolutely nothing to do with drilling for oil on public land. All they're asking for is a bridge for their supplies and total silence. You in?\",TASKS\nUS_01_03_DROPPED_VEHICLE_SEARCHING_TSK_02,Rusty Legacy,_TASKS,US_01,Michigan,truckDelivery,,160,1500,\"Hey there, I'm Ted. I run a repair place in town. So, random story here. Buddy of mine found an abandoned truck. Since I ain't got the time or resources to get it myself, I figured you might help. Whaddaya say, pal?\",TASKS\nUS_01_03_SWAMP_CROSSING_02_TSK,Bridge The Gap,_TASKS,US_01,Michigan,cargoDelivery,2\u00d7 Wooden Planks,100,900,\"Hey, I'm Tony from the oil company. Our drilling team is having a heck of a time trying to get gear to the site. The terrain's too rough and we could really use a bridge. Can you help us out? We'd pay you well.\",TASKS\nUS_01_03_SWAMP_CROSSING_03_TSK,Corporate Expansion,_TASKS,US_01,Michigan,cargoDelivery,2\u00d7 Wooden Planks,110,1150,\"Hey, it's Tony again. We found another area that looks like it might produce oil. We wanna do some testing but we're gonna need -- you guessed it -- another bridge to get our equipment there. Can we count on you?\",TASKS\nUS_01_03_DROPPED_VEHICLE_SEARCHING_TSK_03,A Little Help For My Friends,_TASKS,US_01,Michigan,exploration,,70,1250,\"Oh, man, we could really use a favor here. We were doing some hauling for Dyson and got stuck in the dang mud. Boss is gonna chew us out of we don't get it delivered on time. Can you do us a solid and help us out?\",TASKS\nUS_01_03_LUMBER_MILL_REACTIVATION_OBJ,Lumber Mill Revival,_CONTRACTS,US_01,Michigan,truckDelivery,2\u00d7 Blocks; 1\u00d7 Metal Planks,590,4800,We're getting the old Lumber Mill back up and running. We could use a hand hauling some of the materials to the site. I believe there is a trailer with the necessary materials in the warehouse nearby.,CONTRACTS\nUS_01_03_LOST_CARGO_TSK,Instruments Of Development,_CONTRACTS,US_01,Michigan,cargoDelivery,3\u00d7 Container Large Drilling,770,5950,\"We did some digging out on the old swamp and it looks like there could be oil out there. We wanna start drilling, but first we're gonna need you to get rid of some old equipment that's in the way.\",CONTRACTS\nUS_01_03_CARGO_PORT_OBJ,Harbor Delivery,_CONTRACTS,US_01,Michigan,cargoDelivery,1\u00d7 Container Large Drilling,590,4750,The old drills you've helped us recover are now packed up for shipment. All that's left is to get them over to Drummond Island's port and get 'em off our hands. Get that done so we can bring in the new gear.,CONTRACTS\nUS_01_03_SHORT_CUT_TSK,Hunter Bob's \u0412ridge,_TASKS,US_01,Michigan,cargoDelivery,2\u00d7 Wooden Planks; 2\u00d7 Wooden Planks,140,1700,Always nice to see new faces around here! I have a favor to ask. We've got this old bridge not far from here that could really use fixing up. I'll make it worth your while.,TASKS\nUS_01_03_FIX_THE_ANTENNA_TSK,Broken Antenna,_TASKS,US_01,Michigan,cargoDelivery,1\u00d7 Container Small,140,1200,\"So, we can't afford to fix the TV tower so we've got to fix it ourselves. I'll walk you through it but first we need to grab some parts and get them to the tower.\",TASKS\nUS_01_03_TRUCK_REPAIR,No Country For Old Truck,_TASKS,US_01,Michigan,exploration,,160,1500,\"Man, it's a crying shame the way some people treat their stuff. This truck must've been a Cowboy Cadillac back in the day, now it's a total heap. If you can get it out of here, maybe I can get it running again.\",TASKS\nUS_01_03_FIND_THE_ANTENNA_TSK,Signal In The Mountains,_TASKS,US_01,Michigan,exploration,,40,600,\"It never rains but it pours, right? Seems like the old TV tower up on the ridge is on the blink again. I hate to ask but could you get up there and see if it can be repaired?\",TASKS\nUS_01_03_BARREL_CNT,Flaming Barrels,_CONTESTS,US_01,Michigan,cargoDelivery,1\u00d7 Barrels Oil,110,950,We've got a situation here. A shipment of oil barrels got stuck in the forest. We're on a tight schedule and we need that oil ASAP. Get out there and bring back as much as you can.,CONTESTS\nUS_01_04_BUILD_A_BRIDGE_OBJ_1,Support Local Rangers,_TASKS,US_01,Michigan,cargoDelivery,2\u00d7 Wooden Planks,100,1000,\"Hey, nice to meet ya. Name's Mitch. I'm a forest ranger. Word in town is that you're the man for bridge problems. I happen to have a little bridge that could use some repair. You interested?\",TASKS\nUS_01_04_SUPPLIES_FOR_REGION_2_OBJ,Cement For The Region,_CONTRACTS,US_01,Michigan,cargoDelivery,2\u00d7 Bags; 2\u00d7 Bags; 2\u00d7 Bags; 2\u00d7 Bags,2000,22350,\"Hi! Somehow lots of demand for cement cropped up in this region. Lots of cargo, lots of destinations. Not an easy task. But I am sure you will ace it.\",CONTRACTS\nUS_01_04_MED_LOGS_2_OBJ,Timber At Arm's Reach,_CONTRACTS,US_01,Michigan,cargoDelivery,3\u00d7 Logs Medium; 2\u00d7 Logs Long; 1\u00d7 Logs Medium; 1\u00d7 Logs Long; 2\u00d7 Logs Medium,1850,17250,\"Several local households and a sawmill near a lake on the island request some logs. The batch is rather big, so you may want to bring some help or make several trips. The cheque at the end should make you happy.\",CONTRACTS\nUS_01_04_LONG_LOGS_2_OBJ,Logs To The Workers!,_CONTRACTS,US_01,Michigan,cargoDelivery,1\u00d7 Logs Long; 1\u00d7 Logs Long,1030,9600,\"The Quarry and the Drilling Site desperately need logs. After the recent rain, the heavy machinery is drowning in mud, so something needs to go under the wheels. Work has stopped while the budget gets wasted. Hurry!\",CONTRACTS\nUS_01_04_BUILD_A_BRIDGE_OBJ_2,Of Bridges And Men,_TASKS,US_01,Michigan,cargoDelivery,2\u00d7 Wooden Planks,100,1000,\"Hey there, bridge-fixin' man. It's Mitch again. Believe it or not, I've got another bridge in need of your expert attention. Can you have a look? I'd sure appreciate it.\",TASKS\nUS_01_04_BUILD_A_BRIDGE_OBJ_3,Ten-Hut,_TASKS,US_01,Michigan,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Concrete Slab; 1\u00d7 Metal Planks; 1\u00d7 Blocks,640,6000,\"Greetings. I'm Captain Smith from the Army Corp of Engineers. We'd usually use our own guys for this, but, I gotta tell you, your bridge prowess is legendary. We've got one that needs help. Can you show us how it's done?\",TASKS\nUS_01_04_CARGO_DELIVERING_OBJ,Out With The Old,_CONTRACTS,US_01,Michigan,truckDelivery,2\u00d7 Metal Planks; 1\u00d7 Big Drills,1180,11000,\"With the old sites now shut down, it's time to set up our shiny new ones. Get the necessary equipment over there ASAP so we can break ground and get to that oil!\",CONTRACTS\nUS_01_04_MULTIORDER_OBJ,Super Order,_CONTRACTS,US_01,Michigan,cargoDelivery,1\u00d7 Barrels; 1\u00d7 Crate Large; 1\u00d7 Service Spare Parts; 1\u00d7 Wooden Planks; 1\u00d7 Barrels; 1\u00d7 Crate Large; 1\u00d7 Blocks; 1\u00d7 Vehicles Spare Parts; 1\u00d7 Barrels; 1\u00d7 Metal Planks; 1\u00d7 Concrete Slab,2740,32650,\"Wow, look at you! Ready for work? We have a job for an ace like you\u2014lots of cargo to be taken to various destinations. It requires some complicated logistics, but it'll be quicker if you have a helper.\",CONTRACTS\nUS_01_04_LONG_LOGS_OBJ,Timber For The Locals,_CONTRACTS,US_01,Michigan,cargoDelivery,3\u00d7 Logs Long; 3\u00d7 Logs Medium; 3\u00d7 Logs Medium,2060,22450,Local businesses need lots of timber. It's good that the Timber Station is working again. You have to pick up some logs there and haul them to the customers. You may need special equipment for loading and transportation.,CONTRACTS\nUS_01_04_CARGO_FROM_SHIP_OBJ,Cargo From The Ship,_CONTRACTS,US_01,Michigan,cargoDelivery,2\u00d7 Container Large,1270,14100,\"A businessman contacted us. His cargo is stuck on a ship that went aground. Big containers in a narrow passage. Wouldn't be much of a problem for you now, would it? Although if I were you, I would use help.\",CONTRACTS\nUS_01_04_MED_LOGS_OBJ,Aid At The Island,_CONTRACTS,US_01,Michigan,cargoDelivery,1\u00d7 Logs Medium; 1\u00d7 Logs Medium; 2\u00d7 Logs Medium,990,11300,\"A farm and several residences on the Isle of Drammond requested a shipment of logs. We gotta help them. There are logs at the Timber Stations, but they have to be delivered. Are you up to it?\",CONTRACTS\nUS_01_04_FIND_LOST_TRUCK_TSK,Mud Wrestling,_TASKS,US_01,Michigan,truckDelivery,,140,1200,\"It's Ted from the auto repair place. Some city folk thought it would be nice to get back to nature. Surprise, surprise, they got stuck in the mud. I'd complain, but I make most of my income off these fools. Can you help?\",TASKS\nUS_01_04_FALLEN_CARGO_TSK,Cousin Cletus,_TASKS,US_01,Michigan,cargoDelivery,2\u00d7 Barrels Oil,340,2150,\"Hey. I'm Chris from Dyson. You ever trust somebody and they lose cargo and now you might get fired? I'm not saying that 'somebody' is my cousin Cletus, but I'm not saying it's not Cletus. Can you find the lost cargo?\",TASKS\nUS_01_04_LOST_CARGO_DELIVERY_TSK,Lost Delivery,_TASKS,US_01,Michigan,cargoDelivery,3\u00d7 Container Small Special,240,2150,Hi. I'm Karen from the Steel River mayor's office. Today's shipment for the Main Street Renewal Project came up a few pallets short. Would you be a good citizen and find them and bring them to the construction site?,TASKS\nUS_01_04_LOST_SHIP_OBJ,Forged By The Sea,_TASKS,US_01,Michigan,exploration,,50,850,\"Tony again. A little birdie told me that a cargo ship washed ashore not too far from here. Hypothetically, if my employer was interested in that cargo and would pay you to retrieve it, would that spark your interest?\",TASKS\nUS_01_04_OBSERVATION_DECK_TSK,The Silent Observer,_TASKS,US_01,Michigan,exploration,,40,650,\"You're probably tired of hearing how bad the weather's been, but let's be honest, it's also feathering your nest. One of our observation points went silent in the last storm. Can you head out there and see if it's ok?\",TASKS\nUS_01_04_PATH__PASSING_TSK,Tourist Attraction,_TASKS,US_01,Michigan,exploration,,30,500,\"Hi sweetie. It's Karen again from Steel River Township. With tourist season coming up and all the rain we've had, we've gotta make sure that the road to Hollis Island is in decent shape. Could you check it out for us?\",TASKS\nUS_01_04_EXPLORING_CNT,A Race With The Rain,_CONTESTS,US_01,Michigan,exploration,,40,600,\"This weather, man. We're already getting reports that the road to the farm may be washed out from the rain. We need you to get out there right now to check it out before it gets any worse. We're all counting on you.\",CONTESTS\nUS_02_01_LOGS_01_OBJ,Timber!,_CONTRACTS,US_02,Alaska,cargoDelivery,2\u00d7 Logs Medium; 1\u00d7 Logs Long,624,5670,\"Hi! The sawmill near the port is running out of wood, so we'll pay to have some delivered.\",CONTRACTS\nUS_02_01_MOUNTAIN_CONQUEST_1_CNT,North Mountain Conquest,_CONTESTS,US_02,Alaska,exploration,,50,750,People say you're one of the best drivers around. Maybe. Maybe not. How 'bout a little challenge to see if you can beat my record?,CONTESTS\nUS_02_01_OIL_DELIVERY_02_OBJ,Mountain Delivery,_CONTRACTS,US_02,Alaska,cargoDelivery,4\u00d7 Barrels Oil,290,2500,\"I do love looking up at those beautiful mountains. Driving up there? Now, that's a whole different ball game. I need you to ship these pallets to the Service Hub among the peaks. Take it easy out there.\",CONTRACTS\nUS_02_01_MOUNTAIN_CONQUEST_2_CNT,West Mountain Conquest,_CONTESTS,US_02,Alaska,exploration,,40,650,\"Heard you've got some nerve, pal. Let's check it out. Reach every point marked on the map and I'll admit defeat. Do you accept my challenge or you gonna chicken out?\",CONTESTS\nUS_02_01_DISASS_OBJ,Floating Drill,_CONTRACTS,US_02,Alaska,truckDelivery,,360,3350,\"Black Bird needs your help, old pal! We're removing the floating oil derrick and we need you deliver the Rig Salvage Trailer to the Port.\",CONTRACTS\nUS_02_01_DRILL_DELIVERY_OBJ,Drilling Supplies,_CONTRACTS,US_02,Alaska,cargoDelivery,1\u00d7 Container Small; 1\u00d7 Pipes Medium,670,5900,\"Looks like you're the chosen one! Chosen to save the day and deliver materials to a drilling site, that is! I used to do it all myself in my younger days but my body ain't what it used to be. Well, get going!\",CONTRACTS\nUS_02_01_OIL_DELIVERY_OBJ,Oil Delivery,_CONTRACTS,US_02,Alaska,cargoDelivery,1\u00d7 Barrels Oil; 1\u00d7 Barrels Oil; 1\u00d7 Barrels Oil; 1\u00d7 Barrels Oil,520,5600,Oil is the lifeblood of our economy round here. Nothing moves without it. Get up behind that wheel and haul the latest shipment to its destination and I'll make sure you're paid well.,CONTRACTS\nUS_02_01_SPECIAL_DELIVERY_OBJ,Drilling Parts Delivery,_CONTRACTS,US_02,Alaska,cargoDelivery,4\u00d7 Service Spare Parts Special,470,4400,\"We've dismantled a floating oil derrick and all the spare parts are packed into boxes. Take them to the Factory, where our guys will decide what to do with them.\",CONTRACTS\nUS_02_01_POLAR_BASE_OBJ,Port Polar Base Supply,_CONTRACTS,US_02,Alaska,cargoDelivery,4\u00d7 Bricks; 1\u00d7 Metal Planks; 2\u00d7 Barrels,390,3450,\"We've got a bunch of brainiac scientist fellas that get their kicks drilling holes in the ice. They happen to be big customers of ours so we like having them around. They ask for something, you deliver. You dig?\",CONTRACTS\nUS_02_01_BARRELS_OBJ,Fire In A Barrel,_CONTRACTS,US_02,Alaska,cargoDelivery,2\u00d7 Barrels,250,2750,\"Hello there! Our client, a port service, wants us to deliver a shipment of fuel. Take the barrels, drive to the port and show me you are worth the expense.\",CONTRACTS\nUS_02_01_PIPELINE_OBJ,Pipeline Construction,_CONTRACTS,US_02,Alaska,cargoDelivery,2\u00d7 Service Spare Parts,180,1400,\"We've got an urgent contract for you. Local fuel company is building a pipeline and they just hired us as a subcontractor. Deliver everything they need, fast.\",CONTRACTS\nUS_02_01_CANT_GO_TO_WASTE_TSK,Can't Go To Waste,_TASKS,US_02,Alaska,truckDelivery,,350,2250,\"Hey, name's Mitch. People round here say a lot of good things about you. I need your services. A local fella named Noah left town with his family a few months back and they abandoned their truck. Can you get it to me?\",TASKS\nUS_02_01_OILTANK_DELIVERY_TSK,Oil Tank Delivery,_TASKS,US_02,Alaska,truckDelivery,,300,2100,\"So, we've got a gas station that needs gas. Big surprise there. I need you to get a tank of oil over there. Deadline was, of course, the day before yesterday, so get on with it!\",TASKS\nUS_02_01_STUCK_SCOUT_TSK,Stuck In The Mountains,_TASKS,US_02,Alaska,truckDelivery,,240,1650,We've got an emergency. A group of scientists went into the mountains on an expedition and they must have gotten trapped after the storm. There's a reward for anyone that can find and return them safely.,TASKS\nUS_02_01_SERVICE_RETURN_TSK,Lost Service Trailer,_TASKS,US_02,Alaska,truckDelivery,,210,1500,Our factory has lost a trailer. This is real bad news. The cargo has the potential to cause some pretty nasty damage to the environment so we need it found and retrieved ASAP. Can you do it?,TASKS\nUS_02_01_HUMMER_TSK,Drowned Hummer,_TASKS,US_02,Alaska,truckDelivery,,180,1500,\"Mark Pearson, the owner of a local factory, has gone and lost his car... again. His Hummer fell down from a bridge due to... driver error. See if you can find the car and take it to a nearby parking lot.\",TASKS\nUS_02_01_LOST_OILTANK_TSK,Lost Oil Tank,_TASKS,US_02,Alaska,truckDelivery,,150,1000,Local radio tower was due a shipment of fuel to keep it running. Seems the oil tank ran into trouble and is now lost. Can you find it and get it to the Radio Tower? Lord knows they need it.,TASKS\nUS_02_01_ABANDONED_SUPPLIES_TSK,Abandoned Supplies,_TASKS,US_02,Alaska,truckDelivery,,150,950,We run a Polar Base facility near here. We've been struggling to get shipments in lately because of the weather. We had to ditch a shipment just to make it through. We need a subcontractor that can handle the conditions.,TASKS\nUS_02_01_BAGS_ON_ICE_TSK,Bags On Ice,_TASKS,US_02,Alaska,cargoDelivery,4\u00d7 Bags,480,3100,\"Today is not my day. First I get attacked by a bear while I was crossing the river, and then, when I escape, I realize I've lost all my cargo! They must be in the river. Can you go get them for me?\",TASKS\nUS_02_01_CONTAINERS_IN_RIVER_TSK,Containers In The River,_TASKS,US_02,Alaska,cargoDelivery,2\u00d7 Container Small,350,2600,Name's Pete. I work for a transport company called Alaskan Transit. We've had an unfortunate accident that's resulted in containers of tools stuck in the river. Can you locate them and get them to the base?,TASKS\nUS_02_01_LOST_TUBE_TSK,Lost Tube,_TASKS,US_02,Alaska,cargoDelivery,1\u00d7 Pipe Large,360,1950,\"Hello. Name's Robert. A big, fancy new pipeline is being built in the area. One of the subcontractors operating logistics has lost a section of pipe. We need a driver that can help with the recovery.\",TASKS\nUS_02_01_ROCK_TSK,Rock Obstacle,_TASKS,US_02,Alaska,cargoDelivery,1\u00d7 Metal Planks,180,1750,\"Looks like we've got another rockslide on our hands. Time for you to suit up and save the day, buddy. A shipment of cargo needs delivering to the site of the rockslide to help with clean-up.\",TASKS\nUS_02_01_FIX_A_BRIDGE_TSK,Bridge Repair,_TASKS,US_02,Alaska,cargoDelivery,1\u00d7 Metal Planks,180,1650,Local authorities need a driver. They've got a metal bridge that needs some serious renovations. You'll need to haul the necessary supplies out there so work can begin. Pay looks good.,TASKS\nUS_02_01_ROCK_FALL_TSK,Rocks Fall,_TASKS,US_02,Alaska,cargoDelivery,2\u00d7 Wooden Planks,200,1300,\"The local authorities are looking for folks willing to volunteer to help with a nasty rockslide. The job is well paid. The request is urgent, so get to it!\",TASKS\nUS_02_01_STONE_FALL_TSK,Stone Fall,_TASKS,US_02,Alaska,cargoDelivery,1\u00d7 Metal Planks,200,1300,We're having a season of rockslides! Another one has stirred things up and needs dealing with right away. Can you roll up your sleeves and lend a hand getting it cleared up?,TASKS\nUS_02_01_POWERLINE_CHECK_TSK,Powerlines Check,_TASKS,US_02,Alaska,exploration,,130,2150,Local authorities just sent out a request for volunteers to take care of some downed powerlines. Sounds pretty serious. They'll cover the cost and make it worth your while.,TASKS\nUS_02_01_TRAILER_PARK_TSK,Trailer Park,_TASKS,US_02,Alaska,exploration,,60,950,\"Well hi there! Any chance of a ride? Need to get to a trailer park to handle some, uh... business, but I can't remember where it is. I have a general idea of the location but nothing concrete. What do you say?\",TASKS\nUS_02_01_RADIOSTATION_TSK,Find Radio Station,_TASKS,US_02,Alaska,exploration,,50,900,Your skills come highly recommended. Our scientists have spotted an abandoned radio station but the path is quite treacherous. Can you find a safe route up there?,TASKS\nUS_02_01_EMPLOYEE_DISLOCATION_CNT,Employee Dislocation,_CONTESTS,US_02,Alaska,cargoDelivery,1\u00d7 Service Spare Parts,90,950,Hi there! Name's Chuck. I'm currently a local resident but since the earthquake we decided this place isn't for us. It's just too dangerous. We've booked passage on a ship. Can you transport our things to the port?,CONTESTS\nUS_02_01_FLAGS_CNT,Race Down To Flags,_CONTESTS,US_02,Alaska,exploration,,30,450,Name's Alex. Not a soul in this town can beat my record around this course. You see those flags? Think you can get to all of 'em before nightfall?,CONTESTS\nUS_02_02_LOGS_01_OBJ,It's A Long Way To The Top,_CONTRACTS,US_02,Alaska,cargoDelivery,2\u00d7 Logs Medium; 2\u00d7 Logs Long,1440,16650,\"Remember how you helped get the sawmill started up in the mountains? We need more logs, our initial supply is running out.\",CONTRACTS\nUS_02_02_DISASS_OBJ,Drill Rig Disassembly,_CONTRACTS,US_02,Alaska,truckDelivery,,320,3950,We're in the process of disassembling the drilling platforms and we need some extra gear. We need you to transport the necessary equipment. I'll mark the locations on your map.,CONTRACTS\nUS_02_02_PIPELINE_OBJ,Mountain Pipeline Building,_CONTRACTS,US_02,Alaska,cargoDelivery,2\u00d7 Metal Planks; 2\u00d7 Service Spare Parts; 1\u00d7 Pipes Medium; 1\u00d7 Pipes Small,780,4750,Good news for local residents! We're installing a shiny new oil pipeline! We need you to get the pipes and some spare parts to the construction site. We'll be milking the land dry in no time!,CONTRACTS\nUS_02_02_VILLAGE_DELIVERY_OBJ,Village Delivery,_CONTRACTS,US_02,Alaska,cargoDelivery,2\u00d7 Bags; 2\u00d7 Crate Large,460,3550,\"This one's urgent. Priority one! Mr. Jackson, our district manager, has a contract with a mountain settlement to deliver a shipment of consumables right away. He's probably making a fortune off them. Wouldn't want to lose that customer!\",CONTRACTS\nUS_02_02_MILL_DELIVERY_OBJ,Lumber Mill Supplies,_CONTRACTS,US_02,Alaska,cargoDelivery,4\u00d7 Service Spare Parts; 1\u00d7 Metal Planks; 2\u00d7 Barrels; 1\u00d7 Pipes Small,450,3300,\"I am mr. Blade. I don't normally work with outsiders but I don't have much choice at this point. Our lumber mills are in desperate need of supplies. If you can help me, I'll pay a fair price.\",CONTRACTS\nUS_02_02_TSTOP_DELIVERY_OBJ,On The Top Of The World,_CONTRACTS,US_02,Alaska,cargoDelivery,2\u00d7 Crate Large; 4\u00d7 Barrels; 2\u00d7 Wooden Planks,380,3300,Up on that mountain is a little truck stop with a big heart. It's so popular with truckers that they just ran completely out of supplies! If you don't get some provisions up there soon there may be a truckers revolt!,CONTRACTS\nUS_02_02_DRILLING_PARTS_OBJ,Drilling Equipment,_CONTRACTS,US_02,Alaska,cargoDelivery,1\u00d7 Container Large Drilling,280,2350,\"We've just finished disassembling a drill rig and we want it shipped over to another drilling station to be stocked. Once you arrive, you should find new contracts waiting for you.\",CONTRACTS\nUS_02_02_POLAR_BASE_OBJ,Mountain Maze,_CONTRACTS,US_02,Alaska,cargoDelivery,1\u00d7 Wooden Planks; 2\u00d7 Barrels,260,2150,\"You know what I love most about this job? Chatting with decent folks like yourself. Sadly, duty calls! Miners on the mountain polar base need materials. Deliver it and you'll be rewarded for your time.\",CONTRACTS\nUS_02_02_SPECIAL_DELIVERY_OBJ,More Parts,_CONTRACTS,US_02,Alaska,cargoDelivery,4\u00d7 Service Spare Parts Special,200,1350,\"In the meantime, we'll get the rest of the oil derrick squared away. Take these boxes to the factory and we'll see to it that you're rewarded.\",CONTRACTS\nUS_02_02_WORKING_STIFF_TSK,Working Stiff,_TASKS,US_02,Alaska,truckDelivery,,190,2000,\"I'm George, the foreman on the Lumber Mill. We need help with our workhorse, and by that I mean Caterpillar 745C. It's taken quite a beating over the years. Think you can repair it and get it to the sawmill?\",TASKS\nUS_02_02_ENVIRONMENTAL_ISSUE_TSK,Environmental Issue,_TASKS,US_02,Alaska,truckDelivery,,170,1350,\"I'm Bob Fletcher, local environmentalist. I'm concerned about the construction of the new pipeline. Can you help me and my team get to it so we can perform a survey and make sure everything is above board?\",TASKS\nUS_02_02_SERVICE_CONVOY_TSK,Service Convoy,_TASKS,US_02,Alaska,truckDelivery,,180,1200,Hi. I'm Bennet from Alaska Silver Extraction. The service trailer in our convoy is stuck in some deep mud. We're in dire need of help to get it unstuck. We're happy to pay the going rates.,TASKS\nUS_02_02_BRIDGE_RECOVERY_TSK,Restoring The Bridge,_TASKS,US_02,Alaska,cargoDelivery,1\u00d7 Concrete Slab; 2\u00d7 Service Spare Parts,310,2300,\"Greetings, friend. It's Singing Axe. Sorry to disturb you, but a local bridge near our village has collapsed. We're trying to restore it but we lack drivers who can supply the materials. We could really use your help.\",TASKS\nUS_02_02_BRICKS_ON_RIVER_TSK,Bricks In The River,_TASKS,US_02,Alaska,cargoDelivery,3\u00d7 Bricks,210,1800,Hey! I need help. I had an accident near the river. I was transporting a batch of bricks to a warehouse and they slid off into the water. I need help finding and delivering them to their destination.,TASKS\nUS_02_02_TO_THE_TOWER_CNT,To The Top And Back,_CONTESTS,US_02,Alaska,exploration,,50,750,\"You're a pretty good driver, but are you the best? How 'bout a little contest? Climb up the mud from that landslide, all the way to the top, and then go down as quickly as possible. See if you can break my record.\",CONTESTS\nUS_02_02_RIVER_CONTEST_CNT,River Contest,_CONTESTS,US_02,Alaska,exploration,,30,500,I'm Nick Green. I'm with the Coast Guard. People get lost all too often round here so we leave emergency supply crates dotted around the landscape. Yesterday's storm may have dislodged them. Can you take a look?,CONTESTS\nUS_02_03_LOGS_01_OBJ,Airdrome Logging,_CONTRACTS,US_02,Alaska,cargoDelivery,2\u00d7 Logs Medium; 2\u00d7 Logs Long,1610,18200,\"Trees are still being cut near the airdrome. As many logs as possible should be hauled to the local sawmill. Get ready, bring your friends, it's a long road ahead.\",CONTRACTS\nUS_02_03_DISASS_OBJ,Drill Disassembly,_CONTRACTS,US_02,Alaska,truckDelivery,,190,1750,Bob here. We're in the middle of tearing down one of our oil platforms so we gotta get the drill airlifted out of here. Give us a hand.,CONTRACTS\nUS_02_03_PIPELINE_OBJ,Valley Pipeline Building,_CONTRACTS,US_02,Alaska,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Service Spare Parts; 1\u00d7 Pipes Medium; 1\u00d7 Wooden Planks; 1\u00d7 Container Large,800,6600,\"The pipeline's starting to come together, but there's still much to do. There's another segment we're going to need some help with. You know the drill. Bring the materials to the site, and we'll handle the rest.\",CONTRACTS\nUS_02_03_TOWN_DELIVERY_OBJ,Town Supplies,_CONTRACTS,US_02,Alaska,cargoDelivery,2\u00d7 Blocks; 1\u00d7 Pipes Small; 2\u00d7 Wooden Planks,620,6350,\"We have a situation here. The locals are having issues with their water supply. They can fix the issue, but they require some replacement parts and other materials. The sooner you can get on this, the better.\",CONTRACTS\nUS_02_03_POLAR_BASE_OBJ,Valley Polar Base Research,_CONTRACTS,US_02,Alaska,cargoDelivery,2\u00d7 Wooden Planks; 1\u00d7 Bags; 2\u00d7 Barrels; 1\u00d7 Metal Planks; 2\u00d7 Service Spare Parts,770,6100,\"Greetings. One of our research teams is running low on supplies. Sadly, none of our vehicles are available at the moment, which is why we have no choice but to turn to freelancers. Interested in some extra work?\",CONTRACTS\nUS_02_03_DRILL_DELIVERY_OBJ,Drilling Stuff Delivery,_CONTRACTS,US_02,Alaska,cargoDelivery,1\u00d7 Container Large Drilling,790,5900,\"Hey, I'm Peter from Morrison Mining. There's a new set of drills ready to be shipped to our site. Our guys are already there and waiting for it, so I need you to get over to the site.\",CONTRACTS\nUS_02_03_CRATES_OF_CONSUMABLES_OBJ,Supply Crates,_CONTRACTS,US_02,Alaska,cargoDelivery,4\u00d7 Crate Large,500,3400,\"The winter's been particularly harsh this year, so we've got a shipment of emergency supplies for the local town and local lumber mill. Just basics like medicine and food in case the roads get cut off again.\",CONTRACTS\nUS_02_03_MAZUT_OBJ,Oil Barrels Delivery,_CONTRACTS,US_02,Alaska,cargoDelivery,5\u00d7 Barrels Oil,260,2050,\"We got a big ol' bird flying in to the local airport, packed with cargo. We're ready to handle the goods but we need someone to get a shipment of fuel up there so we can turn the plane around as soon as it's unloaded.\",CONTRACTS\nUS_02_03_SPECIAL_DELIVERY_OBJ,From Water To The Land,_CONTRACTS,US_02,Alaska,cargoDelivery,4\u00d7 Service Spare Parts Special,210,1550,We hauled parts of a neighbouring drilling platform onto land and packed them into boxes. Please take them to a factory nearby.,CONTRACTS\nUS_02_03_DERRY_LONGHORN_TSK,Pluck The Stuck Truck,_TASKS,US_02,Alaska,truckDelivery,,210,2050,\"Seems like our airfield crew managed to get our tow truck stuck in the creek nearby. Usually I'd just call another one to pull it out, but that's the only one we have right now. Think you could give it a try?\",TASKS\nUS_02_03_FAILED_FISHING_A_TSK,Search And Rescue,_TASKS,US_02,Alaska,truckDelivery,,290,1950,\"You're the angel I've been praying for. There's a fishing spot on an island nearby, connected to the shore by a small stretch of land. It got flooded over with my car still on the far side. Can you get my car back here?\",TASKS\nUS_02_03_SCOUT_IN_TROUBLE_TSK,Deep River Diving,_TASKS,US_02,Alaska,truckDelivery,,190,1700,Hey! Help! I was out fishing when the river swelled. I had to run for my life or I'd have been washed away. I had to abandon my car. Can you get it back for me?,TASKS\nUS_02_03_OUT_OF_FUEL_TSK,Out Of Fuel,_TASKS,US_02,Alaska,truckDelivery,,160,1500,\"It's a bit of a long story, but I managed to leave my truck's engine running overnight. Now I'm more or less stranded at the boat station without enough gas to start the car. Could you help me out?\",TASKS\nUS_02_03_THEFT_OF_FUEL_TSK,Trucks And Investigations,_TASKS,US_02,Alaska,truckDelivery,,180,1200,\"Seems like someone's tried to tap into our pipeline... again. Security is already on the case but I doubt they'll find much. That said, they did leave their unmarked trailer behind. Could you tow it for us?\",TASKS\nUS_02_03_BLOCKED_TUNNEL_TSK,Blocked Tunnel,_TASKS,US_02,Alaska,cargoDelivery,1\u00d7 Pipes Small; 1\u00d7 Blocks,270,2350,\"That last earthquake left quite an impact on the whole region. Case in point, the old tunnel's now blocked by rubble. I was gearing up to get in touch and ask you for help, but now that you're here...\",TASKS\nUS_02_03_LONG_BRIDGE_RECOVERY_TSK,Long Bridge,_TASKS,US_02,Alaska,cargoDelivery,2\u00d7 Wooden Planks; 2\u00d7 Service Spare Parts,220,1500,\"Seems like the last earthquake took out quite a few old bridges around these parts. This one never was all that reliable to begin with, but now it's nothing but a pile of planks. Give us a hand here, will ya?\",TASKS\nUS_02_03_BUILDING_MATERIALS_TSK,Building Materials,_TASKS,US_02,Alaska,cargoDelivery,3\u00d7 Bricks,160,1250,\"I'm Mark. I run the local sawmill. I was thinking of expanding my warehouse, but the issue is that the bricks I ordered didn't make it here because of the recent earthquake. Could you fetch them for me?\",TASKS\nUS_02_03_REPAIR_THE_BRIDGE_TSK,Bridge,_TASKS,US_02,Alaska,cargoDelivery,2\u00d7 Service Spare Parts,120,950,\"Hey there, you have a moment? A bridge not far from here got torn into pieces by the recent earthquake. Could you help us put the old thing back together?\",TASKS\nUS_02_03_WEATHER_FORECAST_CNT,Weather Forecast,_CONTESTS,US_02,Alaska,truckDelivery,,300,1950,\"We've got three weather monitoring stations in the area, but we can't get any remote readings from them. We should, in theory, be able to get some readings if you can take a service trailer up to them.\",CONTESTS\nUS_02_04_LOGS_01_OBJ,Far Far Away,_CONTRACTS,US_02,Alaska,cargoDelivery,1\u00d7 Logs Medium; 1\u00d7 Logs Long,1220,14450,\"Residents of a remote area in Alaska need logs, their supplies are running out, and the freezing temperatures do not make it any easier. Please hurry, it is important!\",CONTRACTS\nUS_02_04_NEED_MORE_CARGO_ST2_OBJ,\"Cargopocalypse, pt.2\",_CONTRACTS,US_02,Alaska,cargoDelivery,8\u00d7 Bricks; 4\u00d7 Concrete Slab,1850,18850,\"Ready to hit the road with another mountain of cargo? Great. The warehouse's waiting for another shipment, so you better get going. Oh and, I'd suggest you recruit some help for this one, too.\",CONTRACTS\nUS_02_04_SPECIAL_CARGO_DELIVERYNG_CNT,Cargo Delivery,_CONTRACTS,US_02,Alaska,truckDelivery,,180,1500,\"Bob from Morrison Mining here. We've got a trailer full of tubing that needs delivering to the factory. It's not like the guys at the factory don't have anything else to do, but I'd appreciate if you double-time it.\",CONTRACTS\nUS_02_04_EPIC_DELIVERY_TIME_OBJ,Order Master Supreme,_CONTRACTS,US_02,Alaska,cargoDelivery,1\u00d7 Container Large; 1\u00d7 Container Large; 1\u00d7 Concrete Slab; 1\u00d7 Bags; 1\u00d7 Concrete Slab; 1\u00d7 Bags; 1\u00d7 Metal Planks; 2\u00d7 Concrete Slab; 1\u00d7 Container Small; 1\u00d7 Container Small,3170,31700,\"Hey there, friend! Glad you dropped by. Got a whole bunch of orders while you were gone, and little time to do them. Can you pick them all in one go? And if you can recruit some help, all the better.\",CONTRACTS\nUS_02_04_NEED_MORE_CARGO_OBJ,Cargopocalypse,_CONTRACTS,US_02,Alaska,cargoDelivery,3\u00d7 Pipe Large; 4\u00d7 Blocks,2380,23600,\"What a surprise! Since you're here anyway, how about earning some extra cash? All you need to do is to deliver about three times the norm in one go. Though, if you do recruit some help, you should be done in no time.\",CONTRACTS\nUS_02_04_SERVICE_HUB_REACTIVATION_CNT,Service Hub Reactivation,_CONTRACTS,US_02,Alaska,cargoDelivery,2\u00d7 Blocks; 2\u00d7 Crate Large,420,5100,\"So, management gave us the green-light to set up a base of operations in the old service hub here. Help us out with logistics and, in exchange, you can drop by for any repairs you need, any time. Deal?\",CONTRACTS\nUS_02_04_PIPELINE_BUILDING_CNT,Pipeline Repair,_CONTRACTS,US_02,Alaska,cargoDelivery,2\u00d7 Metal Planks,310,2450,\"We're supposed to repair one pipeline section right now, so we need to finally get the metal beams we requested weeks ago. Could you give us a hand and fetch them for us?\",CONTRACTS\nUS_02_04_SIDEBOARD_SPAWN_TSK,Family Treasure,_TASKS,US_02,Alaska,truckDelivery,,240,2250,\"Listen, I have a favor to ask. My dad owns a farm near here. I wanted to bring him some of our old family furniture from my house, but so far everyone's afraid the roads are too treacherous. Would you help us out?\",TASKS\nUS_02_04_CAR_HELP_TSK,Old Mack And Drowned Car,_TASKS,US_02,Alaska,truckDelivery,,190,1650,Interested in a small side gig? A hunter managed to run his truck into the swamp while chasing a moose. We'd send a tow truck but they're all out on other jobs. You interested?,TASKS\nUS_02_04_MOUNTAIN_CLEANING_TSK,Morrison's Landslide,_TASKS,US_02,Alaska,cargoDelivery,2\u00d7 Wooden Planks,230,2550,\"There's been an accident involving Morrison miners working in the mountains. No one was hurt, but the loose rocks have blocked the road. Could you help us with clearing the rubble?\",TASKS\nUS_02_04_LOST_CARGO_TSK,The Lost Tools,_TASKS,US_02,Alaska,cargoDelivery,1\u00d7 Vehicles Spare Parts,150,1550,\"Hey! It's Mike, Chief Engineer at Morrison Mining. To cut a long story very short, we need supplies. I'd just order them through our logistics, but it'd take too long for them to get here. How 'bout giving us a hand?\",TASKS\nUS_02_04_MATERIAL_DELIVERYING_TSK,Moving The Stock,_TASKS,US_02,Alaska,cargoDelivery,1\u00d7 Barrels Oil; 2\u00d7 Wooden Planks,150,1350,\"We're moving some of our stock to a new plant and could use some help. Our local trucks are on it already, but we could use an extra pair of hands for some of the remaining items.\",TASKS\nUS_02_04_BROKEN_POLE_TSK,Pole Down,_TASKS,US_02,Alaska,cargoDelivery,2\u00d7 Wooden Planks,130,1250,Hey! It's Peter from Morrison's. I'm at our warehouse right now. We're sitting in the dark here! No power throughout the entire facility. A power pole must have fallen down somewhere. You got time to check it out?,TASKS\nUS_02_04_BRIDGE_BUILDING_TSK,Dangerous Route,_TASKS,US_02,Alaska,cargoDelivery,4\u00d7 Wooden Planks,140,1100,Got word from one of my men about a bridge on one of the regular routes here that's been on its last legs for a while now. Could you get us a shipment of planks so we can take a crack at fixing it?,TASKS\nUS_02_04_FARMER_HOME_TSK,The Mountains Road,_TASKS,US_02,Alaska,exploration,,80,1300,Hi. Name's Albert. I'm with the Rescue Service. Do you have any idea how many people go off trail and get lost in the mountains? We used to have a lookout point nearby. Could you go and see what kind of shape it's in?,TASKS\nUS_02_04_FRAGILE_DELIVERY_CNT,Weather Conditions,_CONTESTS,US_02,Alaska,truckDelivery,,140,1300,\"Howdy. Name's Pete. I'm with the Weather Service. We need you to take our expedition vehicle to a site not far from here. The car's full of precisely tuned gear, so don't rock it around too much. Got it?\",CONTESTS\nRU_02_01_SERVICE_HUB_RECOVERY_01_OBJ,Service Hub Recovery,_CONTRACTS,RU_02,Taymyr,truckDelivery,,230,2050,There's a local service hub that was used for vehicle maintenance. It was shut down for lack of demand but we think we can make use of it now. Take fuel from the gas station to the service hub so we can fire it up.,CONTRACTS\nRU_02_01_PROSPECTING_01_OBJ,Georesearch \u2014 Starting Point,_CONTRACTS,RU_02,Taymyr,exploration,,740,4600,\"Data received from a recent survey suggests the largest oil reserves are located further to the west. To confirm that, use a Seismic Vibrator Addon not far from the Quarry.\",CONTRACTS\nRU_02_01_SERVICE_HUB_RECOVERY_02_OBJ,Repair Shop Restock,_CONTRACTS,RU_02,Taymyr,cargoDelivery,3\u00d7 Vehicles Spare Parts,340,3800,\"Now that the service hub can offer refueling services, it's time to set up a small workshop there to allow for proper repairs.\",CONTRACTS\nRU_02_01_PROSPECTING_02_OBJ,Georesearch \u2014 Triangulation,_CONTRACTS,RU_02,Taymyr,exploration,,1510,9950,\"As expected, the first scan around the quarry confirms the search is moving in the right direction. The next step would be to perform another full triangulation by finding and scanning two more suitable locations.\",CONTRACTS\nRU_02_01_LOG_SHIPWRECK_SUPPLY_OBJ,A Bigger Boat,_CONTRACTS,RU_02,Taymyr,cargoDelivery,3\u00d7 Logs Medium,670,5950,\"The ship repairs aren't going as fast as we'd hoped. The tides are about to rise, so we need to prevent them from pulling the ship off the shore before it's ready. If only we had some logs to secure the thing better...\",CONTRACTS\nRU_02_01_LOG_QUARRY_REINFORCEMENT_OBJ,Waterlogged,_CONTRACTS,RU_02,Taymyr,cargoDelivery,3\u00d7 Logs Medium,630,5350,\"The quarry up north is being slowly flooded by the mountain runoff. That alone is bad enough, but without reinforcement, the extra water could eventually collapse its walls, and that's something we'd rather avoid.\",CONTRACTS\nRU_02_01_OILRIG_RECOVERY_OBJ,Oil Rig Restoration,_CONTRACTS,RU_02,Taymyr,cargoDelivery,1\u00d7 Container Large,580,4850,An old oil rig round here was shut down decades ago because the reserves were too problematic to extract. New tech means we might be able to make it work. Use the Seismic Vibrator to see if the area holds any promise.,CONTRACTS\nRU_02_01_REFUEL_TRUCK_SWAMP_TSK,No Good Teens,_TASKS,RU_02,Taymyr,truckDelivery,,450,5950,\"A bunch of youngsters decided to spend the evening out in the country. Lord knows what they got up to but they ran out of gas and left their truck stranded. Refuel it and tow it to the service hub, would you?\",TASKS\nRU_02_01_REPAIR_TRUCK_HIGHWAY_TSK,Off-Roader,_TASKS,RU_02,Taymyr,truckDelivery,,430,5550,Good day to you! We've got a truck that was headed for the old town to the west before it went off the road and ran into a tree. It needs to be repaired and delivered to the service hub. Can you take care of it?,TASKS\nRU_02_01_HERMIT_RESCUE_TSK,Cliffhanger,_TASKS,RU_02,Taymyr,truckDelivery,,350,4250,One of the local's vehicles got seriously banged up above the old quarry. The owner would be grateful if you can repair it and deliver it to their house.,TASKS\nRU_02_01_OILRIG_SAMPLING_TSK,Oil Sampling,_TASKS,RU_02,Taymyr,truckDelivery,,300,4050,The rig is back in action and is already putting out its first batch. It'd be best if the first sample of crude from the Oilrig was evaluated at the factory. Just pull it to the fuel station and we'll handle it from there.,TASKS\nRU_02_01_EXAMINE_SOUTH_TSK,Rogue Connection,_TASKS,RU_02,Taymyr,truckDelivery,,200,1500,Ever notice you can't get a darn signal out here? The old communications tower in the woods could be used to improve comms coverage. Deliver a service trailer to the Southern Tower to perform the necessary repairs.,TASKS\nRU_02_01_TOWER_CLEARING_A_TSK,Clearing The Way,_TASKS,RU_02,Taymyr,cargoDelivery,2\u00d7 Metal Planks,600,5500,An old communication tower has collapsed and blocked part of the road. Clearing the rubble is going to need some specialized equipment. Can you get it out there?,TASKS\nRU_02_01_VILLAGE_RESTORATION_TSK,Settling In,_TASKS,RU_02,Taymyr,cargoDelivery,3\u00d7 Wooden Planks,160,1400,\"With the regional oil rig in working order, the nearby village could prove useful as the new staff quarters. There's a lumber mill nearby that probably has some materials still lying around. See what you can find.\",TASKS\nRU_02_01_HTRUCK_REFUEL_TSK,Lost In The Woods,_TASKS,RU_02,Taymyr,exploration,,330,3900,One of the specialized trucks taking part in the ship's recovery operations ran out of gas not far from the shore. It needs to be refueled before it can be used again.,TASKS\nRU_02_01_EXAMINE_EAST_TSK,Late Inspection,_TASKS,RU_02,Taymyr,exploration,,70,1200,The communications tower used to be a relay between the oil rig and the city to the west. Could prove a useful node for comms to the mainland in the future. I need you to run recon on the tower and check it's condition.,TASKS\nRU_02_01_FIREWATCH_SUPPLY_CNT,Firewatch Tower Supply,_CONTESTS,RU_02,Taymyr,truckDelivery,,120,950,\"Now that people are returning to the area, the old fire watch tower is going to be manned again. We need to resupply the tower before the new staff can take their post. Can you get on it?\",CONTESTS\nRU_02_01_SERVHUB_FUEL_RESTOCK_CNT,Fuel Restock,_CONTESTS,RU_02,Taymyr,cargoDelivery,3\u00d7 Barrels,200,1650,\"We've got a lot of heavy machinery in operation and those things guzzle gas like a herd of mountain giants. I'd like to have a reserve of fuel so we don't run out. Get on it, will you?\",CONTESTS\nRU_02_01_SHIP_REPAIRS_CNT,Shipwreck,_CONTESTS,RU_02,Taymyr,cargoDelivery,2\u00d7 Service Spare Parts,130,1050,Shipwreck,CONTESTS\nRU_02_02_LOST_CARGO_01_TSK,Drowned Oil Tank,_TASKS,RU_02,Taymyr,truckDelivery,,220,2300,One of our oil tanks ended up in deep water on its way to the gas station. Can you wade in there and retrieve it then transport it home?,TASKS\nRU_02_02_DAMAGED_TRUCK_01_TSK,Drowned Truck,_TASKS,RU_02,Taymyr,exploration,,290,3150,Two weeks ago a truck was found abandoned in the water. No one has come to collect it and the waiting period has expired so it's officially mine. Please get it repaired and refuelled as soon as possible.,TASKS\nRU_02_02_FLAG_1_CNT,Rocking And Rolling,_CONTESTS,RU_02,Taymyr,exploration,,60,900,\"You are a good driver, true, but are you an exceptional driver? Yes? Then prove it to me. There are five points. Reach them all in sequence. The faster you do it, the higher the reward. Understand?\",CONTESTS\nRU_02_02_HUB_RECOVERY_2,\"Hub Recovery, 2nd Stage\",_CONTRACTS,RU_02,Taymyr,cargoDelivery,4\u00d7 Vehicles Spare Parts,310,2400,\"This could be good for you. We can unlock the area for your vehicle repairs and even a trailer store, but first you'll need to deliver the necessary service parts boxes. Take care of that and you'll be good to go.\",CONTRACTS\nRU_02_02_STUCK_TRUCK_02_TSK,Retired,_TASKS,RU_02,Taymyr,truckDelivery,,240,2350,One of my drivers lost control of his truck and got it stuck in the swamp! Please get it out and deliver it to the garage while I take care of this incompetence-induced migraine.,TASKS\nRU_02_02_LOST_CARGO_02_TSK,Lost Bricks,_TASKS,RU_02,Taymyr,cargoDelivery,1\u00d7 Bricks,200,2250,\"Pavel has been waiting excitedly all night for his shipment of bricks, like a boy waiting for Santa Claus. Sadly, the driver plowed off the road and lost the cargo. Pavel's fate is now in your hands.\",TASKS\nRU_02_02_DAMAGED_TRUCK_02_TSK,Off The Cliff,_TASKS,RU_02,Taymyr,exploration,,230,2450,\"You would not believe what people lose around here. What's that? Their minds! Ha ha! Yes. Yes, perhaps. But no. There is a car at the bottom of the water that needs to be repaired and refuelled. Please help.\",TASKS\nRU_02_02_FLAG_2_CNT,Swimming And Sinking,_CONTESTS,RU_02,Taymyr,exploration,,60,850,\"A person's reputation is everything. How would you like to boost yours? There are five points on the map. Pass all of them, in sequence, and I will reward you. The faster you go, the more you get. You get me? Good.\",CONTESTS\nRU_02_02_STUCK_TRUCK_03_TSK,Lost In The Rocks,_TASKS,RU_02,Taymyr,truckDelivery,,290,2300,\"What's wrong with people! No wonder insurance premiums are so high in this town. A villager damaged his vehicle and then, of course, abandoned it. Please find it and take it to the overpass at the service station.\",TASKS\nRU_02_02_LOST_CARGO_03_TSK,Lost Light Trailer,_TASKS,RU_02,Taymyr,truckDelivery,,190,1450,\"How do I cope with all these lost vehicles you ask? I don't. At my last check-up, I burst the blood pressure monitor. My only hope is for you to find the lost trailer and take it to the sawmill's parking.\",TASKS\nRU_02_02_DAMAGED_TRUCK_03_TSK,Aquatic,_TASKS,RU_02,Taymyr,exploration,,230,2050,Nope. It's broken and won't budge. I think I would have more chance pulling the mythical sword from the mythical stone! But first we need to repair this vehicle. Could you help us?,TASKS\nRU_02_02_STUCK_TRUCK_04_TSK,Help Cable,_TASKS,RU_02,Taymyr,truckDelivery,,260,2650,Deliver to the <strong>House</strong>:,TASKS\nRU_02_02_LOST_CARGO_04_TSK,Drowned Bricks,_TASKS,RU_02,Taymyr,cargoDelivery,1\u00d7 Bricks,190,1600,\"Am I being pranked? Seriously. Another shipment of bricks has been lost on its way to the warehouse. If I was a betting man I'd say it was sabotage. Get out there, find me that cargo and get it to the warehouse!\",TASKS\nRU_02_02_DAMAGED_TRUCK_04_TSK,SOS,_TASKS,RU_02,Taymyr,exploration,,190,1450,\"Alexander, the local inventor, decided to try and increase speed and efficiency of his truck with some \"\"simple\"\" modifications. Upon testing it he ploughed off the road and blew the engine. Would you mind taking a look?\",TASKS\nRU_02_02_STUCK_TRUCK_05_TSK,Farm Swim,_TASKS,RU_02,Taymyr,truckDelivery,,180,1200,One of the farm's trucks got stuck in the swamp while making a delivery. Can you pull it out and take it to the farm?,TASKS\nRU_02_02_LOST_CARGO_05_TSK,Fallen Bricks,_TASKS,RU_02,Taymyr,cargoDelivery,1\u00d7 Bricks,200,1250,\"The tales get taller and taller. I now, apparently, have a pallet of bricks sat under a bridge because the residents \"\"accidentally dropped\"\" them over the side. Get me those bricks!\",TASKS\nRU_02_02_HUB_RECOVERY,\"Hub Recovery, 1st Stage\",_CONTRACTS,RU_02,Taymyr,truckDelivery,,290,2700,In the center of the map you'll see an inactive maintenance station. To get it working again we first have to bring a tank of fuel in for the operations crew and site machinery. I'd appreciate if you'd prioritize this.,CONTRACTS\nRU_02_02_LOG_FARM_SUPPLY_OBJ,East Wind,_CONTRACTS,RU_02,Taymyr,cargoDelivery,6\u00d7 Logs Medium,1470,16800,\"We've received a supply request from the local farm for another batch of lumber. You know, winter is coming and all that. Might want to ask around one of the log stations near the village.\",CONTRACTS\nRU_02_02_LOG_LUMBERMILL_RESTOCK_OBJ,Up And Running,_CONTRACTS,RU_02,Taymyr,cargoDelivery,3\u00d7 Logs Long,1170,10150,\"The company that usually restocks our sawmill is having some logistical issues, again. They won't have a spare truck available for a while, and the mill is almost out of materials to work with. Can you help?\",CONTRACTS\nRU_02_02_GORLAG_CLEANING,Radioactive Fossils,_CONTRACTS,RU_02,Taymyr,cargoDelivery,4\u00d7 Radioctive; 2\u00d7 Radioctive,730,6900,\"You're not planning to start a family, right? We've got several containers that hold some equipment with radioactive components. I need you to move them so we don't wind up with any unfortunate accidents.\",CONTRACTS\nRU_02_02_RADAR_TOWER_RECOVERY,Looking Beyond The Horizon,_CONTRACTS,RU_02,Taymyr,cargoDelivery,1\u00d7 Metal Planks; 2\u00d7 Service Spare Parts; 1\u00d7 Container Small,710,6150,\"How do they expect me to do business here? I can't even talk to the neighboring territories because there's no radio tower! Please, would you take a look at the old tower and see if it can be repaired?\",CONTRACTS\nRU_02_02_MAZUT_DELIVERY,Dirty Deeds,_CONTRACTS,RU_02,Taymyr,cargoDelivery,2\u00d7 Barrels Oil; 1\u00d7 Barrels Oil; 1\u00d7 Barrels Oil,560,5750,\"A shipment of oil has been left at the railway station. No one has turned up to transport them to their final destinations so now I have to deal with it, which means you have to deal with it!\",CONTRACTS\nRU_02_02_FARM_SUPPLY,Cycle of Cargo,_CONTRACTS,RU_02,Taymyr,cargoDelivery,2\u00d7 Bags; 2\u00d7 Wooden Planks; 1\u00d7 Concrete Slab,510,4800,\"The local farmers are trying to expand their operations and they're all flustered. They need a variety of supplies, and fast. As a reward for helping them out, they're offering some money.\",CONTRACTS\nRU_02_02_WOODEN_PLANKS_DELIVERY,Wooden Planks Order,_CONTRACTS,RU_02,Taymyr,cargoDelivery,5\u00d7 Wooden Planks,260,2750,An order has come through for a large number of wooden planks. Pick up the shipment at the sawmill and take them to the railway station so they can be sent to Norilsk.,CONTRACTS\nRU_02_02_RESEARCH,Geological Exploration,_CONTRACTS,RU_02,Taymyr,exploration,,570,9950,We've got a contract from a fuel company that would like to conduct a soil survey to assess mineral content. Perform a seismic triangulation using the specified locations.,CONTRACTS\nRU_02_02_STUCK_TRUCK_TSK,Saving Country Bread,_TASKS,RU_02,Taymyr,truckDelivery,,240,2350,\"Dostoyevsky said there is nothing more positive than bread. I say, there is nothing worse than a truck full of glorious bread that is stuck in the miserable river! Get it out! And bring it to the railway station!\",TASKS\nRU_02_02_CONTAINER_DELIVERY_CNT,Container Delivery,_CONTESTS,RU_02,Taymyr,cargoDelivery,1\u00d7 Container Small,150,1600,It's one headache after another today! The lumber mill workers have been working like madmen and they've broken all their saw blades! Get them new equipment immediately!,CONTESTS\nRU_02_02_BARRELS_DELIVERY_CNT,Barrels Delivery,_CONTESTS,RU_02,Taymyr,cargoDelivery,2\u00d7 Barrels,150,1550,\"The lumber mill is working around the clock to complete an order. It's all last minute, which means they didn't build up fuel reserves. They'll lose power soon if they don't get more fuel. Get up there, fast!\",CONTESTS\nRU_02_03_LOG_SAWMILL_RESTOCK_OBJ,Test Flight,_CONTRACTS,RU_02,Taymyr,cargoDelivery,3\u00d7 Logs Long,1530,17700,\"The old sawmill seems to be in a decent shape, all things considered. With just a bit of work we should be able to get it to work again, and to test our progress we will need a bit of raw materials of decent quality.\",CONTRACTS\nRU_02_03_UPD_PORTSIDE_RESUPPLY_OBJ,Supply and demand,_CONTRACTS,RU_02,Taymyr,cargoDelivery,2\u00d7 Concrete Slab; 4\u00d7 Vehicles Spare Parts,1320,12200,\"Now that we're set here for the long haul, we're considering repurposing some of the old factories here to suit our current needs. Which means we could use some help getting them fixed and fully stocked.\",CONTRACTS\nRU_02_03_DERRICK_DELIVERY_OBJ,Derrick Delivery,_CONTRACTS,RU_02,Taymyr,cargoDelivery,1\u00d7 Big Drills,1080,8900,\"I have an urgent contract for you. We are three days behind schedule which means I'm getting phone calls from management every thirty minutes. I need an oil rig delivered to the following location, immediately!\",CONTRACTS\nRU_02_03_PIER_RECOVERY_OBJ,Pier Recovery,_CONTRACTS,RU_02,Taymyr,cargoDelivery,2\u00d7 Concrete Slab; 1\u00d7 Container Large,1090,8250,We'd like to restore a pier in the Zimnegorsk town to enable us to bring shipments in by boat. Can you take materials to the site so the crew can get to work?,CONTRACTS\nRU_02_03_DRILLING_EQUIPMENT_DELIVERY_OBJ,Drilling Equipment Delivery,_CONTRACTS,RU_02,Taymyr,cargoDelivery,1\u00d7 Container Large; 2\u00d7 Container Small,1050,7550,We've been approved for a new drilling operation nearby. We want to get construction of the rig started as soon as possible. Please transport the necessary equipment there in good time.,CONTRACTS\nRU_02_03_GARAGE_AND_WAREHOUSE_RESTORATION_OBJ,Key Buildings Restoration,_CONTRACTS,RU_02,Taymyr,cargoDelivery,2\u00d7 Concrete Slab; 1\u00d7 Barrels,840,7500,There is a neglected Warehouse in Zimnegorsk that has been abandoned for many years. We have decided to take over ownership and put it to use. Can you deliver the materials we need to begin renovations?,CONTRACTS\nRU_02_03_CONTRACT_SCAN_POINTS_OBJ,Geological Research,_CONTRACTS,RU_02,Taymyr,exploration,,260,4550,There used to be an oil drilling operation here but it was abandoned when extraction became difficult. We believe there may still be reserves that have not been fully exploited. Can you investigate?,CONTRACTS\nRU_02_03_TASK_FIND_THE_CAR_OBJ,Abandoned Car,_TASKS,RU_02,Taymyr,truckDelivery,,200,1600,\"A drunken local decided to go for a drive into the wilderness. He claims that he was abducted by aliens, experimented on and then returned to his bed. He'd be grateful if you could find his car.\",TASKS\nRU_02_03_TASK_SEARCH_OBJ,Lost Instruments,_TASKS,RU_02,Taymyr,cargoDelivery,4\u00d7 Crate Large,930,6350,\"The renovation of the Zimnegorsk pier was going beautifully, until a shipment of essential equipment was lost in transit. Please find this equipment and deliver it to the pier.\",TASKS\nRU_02_03_SAWMILL_RECOVERY_OBJ,Sawmill,_TASKS,RU_02,Taymyr,cargoDelivery,1\u00d7 Container Large; 2\u00d7 Metal Planks,850,5450,We have a plan to revive an old Sawmill here in Zimnegorsk. In order to achieve this we will need various construction materials and equipment. Can you get these safely delivered to us?,TASKS\nRU_02_03_TASK_BUILD_BRIDGE_OBJ,Zimnegorsk Bridge Recovery,_TASKS,RU_02,Taymyr,cargoDelivery,2\u00d7 Concrete Slab,530,3950,\"Deliver some materials from Quarry, that will let us recover bridge in Zimnegorsk.\",TASKS\nRU_02_03_TASK_METAL_DELIVERY_OBJ,Metal Delivery,_TASKS,RU_02,Taymyr,cargoDelivery,2\u00d7 Metal Planks,390,2300,\"I have a straightforward assignment for you, my friend. Take a shipment of metal from factory to warehouse. What do you say?\",TASKS\nRU_02_03_TASK_FIND_THE_TRUCK_OBJ,Truck Recovery,_TASKS,RU_02,Taymyr,exploration,,370,4550,You seem to be a very resourceful person. There's an abandoned truck in the north swamp that has been there for some time. Maybe you could fix it and find a use for it?,TASKS\nRU_02_03_TASK_DOCUMENTARY_OBJ,Documentary Film,_TASKS,RU_02,Taymyr,exploration,,50,850,It seems an American crowdfunding project has raised the money to make a documentary about Zimnegorsk. What is the world coming to? The cameraman wants to get some shots in the mountain. Can you get him up there?,TASKS\nRU_02_03_CONTEST_METAL_DELIVERY_OBJ,Oilfield Delivery,_CONTESTS,RU_02,Taymyr,cargoDelivery,2\u00d7 Metal Planks,300,2100,\"The Oilfield lacks metal, and I do not. This is what you call supply and demand. You are the bridge between us. Get this metal to it's buyer as fast as possible and you will be rewarded well.\",CONTESTS\nRU_02_03_CONTEST_BARRELS_DELIVERY_OBJ,Cement Delivery,_CONTESTS,RU_02,Taymyr,cargoDelivery,4\u00d7 Bags,280,1750,If Alexander was not my cousin I would have fired him a long time ago. He lost a load of bags with cement in the river and now I have to deal with it. Either get them back or replace them. I don't care which.,CONTESTS\nRU_02_03_CONTEST_WOODEN_DELIVEY_PIRS_OBJ,Pier Delivery,_CONTESTS,RU_02,Taymyr,cargoDelivery,4\u00d7 Wooden Planks,110,1050,\"The pier is almost finished but it seems half the wood shipment they received was rotten. They need more wood, fast!\",CONTESTS\nRU_02_03_CONTEST_WOODEN_DELIVEY_WAREHOUSE_OBJ,Warehouse Delivery,_CONTESTS,RU_02,Taymyr,cargoDelivery,4\u00d7 Wooden Planks,100,950,\"How much wood would a woodchuck chuck if a woodchuck could chuck wood? I don't know, but I hope you can chuck a lot of it over to the warehouse right away!\",CONTESTS\nRU_02_04_MINES_EXPLORATION_OBJ,Datamining,_CONTRACTS,RU_02,Taymyr,truckDelivery,,340,3300,\"The abandoned mine nearby was at some point used as a testbed for testing experimental mining tech. While the tech is now outdated, the local authorities would still like to have a look at the documents sealed there.\",CONTRACTS\nRU_02_04_UPD_VILLAGE_RESTOCK_OBJ,Loud and clear,_CONTRACTS,RU_02,Taymyr,cargoDelivery,2\u00d7 Vehicles Spare Parts,610,7950,\"With the base station restored, it might be a good idea to keep a stock of replacement parts somewhere nearby. We've cleaned up some space at the local lumber yard, it should be a safe enough storage for our equipment.\",CONTRACTS\nRU_02_04_MAST_RESTORATION_OBJ,Uplink,_CONTRACTS,RU_02,Taymyr,cargoDelivery,2\u00d7 Wooden Planks; 2\u00d7 Metal Planks,960,7750,\"A base station in this region could be a viable asset for our drilling operations in this region. Restoring it might be tricky, but we're sure you've handled things like this before.\",CONTRACTS\nRU_02_04_NORTH_ROAD_EXPLORATION_OBJ,Pathfinder,_CONTRACTS,RU_02,Taymyr,exploration,,80,1300,\"The road we're using to reach this area now isn't well-suited for trucks, but there is another road heading off from the village to the North. We need you to check if it's more accessible than our current path.\",CONTRACTS\nRU_02_04_RECOVER_LOAF_TSK,The Great Nomad,_TASKS,RU_02,Taymyr,truckDelivery,,590,5050,\"The local farm might not have been the pinnacle of motorized industry, but it still won't hurt to check if they might still have some salvageable vehicles there. Fix anything you find and tow it to the service station.\",TASKS\nRU_02_04_REFUEL_TRUCK_TSK,Ol' Faithful,_TASKS,RU_02,Taymyr,truckDelivery,,520,4450,\"According to some papers we got on the old mine, there should be at least one truck the workers used for hauling some of their equipment. Find it, and see if you can get it to work.\",TASKS\nRU_02_04_MAST_FUELSTATION_TSK,Covering All Fronts,_TASKS,RU_02,Taymyr,truckDelivery,,360,3550,\"The station on the hill is our main priority in this area, and we'd like to equip a small fuel depot next to it. Retrieve a fuel tank and bring it to the base station.\",TASKS\nRU_02_04_RECOVER_APC_TSK,Pacifist,_TASKS,RU_02,Taymyr,truckDelivery,,470,2950,\"Now that is not something you see often. A repurposed armored carrier just lying out here, waiting to be recovered. Seems like it might be stuck down there. Pull it out and fix it, and you get to keep it.\",TASKS\nRU_02_04_DELIVER_SUPPLY_TRAILER_TSK,Lost And Found,_TASKS,RU_02,Taymyr,truckDelivery,,320,2850,\"We're setting up a supply depot just by the lake, and one of our workers'd caught a glimpse of an abandoned trailer up in the hills. Find it, and take it to the depot.\",TASKS\nRU_02_04_RESUPPLY_TSK,In With The New,_TASKS,RU_02,Taymyr,cargoDelivery,1\u00d7 Bags; 1\u00d7 Bags; 1\u00d7 Bags; 1\u00d7 Bags,1260,7100,The nearby village had definitely seen better days. We do have some supplies we could spare to help making this place more habitable.,TASKS\nRU_02_04_REPAIR_ZIKZ_TSK,Finders Keepers,_TASKS,RU_02,Taymyr,exploration,,200,1550,\"One of our men spotted a lone truck just by the lake. If you could fix it up, we could really put it to good use here.\",TASKS\nRU_02_04_EXAMINE_CAMP_TSK,Unexpected Guests,_TASKS,RU_02,Taymyr,exploration,,90,1550,Places like these seem to attract a good bit of attention from thrillseekers and scavengers alike. It seems like someone'd set up camp in the forest nearby. Head down there and see if they're still here.,TASKS\nRU_02_04_RIFT_MAPPING_CNT,Landslide Mapping,_CONTESTS,RU_02,Taymyr,exploration,,50,850,We need to map out the rift created by the landslide. Navigate the rift it created and visit all areas marked on your map.,CONTESTS\nRU_03_01_POWERLINE_1_REPAIR_TSK,Temporary Measures,_TASKS,RU_03,Kola Peninsula,cargoDelivery,1\u00d7 Metal Planks,460,3700,\"Nothing is more permanent than something temporary, right? Have been patching up this rusty pole, but what we really need is a new one... Next time maybe... Please get us some beams and we shall fix it again.\",TASKS\nRU_03_01_POWERLINE_3_REPAIR_TSK,Unlucky Number,_TASKS,RU_03,Kola Peninsula,cargoDelivery,1\u00d7 Service Spare Parts,280,3000,\"And then they tell me not to believe in superstitions! Every single time number thirteen is out of order. Looks like the isolators are broken, again. Go get some spares, and I will later climb and see what can be done.\",TASKS\nRU_03_01_CAMP_SETUP_OBJ,Cabin Fever,_CONTRACTS,RU_03,Kola Peninsula,truckDelivery,,610,6100,\"Hello. We are preparing an expedition up North, so we are moving some of the supplies from our satellite camps to the main one. That includes a couple of containers we need help with.\",CONTRACTS\nRU_03_01_MOBILE_WATCHTOWER_RECOVERY_OBJ,Forgotten Prototype,_CONTRACTS,RU_03,Kola Peninsula,truckDelivery,,450,4750,\"The local factory used to produce instruments for mining, and per the documents a couple of interesting devices were never removed from here. This place is abandoned, so the equipment may prove really useful.\",CONTRACTS\nRU_03_01_LOG_VILLAGE_DELIVERY_OBJ,Log Of Discord,_CONTRACTS,RU_03,Kola Peninsula,cargoDelivery,3\u00d7 Logs Medium,960,10250,\"When we arrived, some locals weren't very excited by our presence. We've agreed to help them with winter supplies to smooth things out. Now we only need someone to help with logistics. Would you kindly lend us a hand?\",CONTRACTS\nRU_03_01_BUNKER_OBJ,Sites of military glory,_CONTRACTS,RU_03,Kola Peninsula,exploration,,340,5800,\"You probably could not tell right away, but almost a century ago this place was busy with preparation for defense. As we are setting off to a serious expedition, why not look around and feel the team spirit?\",CONTRACTS\nRU_03_01_POWERLINE_INSPECTION_OBJ,Tense Situation,_CONTRACTS,RU_03,Kola Peninsula,exploration,,230,3950,\"Ah, you are here? So we start early. We had a number of power outages at the Sub-station, we fear we might end up in the dark. We need to check out the stations and the lines, and the sooner we do it, the better.\",CONTRACTS\nRU_03_01_SWAMP_ROUTE_CHECK_OBJ,Rescuer\u00a0\u2014 marshes,_CONTRACTS,RU_03,Kola Peninsula,exploration,,210,3550,\"The folks here are quite fearless, and it never occurs to them that by the end of winter the ice may be too thin. Of course it falls to me to rescue them from ice floes. I have to check if anyone has already drowned.\",CONTRACTS\nRU_03_01_NORTH_ROUTE_CHECK_OBJ,Rescuer\u00a0\u2014 the North Tributary,_CONTRACTS,RU_03,Kola Peninsula,exploration,,190,3200,\"You know, frozen marshes are not much safer than rivers, especially when it gets warmer. Falling through the ice is no fun. You should check if anyone is already stuck in the South Marsh.\",CONTRACTS\nRU_03_01_ROUTE_CHECK_OBJ,Almost Afloat,_CONTRACTS,RU_03,Kola Peninsula,exploration,,150,2550,\"Each winter we have to fight the local historians. We need a good way to the docks for our ships, while they are still using ice for traveling. We suggest marking the strong ice and the weak ice, so everyone is happy.\",CONTRACTS\nRU_03_01_SNOWED_IN_TSK,Knee-deep,_TASKS,RU_03,Kola Peninsula,truckDelivery,,680,7300,\"I rode to the Ranger's Cabin the other day but I overestimated myself. I did make it, but damaged my suspension and got stuck. I hardly made it out on foot, but I have no idea what to do with my car. You are my only hope.\",TASKS\nRU_03_01_ZIKZ_RECOVERY_TSK,Like Brothers,_TASKS,RU_03,Kola Peninsula,truckDelivery,,610,6100,\"Hey, could you help me here? I brought some boards to the Meteo Station, but I got stuck and ruined half on my vehicle. Could you get me to the mechanic please? Be a friend, OK?\",TASKS\nRU_03_01_CAMP_GAS_DELIVEY_TSK,Pit-stop,_TASKS,RU_03,Kola Peninsula,truckDelivery,,410,4450,\"When it is cold like this, fuel is just about as important as food. No light, no heat, even no communication maybe. Firewood may be an option but it is much better to just haul that tanker trailer over here.\",TASKS\nRU_03_01_FOREIGNER_TSK,Foreigner,_TASKS,RU_03,Kola Peninsula,truckDelivery,,410,4100,There is this foreign tourist. He really wants to see some live history. Ended up almost going under the ice. Will you help me pull him out? I will make it worth your time.,TASKS\nRU_03_01_ANTIQUE_TRUCK_HUNT_TSK,Self Contained Antique,_TASKS,RU_03,Kola Peninsula,truckDelivery,,350,3050,\"A friend who works at the power plant spotted an old TUZ nearby. That guy knows history well, so he says they used it to drive an officer during the war. What would you say if we towed that TUZ right to his workshop?\",TASKS\nRU_03_01_ON_THIN_ICE_TSK,On Thin Ice,_TASKS,RU_03,Kola Peninsula,truckDelivery,,300,2550,\"That is some bad luck! I drove my SUV to the marshes, and the engine died. I don't know a thing about cars, so I had to walk. Please do me a favor. If you could tow me to the Guard House, and I will take it from there.\",TASKS\nRU_03_01_CAMP_RESUPPLY_TSK,Supplies Delivery,_TASKS,RU_03,Kola Peninsula,cargoDelivery,1\u00d7 Service Spare Parts; 1\u00d7 Service Spare Parts,650,8350,\"Great timing! We need to transfer supplies to the other two camps and we were at a loss. It's not much, a couple of crates full of stuff, instruments, spare parts, a little bit of this and that... Will you do it?\",TASKS\nRU_03_01_BASE_SUPPLIES_TSK,Army Reserve,_TASKS,RU_03,Kola Peninsula,cargoDelivery,1\u00d7 Radioctive,450,6250,\"Here goes. There is a bunker up North, not like one of the coastal ones, no, this one is newer. A friend of mine spotted an interesting looking box there. Who knows what may be in it. Will you go there and take a look?\",TASKS\nRU_03_01_BRIDGE_REPAIR_B_TSK,Old Crossing,_TASKS,RU_03,Kola Peninsula,cargoDelivery,1\u00d7 Metal Planks,460,4850,\"Bridges are not as critical in winter, but as the ice melts, all of the northern shore will be cut off from the harbor. That has to be fixed.\",TASKS\nRU_03_01_MILITARY_SUPPLY_TSK,Abandoned Convoy,_TASKS,RU_03,Kola Peninsula,cargoDelivery,1\u00d7 Barrels Oil,390,4750,\"A friend was exploring empty houses in the North, and he saw a truck. Not a usual one, it looks like an Army Trailer Train that has been there forever. If you could get me the barrels from it, you can keep the rest.\",TASKS\nRU_03_01_HELI_CARGO_RECOVERY_TSK,Inclement Weather,_TASKS,RU_03,Kola Peninsula,cargoDelivery,1\u00d7 Vehicles Spare Parts,370,3800,\"Back in the Soviet times, a helicopter crashed here. Ran out of fuel or just broke. Ended up landing in the woods. And they had some supplies on board that are probably not too interesting for you, but I could use them.\",TASKS\nRU_03_01_METEO_RESTORE_TSK,Forecast Unclear,_TASKS,RU_03,Kola Peninsula,cargoDelivery,1\u00d7 Wooden Planks,250,3250,\"We were revising our stock and found some boards. Probably intended for the old Meteo Station up in the mountains, but the driver lost contact with us, so we forgot about it. Can you help?\",TASKS\nRU_03_01_LOG_CABIN_REPAIR_TSK,Shabby Guard House,_TASKS,RU_03,Kola Peninsula,cargoDelivery,1\u00d7 Wooden Planks,210,2500,\"There was a Guard House here, just a small house to wait out the rain. I see it has got really shabby recently. If you get us some boards, I will do what I can to repair it, okay?\",TASKS\nRU_03_01_RURAL_SPELUNKER_TSK,Rural Spelunker,_TASKS,RU_03,Kola Peninsula,exploration,,560,9800,\"There are plenty of houses here that have been empty since the war. You never know what you may find in those places, a flask, a helmet or some personal belongings. If you see an interesting spot\u00a0- please let me know.\",TASKS\nRU_03_01_FACTORY_CHECK_TSK,Factory Skyline,_TASKS,RU_03,Kola Peninsula,exploration,,210,3650,\"These deserted places have a charm of their own, don't you think? I even used to work here, some thirty years ago... Never mind. It used to be a factory. Let us go take a look how it's all holding up.\",TASKS\nRU_03_01_METEO_CHECK_TSK,Dead Calm,_TASKS,RU_03,Kola Peninsula,exploration,,180,3150,\"Not so long ago we were going to fix one of the Meteo Stations, so we sent over a cargo of boards and some workers. Never heard from them since. Could you go over there and see how things are?\",TASKS\nRU_03_01_CAMP_ROUTE_CHECK_TSK,Trackwalker,_TASKS,RU_03,Kola Peninsula,exploration,,170,3000,\"People may say what they want, but it is so much easier working here in winter\u00a0- you do not drown in the mud, you can cross the river just about anywhere. By the way, would you like to check our route to the North Camp?\",TASKS\nRU_03_01_VILLAGE_SCOUTING_TSK,Frozen in Time,_TASKS,RU_03,Kola Peninsula,exploration,,130,2150,\"I think you can help. It has been fifty years or so since I left this area. There was a village... I know, it's empty, but I would like to know whether any of the houses are still standing, or did we leave no trace...\",TASKS\nRU_03_01_NORTH_RIVER_RACE_CNT,Battle on the Ice,_CONTESTS,RU_03,Kola Peninsula,exploration,,70,1250,\"The locals love this game: they go in circles on the ice around my shop. I will never understand it, but you can try for yourself. Who knows, you might even like it.\",CONTESTS\nRU_03_01_OFF_THE_RAILS_CNT,Overturned Train Carriage,_CONTESTS,RU_03,Kola Peninsula,exploration,,70,1150,\"The historians from the nearby camp have been asking me for some time to see what kind of railway carriages tipped over not far from here, and I kinda forgot. Could you do that for me and then let them know?\",CONTESTS\nRU_03_02_LANDSLIDE_1_TSK,Magnitude of 5,_TASKS,RU_03,Kola Peninsula,cargoDelivery,2\u00d7 Barrels,390,3750,\"We have an emergency. Earthquakes are common here, but the last one struck real hard. The road to the power station is blocked, and we've got nothing to clear it with. Even our last tractor is undergoing repair.\",TASKS\nRU_03_02_CAMPUS_1_TSK,The Aftermath of Hurricane,_TASKS,RU_03,Kola Peninsula,exploration,,250,4350,\"We had a hurricane yesterday, it wrecked stuff all over the place. Can you drive along the route and see if it can be traversed?\",TASKS\nRU_03_02_LANDSLIDE_2_TSK,Landslide in the Mountains,_TASKS,RU_03,Kola Peninsula,cargoDelivery,1\u00d7 Container Small,490,5600,\"The weather's been really mean as of late. The coastal road has been blocked by landslides again, and we need to deliver supplies to the workers. If you are headed in that direction, could you take a crate along?\",TASKS\nRU_03_02_AIRPLANE_CNT,The Star in the Sky,_CONTRACTS,RU_03,Kola Peninsula,truckDelivery,1\u00d7 Big Wing; 1\u00d7 Plane; 1\u00d7 Wing,1910,18650,\"Back in '41, one pilot got shot down not far from here, but managed to survive the crash and return to his comrades on foot. We would like you to recover what's left of his plane. Oh, and you would need a trailer and a crane for that.\",CONTRACTS\nRU_03_02_LOG_WAREHOUSE_SUPPLY_OBJ,Strategic Reserve,_CONTRACTS,RU_03,Kola Peninsula,cargoDelivery,3\u00d7 Logs Long,1470,16800,\"The last few weeks were exceptionally cold, and while we're not out of wood yet, we did dig into our emergency supplies. We ordered another batch of logs from the sawmill, but someone needs to actually deliver them.\",CONTRACTS\nRU_03_02_TANK_CNT,The Price of Victory,_CONTRACTS,RU_03,Kola Peninsula,cargoDelivery,1\u00d7 Ba20; 1\u00d7 Ba20,1600,15350,\"During the war, our forces were on the offensive in this area. Not everyone reached the objective\u2014some armored cars were lost in the snow. Try to find and recover them. And do pack a crane with you, you will need it.\",CONTRACTS\nRU_03_02_LOG_CAMP_DELIVERY_OBJ,Power To The Masses,_CONTRACTS,RU_03,Kola Peninsula,cargoDelivery,3\u00d7 Logs Medium,980,11400,\"The low-voltage power lines around here have definitely seen better days. A lot of the poles are barely standing, what's with the wind and all. If we had some spare logs we could start reinforcing the worst offenders.\",CONTRACTS\nRU_03_02_ARCH_CNT,Forgotten civilization,_CONTRACTS,RU_03,Kola Peninsula,exploration,,540,9450,\"According to journals from past expeditions, this place houses traces of an ancient civilization, many centuries old. Let me know if you find something unusual\u2014we'd be delighted to take a look!\",CONTRACTS\nRU_03_02_BUNKER_CNT,Places of Military Glory,_CONTRACTS,RU_03,Kola Peninsula,exploration,,410,7150,\"It's been a while since we went that far. According to the maps, there were coastal batteries deployed here to defend against enemy landings during the war. We need to investigate their current condition.\",CONTRACTS\nRU_03_02_ANTIQUES_TSK,Old Scrap Metal,_TASKS,RU_03,Kola Peninsula,truckDelivery,,520,6450,\"There are plenty of samovars to be found inside the abandoned village on the island. That's some good money to be made, why don't you go and collect some?\",TASKS\nRU_03_02_NEW_FISHING_TSK,New Pool,_TASKS,RU_03,Kola Peninsula,truckDelivery,,400,3900,Folks speak of a king-size catfish living somewhere to the south. Could you bring my stuff there?,TASKS\nRU_03_02_GARAGE_REPAIR_TSK,Engine Troubles,_TASKS,RU_03,Kola Peninsula,truckDelivery,1\u00d7 Vehicles Spare Parts,440,3350,\"Curse the day when I decided to drive this jalopy. Could you tow it to the garage, please?\",TASKS\nRU_03_02_FISHERCAMP_TSK,The Floating Change House,_TASKS,RU_03,Kola Peninsula,truckDelivery,,350,2700,\"I am an idiot. I left my cabin parked on the ice. Now the river's thawed, and my cabin's sailing the high seas. Can't work without it. Think you could help, eh?\",TASKS\nRU_03_02_CAR_DELIVERY_TSK,Nighttime Delivery,_TASKS,RU_03,Kola Peninsula,truckDelivery,,320,2550,\"Listen up, I've got a job for you. There this guy out there, and he wants a certain car. Beats me why, but it's none of my business\u2014if he wants a certain motor, he's got his reasons. Just keep your mouth shut. Deal?\",TASKS\nRU_03_02_LOST_CAR_TSK,Troublesome Motor,_TASKS,RU_03,Kola Peninsula,truckDelivery,,320,2500,\"Look, yesterday me and my pals were wheeling around on the ice and ended up stuck in the pressure ridges. Care to pull the car free, eh?\",TASKS\nRU_03_02_TRAILER_CARGO_DELIVERY_TSK,Lost Trailer,_TASKS,RU_03,Kola Peninsula,truckDelivery,,280,1900,\"I ordered a new trailer to help moving a few hives around, but it got stuck on the way here. Can you pull it out? It's not a long way off.\",TASKS\nRU_03_02_SUPPLIES_TSK,The Riddle of Geology,_TASKS,RU_03,Kola Peninsula,cargoDelivery,1\u00d7 Container Small,510,5950,\"We're camping here for a week, but the supplies clearly won't last as long. Bring us the goods please? We'll owe ya one.\",TASKS\nRU_03_02_WAREHOUSE_TSK,Broken Power Station,_TASKS,RU_03,Kola Peninsula,cargoDelivery,2\u00d7 Metal Planks,700,4900,\"It's getting crazy. Lines get broken, transformers get flooded, poles fall... We've got spare people at the substation and yet no spare parts to fix it all. By the way, you mind helping me get some components there?\",TASKS\nRU_03_02_POWER_PLANTS_TSK,Repairing Power Lines,_TASKS,RU_03,Kola Peninsula,cargoDelivery,1\u00d7 Wooden Planks; 1\u00d7 Wooden Planks,490,4300,\"We're knee-deep in trouble again. Last night's wind was brutal, tore the power lines to shreds. Can you go and take a look while I'll get the garage running for you?\",TASKS\nRU_03_02_METEO_TSK,Forecast for Tomorrow,_TASKS,RU_03,Kola Peninsula,exploration,,650,11300,Our weather sensors are acting up. They flat out refuse to transmit data. Can you go and manually collect their measurements?,TASKS\nRU_03_02_VILLAGE_TSK,A Picture to Remember,_TASKS,RU_03,Kola Peninsula,exploration,,300,5250,\"I hail from the city\u2014been living there for many years now. Yet I grew up around these parts, and I'd like to take some pictures\u2014a memento I could show to my kids one day. Care to help?\",TASKS\nRU_03_02_ICE_ROAD_TSK,Everything for the Pictogram,_TASKS,RU_03,Kola Peninsula,exploration,,280,4850,\"I came here all the way from Moscow. They say there's an island full of old architecture. Can't reach it though, but I'd really like to take some photos. Care to take some using my camera?\",TASKS\nRU_03_02_OLD_SHIP_TSK,A Keepsake,_TASKS,RU_03,Kola Peninsula,exploration,,280,4800,\"I've got a peculiar customer. He wants to have a memento from a destroyer, and we just so happen to have one moored nearby. Can you bring one? I'll make it worth your while.\",TASKS\nRU_03_02_LIGHTHOUSE_TSK,Old Lighthouse,_TASKS,RU_03,Kola Peninsula,exploration,,210,3550,\"Nice timing there! I've got an idea. Back in the day, there used to be a lighthouse around these parts, before people left. So, I'm thinking of bringing it back online. Can you check if there's a way to reach it?\",TASKS\nRU_03_02_GARBAGE_CNT,Dirty business,_CONTESTS,RU_03,Kola Peninsula,cargoDelivery,1\u00d7 Metal Planks,220,1600,\"I have a job for you, if you're game. There is a pile of metal scrap not far from here. I wish I could take it all to the Recycling Station, but I don't have a vehicle for that. We could split the profit. You in?\",CONTESTS\nUS_04_01_CONV_01_CONT,Conveyor Belt Section 1,_CONTRACTS,US_04,Yukon,cargoDelivery,4\u00d7 Crate Large; 2\u00d7 Barrels; 2\u00d7 Metal Planks; 1\u00d7 Forklift Caravan Container 2,1630,11000,\"To get the ore to the sorting building we need to construct the conveyor. Warning: a conveyor section will partially obstruct the path. Also, the Cabin Zone 2 needs to be outfitted with prefab cabins.\",CONTRACTS\nUS_04_01_TSK_01,Abandoned Trailer,_TASKS,US_04,Yukon,truckDelivery,,460,4600,\"Hello! We have just moved a little uphill from here, but we left a trailer with fuel at the old place. Will you help us retrieve it?\",TASKS\nUS_04_01_TSK_BRIDGE_01,North Bridge,_TASKS,US_04,Yukon,cargoDelivery,2\u00d7 Metal Planks,650,4100,This bridge was broken by the flood. You have to help restore it if you want to use this shortcut.,TASKS\nUS_04_01_TSK_ROCKS_01,Fallen Rocks,_TASKS,US_04,Yukon,cargoDelivery,2\u00d7 Crate Large,450,3550,The rocks have fallen into the railway tracks. Help clear the debris to reopen the path.,TASKS\nUS_04_01_DOWNHILL_01_CNT,Down!,_CONTESTS,US_04,Yukon,exploration,,40,700,\"Hello, we sometimes race here with the guys. Try it!\",CONTESTS\nUS_04_01_CONV_02_CONT,Conveyor Belt Section 2,_CONTRACTS,US_04,Yukon,cargoDelivery,2\u00d7 Blocks; 4\u00d7 Pipes Small; 4\u00d7 Wooden Planks,1510,14650,\"Ore must be delivered to the sorting site as soon as possible, Conveyor Belt Section 2 is waiting. Attention: construction partially blocks the way.\",CONTRACTS\nUS_04_01_TSK_02,Abandoned Truck,_TASKS,US_04,Yukon,truckDelivery,,360,3300,\"Hello wanderer! I have recently moved from the frosty top of the hill closer to the water, and I had nothing to tow my old broken truck here. Will you help?\",TASKS\nUS_04_01_TSK_BRIDGE_02,Wooden bridge,_TASKS,US_04,Yukon,cargoDelivery,2\u00d7 Wooden Planks,380,4600,\"This bridge was washed away in minutes! We restored part of it, but we need help with the rest.\",TASKS\nUS_04_01_CONV_03_CONT,Conveyor Belt Section 3,_CONTRACTS,US_04,Yukon,cargoDelivery,4\u00d7 Metal Roll; 4\u00d7 Bricks; 1\u00d7 Concrete Slab; 1\u00d7 Forklift Caravan Container 2,1100,8300,\"We need to assemble conveyor section near the sorting building. Warning: a conveyor section will partially obstruct the path. Also, the Cabin Zone 1 needs to be outfitted with prefab cabins.\",CONTRACTS\nUS_04_01_TSK_BRIDGE_03,Western bridge,_TASKS,US_04,Yukon,cargoDelivery,2\u00d7 Metal Planks,660,5250,One of the critically important bridges on the way to the railway station was washed away in the flood. Will you help us restore it?,TASKS\nUS_04_01_TSK_03,Support the Meteorologists,_TASKS,US_04,Yukon,cargoDelivery,1\u00d7 Service Spare Parts,230,1850,\"Oh, you're right on time! We have ordered some spare parts for our equipment, and it was shipped down there. Will you help us get it up here?\",TASKS\nUS_04_01_TSK_04,Flood Aftermath,_TASKS,US_04,Yukon,cargoDelivery,2\u00d7 Service Spare Parts,300,2600,\"The recent flood washed my crates of spare parts down the stream. Help me recover them, and I will pay you back.\",TASKS\nUS_04_01_TSK_05,Don't do a barrel roll!,_TASKS,US_04,Yukon,truckDelivery,1\u00d7 Barrels,780,8550,\"I promised to deliver this trailer with barrels to a friend of mine who lives uphill, but I have no vehicle for that. Can you help?\",TASKS\nUS_04_01_TSK_06,Towage at Height,_TASKS,US_04,Yukon,truckDelivery,,480,5400,\"My car recently broke down, and my friend Joe just happens to know how to fix it. Will you help tow it there?\",TASKS\nUS_04_01_TSK_07,Trailer Whirlpool,_TASKS,US_04,Yukon,truckDelivery,,360,2900,\"Whoa! I was just crossing the bridge when I got distracted by a squirrel and drove this trailer in the water. Please, help me get it to the farm, you're a better fit for this job.\",TASKS\nUS_04_01_TSK_08,Camping Gone Wrong,_TASKS,US_04,Yukon,truckDelivery,,760,7300,That Stepan guy is something else! He crashed the car when he was supposed to get work done. I need your help repairing it. For a reward of course.,TASKS\nUS_04_01_TSK_09,Arrogance,_TASKS,US_04,Yukon,truckDelivery,,310,1950,\"Can you imagine, I made a bet with a friend that I can cross this stream at full throttle. Looks like I lost. Will you help me deliver the car to the house?\",TASKS\nUS_04_01_TSK_10,Uphill Delivery,_TASKS,US_04,Yukon,truckDelivery,2\u00d7 Wooden Planks,760,8500,Mr. Stevenson ordered some wooden planks and this trailer from us. Deliver them to him and he will pay you.,TASKS\nUS_04_01_TSK_11,Failed Delivery,_TASKS,US_04,Yukon,cargoDelivery,4\u00d7 Barrels Oil,480,2950,\"We ordered some fuel, but the delivery truck dropped everything on the frozen lake. Can you help us bring it home?\",TASKS\nUS_04_01_TSK_12,Aid Thy Neighbor,_TASKS,US_04,Yukon,truckDelivery,,350,3050,\"Hi. A friend of mine lives on the far slope of the mountain, and the path to them was blocked by the fallen rocks. We managed to clear most of it, but they need some fuel. Help us deliver this trailer, will you?\",TASKS\nUS_04_01_TSK_13,Lost en Route,_TASKS,US_04,Yukon,cargoDelivery,2\u00d7 Vehicles Spare Parts,330,2400,\"My friend Bill loves building cars. I sent him some crates with spare parts, but not only did he break the truck\u2014he also lost the crates. Will you deliver the crates to him? He will give you the truck if you fix it.\",TASKS\nUS_04_01_TSK_14,Lonely Island,_TASKS,US_04,Yukon,truckDelivery,,420,3600,\"Hi, I am Petro, and my car died right in the middle of this marsh. Will you help me tow it home? Just be careful, the soil here is very weak and hard to drive on.\",TASKS\nUS_04_01_TUBING,Mountaintop Delivery,_CONTRACTS,US_04,Yukon,truckDelivery,,900,10150,Making it possible to work with the soil deep under the mountain will require a special trailer delivered to the mine's opening up above. Here's what you'll need to do.,CONTRACTS\nUS_04_01_MINE_CONT,Reinvigorating The Old Mines,_CONTRACTS,US_04,Yukon,cargoDelivery,10\u00d7 Metal Roll; 4\u00d7 Concrete Slab; 6\u00d7 Barrels; 10\u00d7 Bricks; 2\u00d7 Forklift Caravan Container 2; 6\u00d7 Metal Planks; 8\u00d7 Wooden Planks; 2\u00d7 Pipes Small; 6\u00d7 Blocks; 2\u00d7 Forklift Caravan Container 2,5630,49000,\"Goldhorse Mining made a decision to restore an old gold mine. This would require more resources and workforce, and the latter would need some prefab cabins. Install them at the Cabin Zone 2.\",CONTRACTS\nUS_04_01_SORT_CONT,Ore Sorting Center,_CONTRACTS,US_04,Yukon,cargoDelivery,4\u00d7 Metal Planks; 6\u00d7 Wooden Planks; 4\u00d7 Blocks; 6\u00d7 Crate Large; 2\u00d7 Forklift Caravan Container 2; 6\u00d7 Metal Roll; 2\u00d7 Concrete Slab; 8\u00d7 Barrels; 8\u00d7 Bricks; 2\u00d7 Forklift Caravan Container 2,4340,36200,\"Once the ore is mined, it needs to be sorted to separate the gold from the rocks. This requires a dedicated sorting building, as well as the installation of prefab cabins in the Cabin Zone 1.\",CONTRACTS\nUS_04_01_CRAFT_TUTOR,Cargo Crafting,_CONTRACTS,US_04,Yukon,cargoDelivery,1\u00d7 Forklift Caravan Container 2,150,2500,\"Sometimes, completing construction requires new materials or components to be created. Pick up the container on the railway station, find two sets of wooden planks, and make them into a prefab cabin at the crafting zone.\",CONTRACTS\nUS_04_01_TSK_CAT,Dump Truck for Dump Truck,_TASKS,US_04,Yukon,truckDelivery,,680,5800,\"Hey there! We need someone to get a new disassembled quarry truck over here. We'll put it together ourselves, and you can help yourself to our old one, just don't forget to fuel it up.\",TASKS\nUS_04_01_TSK_FORKLIFT,Manual Unloading Practice,_TASKS,US_04,Yukon,cargoDelivery,1\u00d7 Barrels,50,350,\"Greetings! Here's your task for the day: using the forklift or the crane, load a pallet with barrels onto the platform.\",TASKS\nUS_04_01_RACE_L_CNT,Long ride,_CONTESTS,US_04,Yukon,exploration,,90,1450,\"Hi! How do you like the idea of racing on the big ring in the old dredge works? We call it the Dradge Race, ha! You just have to turn here.\",CONTESTS\nUS_04_01_RACE_S_CNT,Short ride,_CONTESTS,US_04,Yukon,exploration,,60,900,\"Hey, this is Quick Nick! My friends and I have been racing here in the old dredge works. Try and beat our records!\",CONTESTS\nUS_04_02_FACTORY_BLOCK_01,Factory: Main Block,_CONTRACTS,US_04,Yukon,cargoDelivery,6\u00d7 Bricks; 3\u00d7 Concrete Slab; 4\u00d7 Metal Planks; 3\u00d7 Pipes Small; 2\u00d7 Forklift Caravan Container 2; 2\u00d7 Pipe Large,3420,26600,\"Building a whole plant in such conditions is no easy task. We'll need all the resources we can possibly find. Try getting support from the locals, they'll be happy to help.\",CONTRACTS\nUS_04_02_BLOCKAGE_01_TSK,Road Block,_TASKS,US_04,Yukon,cargoDelivery,2\u00d7 Wooden Planks,320,3750,The detour has recently been blocked by a landslide. We need your help now!,TASKS\nUS_04_02_FACTORY_BLOCK_02,Factory: Reservoirs,_CONTRACTS,US_04,Yukon,truckDelivery,4\u00d7 Pipes Small; 4\u00d7 Blocks; 2\u00d7 Concrete Slab; 2\u00d7 Metal Planks; 2\u00d7 Forklift Caravan Container 2,2690,21450,\"Material supplies are way behind schedule, but we need the plant now! This area is full of abandoned structures. Besides, I hear there are tons of empty containers along the old railway tracks. You may have use for them.\",CONTRACTS\nUS_04_02_BLOCKAGE_02_TSK,Landslide on the Highway,_TASKS,US_04,Yukon,cargoDelivery,2\u00d7 Wooden Planks,360,4500,Oh no! Our only paved road is blocked by debris. It must be cleared soon! Otherwise we'll run out of food!,TASKS\nUS_04_02_FACTORY_BLOCK_03,Factory: Sewage Treatment Facilities,_CONTRACTS,US_04,Yukon,truckDelivery,4\u00d7 Metal Planks; 3\u00d7 Pipes Medium; 4\u00d7 Blocks; 4\u00d7 Bricks; 3\u00d7 Forklift Caravan Container 2,2660,20150,\"Such large scale construction takes lots of resources\u2014human resources too. Don't forget your workers will need housing. A nearby factory produces great cabins, it may be worth looking at.\",CONTRACTS\nUS_04_02_FINAL_DELIVERY,Final Check,_CONTRACTS,US_04,Yukon,truckDelivery,,1100,13900,\"Now that the plant is ready, we have to test it. Take the ore samples produced earlier and deliver them to the plant.\",CONTRACTS\nUS_04_02_MED_LOGS,Looking After Your Own,_CONTRACTS,US_04,Yukon,cargoDelivery,2\u00d7 Logs Medium; 2\u00d7 Logs Medium; 1\u00d7 Logs Medium; 2\u00d7 Logs Medium,2680,30850,\"Several households are in a tight spot. Lots of trees around but no permit to cut them down, and they're running out of wood to heat their homes. We have to deliver firewood to each of them. You seem fit for the job.\",CONTRACTS\nUS_04_02_LOGS_FOR_LUMBERMILL,Destination: Sawmill,_CONTRACTS,US_04,Yukon,cargoDelivery,4\u00d7 Logs Medium; 2\u00d7 Logs Long,2480,28950,We have a big batch of logs here that need to be taken to the local sawmill. I'm sure you'll do well. Just be sure to secure the logs on the trailer. We don't want anyone to get hurt.,CONTRACTS\nUS_04_02_LONG_LOGS,Stocking Up,_CONTRACTS,US_04,Yukon,cargoDelivery,2\u00d7 Logs Long; 2\u00d7 Logs Long; 1\u00d7 Logs Long,2500,25100,\"My friend! So glad you're back with us! We need to stock up several warehouses with logs. For that, we need a professional driver, like you.\",CONTRACTS\nUS_04_02_TOOLSDELIVERY_TSK,Tools for the Warehouse,_TASKS,US_04,Yukon,truckDelivery,,660,6900,\"Ready for some errands? On the other end of this mountain chain there is a warehouse with some useful resources. It needs some repairs. If you take these tools there, we could share some of our stock with you.\",TASKS\nUS_04_02_TELEMETRY_TSK,Unstable Foundation,_TASKS,US_04,Yukon,truckDelivery,,800,6100,We're worried that the ground may sink under several power towers. We have to check it. Please deliver the trailer with the diagnostic equipment to each point; we need to make sure everything's okay.,TASKS\nUS_04_02_DROWNSCOUT_TSK,Off the Cliff,_TASKS,US_04,Yukon,truckDelivery,,590,5550,\"I need your help! I was fishing off the rocks there and I didn't see my car roll down into the lake! Now I cannot get it out, but you could easily do it.\",TASKS\nUS_04_02_BROKEN_TRUCK_TSK,Wrong Turn,_TASKS,US_04,Yukon,truckDelivery,,550,5100,\"I need your help, friend! My cousin went for a drive in the mountains at night. And his truck broke down right at the peak! Now we're thinking how to get him out of there. Will you help us?\",TASKS\nUS_04_02_FUEL_FOR_HUNTER_TSK,Hunting Needs,_TASKS,US_04,Yukon,truckDelivery,,410,4100,\"Hey, would you like to make some money? I have a small hunting cabin in the mountains, but it needs heating and my old jeep won't start. Could you take the tank there?\",TASKS\nUS_04_02_DROWNED_TITAN_TSK,Sunk Giant,_TASKS,US_04,Yukon,truckDelivery,,360,3550,\"I have no idea how, but my men managed to sink that thing in the woods. And I can't even begin to think how to drag it out. But if you do the impossible, I promise you a big reward.\",TASKS\nUS_04_02_OPEN_STORAGE_TSK,Opening The Warehouse,_TASKS,US_04,Yukon,truckDelivery,,370,3400,\"We recently bought a hangar at the old station; but it has to be disassembled and repaired. If you help get some tools there, we will share some resources from the warehouse with you.\",TASKS\nUS_04_02_HELP_TSK,Support the Locals,_TASKS,US_04,Yukon,cargoDelivery,1\u00d7 Crate Large; 1\u00d7 Crate Large; 1\u00d7 Crate Large,850,7700,\"Several households in the area have suffered from flooding. They need food. However, the crates must be placed on special ramps, so that they're not washed away in case of repeat flooding.\",TASKS\nUS_04_02_STORAGE_TSK,Warehouse in the Mountains,_TASKS,US_04,Yukon,cargoDelivery,1\u00d7 Container Small; 1\u00d7 Container Small,820,7550,\"We have an unusual favor to ask. We're starting exploration in the mountains, but we have to keep our equipment somewhere. The easiest way is to use containers. Deliver containers to both camps. I've heard there are some empty containers along the railway tracks.\",TASKS\nUS_04_02_LOST_CRATES_TSK,Crates in the Woods,_TASKS,US_04,Yukon,cargoDelivery,5\u00d7 Vehicles Spare Parts,900,7050,\"What are they doing there? After the flood, half our belongings got washed away! We really need those crates. If you help us, we'll give you a good reward.\",TASKS\nUS_04_02_TUBES_NEED_NOW_TSK,Piping Problem,_TASKS,US_04,Yukon,cargoDelivery,2\u00d7 Pipes Medium,710,5750,\"I was expecting a delivery of some pipes for the water supply system, but the contractors sank the trailer with them in the woods. I need them now. You can either find my trailer or bring me new pipes. I'll pay you well.\",TASKS\nUS_04_02_BLOCKED_GATE_TSK,Rockfall in the Ravine,_TASKS,US_04,Yukon,cargoDelivery,2\u00d7 Wooden Planks; 1\u00d7 Metal Planks,590,5700,\"This ravine leads to the semi-flooded foothills. Or used to, to be more precise. Now it's nothing but a heap of rocks. However, if you clear them, we won't have to take a detour to get there.\",TASKS\nUS_04_02_LOST_BIG_CONT_TSK,Missing Container,_TASKS,US_04,Yukon,cargoDelivery,1\u00d7 Container Large,650,4600,\"We have a situation here! A container with cargo for our warehouse went missing after a small flood. I suspect water carried it toward the forest. If you bring it back, we'll share some stuff from the warehouse with you.\",TASKS\nUS_04_02_ELECTROFIX_TSK,Power Line Maintenance,_TASKS,US_04,Yukon,cargoDelivery,1\u00d7 Service Spare Parts,220,1800,\"We need to do some planned maintenance on the tower. The trouble is, it's located on a cliff and there's no way we can drag our equipment there. We need someone who could lift the crate with the tools up the cliff.\",TASKS\nUS_04_02_STRANGE_SIGNAL_TSK,Strange Signal,_TASKS,US_04,Yukon,exploration,,200,3400,\"How did you get here? Never mind. My name is Carter, and I think you can help me. There is an abandoned radio tower nearby. Sometimes, I receive a strange signal coming from it. Can you check out what's going on there?\",TASKS\nUS_04_02_DIRT_CNT,Path through the Rocks,_CONTESTS,US_04,Yukon,exploration,,160,2750,These steps are left from excavation work in the past. Looks like an ideal place for a survival competition to me. You in?,CONTESTS\nUS_04_02_TOTHETOP_CNT,To the Top!,_CONTESTS,US_04,Yukon,exploration,,70,1200,\"Want to take part in the local rock climbing competition? If you get to the top within the set time limit, you will get a handsome reward.\",CONTESTS\nUS_03_01_PAPER_01_CONT,\"Paper Factory, Section A\",_CONTRACTS,US_03,Wisconsin,cargoDelivery,4\u00d7 Metal Planks; 12\u00d7 Metal Roll; 6\u00d7 Barrels; 6\u00d7 Blocks; 2\u00d7 Pipe Large,4170,28450,\"We have another construction site here! Section A of the future paper factory requires components, please help deliver them to the construction site.\",CONTRACTS\nUS_03_01_LOGS_CONT_01,Delivery To The Weather Station,_CONTRACTS,US_03,Wisconsin,cargoDelivery,1\u00d7 Logs Medium; 1\u00d7 Logs Long; 4\u00d7 Wooden Planks,1070,9350,\"Local meteorologists need logs! The sooner, the better! Even though these guys are usually good at waiting\u2014their supplies will run out, as will as their patience.\",CONTRACTS\nUS_03_01_BRIDGE_01,Factories Bridge,_TASKS,US_03,Wisconsin,cargoDelivery,6\u00d7 Metal Planks,1750,13800,The main link between the new and the old factory has been destroyed. Will you help restore it?,TASKS\nUS_03_01_TSK_01,Lost Supplies,_TASKS,US_03,Wisconsin,cargoDelivery,2\u00d7 Service Spare Parts,420,4700,\"My friend was bringing me some spare parts, but some of them got lost on the way. Help us recover them please.\",TASKS\nUS_03_01_CONT_01,Quarry Race,_CONTESTS,US_03,Wisconsin,exploration,,80,1300,\"Hi! As you know, the locals love racing, and the quarry nearby is an ideal racing track. Let's see how fast your vehicle can go.\",CONTESTS\nUS_03_01_PAPER_02_CONT,\"Paper Factory, Section B\",_CONTRACTS,US_03,Wisconsin,cargoDelivery,12\u00d7 Bricks; 4\u00d7 Bags; 4\u00d7 Concrete Slab; 6\u00d7 Metal Roll,2790,19000,\"Section B of the Paper Factory needs materials. Help deliver them, and we'll be one step closer to completion.\",CONTRACTS\nUS_03_01_LOGS_CONT_02,Contribution To Farm Restoration,_CONTRACTS,US_03,Wisconsin,cargoDelivery,2\u00d7 Logs Long; 6\u00d7 Wooden Planks,1400,14300,\"Hi! We had this big flood a while ago, but the water is subsiding, and we're planning to restore our town. We need you to deliver some wood here, will you help us?\",CONTRACTS\nUS_03_01_TSK_02,Exploration Gone Wrong,_TASKS,US_03,Wisconsin,truckDelivery,,460,4250,\"My friend Steve decided to take a ride on the rocks and fell from the very top. He's okay, but his car is stuck. Can you tow it out, please?\",TASKS\nUS_03_01_BRIDGE_02,Wooden Bridge,_TASKS,US_03,Wisconsin,cargoDelivery,8\u00d7 Wooden Planks,580,6500,\"There used to be a wooden bridge here, and we are re-building it. We need your help!\",TASKS\nUS_03_01_CONT_02,Cliffhanger Race,_CONTESTS,US_03,Wisconsin,exploration,,60,1000,\"A friend of mine says: \u201cNever relax before you reach your goal.\u201d Let's see how well you concentrate, this race is intended precisely for that.\",CONTESTS\nUS_03_01_PAPER_03_CONT,\"Paper Factory, Section C\",_CONTRACTS,US_03,Wisconsin,cargoDelivery,4\u00d7 Barrels; 2\u00d7 Pipes Small; 6\u00d7 Crate Large; 2\u00d7 Container Small; 4\u00d7 Metal Roll; 4\u00d7 Metal Planks; 2\u00d7 Pipe Large; 6\u00d7 Bricks,4810,35500,\"What's left is the last-but-not-least section of the Paper Factory. Let us finish it, you're doing great.\",CONTRACTS\nUS_03_01_LOGS_CONT_03,Rail Trail,_CONTRACTS,US_03,Wisconsin,cargoDelivery,2\u00d7 Logs Medium; 4\u00d7 Logs Long,1610,12800,\"This railway station will resume its work any time now, and we already got a few orders. Please make the deliveries and we'll pay.\",CONTRACTS\nUS_03_01_TSK_03,To New Horizons,_TASKS,US_03,Wisconsin,truckDelivery,2\u00d7 Vehicles Spare Parts,840,7850,\"Hi! My name is Mr. B, I am a traveler. I was just testing some modifications when I lost my trailer with spare parts, and my car broke down. Will you help me with that? I will pay you!\",TASKS\nUS_03_01_BRIDGE_03,Railway Bridge,_TASKS,US_03,Wisconsin,cargoDelivery,4\u00d7 Metal Planks; 2\u00d7 Concrete Slab,1770,14750,The path to the sawmill is blocked too. Let's clear it.,TASKS\nUS_03_01_LOGS_CONT_04,Big Trees to Small Homes,_CONTRACTS,US_03,Wisconsin,cargoDelivery,1\u00d7 Logs Medium; 4\u00d7 Wooden Planks; 2\u00d7 Logs Long,1570,15100,\"A couple of local residents want to stock up on firewood for the coming winter. Please help us with the delivery, we'll pay you.\",CONTRACTS\nUS_03_01_TSK_04,Look Before You Leap,_TASKS,US_03,Wisconsin,truckDelivery,,380,2900,\"Why, oh why did he go there?.. Please, please help me get my friend's car out of the bog, I will pay you!\",TASKS\nUS_03_01_BRIDGE_04,Creek Bridge,_TASKS,US_03,Wisconsin,cargoDelivery,2\u00d7 Metal Planks,700,5000,\"This bridge used to be made of wood, but we decided to renovate it. I think that will be just what we need. Will you help us?\",TASKS\nUS_03_01_TSK_05,The Aftermath of the Hurricane,_TASKS,US_03,Wisconsin,cargoDelivery,3\u00d7 Vehicles Spare Parts,570,5800,\"A recent hurricane has scattered our belongings all around. We need your help to find and recover them. For a fee, of course.\",TASKS\nUS_03_01_TSK_06,Quarry Incidents,_TASKS,US_03,Wisconsin,truckDelivery,,970,10100,Hi! I wanted to ride down the quarry on my old truck but it fell. Will you help me pull it out and fix it? I will pay you for that.,TASKS\nUS_03_01_TSK_07,Decisions and Consequences,_TASKS,US_03,Wisconsin,truckDelivery,,360,2550,\"I did tell myself not to go there, cause I might sink... But I just had to test that new suspension and the wheels! Help me pull the car out, will you? I will reward you handsomely.\",TASKS\nUS_03_01_TSK_08,Saving the Farmer's Machinery,_TASKS,US_03,Wisconsin,truckDelivery,,630,7200,\"My farm got flooded, so I moved. But I left my truck behind. Can you help bring it here?\",TASKS\nUS_03_01_TSK_09,Lost but not Forgotten,_TASKS,US_03,Wisconsin,truckDelivery,,360,2550,\"Not long ago, I was driving around and toppled off the hill. The car is totaled and I can do without it, but I left some stuff there. Bring it here, and you can keep the car.\",TASKS\nUS_03_01_TSK_10,Repaying the Debts,_TASKS,US_03,Wisconsin,truckDelivery,1\u00d7 Logs Medium,540,5600,\"Recently, I borrowed a trailer and some logs from a friend. Will you help me get what I owe to his house? I will pay for your services.\",TASKS\nUS_03_01_TSK_11,Lost Order,_TASKS,US_03,Wisconsin,cargoDelivery,2\u00d7 Barrels Oil,560,5950,Did you see the train that turned over on the tracks? We have some barrels there. Will you help us bring them here?,TASKS\nUS_03_01_TSK_12,Recovering What's Lost,_TASKS,US_03,Wisconsin,cargoDelivery,1\u00d7 Container Large,810,6600,\"We used to have a nice farm and a pretty town. The hurricane was vicious, but we do not leave our own behind. Help us recover the container with our gear and belongings, and we will pay you.\",TASKS\nUS_03_01_CELL_CONT,Wisconsin Truck Trip,_CONTRACTS,US_03,Wisconsin,cargoDelivery,16\u00d7 Cellulose,2020,17450,\"Finally, we've started making cellulose! Now we have to fulfil a major contract and deliver it to the station in the south, over the hill. Pick up the cargo and take it to the destination.\",CONTRACTS\nUS_03_01_PLANT_CONT,Starting the Power Plant,_CONTRACTS,US_03,Wisconsin,cargoDelivery,8\u00d7 Barrels; 2\u00d7 Container Small; 2\u00d7 Metal Planks; 2\u00d7 Crate Large,1960,15850,\"Starting the Power Plant will help start up steel and brick production, and it will help the region in general. Let there be power!\",CONTRACTS\nUS_03_02_LOST_TRUCK_01_TSK,The Wrong Turn,_TASKS,US_03,Wisconsin,truckDelivery,,810,6700,\"Driving through a forest at night is a terrible idea. We sent out a truck with a trailer for a technical assistance mission. The driver got lost, so the truck and the trailer both got stuck on the coast. We have no adequate equipment, we need your help.\",TASKS\nUS_03_02_BROKEN_BRIDGE_01_TSK,North Bridge,_TASKS,US_03,Wisconsin,cargoDelivery,2\u00d7 Metal Planks; 2\u00d7 Service Spare Parts,1040,8250,This bridge was intended to connect the factory and the railway station. We need to fulfill the plan and complete the bridge.,TASKS\nUS_03_02_BROKEN_BRIDGE_02_TSK,South Bridge,_TASKS,US_03,Wisconsin,cargoDelivery,2\u00d7 Metal Planks; 2\u00d7 Service Spare Parts,1000,7550,This bridge was never finished. We have to complete the job so we stay connected with this part of the region.,TASKS\nUS_03_02_BROKEN_BRIDGE_03_TSK,Wooden Bridge,_TASKS,US_03,Wisconsin,cargoDelivery,2\u00d7 Wooden Planks,300,3350,\"The bridge that leads to my house has become very shabby and recently fell apart. Now, I'm cut off from my land! I have to climb rocks to get home. Will you help me restore the bridge?\",TASKS\nUS_03_02_TRAIN_OBJ,Weightlifting,_CONTRACTS,US_03,Wisconsin,truckDelivery,,1480,9350,\"It's as simple as that\u2014you have to transport a locomotive from the depot to the station. You heard me right. I know, this is crazy and no one would be up to it\u2014except you. The thing is, it can only be fueled and fixed at the station, and we have no other locomotives to tow it.\",CONTRACTS\nUS_03_02_BIG_LOGS_OBJ,Fallen Titans,_CONTRACTS,US_03,Wisconsin,cargoDelivery,1\u00d7 Sequoia; 1\u00d7 Sequoia; 1\u00d7 Sequoia,2300,18550,\"Several big century-old trees were blown over by a recent storm. We would hate for the valuable timber to go to waste, so we have prepared the logs for transportation. The pick-up locations can be quite tricky, but I think you can do it.\",CONTRACTS\nUS_03_02_RAIL_REPAIR_OBJ,Restoring the tracks,_CONTRACTS,US_03,Wisconsin,cargoDelivery,1\u00d7 Railway; 1\u00d7 Railway; 1\u00d7 Railway; 1\u00d7 Railway; 1\u00d7 Railway; 1\u00d7 Railway,2300,16600,\"If we are to restore this region, we have to establish connections. And nothing beats a railway! There's one here, and it looks decent, but several sections need to be replaced. That means ordering them at the factory and then delivering them to the breaches.\",CONTRACTS\nUS_03_02_FINAL_COUNTDOWN_OBJ,Train Departure,_CONTRACTS,US_03,Wisconsin,cargoDelivery,3\u00d7 Metal Planks; 3\u00d7 Service Spare Parts; 3\u00d7 Barrels,1860,14950,\"We are almost ready to send out our first train! There are a few problems however. First, the bridge the train has to go over isn't in top shape. It has to be fixed. Second, no engine will run without fuel. And we're not talking gasoline here. It needs special fuel from the fueling station nearby.\",CONTRACTS\nUS_03_02_CELL_MASHINERY_TSK,Mobile Telephony,_TASKS,US_03,Wisconsin,truckDelivery,1\u00d7 Container Large,1070,8600,\"Hi! We are trying to set up some mobile communication here. The signal is so bad, I can't even call my mom. But we need to haul the equipment to the hilltop. Will you help?\",TASKS\nUS_03_02_CARGOINRIVER_TSK,Trailer In The River,_TASKS,US_03,Wisconsin,truckDelivery,2\u00d7 Crate Large,920,7900,\"My loaded trailer got washed off into the river during a recent flood, but yesterday I saw it on a sandbank. Can you get it out?\",TASKS\nUS_03_02_TRUCK_IN_RIVER_TSK,Out of Nowhere,_TASKS,US_03,Wisconsin,truckDelivery,,760,7300,\"A barrel truck was supposed to arrive at our warehouse. But you know how the roads here are... It turned over on the way. We have no machinery to flip or fix it. Could you help us? For a fee, of course.\",TASKS\nUS_03_02_TOR_TSK,Only for the Worthy,_TASKS,US_03,Wisconsin,truckDelivery,,500,5000,\"We have an emergency! There was this big crane truck we were expecting. And by \u201cbig,\u201d I mean BIG. It was going to the warehouse, but it flipped on its way here and fell into the trench. Folks who tried to help couldn't even budge it. Maybe you're the one who can do it?\",TASKS\nUS_03_02_FARMER_LOG_TSK,Order for Logs,_TASKS,US_03,Wisconsin,cargoDelivery,2\u00d7 Logs Long,1740,17700,\"Hey, can you help me? I'm trying to fix my farm, and I could really use some long logs. I'll pay you well for the haul.\",TASKS\nUS_03_02_DANGER_BARRELS_TSK,Hazardous Cargo,_TASKS,US_03,Wisconsin,cargoDelivery,5\u00d7 Barrels Oil,1230,11000,A landslide happened after a recent storm and carried away a few pallets of fuel oil tanks. They are a hazard for the environment and can even contaminate the river. Can you please pull them out?,TASKS\nUS_03_02_TUBE_IN_RIVER_TSK,Don't push your luck!,_TASKS,US_03,Wisconsin,cargoDelivery,1\u00d7 Pipe Large,1070,8100,\"We were expecting a delivery of a big pipe, and the driver decided to take a shortcut across a shabby bridge. As the result, the pipe ended up in the river. We have to get it out somehow. Can you do it?\",TASKS\nUS_03_02_CARGO_ON_ISLANDS_TSK,Gone with the Wind,_TASKS,US_03,Wisconsin,cargoDelivery,5\u00d7 Bricks,620,6650,\"Thanks for your help with the bridge. But I've found out that the materials for the new structure, that were lying around on the lot, have vanished. I think the hurricane blew them over to the river. I saw pallets with them washed ashore at the islands. Will you get them for me?\",TASKS\nUS_03_02_INVASION_TSK,Invasion!,_TASKS,US_03,Wisconsin,cargoDelivery,2\u00d7 Vehicles Spare Parts,490,5250,\"Finally, I discovered what causes the anomalies in the area! The huge funnel sink as locals call it, is in fact an alien landing spot. Government agents have collected the remains of the ship in crates, but they never took them anywhere. You have to be there first and get the crates to me.\",TASKS\nUS_03_02_STRANGER_TSK,Strangest Things,_TASKS,US_03,Wisconsin,exploration,,300,5250,\"Hey, kid, I see you're not from around here. It's good. My name is Carter. Something fishy is going on in this forest. I've seen a weird glow in the depth of the woods many a time. I think they're doing some prohibited experiments there. I've written down the coordinates of those places. You have to check them out!\",TASKS\nUS_03_02_YFILES_TSK,Classified,_TASKS,US_03,Wisconsin,exploration,,240,4200,\"How'd you find me? Never mind. I think the government knows that I'm after them, so I had to move. But I can trust you. I have discovered paranormal activity again! Tree tops mysteriously disappear at night, in the spots I've marked. Please take a look at that place.\",TASKS\nUS_03_02_MUDSTER_CNT,Mud Tested,_CONTESTS,US_03,Wisconsin,exploration,,150,2500,\"My Grandma urgently needs a batch of pies! But you'll have to travel at night, through half-flooded forest. You'd better take something serious, not every vehicle will go through there.\",CONTESTS\nUS_03_02_SLALOM_CNT,Slalom,_CONTESTS,US_03,Wisconsin,exploration,,140,2450,\"There is an interesting trail here, at first it looks easy, but in the end you're up for a surprise. Not many people reach the final point. But you look brave enough.\",CONTESTS\nRU_04_01_BRIDGE_01_TSK,Bridge to the Sawmill,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Metal Planks; 2\u00d7 Service Spare Parts,950,7900,This old bridge has gone without maintenance for a long time. No wonder there's so little left of it. Bring us some materials and we'll restore the direct route to the sawmill.,TASKS\nRU_04_01_ROCKSLIDE_01_TSK,Landslide on the Path,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Wooden Planks; 1\u00d7 Service Spare Parts,420,3550,\"Generally, this path doesn't get much use, so no one really cared it got blocked. But it is a smart route to the quarry, so we should probably do something about it.\",TASKS\nRU_04_01_BRIDGE_02_TSK,Bridge to the Warehouse,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Metal Planks; 2\u00d7 Service Spare Parts,890,6850,\"There used to be a bridge here. I have no idea how, but rumor has it the locals sold it for scrap. It has to be restored ASAP if we want to use this warehouse at all. Will you help deliver the materials?\",TASKS\nRU_04_01_ROCKSLIDE_02_TSK,Landslide on the Highway,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Wooden Planks; 1\u00d7 Service Spare Parts,540,5700,\"This place must be cursed. The main highway leading to the assembly shop got blocked. We may have to put rockets together with our bare hands, from matches and acorns, unless we do something about it. It's vital.\",TASKS\nRU_04_01_BRIDGE_03_TSK,Bridge to the Island,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Metal Planks; 2\u00d7 Service Spare Parts,950,7950,\"This bridge used to lead to the island where you could get to the city. However, there was some strong weather, so it should be restored. I think you know what to do.\",TASKS\nRU_04_01_ROCKSLIDE_03_TSK,Landslide Near the Quarry,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Wooden Planks; 1\u00d7 Service Spare Parts,570,6200,\"There are only two roads leading to the quarry, this one and the narrow path nearby, and standard trucks can only squeeze through here. Or rather they used to\u2014it got thoroughly blocked, can you fix it?\",TASKS\nRU_04_01_BRIDGE_04_TSK,Bridge to Town,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Metal Planks; 2\u00d7 Service Spare Parts,970,8300,\"This ain't the first bridge that got cut up for scrap by some wise guys. We have to talk to the locals, maybe they know something. And for now, we need a new crossing, so let's get to work.\",TASKS\nRU_04_01_TUNNELBLOCK_OBJ,A Heap Of Work,_CONTRACTS,RU_04,Amur,truckDelivery,2\u00d7 Metal Planks; 4\u00d7 Wooden Planks; 4\u00d7 Service Spare Parts,1970,17700,\"The road to the launch site goes through a tunnel. Trouble is\u2014it's been blocked and neglected for many years. We can allocate machinery for clearing, but we'll need some materials.\",CONTRACTS\nRU_04_01_ROCKET_TRAILER_OBJ,Carrier Rocket Carrier,_CONTRACTS,RU_04,Amur,truckDelivery,,1800,15000,While they're still busy putting the rocket together we should find a way to get it to the launchpad. There's an old military base that used to be a part of the cosmodrome. Let's see if we can find anything useful there.,CONTRACTS\nRU_04_01_ROCKET_OBJ,Orbital Velocity,_CONTRACTS,RU_04,Amur,cargoDelivery,1\u00d7 Rocket Engine; 1\u00d7 Rocket Part 2; 1\u00d7 Rocket Part 1,3090,20450,\"The mill is in order, now we need a carrier. In fact, they wanted to launch here in the 70s, but they never made it, so chunks of a rocket are up for grabs. Hopefully they'll be in good enough condition.\",CONTRACTS\nRU_04_01_POWERPLANT_OBJ,Electric Atmosphere,_CONTRACTS,RU_04,Amur,cargoDelivery,3\u00d7 Metal Planks; 1\u00d7 Container Large,1630,12250,\"Welcome. We are here to conquer space, and the local electricians won't even bother to repair the substation. We barely have power to keep the lights on, let alone run the mill. Bring them what they need, alright?\",CONTRACTS\nRU_04_01_HELP_TSK,Quid pro Quo,_TASKS,RU_04,Amur,truckDelivery,1\u00d7 Blocks; 1\u00d7 Concrete Slab; 1\u00d7 Bricks,1250,12750,\"Hey, wanna make some money? Even better\u2014I could give you some juicy leads. Get the construction materials to my brother in town, and I'll make it worth your time.\",TASKS\nRU_04_01_EXPEDITION_TSK,Unlucky Expedition,_TASKS,RU_04,Amur,truckDelivery,1\u00d7 Forklift Caravan Container 2,1250,10300,\"We have a situation. We have a small business and were doing some research here, but we got in an accident on our way back. To cut it short, we have no access to either the vehicle or to the trailer cabin. Can you help?\",TASKS\nRU_04_01_FACTORY_TSK,Barefoot Cobbler,_TASKS,RU_04,Amur,truckDelivery,5\u00d7 Vehicles Spare Parts,950,8700,\"We hear you're game for some tough work? We were planning to open a workshop at the factory, but we'll need some supplies, and our trailer got stuck in the ravine. Could you try and pull it out for us?\",TASKS\nRU_04_01_LOST_TRAILER_TSK,Warehouse Trouble,_TASKS,RU_04,Amur,truckDelivery,,380,3600,\"Oh, there's another one. Sorry, we're closed. Our supply trailer got stuck in the marsh, so unless someone pulls it out, all I can offer is air. However, if you help us, we can talk.\",TASKS\nRU_04_01_TRUCK_ON_HILL_TSK,At The Apex,_TASKS,RU_04,Amur,truckDelivery,,320,2500,\"I maintain a radio tower on a hill not far from here. My SUV wouldn't start yesterday, I have no idea what the problem is, so I left it there. Could you help me tow it here, and I can take a good look at it.\",TASKS\nRU_04_01_SCOUT_IN_TROUBLE_TSK,Treasure Hunter,_TASKS,RU_04,Amur,truckDelivery,,350,2350,\"I promised you a good lead, so listen. There is a car at the bottom of the ravine here. It's been there a while, but it looks okay. Tow it out, and we'll see what we can do with it.\",TASKS\nRU_04_01_FUELTASK_TSK,Bottoms Up,_TASKS,RU_04,Amur,truckDelivery,,290,2050,\"Sorry, my friend, there will be no fueling today\u2014the fuel tank they were transporting yesterday flipped over and slid into the river. If you can pull it out and bring it here, we can fuel you up as well.\",TASKS\nRU_04_01_CHURCH_TSK,Closer to Heaven,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Logs Medium; 1\u00d7 Logs Long; 1\u00d7 Metal Planks,1050,7350,\"See this church? I am doing my best to maintain it, year after year, with these very hands. But, without supplies, it's getting harder and harder. Would you help me?\",TASKS\nRU_04_01_SUPPLIES_TSK,For Hard-to-reach Places,_TASKS,RU_04,Amur,cargoDelivery,1\u00d7 Forklift Caravan Container 2; 1\u00d7 Container Small; 1\u00d7 Crate Large,1040,6900,\"What should I do?.. I was making some deliveries, but everything is covered with ice, so the trailer slid into a ravine. How on Earth can I deliver it now? Wait... Can I ask you for a favor?\",TASKS\nRU_04_01_CARGOCULT_TSK,Cargo Cult,_TASKS,RU_04,Amur,cargoDelivery,3\u00d7 Crate Large,560,4000,\"They dropped some cargo off a helicopter a few days ago, but no one came to pick it up. Let's get it here at least. Better than it sitting in the snow..\",TASKS\nRU_04_01_SAWMILL_TSK,Means Of Production,_TASKS,RU_04,Amur,cargoDelivery,1\u00d7 Container Small,400,3900,\"How on Earth is the sawmill supposed to run with no tools? Huh? We ordered a crate of those, paid for it and all. We did everything right\u2014till some butterfingers dropped it in the river! What do we do now?\",TASKS\nRU_04_01_ROCK_RACE_CNT,Conquering Summits,_CONTESTS,RU_04,Amur,exploration,,100,1750,\"I can see you have every reason to be proud. How about a stress test? There is a small range with three summits nearby. If you make it to all three on time, you will get a prize. Deal?\",CONTESTS\nRU_04_01_ICE_RACE_CNT,Ice Adventure,_CONTESTS,RU_04,Amur,exploration,,80,1250,\"It is strongly recommended not to get on the ice now, but that's not stopping you, right? We raised some prize money here and want to race to the end of the route. You can join us\u2014if you're not scared, that is.\",CONTESTS\nRU_04_02_ROCK_CLEANUP_1_TSK,Rockslide,_TASKS,RU_04,Amur,cargoDelivery,1\u00d7 Metal Planks,470,4500,\"Looks like the main road is going to be a problem. I think it could still be easier to just clear that one out than to look for an alternate route, so let's get cracking.\",TASKS\nRU_04_02_TREE_CLEANUP_1_TSK,Fallen Trees,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Service Spare Parts,400,2800,Treefall? Well that's a new one. Doesn't change the fact we need to get past them though.,TASKS\nRU_04_02_ROCK_CLEANUP_2_TSK,Shore Collapse,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Metal Planks,720,5400,\"Another rockslide. We're already here anyway, so why stop now. I think you know what to do.\",TASKS\nRU_04_02_TREE_CLEANUP_2_TSK,Cliffside Treefall,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Service Spare Parts,420,3150,\"I'm beginning to think it would've been easier to pave a new road instead of clearing that one out. Oh well, too late for that, time to get our hands dirty.\",TASKS\nRU_04_02_TREE_CLEANUP_3_TSK,Tree Blockade,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Service Spare Parts,460,3700,\"More fallen trees, of course. At least, we don't need to fit the trailer through here. You know the drill.\",TASKS\nRU_04_02_ROCKET_TRAIN_DELIVERY_OBJ,Liftoff,_CONTRACTS,RU_04,Amur,truckDelivery,,1070,29000,\"Okay, final push. We couldn't get our hands on an actual locomotive so you'd have to give the platform a good push to the launchpad. Once that's done head for the Mission Control, should be quite a view.\",CONTRACTS\nRU_04_02_MISSION_CONTROL_OBJ,Ground Control,_CONTRACTS,RU_04,Amur,truckDelivery,2\u00d7 Bricks; 1\u00d7 Concrete Slab,1290,12250,\"Time to get this place up to shape. First things first, the bunker is in good condition, but the rest isn't, especially the radar. You go and get some bricks to get us going, I'll figure what to do about the dish.\",CONTRACTS\nRU_04_02_ROCKET_DELIVERY_OBJ,Reach For The Stars,_CONTRACTS,RU_04,Amur,truckDelivery,,720,9600,\"The rocket's ready, now it's time to ship it over to the guys at the HAB. It's already big as it is, so we'll put the boosters on it later. I hope I don't have to remind you about being careful with it.\",CONTRACTS\nRU_04_02_FUEL_LINE_REPAIR_OBJ,Lifeline,_CONTRACTS,RU_04,Amur,cargoDelivery,2\u00d7 Pipe Large,1610,12500,\"Looks like someone overestimated how intact the fuel line is. The auxiliary fuel storage is there alright, but we can't really fill it up with a chunk of pipeline missing. I do think we can find a replacement though.\",CONTRACTS\nRU_04_02_TOWN_RESUPPLY_TSK,Strategic Reserve,_TASKS,RU_04,Amur,truckDelivery,,1090,10100,\"I'll cut straight to the chase. We've got a couple of supply trailers scattered around the place that I want you to tow back to the town. You do that, and I'll help you with that launchpad. Deal?\",TASKS\nRU_04_02_DAN_RESCUE_TSK,Misadventure,_TASKS,RU_04,Amur,truckDelivery,,760,7200,\"Oh, aren't you with those cosmonaut people? It's been years since anyone tended to our local church, so I figured I'll help out. Turns out, the road's a bit too rough for my baby. Could you please get her back to me?\",TASKS\nRU_04_02_FISHERMAN_SCOUT_RESCUE_TSK,Takes One To Find One,_TASKS,RU_04,Amur,truckDelivery,,450,4500,\"So a pal of mine's been here, kind of a fisherman like myself. Had a bit too much tea while camping so he fell into the river. The guy made it out, but his car needs towing\u00a0- doubt he'll ever set foot on that ice again.\",TASKS\nRU_04_02_KHAN_RECOVER_TSK,Unlucky Lineman,_TASKS,RU_04,Amur,truckDelivery,,410,3750,\"Now, I have no idea what some of these people are thinking sometimes, but one my guys decided to check the railroad across the river and managed to roll the truck off a cliff. Would you kindly pull him out?\",TASKS\nRU_04_02_DELIVERY_TRUCK_RECOVER_TSK,Knee Deep,_TASKS,RU_04,Amur,truckDelivery,,370,3400,\"Looks like the previous courier didn't make it all the way to the town. It's probably best to pull his truck back to the repair shop, it could still be of use to the locals.\",TASKS\nRU_04_02_ABANDONED_TRUCK_RECOVERY_TSK,Lost But Not Forgotten,_TASKS,RU_04,Amur,truckDelivery,,320,2550,\"You got a minute? Back in the day my grandpa got his truck stuck in the swamp nearby. It's been there for years now, but I'm wondering if something can still be done about it.\",TASKS\nRU_04_02_FARM_RESTOCK_BRICKS_TSK,Build Additional Farmhouses,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Bricks; 2\u00d7 Metal Planks; 3\u00d7 Logs Short,1400,13050,\"Ah, always nice to see a new face around these parts. Tell me, friend, do you have a moment to spare? We were going to patch up this here farm a touch, but we're going to need a few things for that. Think you could help?\",TASKS\nRU_04_02_LOG_BRIDGE_CONSTRUCTION_TSK,Log Crossing,_TASKS,RU_04,Amur,cargoDelivery,3\u00d7 Logs Medium,560,5550,I don't even want to know the shape our cargo would be if you try to go over that crossing right now. Just bring a few logs here and I'll have someone finish the crossing for you.,TASKS\nRU_04_02_METEO_RESTOCK_TSK,Meteo Point,_TASKS,RU_04,Amur,cargoDelivery,1\u00d7 Barrels,390,4750,\"Living away from civilization isn't as tough as some may believe. Just remember to keep reserves of everything you really need. Speaking of, I could do with some spare fuel for my generator, if you could lend a hand.\",TASKS\nRU_04_02_CHURCH_REPAIR_TSK,Church,_TASKS,RU_04,Amur,cargoDelivery,1\u00d7 Wooden Planks,330,4000,\"Maintaining a church is an honest and tough work, especially without any materials. One kind soul offered their help with that, but couldn't handle the delivery. Perhaps you could show kindness to those in need, too?\",TASKS\nRU_04_02_BRIDGE_CONSTRUCTION_TSK,Old Bridge,_TASKS,RU_04,Amur,cargoDelivery,3\u00d7 Wooden Planks,350,3950,\"So here we are, huh. I was afraid something like this would happen... No matter. Someone has to have some planks we could borrow around here, right?\",TASKS\nRU_04_02_FARM_FUEL_RESTOCK_TSK,Fuel For The Masses,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Barrels,380,3650,\"You may be wondering why we have those pipes in storage over there. Well, we were going to set up water supply from the town, but that went nowhere, so we use generators to power the boilers. Speaking of which...\",TASKS\nRU_04_02_PICTURESCUE_TSK,Picturesque,_TASKS,RU_04,Amur,exploration,,450,7750,\"Oh, hey there! So, once they officially open the spaceport, we'll need to get the PR rolling. And I figured, why not take some pictures for future merch and such, right? So, could you give me a quick tour?\",TASKS\nRU_04_02_ABANDON_CHECKUP_TSK,Snowbound,_TASKS,RU_04,Amur,exploration,,360,6250,\"The winters here are quite something. Sometimes, that much snow can block entire roads off completely. Since the weather'd cleared up a bit, can I ask you to go check on a couple places around here?\",TASKS\nRU_04_02_ZIKZ_RESCUE_TSK,Stargazer,_TASKS,RU_04,Amur,exploration,,330,2750,\"They say something fell not far from here before the cosmodrome was built. Whatever it was, it's not there anymore, but looks like they've left a truck behind. Probably unusable, we still better check.\",TASKS\nRU_04_02_FUEL_RESTOCK_TARGET_CNT,Belated Delivery,_CONTESTS,RU_04,Amur,truckDelivery,,260,2150,\"Oh hey, you might be just who I need. I've got this barrel that needs delivering to the fuel station. I'm already way behind schedule, and I've got nothing to tow it there with, so... help a guy out, will you?\",CONTESTS\nRU_04_02_FUEL_RUN_CNT,Fuel-Eight,_CONTESTS,RU_04,Amur,exploration,,150,2600,\"The locals have a thing going, a little race from here and all the way to the launchpad. They call it the uh... Fuel-Eight, something or other. Me, I'm not a fan of that kinda thing, but you might want to give it a shot.\",CONTESTS\nRU_04_03_SUBSTATION_1,Unlimited Power,_CONTRACTS,RU_04,Amur,cargoDelivery,2\u00d7 Metal Planks,930,8950,\"There's an electric substation near the bridge across the river. We didn't have much use for it while the whole line was down, but the situation has changed. We'd be grateful if you would help us fix it.\",CONTRACTS\nRU_04_03_BRIDGE_BUILDING_01,Fallen Bridge,_TASKS,RU_04,Amur,cargoDelivery,1\u00d7 Metal Planks; 2\u00d7 Wooden Planks,930,10950,\"Yes, the roads here are pretty beat-up. And the bridge... If we want to have free access to the substation, we have to fix it. And, somehow, I think you're the right person for the job.\",TASKS\nRU_04_03_SCOUT_01,Expanding The Borders,_TASKS,RU_04,Amur,exploration,,440,7650,\"The place is remote, that's for sure. And our maps seem outdated. Will you help us do some exploring? There are some points of special interest here.\",TASKS\nRU_04_03_BRIDGE_BUILDING_02,Bridge By The Checkpoint,_TASKS,RU_04,Amur,cargoDelivery,1\u00d7 Metal Planks; 2\u00d7 Wooden Planks,1020,11750,\"Wow, the main access road to the island looking like this... I would strongly advise against using it as-is. So, let's get our hands dirty\u2014the bridge won't build itself.\",TASKS\nRU_04_03_OBSTACLE_02,Small landslide,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Wooden Planks; 1\u00d7 Metal Planks,800,8650,\"Now, this is bad. According to the map, this route is very promising for exploration, so we have to do something about the landslide.\",TASKS\nRU_04_03_SCOUT_02,Into The Deep,_TASKS,RU_04,Amur,exploration,,480,8250,\"There is this local guy here, a hermit. He told us about a few interesting places that need exploring as soon as we get the chance. Will you do it?\",TASKS\nRU_04_03_BRIDGE_BUILDING_03,Eroded road,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Wooden Planks,480,6600,\"Looks like we won't be able to drive through here, but there's no other good road to the substation. It means we have to build a new crossing. Are you in?\",TASKS\nRU_04_03_OBSTACLE_03,Rockslide,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Metal Planks,730,6450,\"It's just one thing on top of another, literally. Another rockslide. I know, the area here is pretty wild and all, but moving like this\u2014it'll take us weeks to get through. We want to move on, we have to clear the road.\",TASKS\nRU_04_03_OBSTACLE_04,Landslide In The Woods,_TASKS,RU_04,Amur,truckDelivery,,420,3600,\"There is a monastery nearby, I got in touch with them. They can help with the landslide, but they want us to tow out their supply trailer nearby. You deliver it to the monastery while they're clearing the path for us.\",TASKS\nRU_04_03_OBSTACLE_05,Landslide In The City,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Wooden Planks,330,2300,\"Well, we're almost there! Yeah, we expected the city would be like this, but it doesn't mean we're happy about it. Unless the road is cleared, it won't be of much use.\",TASKS\nRU_04_03_OBSTACLE_06,Fallen Tower,_TASKS,RU_04,Amur,cargoDelivery,1\u00d7 Metal Planks,360,2900,\"Nearly there! Just the last stretch, literally, but we have to get this tower away somehow. Let's look around, maybe we can find some supplies.\",TASKS\nRU_04_03_GIANT,Top Secret,_CONTRACTS,RU_04,Amur,truckDelivery,,420,4250,\"A serious person got in touch with me, he says there's some equipment here at the base that belongs to them. I was smart enough not to ask for the details\u2014you know what I mean. Long story short, we were asked to get it out.\",CONTRACTS\nRU_04_03_FUEL,Bare Minimum,_CONTRACTS,RU_04,Amur,truckDelivery,,420,3950,\"According to the maps, there's a good spot for a temporary gas station near one of the military objects on the island. We're ready to have some people set it up, if you provide the fuel.\",CONTRACTS\nRU_04_03_LOST_C,Evacuation Point,_CONTRACTS,RU_04,Amur,truckDelivery,,380,2800,\"We're scouting the area here, but our driver didn't make the turn. If you can tow the car out, we'd be grateful. If we can make it to the road, we can take over from there.\",CONTRACTS\nRU_04_03_SUPPLIES,Humanitarian Mission,_CONTRACTS,RU_04,Amur,cargoDelivery,2\u00d7 Container Small; 2\u00d7 Wooden Planks,1020,11250,\"It's amazing, but it looks like people still live here. We're trying to befriend the locals, so we decided to get them some supplies. And you, in turn, can help us deliver them.\",CONTRACTS\nRU_04_03_FALLEN_POWER_LINES,We Have Contact!,_CONTRACTS,RU_04,Amur,cargoDelivery,2\u00d7 Blocks; 1\u00d7 Metal Planks,830,9650,\"Okay, so the substation's up and running, but the line from the village is still wrecked. Look like it's broken in a few places. We have to see what happened and how to fix it.\",CONTRACTS\nRU_04_03_SEISMIC,Richter Scale,_CONTRACTS,RU_04,Amur,exploration,,380,6650,\"Our partners from the Geophysics Institute asked us to do some exploring. The terrain here is seismically active, so they could really use the data. If you're in, you'll need a seismic vibrator.\",CONTRACTS\nRU_04_03_LOST_B,Left Behind,_TASKS,RU_04,Amur,truckDelivery,,470,5100,\"Hello. We were just... let's say, we were taking some measurements here, then the weather got worse. One of the cars wouldn't start, so we had to leave it behind. Can you help us get it back to the Weather Station?\",TASKS\nRU_04_03_RUN,Cliff Surfer,_CONTESTS,RU_04,Amur,exploration,,100,1600,\"So you like extreme sports? I thought so. The route ain't an easy one, but I think you can handle it. Let's see how fast you can make it to the finish line.\",CONTESTS\nRU_04_03_OFF_ROAD,Five Roads Island,_CONTESTS,RU_04,Amur,exploration,,80,1350,\"Rumor has it, you're a wicked good driver. How about you prove it? Not too many off-road enthusiasts here, but we've put together some dough for whoever drives around the island the fastest. Are you in?\",CONTESTS\nRU_04_04_NPZ_RESTORATION_C,Block Gamma,_CONTRACTS,RU_04,Amur,cargoDelivery,1\u00d7 Pipes Small; 2\u00d7 Concrete Slab; 2\u00d7 Metal Planks; 6\u00d7 Bricks; 1\u00d7 Pipe Large; 1\u00d7 Pipes Medium; 2\u00d7 Barrels Oil; 2\u00d7 Pipes Small,3790,27200,\"The last stretch. We're doing great, thanks to you. As usual, we have the list, since we're finishing the renovation and are ready to start working on Gamma.\",CONTRACTS\nRU_04_04_NPZ_RESTORATION_A,Block Alpha,_CONTRACTS,RU_04,Amur,cargoDelivery,2\u00d7 Blocks; 4\u00d7 Bags; 2\u00d7 Barrels Oil; 2\u00d7 Bags 2; 2\u00d7 Metal Planks; 2\u00d7 Pipes Medium; 2\u00d7 Wooden Planks; 2\u00d7 Blocks,3610,25900,\"Hello. People from the Maneuver gave me your contact info, they said you're a reliable driver. I think you already know why we're here, so let's skip the formalities. The complex is a big structure, we'll work in stages.\",CONTRACTS\nRU_04_04_NPZ_RESTORATION_B,Block Beta,_CONTRACTS,RU_04,Amur,cargoDelivery,1\u00d7 Pipe Large; 1\u00d7 Pipes Medium; 2\u00d7 Pipes Small; 1\u00d7 Concrete Slab; 2\u00d7 Wooden Planks; 2\u00d7 Metal Planks; 4\u00d7 Bags,3360,24150,\"We go on to the next stage. The list of necessary materials is ready, my people are waiting for the delivery. We still have some supplies left, so the workers aren't idle, but we'd like to avoid it in the future.\",CONTRACTS\nRU_04_04_GARAGE_REPAIR,Back In Action,_CONTRACTS,RU_04,Amur,cargoDelivery,2\u00d7 Metal Roll; 4\u00d7 Pipes Small; 3\u00d7 Bricks; 2\u00d7 Blocks,2210,18800,\"Hello. There is an old garage nearby. Right now, it's pretty wrecked, but after some repairs\u2014it can be re-equipped. You might be interested in it.\",CONTRACTS\nRU_04_04_WINTER_SUPPLIES,Winter Reserves,_CONTRACTS,RU_04,Amur,cargoDelivery,1\u00d7 Logs Medium; 4\u00d7 Logs Long; 2\u00d7 Logs Short,2080,17850,\"In addition to food, the locals have a problem with firewood. The winter was severe, and the sawmill warehouse is getting depleted. It's not critical just yet, but we have to do something now.\",CONTRACTS\nRU_04_04_FACTORY_RESTORATION,Plant Restoration,_CONTRACTS,RU_04,Amur,cargoDelivery,4\u00d7 Bricks; 2\u00d7 Blocks; 2\u00d7 Metal Roll; 4\u00d7 Bags; 2\u00d7 Metal Planks,2250,17300,\"The plant, right... We used to have one, now it's closed. But the government thinks it has to reopen\u2014we have our interest in that, too. If we help with repairs, we'll be able to place orders there, to shop locally.\",CONTRACTS\nRU_04_04_OLD_TRAIN,Covered In Smoke,_CONTRACTS,RU_04,Amur,cargoDelivery,6\u00d7 Logs Medium,1500,15950,\"Turns out the locals have a rarity here, a real steam train engine, mind you\u2014a working one! They fixed it all right, but they have no fuel for it. Care to get them some wood for the coal?\",CONTRACTS\nRU_04_04_HEATING_SEASON,Repair Works,_CONTRACTS,RU_04,Amur,cargoDelivery,1\u00d7 Logs Medium; 2\u00d7 Logs Long; 2\u00d7 Logs Short,1500,15300,\"A sudden request came in. With all the firewood reserved in case of extreme cold, the sawmill was idle. So now they have trouble with planks\u2014the warehouse is empty, and the locals have no materials to use.\",CONTRACTS\nRU_04_04_WAREHOUSE_RESTORATION,Spring Cleaning,_CONTRACTS,RU_04,Amur,cargoDelivery,2\u00d7 Bricks; 1\u00d7 Blocks; 2\u00d7 Metal Planks,1850,13100,\"I've found an interesting paper\u2014looks like the original deed on the launch complex included a warehouse. It's not much, but if we clean it, I can put it to good use. And you may recover something useful too.\",CONTRACTS\nRU_04_04_LUMBER_MILL_RESTORATION,Sawmill Blues,_CONTRACTS,RU_04,Amur,cargoDelivery,4\u00d7 Bricks; 2\u00d7 Blocks; 2\u00d7 Metal Roll; 2\u00d7 Bags 2,1470,10650,\"I guess I look like an errand boy. Obviously, we don't want to quarrel with the local government... In short, they asked for help with the sawmill. Apparently, it has a special saw, so they want to get it up and running.\",CONTRACTS\nRU_04_04_FISHER,Fisherman's House,_CONTRACTS,RU_04,Amur,cargoDelivery,2\u00d7 Logs Short; 1\u00d7 Logs Medium,690,6000,\"Lots of locals live outside the village, and connecting with them is sometimes a problem. Currently, the most urgent issue is a local fisherman running out of firewood, he needs help with delivery.\",CONTRACTS\nRU_04_04_LOCAL_RESIDENTS,Supply Lines,_CONTRACTS,RU_04,Amur,cargoDelivery,1\u00d7 Container Large,730,5950,\"Recently, the village has had supply disruptions. The storms and the ice don't make it any better. We've prepared a batch of food that has to be delivered to the village. Hope we can count on you.\",CONTRACTS\nRU_04_04_WOOD_CARVING,Personal Favor,_CONTRACTS,RU_04,Amur,cargoDelivery,4\u00d7 Logs Medium,570,5850,\"I have a good friend, back from the Army. He has a wood-cutting shop here. He needs help with firewood. It's personal and off the record, but I'll make it worth your while.\",CONTRACTS\nRU_04_04_EXPLORATIONS_WORKS,Solid Rock,_CONTRACTS,RU_04,Amur,exploration,,210,3600,\"Okay, soldier. Aside from the pipes, we have a request from the Amur residents. They will set up production for us, but we'll have to provide some supplies and help. For starters, let's find out how stable the soil is.\",CONTRACTS\nRU_04_04_TSK_REALLY_BIG_ONE,Nothing To See Here,_TASKS,RU_04,Amur,truckDelivery,5\u00d7 Vehicles Spare Parts,940,8650,\"Hey, wanna make some coin? There's an old church here, and I remember a really nice trailer near it. You get it here, I'll pay for the work. Deal?\",TASKS\nRU_04_04_TSK_DREAM_CAR,Dream Car,_TASKS,RU_04,Amur,truckDelivery,,530,5800,\"Hi! I am having a new car delivered, but I cannot pick it up myself, I have a business to run. Maybe you could help me?\",TASKS\nRU_04_04_TSK_DANGEROUS_ROAD,Dangerous Road,_TASKS,RU_04,Amur,truckDelivery,1\u00d7 Logs Medium,690,5600,\"Our winters sure are cold, just like this one. We need to get firewood to the village, but everyone's scared of the ice. If you do it, we'll pay.\",TASKS\nRU_04_04_TSK_BIG_AND_ABANDONED,Heavy Duty,_TASKS,RU_04,Amur,truckDelivery,,530,5400,\"Hey! I hear you do cargo transportation? We have a trailer at the station. It's so big, we can't tow it on the ice. Can you help?\",TASKS\nRU_04_04_TSK_OLD_BUT_GOLD,Old-timer,_TASKS,RU_04,Amur,truckDelivery,,460,4600,\"I remember seeing a trailer in an abandoned village. Sure it was old, but it looked pretty decent. I'm thinking it might still be serviceable. Will you bring it here? I'll pay you for that.\",TASKS\nRU_04_04_TSK_UNLUCKY_BOY,Bad Luck Charm,_TASKS,RU_04,Amur,truckDelivery,,370,3000,\"What kinda day is that? Like I've broken a crate of mirrors... And ruined such a nice car... Wait a minute, maybe you could help? Pull out my car? I have no idea who else to turn to.\",TASKS\nRU_04_04_TSK_FAMILY_BUSINESS,Family Business,_TASKS,RU_04,Amur,truckDelivery,,370,3000,\"As if we didn't have enough problems already. We almost made it home\u2014and we get stuck. Would you be so kind as to tow us to town? We can't walk there with kids, not in this weather...\",TASKS\nRU_04_04_TSK_NEIGHBOR_HELP,Help Thy Neighbor,_TASKS,RU_04,Amur,truckDelivery,,320,2250,\"What do I do now... Can you imagine, I was taking some produce to the market, and I got stuck real bad. The weather is cold, so the food won't spoil too quick, but the car has to be rescued.\",TASKS\nRU_04_04_TSK_BRIDGE_A,Main Bridge,_TASKS,RU_04,Amur,cargoDelivery,3\u00d7 Metal Roll; 2\u00d7 Metal Planks,1150,8000,The infrastructure here looks prehistoric. No wonder the bridge is ruined. But we have to repair it anyway. Will you do it?,TASKS\nRU_04_04_TSK_ROCKS_C,Landslide in the West,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Metal Planks,920,7900,\"And, of course, as soon as we have bad weather, we'd better start looking for road cleaning machinery. Really, I'm just about to buy a tractor... Well, okay. Will you help?\",TASKS\nRU_04_04_TSK_ROCKS_A,Landslide in the South West,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Metal Planks,890,7350,\"I have this feeling that all I ever do is look for someone to help us with the roads. Speaking of which, could you spare some time?\",TASKS\nRU_04_04_TSK_BRIDGE_B,Crossing,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Logs Medium; 4\u00d7 Wooden Planks,740,6350,\"We have a situation. There used to be a bridge here, but it fell apart the other day. We cannot do without a crossing, so it's pretty urgent. Can I count on you?\",TASKS\nRU_04_04_TSK_ROCKS_B,Landslide in the North East,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Metal Planks,810,6000,\"The soil here is so unstable, it just slides away whenever it gets wet. The slightest rainfall blocks all the roads. Do you have a minute to help us clear it up?\",TASKS\nRU_04_04_TSK_ROCKS_D,Landslide in the North,_TASKS,RU_04,Amur,cargoDelivery,2\u00d7 Metal Planks,710,4200,The weather is horrible. Any rain causes a landslide. Will you help us clear it? I'll make it worth your while.,TASKS\nRU_04_04_TSK_STEEL_MACHINE,Breach detected,_TASKS,RU_04,Amur,exploration,,490,5150,\"Oh, do you know anything about repairs? We have a problem, and it looks like we can't fix it. Can you help us? Will you?\",TASKS\nRU_04_04__RACE_C_CNT,Round And Round,_CONTESTS,RU_04,Amur,exploration,,100,1750,How about a round race? We have that too. And we offer a prize\u2014winner takes all. That enough to pique your interest?,CONTESTS\nRU_04_04__RACE_B_CNT,Endurance Run,_CONTESTS,RU_04,Amur,exploration,,90,1500,\"The army of off-road fans includes those who like long distances. For them, we have a long route. Just be sure to fill up that tank if you want to try out your skills.\",CONTESTS\nRU_04_04_RACE_A_CNT,Highland King,_CONTESTS,RU_04,Amur,exploration,,80,1350,The local off-road fans have a sort of initiation rite here\u2014speed driving in the mountains. Care to see if the mountains welcome you?,CONTESTS\nRU_04_04__RACE_D_CNT,Street Race,_CONTESTS,RU_04,Amur,exploration,,60,1000,\"Astronaut town's no metropolis, but even here there's room to maneuver. That means there are those who like street racing, if you didn't get it. Maybe you'd be interested.\",CONTESTS\nRU_05_01_CAR_FACTORY_01,Titan Awakening,_CONTRACTS,RU_05,Don,truckDelivery,4\u00d7 Bricks,1000,10100,\"There are three main buildings we need to restore before the factory will be operational. To start, we'll need some machinery and quite a lot of bricks. There's a long road ahead, so get cracking.\",CONTRACTS\nRU_05_01_DELIVERY_01,Poddonskoye Village,_TASKS,RU_05,Don,cargoDelivery,2\u00d7 Wooden Planks,280,3300,\"I'm working on patching up a few houses here, but I'm about to run out of planks. I'm sure I've seen some decent logs in the forest, just by the powerline pole. Could you get them to the sawmill to get them cut?\",TASKS\nRU_05_01_TOWER_01,Soul Music,_TASKS,RU_05,Don,cargoDelivery,1\u00d7 Container Small,550,3250,\"Our workers have a request. Working at the river port gets so boring without some music to listen to. There's an old radio tower not far from here. We even know where to find the right parts to fix it, but we'll need your help with the delivery.\",TASKS\nRU_05_01_BOILER_ROOM_02,The Steel Phoenix,_CONTRACTS,RU_05,Don,cargoDelivery,2\u00d7 Metal Roll; 1\u00d7 Boiler,1900,15900,\"Time to spark new life into this place. And by that, I mean it's time to bring in the new boilers. The train carrying them is already on its way - if the bridge and the rail line are fixed, it should be here any minute.\",CONTRACTS\nRU_05_01_WAREHOUSE_03,Slow And Steady,_CONTRACTS,RU_05,Don,cargoDelivery,2\u00d7 Container Large; 5\u00d7 Vehicles Spare Parts,960,7950,\"One of the key structures here is the warehouse, and not gonna lie, it's basically in ruins. Worse yet, a good chunk of supplies we were sent got washed off into the river. Those need to be reclaimed first.\",CONTRACTS\nRU_05_01_RECOVERY_PIPES,Winter Is Coming,_CONTRACTS,RU_05,Don,cargoDelivery,2\u00d7 Pipe Large; 1\u00d7 Pipe Large,2280,19900,Another issue is the unfinished hot water line. If we want to keep the factory running through the winter we'll need to complete it. By now the necessary piping should already be waiting for you at Antonovka station.,CONTRACTS\nRU_05_01_RESTORATION_VILLAGE,Broken Pipes,_CONTRACTS,RU_05,Don,cargoDelivery,1\u00d7 Pipes Small; 1\u00d7 Pipes Small; 1\u00d7 Pipes Small,870,8600,\"The village could also use some work. The locals don't even have running water in their homes. What makes things worse is that the ship carrying new pipes was washed ashore, so we'll need to fetch them by land.\",CONTRACTS\nRU_05_01_PORT_RECOVERY,Don's Right Hand,_CONTRACTS,RU_05,Don,cargoDelivery,2\u00d7 Blocks; 3\u00d7 Bags,960,8550,\"First things first, the river port needs patching up. The ships are on their way, and we can't keep them waiting. Without the port, we'll have no materials to work with, and without those, there won't be a factory.\",CONTRACTS\nRU_05_01_PORT_RECOVERY_ENERGY,River Port,_CONTRACTS,RU_05,Don,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Metal Planks,630,4700,\"The ships are ready to dock, but we can't use the cranes without power. We'll have to fix a broken power pole and patch up another one. With supplies still on their way, we'll have to make do with what we can find here.\",CONTRACTS\nRU_05_01_TRACK_DELIVERY,The Toppled Giant,_TASKS,RU_05,Don,truckDelivery,,280,2550,\"So we ran into a problem here. A truck carrying the tools we need went off track on one of the nearby crossings. Now it's stuck, and if something isn't done quick we'll lose both the truck and its cargo. Will you help?\",TASKS\nRU_05_01_BRIDGE,Rural Bridge,_TASKS,RU_05,Don,truckDelivery,,290,2450,\"Our village may have seen better days, but we still need this bridge. There is a scrapyard not far from here. There, you'll find an old trailer with the tools we'll need for repairs. Could you tow it to my house?\",TASKS\nRU_05_01_WOODEN_BRIDGE,Wooden Bridge,_TASKS,RU_05,Don,cargoDelivery,2\u00d7 Wooden Planks,640,7250,\"If you're interested in earning some quick cash and getting on the locals' good side, there's a suggestion to put together a bridge over here. We've even negotiated a decent reward for it with the administration. You in?\",TASKS\nRU_05_01_BIG_BRIDGE,Old Bridge,_TASKS,RU_05,Don,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Bags,750,7000,\"They say this bridge had stood here for over 40 years before collapsing. Since it's vital that we connect the factory to the river port, it's time we get that bridge fixed.\",TASKS\nRU_05_01_FUEL,Old Gas Station,_TASKS,RU_05,Don,cargoDelivery,3\u00d7 Barrels; 2\u00d7 Service Spare Parts,690,5000,\"Since we started working on the factory, might as well fix up the nearby fuel station. There shouldn't be a problem with spare parts, as we can borrow those from the port. Sourcing the fuel though might be an issue.\",TASKS\nRU_05_01_TRACK_TATRA,The Sleeping King,_TASKS,RU_05,Don,cargoDelivery,1\u00d7 Metal Planks,640,4900,\"You know, I saw this land rise and fall. But you give me hope again. There's an old truck out on the scrapyard that got there by mistake. But you know what I think? I think it's fate that brought you to me like that truck's been waiting for you. Do me a favor, and I'll hand you its keys.\",TASKS\nRU_05_01_STONES,Scrap Pile,_TASKS,RU_05,Don,cargoDelivery,2\u00d7 Metal Planks,340,2200,\"When the factory was put out of commission, all the old scrap was simply piled up on the road. Wonder whose bright idea that was... Either way, the trash has to go, we'll need road access here. Up for it?\",TASKS\nRU_05_01_SWAMP_RACE,Lord Of The Frogs,_CONTESTS,RU_05,Don,exploration,,50,600,\"It may not look like it, but this swamp used to have a road running through it. These days it became more of a resting place for many a truck that dared to brave the murky waters. Maybe you'll be up for the challenge.\",CONTESTS\nRU_05_01_FACTORY_RACE,Driving Exam,_CONTESTS,RU_05,Don,exploration,,100,400,I've heard they're putting the factory back together. I hope they'll leave the test track as it is. Want to give it a shot?,CONTESTS\nRU_05_02_BIG_BRIDGE_01,Nature Reserve Bridge,_CONTRACTS,RU_05,Don,cargoDelivery,4\u00d7 Bags,610,4300,\"Our main objective here is to get the bridge fixed. They called it the Nature Reserve bridge before it collapsed. With the reserve so close by, repairing it could be quite an undertaking, but I'm sure we can manage.\",CONTRACTS\nRU_05_02_ROCK_01,Light Rockfall,_TASKS,RU_05,Don,truckDelivery,,280,2250,\"We get a landfall once every few years, and it just had to block off the road we need this time around. The locals will manage the repairs, but you need to deliver our tool trailer from the worker shack.\",TASKS\nRU_05_02_BRIDGE_01,The Washed-Off Bridge,_TASKS,RU_05,Don,cargoDelivery,4\u00d7 Wooden Planks; 2\u00d7 Logs Long,990,8900,\"This bridge is a real pain. Every summer, we rebuild it\u2014and the next spring, the flood washes it away. This year, we didn't even bother. But if you are planning to go to Antonovka, you could use it.\",TASKS\nRU_05_02_DELIVERY_01,Cunning Forester,_TASKS,RU_05,Don,cargoDelivery,1\u00d7 Radioctive,210,2500,\"G'day young man. There's this sawmill nearby, about as old as I am, and just past it, on the shoreline, there's a bit of old junk there. Bring it to me, and I'll give you something special in return.\",TASKS\nRU_05_02_SCOUT_01,Excursion,_TASKS,RU_05,Don,exploration,,180,1800,\"Welcome to our parts, friend. It's peaceful here, isn't it? Right, you're here for work... Well, still, do have a look around our few main attractions when you have a moment. And do let me know if everything is fine.\",TASKS\nRU_05_02_BIG_BRIDGE_02,Guardian Of The Grove,_CONTRACTS,RU_05,Don,cargoDelivery,2\u00d7 Concrete Slab; 2\u00d7 Iron Road,3400,28300,\"Now for the final push. Concrete panels should already be at the port, and there are spare rail sections at the Poddonskaya. You know what to do. Meanwhile, we'll get the site ready for your return.\",CONTRACTS\nRU_05_02_BRIDGE_02,Swamp Crossing,_TASKS,RU_05,Don,cargoDelivery,3\u00d7 Logs Medium,710,2100,\"That swamp isn't making life any easier. I'm thinking of asking for a raise just for that thing alone. Listen, there's a lumberjack camp nearby, could you fetch some logs from there for us to lay down through it?\",TASKS\nRU_05_02_DELIVERY_02,Wipe Away The Debt,_TASKS,RU_05,Don,exploration,,60,1050,\"You again? What do you want? You're still in credit, huh? Oh well. A certain something got stuck in the nearby swamp a while back, and I'm the only one in the know. Can be yours if you pry it from the swamp's cold grip.\",TASKS\nRU_05_02_RESTORATION_VILLAGE_WORKSHOP,Workshop In The Forest,_CONTRACTS,RU_05,Don,truckDelivery,,400,4300,\"There is a workshop here, back from the Soviet era. It may be old, but the mechanic there is a master of his trade. If we can get him supplies, he promised to help us out with repairs whenever we need them.\",CONTRACTS\nRU_05_02_RESTORATION_VILLAGE_REPAIR,Environmental Disaster,_CONTRACTS,RU_05,Don,truckDelivery,,280,2250,\"There is a small lake just past Antonovka favored by the locals. Guys from the sawmill agreed to clean it out, but they'll need some fuel for their equipment. There's a cistern somewhere around the service station.\",CONTRACTS\nRU_05_02_RAIL_REPAIR,Of Rails And Flash Floods,_CONTRACTS,RU_05,Don,cargoDelivery,1\u00d7 Railway; 1\u00d7 Railway; 1\u00d7 Railway,1810,20200,\"Bridge construction is all fine and dandy, but the rails themselves got about as much beating because of the rain. If we don't get that fixed, a bridge alone won't make sure a train can go by. Let's get cracking.\",CONTRACTS\nRU_05_02_RESTORATION_VILLAGE_PUMPTOWER,Let There Be Water,_CONTRACTS,RU_05,Don,cargoDelivery,2\u00d7 Bags; 4\u00d7 Metal Roll,740,6500,\"Good start, but there's still no water in the village if you don't count the puddles. The old water tower is just a heap of scrap. We've had requests to get it fixed before but now we actually have the resources for it.\",CONTRACTS\nRU_05_02_RESTORATION_VILLAGE_SUPPLIES,Antonovka's Champion,_CONTRACTS,RU_05,Don,cargoDelivery,2\u00d7 Container Small,570,5650,\"It might not look like it, but Antonovka is one of the key settlements in the region. Meaning getting on the locals' good side might be a good idea. Deliver them some supplies so we could help to restore the village.\",CONTRACTS\nRU_05_02_MAIL,Going Postal,_CONTRACTS,RU_05,Don,exploration,,230,4000,\"I know it's not really your thing, but I'm out of options. We've got a lot of mail stacked up, and there's no way our mailman can make it through these roads. You're going past Antonovka, could you lend us a hand?\",CONTRACTS\nRU_05_02_LAKE_RACE,The Quick And The Wet,_CONTESTS,RU_05,Don,exploration,,200,800,\"I've been waiting for you. Word is, you're the fastest driver out there. We used to drive around the lake for sport, and my record time still stands. Want to prove the rumors about you aren't just hot air?\",CONTESTS\nRU_05_02_HONEY,Sweet Tooth,_CONTESTS,RU_05,Don,exploration,,200,500,\"Howdy, friend. Can you do us a favor? See that church up in the lake? We were supposed to bring them some honey a month ago. Bring these cans to them. Oh, and be careful, they're quite fragile.\",CONTESTS\nUS_06_01_LOGGING_01,Delivery Through The Lowlands,_CONTRACTS,US_06,Maine,cargoDelivery,1\u00d7 Wooden Planks Medium; 2\u00d7 Logs Short; 2\u00d7 Wooden Planks Medium; 2\u00d7 Logs Short; 1\u00d7 Logs Medium; 1\u00d7 Logs Long; 1\u00d7 Wooden Planks Long,2820,27150,The locals have ordered various types of timber from us. Will you help with the delivery?,CONTRACTS\nUS_06_01_GARAGE_STAGE_01_SLOTS,Cleaning Out The Garage,_CONTRACTS,US_06,Maine,cargoDelivery,4\u00d7 Blocks; 1\u00d7 Container Large,1540,13400,\"Do you copy? After this stage is over, we'll clear some space for the vehicles in the garage. Come on, let's get it done, we'll make it worth your while\u2014you can keep some of the junk.\",CONTRACTS\nUS_06_01_GARAGE_STAGE_01_REPAIR,Constructing A Repair Garage,_CONTRACTS,US_06,Maine,cargoDelivery,2\u00d7 Vehicles Spare Parts; 4\u00d7 Barrels; 4\u00d7 Bricks,1390,13050,\"Hi! Why don't we re-open the repair shop here? Get the necessary stuff, and repairs in the garage will be available again.\",CONTRACTS\nUS_06_01_TSK_01,Big Mistake,_TASKS,US_06,Maine,cargoDelivery,3\u00d7 Barrels Oil,720,8200,\"Can you imagine, I was transporting some oil barrels, but I didn't secure them properly. So they all fell into the river and floated downstream! Please help us fish them out and bring them here before they spill!\",TASKS\nUS_06_01_ROCKS_01,Landslide Near Production Site,_TASKS,US_06,Maine,cargoDelivery,4\u00d7 Wooden Planks,410,4700,\"Did you see the size of that landslide?! I know how to clear it. Just get me what I need, and I'll help with those stones.\",TASKS\nUS_06_01_CON_01,Local Racing,_CONTESTS,US_06,Maine,exploration,,110,1900,Hi! My friends and I sometimes race around the local landmarks. Can you beat our record? We guarantee you a prize!,CONTESTS\nUS_06_01_MILL_02_STUFF,Old Sawmill,_CONTRACTS,US_06,Maine,cargoDelivery,4\u00d7 Metal Planks; 4\u00d7 Bags; 2\u00d7 Concrete Slab; 4\u00d7 Forklift Caravan Container 2,3990,37200,\"Hi! We're starting to build a new sawmill. Get the cabins for the workers and some starter cargo. Word is, they build cabins not far from here. But that place is only accessible from the mountainside.\",CONTRACTS\nUS_06_01_LOGGING_02,Delivery Through The Plateau,_CONTRACTS,US_06,Maine,cargoDelivery,1\u00d7 Wooden Planks Long; 2\u00d7 Logs Medium; 2\u00d7 Wooden Planks; 2\u00d7 Logs Short; 1\u00d7 Wooden Planks Long; 2\u00d7 Logs Medium,2170,21100,\"We got an order for wood from someone up there where the river is (which, luckily, does not flood the banks as easily). We guarantee pay.\",CONTRACTS\nUS_06_01_GARAGE_STAGE_02_TRUCKSTORE,Truck Store Construction,_CONTRACTS,US_06,Maine,cargoDelivery,2\u00d7 Container Small; 2\u00d7 Concrete Slab,1660,14100,\"Our contractor arranged for a Truck Shop to be built right here! We have to build it, and we can't do it without you.\",CONTRACTS\nUS_06_01_GARAGE_CONT_02_TUNING,Restoring The Tuning Shop,_CONTRACTS,US_06,Maine,cargoDelivery,4\u00d7 Metal Roll; 2\u00d7 Metal Planks,1220,9650,\"In order to make tuning available in the garage, we need some materials. As soon as you do it, you can improve your vehicles.\",CONTRACTS\nUS_06_01_TSK_02,Lost in the distance,_TASKS,US_06,Maine,truckDelivery,,450,4750,\"My friends and I went to take a look around and sank our car in the bog, you see! Please help us get it out! We'll fix the car ourselves, we just need to get it to the garage.\",TASKS\nUS_06_01_ROCKS_02,Shore Collapse,_TASKS,US_06,Maine,cargoDelivery,2\u00d7 Crate Large,470,3900,\"Some rocks have collapsed on the path to the shore. Let's clear the path together, just get us what we need.\",TASKS\nUS_06_01_CON_02,Port-side Racing,_CONTESTS,US_06,Maine,exploration,,120,1650,Yo-ho-ho! Who will dare beat the record of One-Eyed Bo? He may be short an eye but not of the driving skill\u2014fast but cautious! Will you improve on his best time?,CONTESTS\nUS_06_01_MILL_03_TRAILER,Trailer With New Equipment,_CONTRACTS,US_06,Maine,truckDelivery,,600,7050,\"Some equipment for our new sawmill arrived. It is up the mountains, you have to deliver it here.\",CONTRACTS\nUS_06_01_LOGGING_03,Up The Mountains,_CONTRACTS,US_06,Maine,cargoDelivery,1\u00d7 Wooden Planks Medium; 2\u00d7 Logs Medium; 2\u00d7 Wooden Planks Medium; 1\u00d7 Logs Long; 2\u00d7 Wooden Planks Long; 2\u00d7 Logs Short,2570,26500,\"Have you heard? It's all covered in snow north from here. And yet, they order wood from us. Get the cargo there, and let's be done with it.\",CONTRACTS\nUS_06_01_GARAGE_STAGE_03_TRAILERS,Trailer Store,_CONTRACTS,US_06,Maine,cargoDelivery,2\u00d7 Bags; 2\u00d7 Crate Large,1040,9800,\"Apart from the garage, we would like to arrange a trailer shop here. After you're done with this list of cargo, you can buy trailers here.\",CONTRACTS\nUS_06_01_TSK_03,Mistakes of Others,_TASKS,US_06,Maine,cargoDelivery,5\u00d7 Radioctive,1110,14200,\"Hi! Here's the deal. We had some radioactive material coming in for science experiments. Naturally, someone made a mistake, so the stuff got dropped into the river and scattered all over. Will you help us collect it?\",TASKS\nUS_06_01_ROCKS_03,North Landslide,_TASKS,US_06,Maine,cargoDelivery,1\u00d7 Wooden Planks Long,180,2250,\"Whoa, the path in the woods is blocked as well! I'll help you clear it if you help me first.\",TASKS\nUS_06_01_MILL_04_BUILD,Sawmill: The Final Stage,_CONTRACTS,US_06,Maine,cargoDelivery,4\u00d7 Crate Large; 4\u00d7 Metal Roll,1410,11400,\"This is the final stage of our sawmill construction. Deliver the last batch of cargo, and we'll start making new types of planks there.\",CONTRACTS\nUS_06_01_TSK_04,Workhorse,_TASKS,US_06,Maine,truckDelivery,,510,5550,\"Our not-so-smart meteorologists went to check out an old hut that used to belong to the station and sank their working vehicle in the bog, breaking it in the process. Will you help us return the car and fix it?\",TASKS\nUS_06_01_TSK_05,Delivery From the Pier,_TASKS,US_06,Maine,truckDelivery,2\u00d7 Bags 2,370,3800,Hi! They say you can help. We have to deliver this trailer with cargo to the customer up the river. Will you do it? We'll make it worth your while.,TASKS\nUS_06_01_TSK_06,Big Loss,_TASKS,US_06,Maine,cargoDelivery,5\u00d7 Cellulose,440,5200,\"I manage a warehouse nearby. They store cargo there before it's sent to the pier. Now, we recently got cargo from Wisconsin, and they managed to somehow flip it over and into the river, can you imagine that? We need your help.\",TASKS\nUS_06_01_TSK_07,Restorers' Trouble,_TASKS,US_06,Maine,truckDelivery,5\u00d7 Bricks,560,5500,\"The old fort on the river bank will soon be restored, and we need to get trailer with the necessary cargo there. Will you help? We'll pay generously.\",TASKS\nUS_06_01_TSK_08,All the Money's Worth,_TASKS,US_06,Maine,truckDelivery,5\u00d7 Wooden Planks,480,5250,\"I placed an order, but they refused to deliver it beyond the piers. Please, help me get it home, and I'll pay.\",TASKS\nUS_06_01_TSK_09,Very Strange Things,_TASKS,US_06,Maine,truckDelivery,,580,6650,\"Howdy, traveler. Imagine, I stepped out of the car for a mere minute, and it somehow rolled down the cliff. Must be some problem with the brakes. Can you help me pull it out and bring it home?\",TASKS\nUS_06_01_TSK_10,Small Order,_TASKS,US_06,Maine,truckDelivery,2\u00d7 Crate Large,460,4300,\"I ordered this trailer with cargo, and I need it delivered to my home. If you do it, I'll pay you.\",TASKS\nUS_06_01_TSK_11,Abandoned Titan,_TASKS,US_06,Maine,exploration,,220,1580,Someone left a real loghauling monster in the nearby swamp. Get it fixed and it's yours!,TASKS\nUS_06_01_GAS_CONT,\"Fuel Station, Revived\",_CONTRACTS,US_06,Maine,truckDelivery,,350,3050,\"Not far from the garage, there's an old gas station in need of fresh fuel. Get it there from the nearest storage, and you can get refueled here.\",CONTRACTS\nUS_06_01_MILL_LOGS,Starting The Plank Production,_CONTRACTS,US_06,Maine,cargoDelivery,1\u00d7 Logs Long; 2\u00d7 Logs Medium; 2\u00d7 Logs Short,2160,23850,\"The plant is up and running, but it has no materials! Get us some logs, and we'll be able to make excellent planks of various length.\",CONTRACTS\nUS_06_01_LOGGING_PIER,Deliver To The Pier,_CONTRACTS,US_06,Maine,cargoDelivery,2\u00d7 Wooden Planks Medium; 1\u00d7 Wooden Planks Long,600,8800,\"We're back on track, and that means we have orders! Take the planks to the pier, and we'll pay you, okay?\",CONTRACTS\nUS_06_02_BROKEN_WOOD_BRIDGE_1_TSK,Road to the Plant,_TASKS,US_06,Maine,cargoDelivery,4\u00d7 Wooden Planks; 4\u00d7 Wooden Planks,980,8500,\"The plant that supplied the region with resources was blocked off by bridges that couldn't withstand the high water. If you restore them, you can use this plant's facilities to benefit the region.\",TASKS\nUS_06_02_BROKEN_WOOD_BRIDGE_2_TSK,Crossing,_TASKS,US_06,Maine,cargoDelivery,4\u00d7 Wooden Planks,670,8050,\"This part of the way is not so easy. Unless we want to sink our truck, we need to restore the wooden crossing. Otherwise, we can't go further.\",TASKS\nUS_06_02_EXIT_C,Access To Civilization,_CONTRACTS,US_06,Maine,cargoDelivery,4\u00d7 Metal Planks; 2\u00d7 Concrete Slab,1450,18300,\"If we restore the Northeast Tunnel, we can use the highway to deliver cargo! Now we use boats, which takes forever. Get the resources to the tunnel entrance, and our workers will do the rest.\",CONTRACTS\nUS_06_02_EXIT_B,South-Bound Road,_CONTRACTS,US_06,Maine,cargoDelivery,4\u00d7 Metal Planks; 4\u00d7 Blocks,1150,16500,\"The West Tunnel has been blocked for two years. No one has made a commitment to restore it ever since, but it's vital for cargo transportation. It's not easy, but if you are up to it, the reward will be handsome.\",CONTRACTS\nUS_06_02_MOTEL_SUPPLY,Hospitality Business,_CONTRACTS,US_06,Maine,cargoDelivery,2\u00d7 Logs Short; 1\u00d7 Logs Medium; 3\u00d7 Wooden Planks Medium; 1\u00d7 Wooden Planks Long; 3\u00d7 Service Spare Parts; 3\u00d7 Crate Large; 1\u00d7 Container Small,1500,16200,\"We're trying to make this area a tourist attraction. But tourists need a place to stay, and this motel needs renovation. We have ordered some materials, but we've got a big problem with delivering them. Can you help?\",CONTRACTS\nUS_06_02_ICEWATCH,Ice Watch,_CONTRACTS,US_06,Maine,cargoDelivery,3\u00d7 Logs Medium; 4\u00d7 Wooden Planks; 2\u00d7 Service Spare Parts; 2\u00d7 Barrels; 2\u00d7 Vehicles Spare Parts,1100,12100,\"We need an observation point to control the region. There's a big watchtower in the center of the forest. Its exterior looks fine, but the inside is totaled. We need to get some equipment and materials there.\",CONTRACTS\nUS_06_02_BROKENTRUCK_TSK,\"Wrong Place, Wrong Time\",_TASKS,US_06,Maine,truckDelivery,,1030,9200,\"My truck, that I use for household needs, broke down in the worst possible place, and my work has stalled. No one is willing to tow it out and fix it. Maybe you can do it?\",TASKS\nUS_06_02_CABINSUPPLY_TSK,\"Dirt, Ice and Delivery\",_TASKS,US_06,Maine,truckDelivery,1\u00d7 Vehicles Spare Parts; 1\u00d7 Vehicles Spare Parts; 1\u00d7 Vehicles Spare Parts; 1\u00d7 Vehicles Spare Parts,1650,8700,\"We have some supplies to be delivered to observation points in the woods. But now that the snow has melted, our vehicles just won't make it. Will you be able to?\",TASKS\nUS_06_02_CABINS_TSK,\"Home, Sweet Home\",_TASKS,US_06,Maine,truckDelivery,,600,8400,\"We really need your help! I've put up a few mobile hunting cabins in the nearby woods for the winter. Now, the snow is melting, and I attempted to take them out, but my SUV is stuck. Can you get me my SUV and my cabins?\",TASKS\nUS_06_02_OILTRUCK_TSK,Crooked Path,_TASKS,US_06,Maine,truckDelivery,,780,7900,\"Sorry for bothering you, but I have no one else to ask. This truck and the fuel tank have to be delivered to a remote base. The road is rough, and our truck is broken. If you help, I will reward you well.\",TASKS\nUS_06_02_FUELSTATION_TSK,Fuel at Any Cost,_TASKS,US_06,Maine,truckDelivery,,880,7800,\"As long as the fuel supply is scarce, every drop counts in this region. Our fueling station tanks are empty. I got a lead on two small tanks with fuel that got washed off the road. Can you bring them here?\",TASKS\nUS_06_02_TURNED_SCOUT_TSK,Treacherous Ravine,_TASKS,US_06,Maine,truckDelivery,2\u00d7 Barrels Oil,1060,7100,\"It's a disaster! I was transporting some cargo with my SUV and a trailer, but I lost control of the vehicle and drove into a ravine. I was lucky I only got a few scratches. Can you help me tow out the SUV and the cargo?\",TASKS\nUS_06_02_FIREWORKS_TSK,Illegal Entertainment,_TASKS,US_06,Maine,truckDelivery,,600,6800,Someone is illegally lighting fireworks and making bonfires to the east of here. They're carrying around a barrel of fuel for their fires and car. Find it at one of the points and take it to the Impoundment Lot.,TASKS\nUS_06_02_LOADSTAR_TSK,Lost but Not Forgotten,_TASKS,US_06,Maine,truckDelivery,,430,4900,\"When the water rose, my family and I hurried to this camp. But I left behind my old truck, and I really need it. I'm afraid to go on ice, but maybe you can do it for a fitting reward?\",TASKS\nUS_06_02_TRAILER_TSK,Support Local Ranger,_TASKS,US_06,Maine,truckDelivery,,400,3900,\"I need help with the trailer I left near my cabin in the woods. All my stuff is there, but I can't reach it, the land is flooded. I'm sure your cross-country truck will have no problem.\",TASKS\nUS_06_02_BROKEN_BRIDGE_TSK,Fallen Bridge,_TASKS,US_06,Maine,cargoDelivery,3\u00d7 Concrete Slab; 3\u00d7 Metal Planks,1900,11000,\"They say you're experienced in restoring bridges. Maybe you can help with this one too? It's the only way to the Big Land, besides the river crossings, so we have to restore it ASAP.\",TASKS\nUS_06_02_BROKEN_POWERLINE_TSK,Power Tower,_TASKS,US_06,Maine,cargoDelivery,3\u00d7 Metal Planks,980,8250,\"A mudflow has recently passed here. My, did that look scary! Good thing no one was hurt... except for this power tower that's blocking the road. It has to be restored. I'd be grateful if you want to help.\",TASKS\nUS_06_02_DROWNED_CARGO_TSK,Last Season's Treasure,_TASKS,US_06,Maine,cargoDelivery,3\u00d7 Crate Large,780,6450,\"Help! Last fall, I took a shortcut while I was delivering some supply crates, and they fell in the water and froze over the very next day. Now, the ice has melted. Can you get those crates out and bring them here?\",TASKS\nUS_06_02_CHECKPLACES_TSK,Quality Assurance,_TASKS,US_06,Maine,exploration,,160,2650,\"My business is non-standard, and I am willing to pay you well. My colleagues installed water quality detectors on the river. What they didn't think about was how to access them when the ice melts. Maybe you can help?\",TASKS\nUS_06_02_KARTER_CNT,Mysterious Monoliths,_CONTESTS,US_06,Maine,truckDelivery,,110,1560,\"Have we met? Never mind. I found some weird monoliths nearby. You have to take my trailer and scan each of them, I've equipped it with detectors. Do it at night, while the monoliths are active.\",CONTESTS\nUS_06_02_DANGERRACE_CNT,Dangerous Games,_CONTESTS,US_06,Maine,exploration,,80,1250,\"Every year, this park hosts cross-country races. This year, the situation is aggravated by the frozen high water. Yet, the most reckless drivers are still willing to race. Are you?\",CONTESTS\nUS_07_01_DELIVER_RIDER_1,Lost Souls,_CONTRACTS,US_07,Tennessee,truckDelivery,,1240,11350,\"Not everyone could stand the test that is the third Stage, and a few drivers could use some help now. We don't have enough service trucks to tow everyone, so we'd appreciate if you could give them a hand.\",CONTRACTS\nUS_07_01_QUALIFICATION_01,Qualification Race,_CONTRACTS,US_07,Tennessee,exploration,,350,3500,\"This is the first step of our championship. Your goal is to beat the timer on a relatively simple off-road track. Shouldn't be something you can't handle, but you might want to pick a good off-road truck for this one.\",CONTRACTS\nUS_07_01_CIRCUIT_RACE_1,The Ring,_CONTRACTS,US_07,Tennessee,exploration,,360,1400,\"The main challenge of this stage is a single-lap race. The best advice is to capitalize on your truck's top speed and to pick the right gearbox and tires, as this one is all about making it first to the finishing line.\",CONTRACTS\nUS_07_01_DELIVER_RACING_01,Racing Truck Delivery,_TASKS,US_07,Tennessee,truckDelivery,,540,6100,\"So, we've had to let our previous driver go, but hopefully you'll be a better fit for this. Get the truck to the garage so we could patch it up, and show us what you can do.\",TASKS\nUS_07_01_DELIVER_BUGGY_01,Buggy Delivery,_TASKS,US_07,Tennessee,truckDelivery,,420,4250,\"The second stage calls for a very special vehicle. We couldn't get it fully prepped until now, so we'll need to have another good look at it before calling it done. Head for the depot, it'll be waiting for you there.\",TASKS\nUS_07_01_RESTORE_TECH_ZONE_01,Fixing The Service Station,_TASKS,US_07,Tennessee,cargoDelivery,4\u00d7 Vehicles Spare Parts,640,6600,\"So listen, we have this field repair shop, except it's been so busy lately with all the trucks we had to fix up they're running out of pretty much all supplies, not even any duct tape left. Could you do them a solid?\",TASKS\nUS_07_01_ROCK_01,Collapsed Rocks,_TASKS,US_07,Tennessee,cargoDelivery,2\u00d7 Wooden Planks Medium,180,1800,\"A guy from our team decided to take our new truck setup for a ride, but had a bit of an accident on the way. A small lapse of judgement to be sure, but he managed to cause a whole rockfall. Will you help us clear it out?\",TASKS\nUS_07_01_QUALIFICATION_01_START_SOLO,Qualification (SOLO),_CONTESTS,US_07,Tennessee,exploration,,150,1500,\"Instead of a time trial, we decided to use this track for a good old-fashion race, where you can test your driving prowess against your competitors directly. Start your engines!\",CONTESTS\nUS_07_01_EXTREME_DESCENT_01,Extreme Downhill (CO-OP),_CONTESTS,US_07,Tennessee,exploration,,100,450,\"This route wasn't initially designed for multiple vehicles, but on popular demand we decided to make an exception. The road is narrow, so plan your overtakes ahead and be ready to use every opportunity to take the lead.\",CONTESTS\nUS_07_01_ROCK_02,Old Ranch Rubble,_TASKS,US_07,Tennessee,cargoDelivery,1\u00d7 Metal Planks,620,6600,\"Used to be a round nearby, but one of them drivers rammed into the cliffside near it and, well... Let's just say there ain't no road here no more. Unless you're in the mood to help us out, that is.\",TASKS\nUS_07_01_RESTORE_TECH_ZONE_03,Repair Crisis,_TASKS,US_07,Tennessee,cargoDelivery,4\u00d7 Vehicles Spare Parts,480,4250,\"The Stone-pit is a huge waste of repair parts I tell you. Can barely catch my breath between having to fix more trucks, and I'm already almost out of spare parts. If only someone could get us a crate from the depot...\",TASKS\nUS_07_01_BRIDGE_RESTORE_03,Old Bridge,_TASKS,US_07,Tennessee,cargoDelivery,2\u00d7 Wooden Planks Medium,320,3750,\"The shortest way from the tent camp to the Pit is through that broken bridge over there. If you don't like having to take the longer way through the swamp, you could help us get the thing fixed.\",TASKS\nUS_07_01_EVACUATION,Bad Take,_CONTRACTS,US_07,Tennessee,truckDelivery,,1380,14150,\"One of our sponsors wanted to film their ad here, but something went wrong. The gear's insured, but someone would need to fix up the track itself. I thought you could be just the person we need for the job.\",CONTRACTS\nUS_07_01_CONNECTION,Service Unavailable,_CONTRACTS,US_07,Tennessee,truckDelivery,4\u00d7 Metal Planks,1770,11740,The communications station is toast! How can we broadcast live feed if we don't even have cell coverage? Need to get the thing fixed ASAP!,CONTRACTS\nUS_07_01_RENEWAL_ELECTRICITY,Supply Depot,_CONTRACTS,US_07,Tennessee,truckDelivery,4\u00d7 Barrels,460,4600,\"That's just our luck, the event is about to start, and the power is out again. We have a backup generator here, but it needs to be moved to the supply depot. Will you help us out?\",CONTRACTS\nUS_07_01_CAREER_SCOUT,Wise Old Bird,_CONTRACTS,US_07,Tennessee,truckDelivery,,560,4450,Looks like we've got another couple unlucky drivers fly off one of the nearby cliffs. Could you give them a hand?,CONTRACTS\nUS_07_01_DELIVERY_SPECIAL,Stone-pit,_CONTRACTS,US_07,Tennessee,truckDelivery,4\u00d7 Service Spare Parts Special,420,3800,\"Seems like some of our contestants weren't as gentle with the pit track during the trial runs. We've got the necessary equipment to fix this mess on standby, but we could use a hand getting it into the quarry itself.\",CONTRACTS\nUS_07_01_RESTORATION_SOLAR_STATION,Solar Power Station,_CONTRACTS,US_07,Tennessee,cargoDelivery,2\u00d7 Metal Planks; 4\u00d7 Solar Panel,950,7500,\"The depot is now powered, but a single generator can hardly cover all our needs. We were going to put some solar panels up eventually, but with the powerlines down we have a good reason to expedite the process.\",CONTRACTS\nUS_07_01_TRACK_RESTORATION,Closing The Loop,_CONTRACTS,US_07,Tennessee,cargoDelivery,3\u00d7 Wooden Planks Medium,570,7350,\"These roadhogs managed to turn the whole track into a complete mess. It's not something unexpected, but we could use some help with procuring some planks to cover the worst parts with a couple springboards.\",CONTRACTS\nUS_07_01_PREPARATION_TRAILS,Bridge Deconstruction,_CONTRACTS,US_07,Tennessee,cargoDelivery,2\u00d7 Crate Large,320,4200,Before Stage 3 can start we need to dismantle the old bridge here. It doesn't look all that safe and I'm pretty sure won't survive some of our contestants' trucks either way. Just need someone to get us the tools.,CONTRACTS\nUS_07_01_RACE_HEROES,Lap Of Honour,_CONTRACTS,US_07,Tennessee,exploration,,1140,19850,\"This is less of a competition and more like a lap of honor. The viewers would want to see you in action once more, so pick your coolest looking car and give us a good final show!\",CONTRACTS\nUS_07_01_ORIENTEERING,Off-Road Navigation,_CONTRACTS,US_07,Tennessee,exploration,,1500,9000,Time for the final round! The goal is simple - get through all the checkpoints on the swamp before the timer runs out. Good planning and a keen eye for the best routes are an absolute must in this challenge.,CONTRACTS\nUS_07_01_EXTREME_DESCENT,Extreme Downhill,_CONTRACTS,US_07,Tennessee,exploration,,200,1000,\"I bet you already know what's next. That's right, get to the finishing line in the allotted time, and the faster, the better! No need to take pity on your truck either, we've got a repair crew waiting for you already.\",CONTRACTS\nUS_07_01_EXTREME_SLOPE,Extreme Climb,_CONTRACTS,US_07,Tennessee,exploration,,100,950,This is the starting point of the second Stage. The objective is to make your way up the treacherous mountain trail. It could be wise to study the track and its surroundings before diving headlong into this challenge.,CONTRACTS\nUS_07_01_CAREER,Stage Two Finale,_CONTRACTS,US_07,Tennessee,exploration,,50,500,\"Time for another race against the clock, but with a new twist - there are three routes to choose from, and it is up to you make that call. The right truck and the right route can make all the difference here.\",CONTRACTS\nUS_07_01_CAREER_START_HARD,Gold Trail,_TASKS,US_07,Tennessee,exploration,,200,1500,\"This one's the hardest trail of the three. The staff have even started a betting pool on whether anyone can clear this trail today, and the stakes are getting quite high. You in?\",TASKS\nUS_07_01_CAREER_START_MEDIUM,Silver Trail,_TASKS,US_07,Tennessee,exploration,,100,1000,\"This one's the most popular trail, even though some drivers clearly overestimated their own skills here. Show them how it's done!\",TASKS\nUS_07_01_CAREER_START_LIGHT,Bronze Trail,_TASKS,US_07,Tennessee,exploration,,50,500,\"That's the easiest route through the Stone-pit. We need some promo shots for this one, so if you could give us a good show...\",TASKS\nUS_07_01_CIRCUIT_RACE,The Ring (CO-OP),_CONTESTS,US_07,Tennessee,exploration,,350,1000,\"As classic as a race can get, a direct competition between live opponents on a looping track. Be mindful of your surroundings and use every advantage you can to win.\",CONTESTS\nUS_07_01_QUALIFICATION,Qualification (CO-OP),_CONTESTS,US_07,Tennessee,exploration,,250,1000,\"Instead of a time trial, we decided to use this track for a good old-fashion race, where you can test your driving prowess against your competitors directly. Start your engines!\",CONTESTS\nUS_07_01_EXTREME_SLOPE_SOLO,Extreme Climb,_CONTESTS,US_07,Tennessee,exploration,,200,700,This is the starting point of the second Stage. The objective is to make your way up the treacherous mountain trail. It could be wise to study the track and its surroundings before diving headlong into this challenge.,CONTESTS\nUS_07_01_RACE_START_SOLO,The Ring (SOLO),_CONTESTS,US_07,Tennessee,exploration,,100,600,\"The main challenge of this stage is a single-lap race. The best advice is to capitalize on your truck's top speed and to pick the right gearbox and tires, as this one is all about making it first to the finishing line.\",CONTESTS\nUS_07_01_EXTREME_DESCENT_START_SOLO,Extreme Downhill (SOLO),_CONTESTS,US_07,Tennessee,exploration,,60,300,\"I bet you already know what's next. That's right, get to the finishing line in the allotted time, and the faster, the better! No need to take pity on your truck either, we've got a repair crew waiting for you already.\",CONTESTS\nUS_07_01_PARKOUR,Parkour,_CONTESTS,US_07,Tennessee,exploration,,50,100,\"This race is one huge obstacle course, where your only goal is to reach the finishing line without rolling your truck upside down. This is the one case where you could say \"\"easy does it\"\" about an actual race.\",CONTESTS\nUS_07_01_WORKOUT,Gearswitch Training,_CONTESTS,US_07,Tennessee,exploration,,10,20,\"It's all about a good racing gearbox, the right highway tires, and impeccable gear shifting. To get a headstart, try quickly going straight into high gear and back to automatic as you start to accelerate.\",CONTESTS\nRU_08_01_FARMING_FIELD_1,Farming Plot,_TASKS,RU_08,Glades,exploration,,370,5800,\"A successful harvest is always a joy, but here's the problem: my tractor broke down the other day, and now I can't plant the crop. You're an expert in this business from what I've heard. Could you help me?\",TASKS\nRU_08_01_INF_FARMING_1,Sample Field,_TASKS,RU_08,Glades,exploration,,310,50,\"Time for some more field work. As usual, we start by cultivating the field, then plant our potatos using a seeder, and finally, recover the grown crops with a harvester. Now get to it.\",TASKS\nRU_08_01_HAYSTACKS_2,Hay Delivery,_TASKS,RU_08,Glades,cargoDelivery,6\u00d7 Stack,370,3400,\"Could you give us a hand here? We've gathered hay from our fields and need to get it to the barn. Thing is, our driver had left with the first batch a while ago, and we haven't seen him since. Can you check up on him?\",TASKS\nRU_08_01_FARMING_FIELD_2,Help on the field,_TASKS,RU_08,Glades,exploration,,470,6400,\"The planting season has just begun, and I'm already running out of steam. Could you help an old man out? You get half of whatever we'll sell it for, and I'll even tell you where you can borrow the gear we'd need.\",TASKS\nRU_08_01_WIND_POWER,Clean Energy,_CONTRACTS,RU_08,Glades,truckDelivery,1\u00d7 Logs Medium; 2\u00d7 Bags; 4\u00d7 Bags 2,2100,19800,\"We've ordered the last batch of turbine fan blades recently, but one of them seems to have gone missing. We know it was unloaded at the railway station.\",CONTRACTS\nRU_08_01_ELEVATOR_STUFF,Fixing The Elevator,_CONTRACTS,RU_08,Glades,truckDelivery,4\u00d7 Service Spare Parts,870,9300,\"We've checked the grain elevator, as it's been out of use for a while now. Luckily, it only needs some minor fixes here and there, but we'll still need some supplies and tools to get this done.\",CONTRACTS\nRU_08_01_FARMING_FIELD_TUTORIAL,Sample Field,_CONTRACTS,RU_08,Glades,truckDelivery,,950,9200,\"Right, that thing looks kind of scary. But we're not here for a beauty contest, either. Let's make sure it's actually able to cover all the basics - cultivating, harvesting, you know the drill.\",CONTRACTS\nRU_08_01_HELP_RAILWAY_WORKERS,Missing Workers,_CONTRACTS,RU_08,Glades,truckDelivery,,430,4800,\"There's been a few issues with the railway just short of the northern tunnel, so we've sent a team out to give it a look. Didn't hear from them in a while though. Could you go find them, ask if everything's alright?\",CONTRACTS\nRU_08_01_KIROVEC,The Beast Rising,_CONTRACTS,RU_08,Glades,truckDelivery,,250,2000,\"Listen up. The new tractors we've ordered are taking their sweet time reaching us here, so we'll have to improvise. I'm told there's an old tractor nearby that might still be somewhat functional, it'd have to do for now.\",CONTRACTS\nRU_08_01_FACTORY_RECYCLE,Supply Recovery,_CONTRACTS,RU_08,Glades,cargoDelivery,2\u00d7 Barrels; 2\u00d7 Barrels Chemicals; 2\u00d7 Metal Planks,1270,12250,\"It's been a while since the factory close down, but we still were able to find some good materials down there. We don't really have a free truck available right now, so we ccould use some help moving them though.\",CONTRACTS\nRU_08_01_METALSCOUT,Finder's Fee,_CONTRACTS,RU_08,Glades,cargoDelivery,1\u00d7 Pipes Small; 1\u00d7 Container Large,950,9200,\"Before your arrival we tried to deal with the local drivers, even though we really shouldn't have. Now some of our stuff is strewn all over the place, and finding it might be a problem. Still, we have faith in you.\",CONTRACTS\nRU_08_01_THROW_OUT_TRASH,Taking Out The Trash,_CONTRACTS,RU_08,Glades,cargoDelivery,2\u00d7 Rubbish,590,6650,\"We've gathered a couple containers with trash from around the railway terminal. Would be nice to drivethem down to the junkyard. Might not be a very short ride, but we can't just leave it here.\",CONTRACTS\nRU_08_01_HAZARD,Risky Cleanup,_CONTRACTS,RU_08,Glades,cargoDelivery,4\u00d7 Radioctive,550,4500,\"When the local factory closed down, they tried moving some of their waste by train. The rail line got flooded though, and the whole thing was just left here to rot. How about we take care of this, now that we're here?\",CONTRACTS\nRU_08_01_VILLAGE_SUPPLIES,A Bit Of This And A Bit Of That,_TASKS,RU_08,Glades,truckDelivery,4\u00d7 Crate Large,660,6400,\"We're waiting for our driver to bring our tools from the station, but it's been a while since we've heard from him. Could you check if he's on the way? Oh, and if you could pick up the rest of our stuff, that'd be great.\",TASKS\nRU_08_01_EXPEDITION_HELP,Stocking Up,_TASKS,RU_08,Glades,truckDelivery,,540,6200,\"We're from- Not like it matters, really. Point is, we wanted to buy some fuel for our expedition, but our own driver is currently away, and there isn't anyone to help us tow the tank to our camp. Maybe you could help us?\",TASKS\nRU_08_01_ILLEGAL_BUSINESS,Shady Dealings,_TASKS,RU_08,Glades,truckDelivery,,440,4550,\"The old factory over there, tis a veritable mine of useful stuff. We scouted it out even, but we were... very rudely interrupted - even had to leave our car behind. And now we was wondering if you could go get it for us.\",TASKS\nRU_08_01_SUPPLIES_FOR_WATCHERS,Let's Pack Up,_TASKS,RU_08,Glades,truckDelivery,,250,2500,\"With the new owners in town we're dismantling the old oil pumps, and so it happens we could use some help sourcing a few tools. Any help's appreciated.\",TASKS\nRU_08_01_LOST_EVACUATION,Lapse Of Judgement,_TASKS,RU_08,Glades,truckDelivery,,240,1800,\"So a colleague of mine decided he was feeling adventurous the other day, and tried to cut strsight through the lake here. He did make it out, which isn't something I can say about his truck. Could you help him out?\",TASKS\nRU_08_01_ECOLOGICAL_CHANGE,Power Of Change,_TASKS,RU_08,Glades,cargoDelivery,1\u00d7 Container Small Special; 1\u00d7 Container Large Drilling,730,6660,\"There's a couple oil rigs over here that are long due for a retirement. Thing is, we could really use some help moving our gear out of there first. Will you help us out?\",TASKS\nRU_08_01_GRAIN_ELEVATOR,Seats Of Power,_TASKS,RU_08,Glades,cargoDelivery,4\u00d7 Crate Large,620,6350,\"Fixing the machinery is all fine and good, but our administrative wing here looks downright terrible. Ancient cracked desks, raggedy couches and chairs - far from what is appropriate for an honest business.\",TASKS\nRU_08_01_VILLAGE_RENOVATION,Supplies for Village,_TASKS,RU_08,Glades,cargoDelivery,2\u00d7 Logs Medium; 2\u00d7 Bags 2,420,4650,\"Oh hey, you're just in time. We've oredered some supplies here, but there's no one available to deliver them. Will you help us? I'll make it worth your while.\",TASKS\nRU_08_01_FARM_VEHICLE_PARTS,The Grand Restock,_TASKS,RU_08,Glades,cargoDelivery,3\u00d7 Vehicles Spare Parts,380,4450,\"It's almost the sowing season, and most of our vehicles aren't in a good shape. We've ordered the parts we need, but the delivery is taking forever. If you could deliver them instead, we'll be beyond grateful.\",TASKS\nRU_08_01_POTATO_SELL,Honest Work,_TASKS,RU_08,Glades,cargoDelivery,2\u00d7 Potato,50,3500,\"We're looking into possible shipping options for our produce. The easiest solution is to ship everything to the station, and then take it to other cities or even coutnries by train. Feel free to drop off the bags there.\",TASKS\nRU_08_01_PLANKS_SUPPLIES,Refurbishing,_TASKS,RU_08,Glades,cargoDelivery,4\u00d7 Wooden Planks,200,2250,\"Yeah, this place clearly haven't seen actual repairs in a while. Makes me wonder how our predecessors used to work here, really. Anyway. We're going to need some planks at the very least. Think you can do that?\",TASKS\nRU_08_01_DANGEROUS_BARRELS,Discarded Chemicals,_TASKS,RU_08,Glades,cargoDelivery,2\u00d7 Barrels Chemicals,270,2000,\"We've been setting up our camp a while ago, and while we went to have a look around we've spotted these barrels with some suspicious logos on them. No idea what's in those, but it's best someone moves them out of here.\",TASKS\nRU_08_01_HAYSTACKS,Roll Out!,_TASKS,RU_08,Glades,cargoDelivery,4\u00d7 Stack,100,1700,\"We've gathered a good few hay bales, but we don't really have a good way of moving them all to the barn. And the word has it that you have just the truck for the job, the Kirovets one. We'll make it worth your while!\",TASKS\nRU_08_01_SILLY_WORKERS,Exemplary Work,_TASKS,RU_08,Glades,cargoDelivery,2\u00d7 Service Spare Parts,200,1500,\"Who in their right mind unloads a train like this?! And all that cargo needs to be at the warehouse, pronto. Help me out, will you? You can take that rusty old tin can they call a crane, assuming it'll even start.\",TASKS\nRU_08_01_PROMO_PICS,Photoshoot,_TASKS,RU_08,Glades,exploration,,200,2100,\"The nature here is gorgeous, can't deny that. We're working on getting this place prepped for the upcoming tourist season, and that includes some promotions. To that end, we need all the photos we can get our hands on.\",TASKS\nRU_08_01_CHECKING_TOURISTS,Welfare Check,_TASKS,RU_08,Glades,exploration,,150,1000,\"Listen, we might have a problem here. Some tourists set up a few camps some ways from here, but it's been a while since they last checked in. Could you make sure if they're alright? I'm getting a bit worried.\",TASKS\nRU_08_01_LAKE_RACE,Lakeside Racing,_CONTESTS,RU_08,Glades,exploration,,260,260,\"The local offroaders have a pastime of driving around our lakes, a race of sorts, if you will. There's even a prize pool, too. Feel free to give it a try if you want, too.\",CONTESTS\nRU_08_01_LOCAL_FUN,Local Rally,_CONTESTS,RU_08,Glades,exploration,,150,150,\"Me and the guys have scouted out that old train down the rail line, it actually makes for a cool racing track - not that either of us managed to beat it yet though. Maybe you'll do better, what do you think?\",CONTESTS\nRU_08_02_JUNK_DELIVERY_01_CONTR,\"Waste Not, Want Not\",_CONTRACTS,RU_08,Glades,cargoDelivery,2\u00d7 Crate Large; 2\u00d7 Bricks,630,6850,\"The Institute consisted of multiple buildings scattered around the farmland. Since they appear to be abandoned, we'd like to check if there's anything of use left in either of those sites.\",CONTRACTS\nRU_08_02_BRIDGE_FIX_01,Unfinished Bridge,_TASKS,RU_08,Glades,cargoDelivery,2\u00d7 Metal Planks; 2\u00d7 Blocks,710,6800,\"Yeah, can't say this was unexpected, but... We got around to fixing this bridge, but ran way out of budget almost from the get-go. Could you help us deliver the supplies while we take care of the contstruction?\",TASKS\nRU_08_02_BRIDGE_LOGS_FIX_01,Log Bridge,_TASKS,RU_08,Glades,cargoDelivery,1\u00d7 Logs Medium,260,2400,\"Since you seem to have some business at the loacl warehouse, I bet you could use a functional bridge here, too? Shouldn't take long, and you'll be doing the locals and yourself a great favor, too.\",TASKS\nRU_08_02_JUNK_DELIVERY_02_CONTR,Scrap Stalker,_CONTRACTS,RU_08,Glades,truckDelivery,3\u00d7 Metal Roll,390,5700,\"Don't be mistaken, these aren't just piles of useless scrap. You'll never know how much of it can be cleaned and put to good use. Just bring us our equipment and we'll start working on it.\",CONTRACTS\nRU_08_02_JUNK_DELIVERY_03,Burning Bridges,_CONTRACTS,RU_08,Glades,cargoDelivery,3\u00d7 Concrete Slab,1200,13700,\"Restoring the bridge would be a very costly enterprise, not to mention, we don't have a need for something that big here. That said, I'm sure we can find something useful if we dig through what's left of it.\",CONTRACTS\nRU_08_02_WATER_TUBING,Damage Evaluation,_CONTRACTS,RU_08,Glades,truckDelivery,1\u00d7 Tank Wagon; 1\u00d7 Tank Wagon,740,9200,\"It appears the Institute wasn't keen on following safety regulations when handling their chemicals, which resulted in a nasty spill poisoning the area around the science wing. First things first, we need to pump it out.\",CONTRACTS\nRU_08_02_VEHICAL_PARTS_HELP,Down To The Last Screw,_CONTRACTS,RU_08,Glades,truckDelivery,3\u00d7 Vehicles Spare Parts,810,7750,\"We've found a couple abandoned vehicles not too far from here. While they're not really in working order, they can still be stripped for some spare parts for our own vehicles, so we could use your help towing them here.\",CONTRACTS\nRU_08_02_CHEMICAL_RESTORER,Fighting Fire With Fire,_CONTRACTS,RU_08,Glades,cargoDelivery,1\u00d7 Radioctive; 1\u00d7 Radioctive; 1\u00d7 Radioctive; 1\u00d7 Radioctive; 1\u00d7 Radioctive,890,11350,\"While we got rid of the sources of pollution, we still need to treat the nearby area with a neutralizing agent. All you need is to deliver it. And be careful - the neutralizer can still be dangerous in its own right.\",CONTRACTS\nRU_08_02_DRAIN_DRY,Living Space,_CONTRACTS,RU_08,Glades,cargoDelivery,1\u00d7 Forklift Caravan Container 2; 1\u00d7 Forklift Caravan Container 2,980,10850,\"We need to take care of the workers' living situation. There's not really a hotel or something we could rent, but we can arrange for some shacks to be built for us. All that's left is delivering the needed materials.\",CONTRACTS\nRU_08_02_CHEMICAL_BARRELS,Eco-Action,_CONTRACTS,RU_08,Glades,cargoDelivery,3\u00d7 Barrels Chemicals,650,8200,\"When the Institute was shut down nobody really bothered to properly dispose of the leftover refuse, instead dumping most of it into the river. We'll need to locate and retrieve what's left of it for further disposal.\",CONTRACTS\nRU_08_02_FARMING,Experimental Field,_CONTRACTS,RU_08,Glades,cargoDelivery,2\u00d7 Potato Gmo,710,7900,\"Time to give this new fertilizer a shot. I've been told we should be able to find all the necessary farming equipment on site. Once this is done, you'll have to deliver the samples to our lab for further study.\",CONTRACTS\nRU_08_02_MANURE_TANKS,Fertilizer Trouble,_CONTRACTS,RU_08,Glades,cargoDelivery,1\u00d7 Tank Wagon,480,7250,\"According to these papers, one of the tanks loaded with some experimental fertilizer was lost somewhere midway to the facility. We want to have a look at it, so we'd appreciate it if you could find that cistern for us.\",CONTRACTS\nRU_08_02_GMO_POTATO_BUYING,Like Hot Potatoes,_CONTRACTS,RU_08,Glades,cargoDelivery,2\u00d7 Potato Gmo,60,6000,\"We're ready to pay well for this new kind of potato. All you have to do is deliver it to us, and we'll make it worth your while.\",CONTRACTS\nRU_08_02_POWER_FIX,High Voltage,_CONTRACTS,RU_08,Glades,cargoDelivery,2\u00d7 Metal Planks; 2\u00d7 Crate Large,660,5900,\"To link the nearby district's power grids we need to restore the Institute's power lines. It's as old as the dirt it stands on, so we'd have to work with some truly ancient equipment, but we don't have much of a choice.\",CONTRACTS\nRU_08_02_TECH_FIX_CONTR,Risky Endeavor,_CONTRACTS,RU_08,Glades,cargoDelivery,2\u00d7 Barrels Oil; 2\u00d7 Vehicles Spare Parts,440,4600,\"Since we're planning to stay here for some time, we should find somewhere to fix and fuel our vehicles. The locals offered to lend us a hand if we help them get their repair station in working order.\",CONTRACTS\nRU_08_02_SEISMO_SCOUT,Deep Study,_CONTRACTS,RU_08,Glades,exploration,,300,5150,\"We have to make sure we didn't miss anything, and that the soil wasn't too damaged by the chemicals. To that end, you will need to analyze the most polluted areas. Let's get this done.\",CONTRACTS\nRU_08_02_SCIENCE_SCOUT,Lost Tech,_CONTRACTS,RU_08,Glades,exploration,,230,3900,\"The local institute researched crops back in the day, and seem to have developed a new sort of potato that grows in harsh environments. We could use any kind of documentation you can find around their facilities.\",CONTRACTS\nRU_08_02_HARVESTER_DELIVERY,Exhibit A,_TASKS,RU_08,Glades,truckDelivery,,300,3600,\"Since you seem to be working the fields often, you could use a new combine. And speaking of, there used to be one on display at the Institute that was left behind once the place shut down. You could have it if you want.\",TASKS\nRU_08_02_POSTMAN_CAR_DELIVERY_TASK,The Lost Mailman,_TASKS,RU_08,Glades,truckDelivery,,260,2550,\"Our mailman managed to get stuck somewhere in the swamps. He did manage to escape, but he had to leave the truck behind. I'm sure towing his truck back to us should be a piece of cake for you, right?\",TASKS\nRU_08_02_CONSUMABLE_MATERIAL,Final Request,_TASKS,RU_08,Glades,cargoDelivery,3\u00d7 Crate Large,390,3900,\"Thanks again for helping out with the repairs. Still, we could use some extra tools and supplies... Do us another solid, will you?\",TASKS\nRU_08_02_LOST_CARGO,The Weak Link,_TASKS,RU_08,Glades,cargoDelivery,2\u00d7 Wooden Planks,260,3400,\"We hired a driver to move a few things just down the road. He didn't secure the cargo well and lost some of it on the way. We won't hire him again for sure, but there's still the matter of retrieving our supplies.\",TASKS\nRU_08_02_DELIVERY_TASK,Priority Mail,_TASKS,RU_08,Glades,cargoDelivery,1\u00d7 Container Large,340,2800,\"We've found this container here whiler digging through our stuff. Locked shut, looks heavy, and hasn't rusted through just yet. Get it to the warehouse, we'd like to have a look inside.\",TASKS\nRU_08_02_FOR_REPAIR,A Gift Horse,_TASKS,RU_08,Glades,exploration,,150,1300,\"See this bad boy? He's already way past his prime, and I've got neither money nor patience to get it back in the saddle. Tell you what, if you'll put him back together and get him off my land, he's all yours. Deal?\",TASKS\nRU_08_02_ENDLESS_FARMING,Out In The Field,_TASKS,RU_08,Glades,exploration,,530,50,\"This experimental field should serve our little experiment well. Since we're still limited on manpower, it's up to you to tend to it - if you have a moment, of course.\",TASKS\nRU_08_02_DANGEROUS_DELIVERY,This Side Up,_CONTESTS,RU_08,Glades,cargoDelivery,1\u00d7 Container Small,100,800,\"We've gathered some of the leftover lab equipment that we could still use. The issue is, it's all fragile, and the lab wants it delivered yesterday. You can do an express delivery, right?\",CONTESTS\nRU_08_02_SWAMP_RACE,Swamp Racer,_CONTESTS,RU_08,Glades,exploration,,50,500,\"Quite a few locals had left their cars to rust here in these swamps over the year. It's all about keeping your momentum, or else you'll get stuck. Would you risk yours to see if you could beat our best time?\",CONTESTS\nRU_08_02_DOWNHILL_RACE,Bravery Is A Virtue,_CONTESTS,RU_08,Glades,exploration,,100,300,Me and the boys were placing bets on who can make it downhill fastest and without rolling. You in?,CONTESTS\nRU_08_03_SOLAR_STATION_PANEL_01_CONTR,Power Of The Sun,_CONTRACTS,RU_08,Glades,truckDelivery,3\u00d7 Solar Panel,700,6950,\"Seems like some eco-friendly trends are making their way even to remote places like this. There's an order of solar panels waiting to be delivered, as well as a trailer that was... misplaced. Will you help?\",CONTRACTS\nRU_08_03_FARMING_TASK_01,Out In The Field,_TASKS,RU_08,Glades,exploration,,550,50,\"Right, time to take care of these fields. Remember, we're paying for each field harvested and for any greens you bring us.\",TASKS\nRU_08_03_SOLAR_STATION_PANEL_02_CONTR,Power Of Two Suns,_CONTRACTS,RU_08,Glades,cargoDelivery,2\u00d7 Solar Panel; 1\u00d7 Container Large,1000,9700,\"Looks like the company likes doing things big, and just one solar power plant isn't enough in their book. Like before, they need a full shipment of panels and another trailer loaded with fittings for them. Get to it.\",CONTRACTS\nRU_08_03_SOLAR_STATION_PANEL_03_CONTR,Power Of A Thousand Suns,_CONTRACTS,RU_08,Glades,cargoDelivery,3\u00d7 Solar Panel; 2\u00d7 Metal Roll,880,9100,\"You'd be surprised, but they want another power plant set up - last one, this time. Same as before, solar panels already awaiting pickup. You'll need another few things, too, but the local farm can help you with that.\",CONTRACTS\nRU_08_03_GENERATOR_DELIVERY_CONTR,Let There Be Light,_CONTRACTS,RU_08,Glades,truckDelivery,,220,2300,\"The power grid here is nowhere near stable enough for us to power our machinery. While the local electricians are busy with the nearby villages, could you fetch a spare generator for us?\",CONTRACTS\nRU_08_03_GARBAGE_DELIVERY_CONTR,\"Reduce, Reuse\",_CONTRACTS,RU_08,Glades,cargoDelivery,4\u00d7 Rubbish,1430,15450,\"Our region is famous for its natural beauty and well-developed farming industry. Garbage disposal, on the other hand? Not so much. Right now, we could really use a hand dealing with some more trash here.\",CONTRACTS\nRU_08_03_TRANSFORMATOR_RESTORE,Power Transformer,_CONTRACTS,RU_08,Glades,cargoDelivery,4\u00d7 Metal Planks; 1\u00d7 Container Small Special,980,8600,We'll need to fix one last thing to link the local and the corporate power grids - the main transformer. We can get it done as soon as you bring us some tools and find another parts shipment that hadn't arrived yet.,CONTRACTS\nRU_08_03_FARMING,Thirty Harvestless Years,_CONTRACTS,RU_08,Glades,cargoDelivery,1\u00d7 Potato; 1\u00d7 Potato,730,7900,\"You may have noticed that the nearby farm isn't exactly doing great these days. Most of the 'farmers' are... less-than-cooperative as of late. Maybe we can lead them by example, what do you think?\",CONTRACTS\nRU_08_03_ROAD_WAYS_FIX_CONTR,Railway Blues,_CONTRACTS,RU_08,Glades,cargoDelivery,1\u00d7 Railway,530,6000,\"The railroad here is all but abandoned. We've already taken a look at the damage there, and realistically, only a single rail segment needs to be completely replaced. Now, getting it there is another matter entirely...\",CONTRACTS\nRU_08_03_SOLAR_STATION_WIRING,Closing The Circuit,_CONTRACTS,RU_08,Glades,cargoDelivery,1\u00d7 Logs Medium; 1\u00d7 Logs Medium,500,5150,\"The plants are up and running, and their yield is enough to help the local villages, too. The power line wasn't properly set up yet, so it's up to us to get it fixed.\",CONTRACTS\nRU_08_03_RESYCLE_DELIVERY,Clean Slate,_CONTRACTS,RU_08,Glades,cargoDelivery,1\u00d7 Vehicles Spare Parts; 1\u00d7 Vehicles Spare Parts; 1\u00d7 Vehicles Spare Parts,410,4600,\"We're slowly ramping up our recycling efforts on the junkyard, and some progress is already being made on getting it cleaned out. We'd really appreciate it if you could deliver these containers to the nearby villages.\",CONTRACTS\nRU_08_03_COLLECTIVE_FARM_METALLDETECTING,Treasure Hunt,_CONTRACTS,RU_08,Glades,cargoDelivery,1\u00d7 Container Small Special,270,2750,\"Before we can begin construction we need to make sure we're stocked on all the necessary tools and materials. The bad news is, the container we leased was lost in the nearby swamp. You know what to do.\",CONTRACTS\nRU_08_03_TRAILER_MATERIALS_DELIVERY,Missing Supplies,_TASKS,RU_08,Glades,truckDelivery,,640,6650,\"A while ago, our farmers ordered some basic supplies that never made it here. Could you have a look around, maybe you'll be able to find them?\",TASKS\nRU_08_03_HAUL_TRANSPORT,The Common Cause,_TASKS,RU_08,Glades,truckDelivery,,600,6650,\"One of our farmers managed to get our only tractor stuck out there, and now we have nothing to work on our fields with. We've contacted the insurance, but in the meantime, could you see if there's anything you can do?\",TASKS\nRU_08_03_PRODUCT_DELIVERY,Supply Chain,_TASKS,RU_08,Glades,truckDelivery,,460,5800,\"Some of the cargo that was lost due to the storm was a restock container for the local store, and now they're basically out of stock. Now they have to find that shipment before they could make any profit whatsoever.\",TASKS\nRU_08_03_CARGO_DELIVERY_TRUCK,Abandoned Gear,_TASKS,RU_08,Glades,truckDelivery,,460,5700,\"So, there's that island here that we tried to settle a while back. We had to drop that idea when the storms came, but we had to leave one of our trucks there. Could you please fetch it for us?\",TASKS\nRU_08_03_TRANSPORT_EVACUATION,Emergency Retrieval,_TASKS,RU_08,Glades,truckDelivery,,420,5100,\"One of our trucks was caught in the nearby swamp when the storm hit. The driver made it out, but there's no way we can get that truck out on our own.\",TASKS\nRU_08_03_POTATO_SELLING,Grocery Run,_TASKS,RU_08,Glades,cargoDelivery,2\u00d7 Potato,50,5800,\"As we agreed before, we're ready to pay for whatever those fields would yield. You just have to get it for us.\",TASKS\nRU_08_03_ROAD_BLOCKAGE,River Trouble,_TASKS,RU_08,Glades,cargoDelivery,2\u00d7 Service Spare Parts Special,310,3650,\"The flooding that followed the recent storm washed a lot of uprooted trees into the river, creating a few blockages down the stream. We could really use some help removing the biggest one of them.\",TASKS\nRU_08_03_BROKEN_PIPES_RESTORE,Force Of Nature,_TASKS,RU_08,Glades,cargoDelivery,1\u00d7 Pipes Medium; 1\u00d7 Pipes Medium,400,3550,\"You've probably heard about the reacent weather anomalies that devastated the region. Sadly, even the water supply was not spared by Mother Nature. Now they could really use some help getting the water pipeline fixed.\",TASKS\nRU_08_03_LOST_GARBAGE_DELIVERY,Lost Treasure,_TASKS,RU_08,Glades,cargoDelivery,1\u00d7 Rubbish,290,2750,\"So get this. There's a guy living out here, and he contacted us the other day, said he wants his trash container back. Even says he'd pay us if we bring back his little treasure. Will you help the poor guy?\",TASKS\nRU_08_03_HAY_DELIVERY,Empty Hayloft,_TASKS,RU_08,Glades,cargoDelivery,3\u00d7 Stack,200,2250,\"That's quite a mess alright. We got some hay for our livestock, but it got dropped off just by the fields and not where it's needed. At least we didn't have to pay for the delivery, so there's money in it for you, too.\",TASKS\nRU_08_03_TV_TOWER_RESTORE,Tune In,_TASKS,RU_08,Glades,cargoDelivery,2\u00d7 Metal Roll,240,2200,\"A recent storm has seriously damaged the local telecommunications network. The thing that needs the most work is the TV tower, and we think you might be just the man for the job.\",TASKS\nRU_08_03_BARN_RESTORE,High Wind Warning,_TASKS,RU_08,Glades,cargoDelivery,1\u00d7 Logs Long,240,2100,\"The recent Southern winds have really battered the local villages. One of the nearby barns was quite literally blown away. If you can help them deliver some logs to get it fixed, they'll make it worth your while.\",TASKS\nRU_08_03_URGENT_DELIVERY_CARGO,Special Delivery,_CONTESTS,RU_08,Glades,cargoDelivery,1\u00d7 Container Small,200,1500,\"There's a sensitive bit of cargo that we need to get to the village ASAP. Time is of the essence, so the faster you can manage it, the better. Think you can do that?\",CONTESTS\nRU_08_03_FIELDS_RACING,Aim To Misbehave,_CONTESTS,RU_08,Glades,exploration,,150,150,\"Hey uh, I've a weird request for you. We need to remove all the old scarecrows from the field so we can go over it with tractors, and the sooner it's done, the better. You in?\",CONTESTS\nRU_08_04_CONNECTION_01,Comms Tower,_CONTRACTS,RU_08,Glades,truckDelivery,,500,5750,\"There is a communications tower not far away. An old thing, this, and it hardly gives any coverage. Would be nice if someone could have a look at it, maybe patch it up. We'll find a way to pay you back.\",CONTRACTS\nRU_08_04_COMPLEX_RECOVERY_01,Byre,_CONTRACTS,RU_08,Glades,cargoDelivery,2\u00d7 Wooden Planks Medium; 2\u00d7 Metal Planks,490,3900,\"Making sure we have the space to house our animals is instrumental to turning this place into one massive farm. To this end, we'll need your help constructing a few things - first and foremost, a byre for our cows.\",CONTRACTS\nRU_08_04_SPECIAL_DELIVERY_01,When Luck Runs Out,_TASKS,RU_08,Glades,truckDelivery,,420,4250,\"Help a brother out, we've been waiting for a car to arrive for like a day now, and it turns out it's been stuck in the mud just down the road, and you're the first person to drive by us so far.\",TASKS\nRU_08_04_LOGS_01_A,Log Bridge,_TASKS,RU_08,Glades,cargoDelivery,2\u00d7 Logs Medium,520,5300,\"Not long ago we used this trail to reach the river crossing, but after the recent storms the mud here got way too deep for our trucks. You think you could help us fix it?\",TASKS\nRU_08_04_FIELD_PROCESSING_01,Out In The Field,_TASKS,RU_08,Glades,exploration,,400,50,\"We've got some work just for you. We're getting the fields ready for the upcoming season. All that's left is a bit of manual labor, and we're ready to pay well to get it done.\",TASKS\nRU_08_04_COMPLEX_RECOVERY_02_CONTR,Keep Refrigerated,_CONTRACTS,RU_08,Glades,cargoDelivery,2\u00d7 Metal Planks; 2\u00d7 Vehicles Spare Parts,1130,11200,\"Since we have some space to spare, we were thinking of improving our dry storage situation a bit. The warehouse we have right now is really not in a good shape, so we were thinking of getting it fixed.\",CONTRACTS\nRU_08_04_LOGS_02_A,Flooded Crossing,_TASKS,RU_08,Glades,cargoDelivery,2\u00d7 Logs Medium,510,5150,\"There used to be a crossing just next to the old bridge, but it's been flooded over as of late. It'd be nice to find at least a temporary bypass for it.\",TASKS\nRU_08_04_BRIDGE_RESTORE_02,Unfinished Bridge,_TASKS,RU_08,Glades,cargoDelivery,2\u00d7 Metal Planks,350,2450,\"There's only one bridge that leads to our fields, and it stood there unfinished for a while now. We'll have to patch it up if we want to avoid taking a detour each time we need to cross the river.\",TASKS\nRU_08_04_COMPLEX_RECOVERY_03,Chicken Coop,_CONTRACTS,RU_08,Glades,cargoDelivery,2\u00d7 Wooden Planks Medium; 4\u00d7 Metal Roll,640,5500,\"A farmer's life isn't just about tilling fields - tending to livestock is just as important. We'll need to set up some extra coops if we want to supply the whole region with produce - and to make some profit, of course.\",CONTRACTS\nRU_08_04_WINDMILL,Wind Turbine,_CONTRACTS,RU_08,Glades,truckDelivery,,1070,14000,\"When first settling in this area we decided to focus on renewable energy sources. Most of our turbines are already online, but one of the blades for them was damaged during shipping. This means we'll need a replacement.\",CONTRACTS\nRU_08_04_LOST_TRUCKS,Cutting Losses,_CONTRACTS,RU_08,Glades,truckDelivery,,780,7300,\"Listen, we could really use your help again. A truck was suposed to arrive here earlier today, but the driver got into a bit of an accident and now needs to be towed. Could you do that, please?\",CONTRACTS\nRU_08_04_GARAGE_RECOVERY_CONTR,Fixing the Fixers,_CONTRACTS,RU_08,Glades,cargoDelivery,4\u00d7 Service Spare Parts; 2\u00d7 Metal Planks,2120,23800,\"It's been a good while since anyone worked on actual trucks here, so it'll take some good effort to get this place ready for business. Of course, things'd go much smoother if you could bring us a few bits and ends.\",CONTRACTS\nRU_08_04_POTATO_BUYING_META,Bulk Loading,_CONTRACTS,RU_08,Glades,cargoDelivery,8\u00d7 Potato,930,6700,\"It's time to prepare a full batch of the new experimental taters to be shipped off. You know the drill by now. Hopefully, your expertise will get us all through.\",CONTRACTS\nRU_08_04_WIRE_PULLING,Not-So-High Voltage,_CONTRACTS,RU_08,Glades,cargoDelivery,1\u00d7 Wooden Planks Medium; 1\u00d7 Wooden Planks Medium; 1\u00d7 Wooden Planks Medium,540,5550,\"One of the power lines was damaged in the recent storm, and if we won't do something about it fast, we'll lose power any minute now. Naturally, extra work requires extra pay, too.\",CONTRACTS\nRU_08_04_AIRPORT_RECOVERY,Airport,_CONTRACTS,RU_08,Glades,cargoDelivery,4\u00d7 Barrels Oil,680,5400,\"There used to be an airfield just up the hill, and we could really use it, too. Thing is, the landing strip there is worse than trying to land a plane in an open field, and fixing that requires some additional materials.\",CONTRACTS\nRU_08_04_FISHING,Gone Fishing,_TASKS,RU_08,Glades,truckDelivery,,530,6250,\"You know that little lake down South? There's a tiny island there, a great place to relax and get away from everything. Thing is, I managed to get my truck stuck there, so could you help me out?\",TASKS\nRU_08_04_LOST_CARGO,Down The Hatch,_TASKS,RU_08,Glades,cargoDelivery,4\u00d7 Service Spare Parts,500,6000,\"One of our drivers missed the right turn while crossing the river nearby and managed to drop some of our cargo into the water. Could you fetch them for us, if it's not too much trouble?\",TASKS\nRU_08_04_HAY_DELIVERY,Feeding time,_TASKS,RU_08,Glades,cargoDelivery,3\u00d7 Stack,400,4650,\"We have already purchased some of the livestock feed, but this is not enough, and the budget is limited. Here in many fields there are abandoned haystacks. Bring us a few and we'll arrange delivery with a premium.\",TASKS\nRU_08_04_POTATO_BUYING,Grocery Run,_TASKS,RU_08,Glades,cargoDelivery,2\u00d7 Potato,50,3600,\"Always a pleasure to deal with you. As always, just bring us the goods, and we'll pay you back in full.\",TASKS\nRU_08_04_IMPORTANT_CARGO,Important Cargo,_TASKS,RU_08,Glades,cargoDelivery,2\u00d7 Container Small,430,3300,\"Oh man, are we in trouble. Our driver managed to lose his cargo that we're supposed to ship real soon, and there's no way we can get it out on our own. Can you help us out?\",TASKS\nRU_08_04_LAKE,In Search Of Inspiration,_TASKS,RU_08,Glades,exploration,,460,8000,\"Did you have a moment to look around here? No? Well, you just have to see all this beauty with your own eyes - after all, many a poet and artist were inspired by it.\",TASKS\nRU_08_04_CONTEST,Treading Water,_CONTESTS,RU_08,Glades,exploration,,100,1200,\"Since you've been driving around these parts for a while, how about a little test of your mettle? I'm sure you've seen worse, but it's still all too easy to get completely stuck in this mud.\",CONTESTS\nRU_08_04_RING,Crop Circle,_CONTESTS,RU_08,Glades,exploration,,125,1000,\"It's a little pastime we have here, circuit racing. The nice thing is that it's an opportunity to have a good look around the fields here. Just don't get too distracted, or you'll risk going straight off the track.\",CONTESTS\nUS_09_01_LOG_HOUSE_CONTRACT_01,Timber For The Locals 1,_CONTRACTS,US_09,Ontario,cargoDelivery,1\u00d7 Wooden Planks Medium; 1\u00d7 Logs Long; 1\u00d7 Wooden Planks Long; 2\u00d7 Logs Short,1500,17950,\"The local government asks us to provide some wood for the residents here. The trick is that they live far from the town, a long way from the sawmill. Please, help us, and we'll provide a handsome reward.\",CONTRACTS\nUS_09_01_WATER_TOWER_CONTRACT_01,Restoring The Water Tower 1,_CONTRACTS,US_09,Ontario,cargoDelivery,1\u00d7 Forklift Caravan Container 2; 2\u00d7 Metal Roll,500,4500,\"To ensure prompt water supply to fire sites, we need to set up proper structures. Namely, restore the old water towers that we used many years ago.\",CONTRACTS\nUS_09_01_WATER_TOWER_TASK_01,Water Boom 1,_TASKS,US_09,Ontario,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Metal Roll; 1\u00d7 Pipes Small,500,4500,\"For easier fire extinguishing, we should think of restoring all the towers in the area, and we'll start with this one.\",TASKS\nUS_09_01_LOG_HOUSE_CONTRACT_02,Timber For The Locals 2,_CONTRACTS,US_09,Ontario,cargoDelivery,1\u00d7 Wooden Planks Long; 1\u00d7 Logs Medium; 1\u00d7 Wooden Planks Medium; 3\u00d7 Logs Short,1730,21800,\"Two more houses here, on the farthest island between both shorelines. Same plan: you deliver the wood, and we pay you back.\",CONTRACTS\nUS_09_01_WATER_TOWER_CONTRACT_02,Restoring the Water Tower 2,_CONTRACTS,US_09,Ontario,cargoDelivery,1\u00d7 Forklift Caravan Container 2; 2\u00d7 Pipes Small,500,4500,\"The pipes are set so that we can pump water in two directions, so it would be great if you help finish this tower as well. Obviously, we'll pay.\",CONTRACTS\nUS_09_01_WATER_TOWER_TASK_02,Water Boom 2,_TASKS,US_09,Ontario,cargoDelivery,1\u00d7 Forklift Caravan Container 2; 1\u00d7 Metal Planks; 1\u00d7 Metal Roll,500,4500,\"Listen buddy, we thought we'd fix the tower ourselves, but something went wrong. Could you help us out, please? We'd really appreciate it.\",TASKS\nUS_09_01_WATER_TOWER_TASK_03,Water Boom 3,_TASKS,US_09,Ontario,cargoDelivery,1\u00d7 Forklift Caravan Container 2; 1\u00d7 Metal Roll; 2\u00d7 Bags 2,500,4500,\"Perhaps restoring a tower in the middle of this chaos was not the best idea, but this will help you do your job. Can you give us a hand?\",TASKS\nUS_09_01_WATER_TOWER_TASK_04,Water Boom 4,_TASKS,US_09,Ontario,cargoDelivery,2\u00d7 Bags 2; 2\u00d7 Pipes Small,500,4500,\"And another one. Same story, you help us transport the cargo, we provide working water storage and an additional bonus.\",TASKS\nUS_09_01_GOLD_TRUCK_DELIVERY_CONTRACT,Work Transport Evacuation,_CONTRACTS,US_09,Ontario,truckDelivery,,890,9700,\"Apparently, the fire has not spread beyond that forest patch, but we would really appreciate it if you could evacuate our expensive transport, just to be on the safe side. Do it, and we'll reward you.\",CONTRACTS\nUS_09_01_FIRE_STATION_VEHICLE_CONTRACT,Looking For The Fire Monster,_CONTRACTS,US_09,Ontario,truckDelivery,,370,4200,\"My team managed to evacuate in the middle of their mission, leaving our working vehicle behind. From what we know, it hasn't suffered any significant damage, so we need to find it, bring it back and fix it. If you find it, you can keep it.\",CONTRACTS\nUS_09_01_GOLD_DELIVERY_CONTRACT,Serious Business,_CONTRACTS,US_09,Ontario,cargoDelivery,8\u00d7 Gold,2450,25200,\"You proved yourself a real hero, we put out the fire and restored almost all of the infrastructure. You earned our trust, so we ask you to transport our most valuable cargo\u2014gold! For a matching reward, of course.\",CONTRACTS\nUS_09_01_WATER_DELIVERY_RAILWAY_CONTRACT,Putting Out The Fire At The Station,_CONTRACTS,US_09,Ontario,cargoDelivery,6400\u00d7 Water; 6400\u00d7 Water,2660,23400,\"Okay, step by step, we're getting there, thanks to you. But it's not quitting time yet. This railway station is the main logistic hub for local business, so we need to stop the fire from spreading.\",CONTRACTS\nUS_09_01_WATER_DELIVERY_WAREHOUSE_CONTRACT,Putting Out The Fire At The Storage 1,_CONTRACTS,US_09,Ontario,cargoDelivery,4400\u00d7 Water; 4400\u00d7 Water; 4400\u00d7 Water,1890,14250,We used the data you helped collect and found out that the resource storage facilities that belong to the gold mining company have almost completely burned down. We have to put out the fires and use what remains of the buildings for materials.,CONTRACTS\nUS_09_01_GARAGE_RESTORE_CONTRACT,Service Center,_CONTRACTS,US_09,Ontario,cargoDelivery,2\u00d7 Metal Roll; 2\u00d7 Metal Planks; 2\u00d7 Bricks,1150,10550,\"First things first: the whole Fire Department thanks you for coming to our rescue! Now, back to business: we'll start with delivering some materials for the Service Center construction.\",CONTRACTS\nUS_09_01_RAILWAY_DELIVERY_CONTRACT,Restoring The Railroad Tracks,_CONTRACTS,US_09,Ontario,cargoDelivery,1\u00d7 Iron Road,1040,9100,\"To fully restore logistics, we need to repair the railroad tracks. Pick up a few sections from the mine and deliver them straight to the railway station. This is a very important job, and we are counting on you!\",CONTRACTS\nUS_09_01_BURNED_LOGS_CLEARING_CONTRACT,Forest Cleaner,_CONTRACTS,US_09,Ontario,cargoDelivery,4\u00d7 Burnt Logs,840,8400,\"Alas, our area's best forests were destroyed by the fire, but some of the trees can still be rescued. The local volunteers have already gathered some of the logs and you have to haul them to the sawmill.\",CONTRACTS\nUS_09_01_AFFECTED_INFROSTRUCTURE_SCOUT_CONTRACT,Exploring The Burned-Out Infrastructure,_CONTRACTS,US_09,Ontario,exploration,,120,2000,\"Let's start from the beginning. We've put out the fire and contained it, for the most part. A lot of the infrastructure is still covered in smoke. We have to explore the surrounding area and see what we can do.\",CONTRACTS\nUS_09_01_WORKER_SCOUT_EVACUATION_TASK,Abandoned Transport,_TASKS,US_09,Ontario,truckDelivery,,440,5450,\"Can you imagine? I worked in the middle of the fire and, when it got really hot, I ran away and left my car behind. Luckily, it wasn't in the fire. Could you please bring it here, for a price, obviously.\",TASKS\nUS_09_01_RANGER_SCOUT_EVACUATION_TASK,Bad trip,_TASKS,US_09,Ontario,truckDelivery,,410,3950,\"Hey, buddy, can you please help me get my car back? It's vital for my business! My protege screwed up a trip to one of the camps and the car just rolled down. You help me, I help you.\",TASKS\nUS_09_01_BAGS_DELIVERY_TASK,Delivery From the Port,_TASKS,US_09,Ontario,truckDelivery,2\u00d7 Bags,410,3950,\"There is a family living not far from here, in the hills. We need to deliver this trailer with cargo to them and we would like to hire you for that. We will make it worth your while.\",TASKS\nUS_09_01_LOST_RADIOACTIVE_DELIVERY_TASK,Contamination Removal,_TASKS,US_09,Ontario,cargoDelivery,5\u00d7 Radioctive,1260,17700,\"Our guys were transporting hazardous chemicals but lost all the cargo en route. Someone needs to ride to the mine, get the location of losses, then collect it all and deliver to us. Will you do that?\",TASKS\nUS_09_01_LOST_BOAT_TASK,Swimming The Wrong Way,_TASKS,US_09,Ontario,cargoDelivery,3\u00d7 Barrels Chemicals,800,9600,\"You know, this dupe had to haul something across the river, but he took the wrong route and got stuck in the rift. Can you please drive there and check if there's anything that can be rescued?\",TASKS\nUS_09_01_LOST_MECHANISM_TASK,Floating Detectors,_TASKS,US_09,Ontario,cargoDelivery,1\u00d7 Service Spare Parts Special; 1\u00d7 Service Spare Parts Special; 1\u00d7 Service Spare Parts Special,660,8100,\"We were driving along a dangerous route near the waterfall, and we lost all the cargo with our underground fire detectors. It should have survived the fall, so we need to gather the detectors and deliver them to the burned forest.\",TASKS\nUS_09_01_SEEMAN_TASK,Far Far Away,_TASKS,US_09,Ontario,cargoDelivery,2\u00d7 Metal Roll; 2\u00d7 Bags 2,780,7700,\"So what if I live in a \u201cweird\u201d place? You help me, I help you. Sounds fair to me.\",TASKS\nUS_09_01_TRAILERS_LOST_STUFF_TASK,Help the Evacuees,_TASKS,US_09,Ontario,cargoDelivery,2\u00d7 Container Small,620,7150,\"Please help us, friend, we had to evacuate quickly, so we abandoned our trailer in a hurry and left some personal belongings behind. Bring the trailer here, and we'll make it worth your while.\",TASKS\nUS_09_01_PORT_RESOURCES_DELIVERY_TASK,Deliver to the Port,_TASKS,US_09,Ontario,cargoDelivery,1\u00d7 Forklift Caravan Container 2; 2\u00d7 Blocks,630,5550,\"With everything that happened, the local boaters are overwhelmed. Please, bring something to help fix the boats, and we'll pay you back!\",TASKS\nUS_09_01_MINE_GARBAGE_TASK,Taking Out The Junk From The Mine,_TASKS,US_09,Ontario,cargoDelivery,2\u00d7 Rubbish,550,5300,Looks like they forgot to clear out some junk from one of the recently abandoned mines. There are a lot of usable materials there that may be handy for our guys at the camp. Would you be so kind as to get the containers there?,TASKS\nUS_09_01_BRIDGE_TASK,\"The Bridge, Again\",_TASKS,US_09,Ontario,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Blocks,450,4350,\"Following this highway any further will be a problem, unless we fix this bridge. We could've done it ourselves, but all this fire havoc messed up our plans. So, will you help us?\",TASKS\nUS_09_01_TOURIST_CAMPING_TASK,Caring for tourists,_TASKS,US_09,Ontario,exploration,,550,9400,\"Hey! As you may know, our region is very popular with tourists and there are some very popular outdoor recreation areas in the area. Could you run over them and check that everything is fine?\",TASKS\nUS_09_01_BARRELS_EXPRESS_DELIVERY_CONTEST,Express Fuel Delivery,_CONTESTS,US_09,Ontario,cargoDelivery,1\u00d7 Barrels; 1\u00d7 Barrels,260,1500,\"Buddy, we badly need some fuel delivered to our mines, ASAP, their machinery has halted, so hurry!\",CONTESTS\nUS_09_02_WATER_TOWER_CONTRACT_01,Restoring The Water Tower 3,_CONTRACTS,US_09,Ontario,cargoDelivery,1\u00d7 Crate Large; 3\u00d7 Pipes Small,500,4500,\"There's not a single working water tower in this area, even though we started some work already. As usual, you deliver the materials, we do the repairs.\",CONTRACTS\nUS_09_02_VR_TASK_01,Help The Villagers 1,_TASKS,US_09,Ontario,cargoDelivery,3\u00d7 Wooden Planks; 2\u00d7 Bags 2,610,6400,\"The recent accident and the flooding damaged our infrastructure pretty bad. We need materials to repair houses and hangars, could you help us deliver them? For a reward, of course.\",TASKS\nUS_09_02_WATER_TOWER_TASK_01,Water Boom 5,_TASKS,US_09,Ontario,cargoDelivery,1\u00d7 Forklift Caravan Container 2; 3\u00d7 Metal Roll,500,4500,\"Here we are, almost in the heart of the fire. If you restore this tower, your fire-extinguishing job will be a tad easier. And we'll make it worth your while, too.\",TASKS\nUS_09_02_WATER_TOWER_CONTRACT_02,Restoring The Water Tower 4,_CONTRACTS,US_09,Ontario,cargoDelivery,1\u00d7 Crate Large; 2\u00d7 Metal Roll,500,4500,\"Here's another good place to get water from. The tower isn't new, but we can put it in order as well. Waiting for your signal.\",CONTRACTS\nUS_09_02_WATER_TOWER_TASK_02,Water Boom 6,_TASKS,US_09,Ontario,cargoDelivery,1\u00d7 Crate Large; 2\u00d7 Bags 2,500,4500,\"The final refuge! These are the closest points for tower restoration. Out there, it's just fires and scorched earth. Let's finish these water monsters already and flood the whole area!\",TASKS\nUS_09_02_VR_TASK_02,Help The Villagers 2,_TASKS,US_09,Ontario,cargoDelivery,4\u00d7 Vehicles Spare Parts,370,3900,\"After what happened, we ordered some spare parts to repair our machines, but the truck that was carrying them turned over. Could you please gather the cargo and bring it here?\",TASKS\nUS_09_02_VR_TASK_03,Help The Villagers 3,_TASKS,US_09,Ontario,truckDelivery,,600,7850,\"This is practically our only working vehicle, and we lent it to our friends from the gold mine, so they can evacuate people from the factory. They are done, so could you bring the vehicle back?\",TASKS\nUS_09_02_WATER_TOWER_TASK_03,Water Boom 7,_TASKS,US_09,Ontario,cargoDelivery,1\u00d7 Forklift Caravan Container 2; 1\u00d7 Metal Planks; 1\u00d7 Metal Roll,500,4500,\"One more! A dangerous spot, but who wouldn't like a little risk? In fact, this is the last good spot, so we're determined to finish what we started. You with us?\",TASKS\nUS_09_02_WATER_DELIVERY_FACTORY_CONTRACT,Putting Out The Fire At The Factory,_CONTRACTS,US_09,Ontario,cargoDelivery,5900\u00d7 Water; 5900\u00d7 Water; 5900\u00d7 Water,3750,30900,\"Indeed, the most dangerous spot, practically all burned, nothing left. Let's try saving the infrastructure and whatever else can be saved. I can only wish good luck to all of us.\",CONTRACTS\nUS_09_02_SEISMO_WATER_DELIVERY_CONTRACT,Putting Out The Underground Fires,_CONTRACTS,US_09,Ontario,cargoDelivery,2200\u00d7 Water; 2\u00d7 Bags 2; 2200\u00d7 Water; 2\u00d7 Bags 2; 2200\u00d7 Water; 2\u00d7 Bags 2,2610,19800,\"Now that we established the causes and exact coordinates, we need to put out those underground fires. The liquid sand mixture should help. This is extremely dangerous, but we're counting on you!\",CONTRACTS\nUS_09_02_FIRE_WATCHTOWER_CONTRACT,Cautiousness,_CONTRACTS,US_09,Ontario,cargoDelivery,2\u00d7 Wooden Planks; 2\u00d7 Logs Short; 3\u00d7 Service Spare Parts; 2\u00d7 Barrels,1680,19500,\"There is an old watchtower somewhere in the mountains. The gold mining company suffered huge damages from their burned facilities, so they want to have them guarded from now on. Are you up to it?\",CONTRACTS\nUS_09_02_FUEL_FOR_STATION_DELIVERY_CONTRACT,Data Collection,_CONTRACTS,US_09,Ontario,cargoDelivery,3\u00d7 Service Spare Parts; 2\u00d7 Barrels; 2\u00d7 Service Spare Parts; 1\u00d7 Metal Planks; 1\u00d7 Barrels,1410,14850,\"Our weather towers had a glitch just before the disaster, otherwise they would've given us the necessary data and help us be prepared. Could you please deliver the materials so we can repair and restart them?\",CONTRACTS\nUS_09_02_UNCLEAR_GOLD_CONTRACT,Delivery From The Quarry,_CONTRACTS,US_09,Ontario,cargoDelivery,2\u00d7 Container Large,1470,13600,\"We would usually send containers with soil, gold and other rock straight to the factory, but the fires spared very little of it. Our railway station is back to work, so we'd like you to deliver everything there.\",CONTRACTS\nUS_09_02_DISASSEMBLY_DRILL_CONTRACT,Old Oil Rig,_CONTRACTS,US_09,Ontario,cargoDelivery,1\u00d7 Container Large Drilling; 2\u00d7 Blocks; 2\u00d7 Wooden Planks,1360,12350,\"We were going to drill a quarry at this location, but the place just wasn't good enough. Worse yet, now we've got a sewer breach on our hands. I think we've messed up enough. Time to make amends with the locals.\",CONTRACTS\nUS_09_02_PIPES_CONTRACT,The Flooded Land,_CONTRACTS,US_09,Ontario,cargoDelivery,2\u00d7 Pipes Medium; 2\u00d7 Pipes Medium,1110,10250,\"A small local town is in trouble over broken sewage. The land is flooded and the locals are suffering huge damages. To solve the problem, they need new pipes delivered to them.\",CONTRACTS\nUS_09_02_WATER_DELIVERY_WAREHOUSE_CONTRACT,Putting Out The Fire At The Storage 2,_CONTRACTS,US_09,Ontario,cargoDelivery,4400\u00d7 Water; 4400\u00d7 Water,1260,9500,Another couple of storage facilities near the factory have practically burned down and are still a fire hazard. We need to put the fires out.,CONTRACTS\nUS_09_02_EXIT_CONTRACT,Let There Be Road,_CONTRACTS,US_09,Ontario,cargoDelivery,3\u00d7 Metal Planks; 3\u00d7 Blocks,860,7350,\"One of the passages was buried under the rockslide a while ago. We took a detour for a while, but the fire made that path really dangerous. Please, help us deliver some cargo for the repairs, and we'll do the rest.\",CONTRACTS\nUS_09_02_EXPORT_HAY_CONTRACT,Preventive Care,_CONTRACTS,US_09,Ontario,cargoDelivery,6\u00d7 Stack,560,6350,\"There's a ranch right near the edge of the burned forest. The owner is really afraid the fire will spark back up, so he asked us to haul his hay right to our storage. Will you help him with that?\",CONTRACTS\nUS_09_02_LUMBER_MILL_REPAIR_CONTRACT,Sawmill Renovation,_CONTRACTS,US_09,Ontario,cargoDelivery,4\u00d7 Service Spare Parts; 1\u00d7 Forklift Caravan Container 2,600,5650,\"An accident at the sawmill rendered a whole production line useless, so we had to pause log processing. We need some materials delivered for the repairs, and after that, all types of planks and boards will be available.\",CONTRACTS\nUS_09_02_SEISMO_SCOUT_CONTRACT,Searching For Underground Fires,_CONTRACTS,US_09,Ontario,exploration,,150,2250,\"We've never quite established the cause of the fires, but one of the sources might have been located underground. We have to inspect the area and check this out. You could use a thumper to help you with that.\",CONTRACTS\nUS_09_02_FORGOTTEN_TRAILER_DELIVERY_TASK,Abandoned Trailer,_TASKS,US_09,Ontario,truckDelivery,2\u00d7 Service Spare Parts,840,9900,\"Imagine, I moved here just recently, and the lads who've been helping me move my stuff took the wrong road and left my trailer somewhere out there. Could you bring it back to me? For a fee, of course.\",TASKS\nUS_09_02_CRASH_TASK,Bad Accident,_TASKS,US_09,Ontario,truckDelivery,,620,5800,\"At the height of the fires, a local resident and a truck driver were unable to share the road, which caused both to fly off the road. The cars were abandoned at the scene. It is necessary to deliver them to the owners, as well as to repair them.\",TASKS\nUS_09_02_RENEGADE_SCOUT_DELIVERY_TASK,Awkward Situation,_TASKS,US_09,Ontario,truckDelivery,,340,3550,\"Someone was so anxious to get away from the fires that he just took my car. He didn't get far, though. Could you please bring it back to me?\",TASKS\nUS_09_02_TATRA_FARM_TASK,Mutual Aid,_TASKS,US_09,Ontario,truckDelivery,,320,3300,\"One of our vehicles got stuck in the flooded field and is unusable. With our sewage problems, we have no use for it anyway, but the neighboring ranch would be happy to take it. However, there is a condition.\",TASKS\nUS_09_02_DON_SCOUT_TASK,A Hurtful Loss,_TASKS,US_09,Ontario,truckDelivery,,290,2700,\"Can you imagine? I was hurrying home with my family, so I decided to take a shortcut. Sadly, it got me stuck in the marsh. We left the car behind and walked home. Could you help us pull out our car?\",TASKS\nUS_09_02_LOGS_VEHICLE_TASK,Support Local Rangers,_TASKS,US_09,Ontario,truckDelivery,,210,1800,\"We had this truck transporting the logs along a dangerous route. The driver lost control of it, and the truck slid off the road. If you tow it out, you can use it for your own goals.\",TASKS\nUS_09_02_QUARRY_LOST_CARGO_TASK,Lost Cargo,_TASKS,US_09,Ontario,cargoDelivery,3\u00d7 Concrete Slab,1010,8550,\"Recently, we deepened the quarry, but we miscalculated the support strength. And when the concrete reinforcement slabs were almost there, the truck carrying them flipped over on the road. Could you please deliver the cargo?\",TASKS\nUS_09_02_METALLDETECTING_TASK,\"Something, Somewhere...\",_TASKS,US_09,Ontario,cargoDelivery,1\u00d7 Container Small Special,580,7450,\"As you see, we evacuated successfully. Obviously, it wasn't that easy. The main trouble is that practically everything is buried under a thick layer of ash. You could use a metal detector to find something important, will you help us?\",TASKS\nUS_09_02_NEW_QUARRY_TASK,Gold Rush,_TASKS,US_09,Ontario,exploration,,700,12100,\"Our quarry is almost depleted, we'd like to explore the nearby area and choose what's best. A thumper will be very helpful.\",TASKS\nUS_09_02_MOUNTAIN_TOURISM_CONTEST,Mountain Ride,_CONTESTS,US_09,Ontario,exploration,,105,1000,\"Hi, friend! We have a real challenge for you! Tourists adore the local fauna and arrange a real adrenaline ride here. Do you want to ride at speed and assess the condition of this route?\",CONTESTS\nUS_10_01_CONSTRUCTION_DRILLING_STATION_01_OBJ,Drilling Station Accident,_CONTRACTS,US_10,British Columbia,truckDelivery,2\u00d7 Metal Planks,1250,11150,\"When it rains, it pours. We're aware of the accident at the oil rig. First of all, we need a construction platform and then materials to build new reservoirs.\",CONTRACTS\nUS_10_01_DELIVERY_TRAILER_01_OBJ,Humanitarian Aid,_CONTRACTS,US_10,British Columbia,truckDelivery,,660,6250,Supplies are scarce in Duncan Bay now. People need any help they can get. Deliver the humanitarian supplies to the hunter's house and to the observation deck on the west coast.,CONTRACTS\nUS_10_01_ELECTRIC_POLE_01_OBJ,Short Circuit,_CONTRACTS,US_10,British Columbia,cargoDelivery,1\u00d7 Service Spare Parts; 1\u00d7 Service Spare Parts,580,6300,\"Not only was the road ruined by the flood - the power line was damaged as well. We need materials for the repair. Deliver all the supplies they need, and you'll be helping us a lot!\",CONTRACTS\nUS_10_01_DELIVERY_LOG_01_OBJ,Forestry Business,_CONTRACTS,US_10,British Columbia,cargoDelivery,2\u00d7 Logs Medium,440,3400,\"Several employees were hastily evacuated. We will gladly accept any help! Can you deliver some logs to our sawmill? For pay, of course.\",CONTRACTS\nUS_10_01_DELIVERY_OILTANK_01_TSK,Fuel For Vehicles,_TASKS,US_10,British Columbia,truckDelivery,,340,2900,\"Our truck left a fuel trailer on the road. The truck needs emergency repairs, and the trailer's just standing there. Get the trailer to our fueling station, and you'll get a financial reward!\",TASKS\nUS_10_01_FALLEN_POWER_LINE_01_TSK,Sky's The Limit,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Service Spare Parts,730,8050,We have an unusual job for you. Some materials have to be delivered to the top of the Power Line Post using a crane. We need to finish repairing it. This contract offers handsome pay.,TASKS\nUS_10_01_PIPELINE_01_TSK,It Gave Way,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Pipe Large,790,7400,\"The pipe literally burst, so the post didn't hold. Deliver the new pipe, and we'll be grateful.\",TASKS\nUS_10_01_PASS_01_TSK,Rockslide,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Concrete Slab; 1\u00d7 Service Spare Parts,650,6250,\"We'd barely repaired the road, and then a rockslide came! Clear it, and we'll reward you handsomely!\",TASKS\nUS_10_01_FIX_BRIDGE_01_TSK,North City Bridge,_TASKS,US_10,British Columbia,cargoDelivery,2\u00d7 Metal Planks,750,5850,We'd like to build a new bridge to replace the old one. We'll pay for materials delivery.,TASKS\nUS_10_01_CONSTRUCTION_DRILLING_STATION_02_OBJ,Pumping Revival,_CONTRACTS,US_10,British Columbia,cargoDelivery,2\u00d7 Pipes Small; 2\u00d7 Service Spare Parts; 1\u00d7 Metal Planks,1530,15200,\"You helped repair the reservoir, now we need to resume construction. Deliver the materials to the pumping station, and you'll be paid well!\",CONTRACTS\nUS_10_01_ELECTRIC_POLE_02_OBJ,Deenergized Coast,_CONTRACTS,US_10,British Columbia,cargoDelivery,1\u00d7 Service Spare Parts; 1\u00d7 Service Spare Parts,600,7500,\"You did a great job on your previous repair contract. Now, you have to finish what's left to do with the posts. The repair brigade will pay you back!\",CONTRACTS\nUS_10_01_CONSTRUCTION_WAREHOUSE_02_OBJ,Urban Industry,_CONTRACTS,US_10,British Columbia,cargoDelivery,2\u00d7 Crate Large; 1\u00d7 Wooden Planks,430,4750,\"As long as Duncan Bay hosts a meeting for the best, it would be great if they help with the town warehouse construction. This contract is a win-win. Show us what you can do!\",CONTRACTS\nUS_10_01_DELIVERY_LOG_02_OBJ,Town Requests,_CONTRACTS,US_10,British Columbia,cargoDelivery,4\u00d7 Wooden Planks,210,1750,\"Another job for you that's easy but vital! Please deliver some wooden planks to the town, and we will pay for the delivery.\",CONTRACTS\nUS_10_01_DELIVERY_OILTANK_02_TSK,Quid Pro Quo,_TASKS,US_10,British Columbia,truckDelivery,,370,3100,\"Hey, we need some household fuel. Besides, we have a new truck we don't use much. If you deliver the fuel trailer and repair my monster, that truck's yours.\",TASKS\nUS_10_01_FALLEN_POWER_LINE_02_TSK,Don't Drop It!,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Service Spare Parts,850,10050,\"If you have a crane and could deliver the necessary parts to the top of the post, that would be awesome. Could you do that? We'll pay for everything.\",TASKS\nUS_10_01_PIPELINE_02_TSK,Pipe Replacement,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Pipe Large,730,6450,\"The pipe replacement is in its final stages. The ones by the road are the last. Help us install them, and you'll get good money.\",TASKS\nUS_10_01_FIX_BRIDGE_02_TSK,South City Bridge,_TASKS,US_10,British Columbia,cargoDelivery,3\u00d7 Wooden Planks,240,2600,\"One of the ferries could barely fit in these cracks, and it managed to crash its roof against the bridge. We've estimated the damage, and it needs repairing. We will pay for your work.\",TASKS\nUS_10_01_PASS_02_TSK,Landslide On The Road,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Concrete Slab; 1\u00d7 Service Spare Parts,920,1050,\"The rocks have been piling up here for quite some time. And, just recently, a truck bumped its trailer on the edge of the cliff, sending all those rocks sliding! Will you help us deal with this problem?\",TASKS\nUS_10_01_CONSTRUCTION_DRILLING_STATION_03_OBJ,Service Station Construction,_CONTRACTS,US_10,British Columbia,cargoDelivery,1\u00d7 Container Large; 1\u00d7 Service Spare Parts,970,8300,Keep up the great work! Your next contract is the service station construction. Bad weather will not ruin our plans!,CONTRACTS\nUS_10_01_FIX_BRIDGE_03_TSK,Drilling Station Bridge,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Logs Medium,560,7750,The flooding left no chance of survival for this wooden bridge. Please help us rebuild it.,TASKS\nUS_10_01_CONSTRUCTION_DRILLING_STATION_04_OBJ,Oil Deposit,_CONTRACTS,US_10,British Columbia,truckDelivery,2\u00d7 Pipes Small; 1\u00d7 Container Large Drilling,1690,13550,\"You helped us build the Service Station, now we need the main thing, the Drilling Station.\",CONTRACTS\nUS_10_01_FIX_BRIDGE_04_TSK,Temporary Measures,_TASKS,US_10,British Columbia,cargoDelivery,2\u00d7 Logs Short,280,3700,The paved road has been washed away. The road workers obviously neglected their jobs! We have no time for construction. We need a wooden bridge as a temporary detour.,TASKS\nUS_10_01_FIX_BRIDGE_05_TSK,Sawmill Bridge,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Logs Medium; 2\u00d7 Logs Short,300,2450,\"The old bridge has been washed off to the ground. We need a new bridge, and it will be longer, too. If you deliver some logs for us, we will pay for that.\",TASKS\nUS_10_01_CONSTRUCTION_TWO_WAREHOUSE_OBJ,Twins In The Woods,_CONTRACTS,US_10,British Columbia,cargoDelivery,2\u00d7 Metal Planks; 2\u00d7 Service Spare Parts; 2\u00d7 Container Small; 2\u00d7 Pipes Small,2060,17200,\"We have a sawmill, but it's got no service facilities for our iron monsters. Please, help us build two service buildings. We need cargo for construction. Will you take this contract?\",CONTRACTS\nUS_10_01_CONSTRUCTION_LOCAL_BUSINESS_OBJ,Support Local Business,_CONTRACTS,US_10,British Columbia,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Container Large; 1\u00d7 Service Spare Parts,1700,17050,There is an outstanding contract up in town. Help a local business owner complete some construction. He will make it worth your while!,CONTRACTS\nUS_10_01_CONSTRUCTION_RAILWAY_OBJ,Railway Station Warehouse,_CONTRACTS,US_10,British Columbia,cargoDelivery,1\u00d7 Concrete Slab; 1\u00d7 Wooden Planks; 1\u00d7 Container Small,1240,13000,Our railway station needs a warehouse for storage. It won't only benefit our company but also expand the range of delivered cargo. Will you take this job?,CONTRACTS\nUS_10_01_CONSTRUCTION_FACTORY_OBJ,The Unfinished Business At The Plant,_CONTRACTS,US_10,British Columbia,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Service Spare Parts; 1\u00d7 Container Small,1080,10800,\"We have unfinished business - the plant construction. Help us complete this important task, and you'll be paid generously.\",CONTRACTS\nUS_10_01_EXIT_CLEANING_OBJ,Paralyzed Transportation,_CONTRACTS,US_10,British Columbia,cargoDelivery,1\u00d7 Service Spare Parts; 2\u00d7 Concrete Slab,1140,10200,This task is a top priority for the region. The residents are cut off from each other because of the roadblock. It is crucial that we reestablish the connection ASAP. The administration will reward you!,CONTRACTS\nUS_10_01_DELIVERY_FERRY_OBJ,Consumables For The Ferry,_CONTRACTS,US_10,British Columbia,cargoDelivery,2\u00d7 Crate Large,650,6550,\"The ferry at Duncan Bay is doing an important job safely taking vehicles across the river. It needs consumables for maintenance. Take this job, and we'll be grateful.\",CONTRACTS\nUS_10_01_DELIVERY_TRANSFORMER_TSK,Backup Source,_TASKS,US_10,British Columbia,truckDelivery,,350,3350,\"Power supply in the city gets interrupted all the time, we need a backup generator. Bring it to us, and we will reward you handsomely!\",TASKS\nUS_10_01_DELIVERY_TRUCK_TSK,Inexperienced Driver,_TASKS,US_10,British Columbia,truckDelivery,,340,2900,\"Please help me with my truck. I went off the road, and my engine died in the middle of the marsh. The truck must be delivered to the service center. I will make it worth your while.\",TASKS\nUS_10_01_DELIVERY_OIL_RIGS_TSK,Let There Be Oil,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Big Drills; 1\u00d7 Big Drills; 1\u00d7 Big Drills,1200,13900,\"Oil! Our oil rigs need special care, they need spare parts! Deliver three parts and get paid.\",TASKS\nUS_10_01_FALLEN_TRAIN_TSK,Off The Tracks,_TASKS,US_10,British Columbia,cargoDelivery,4\u00d7 Gold,1540,13000,\"Some carriages got derailed. We need to get the lost containers as soon as possible. For that, you'll need a crane. Deliver the containers to the railway station. We'll pay you for that!\",TASKS\nUS_10_01_CONSTRUCTION_GAS_WAREHOUSE_TSK,Service Center For The Drilling Station,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Container Small,1110,11250,\"In addition to the drilling station contracts, we also need to build a service center. Whether you help us or not, we'll pay for the delivery.\",TASKS\nUS_10_01_DELIVERY_METEO_TSK,Restoring Communication,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Container Small,500,8600,\"We have a special request for you, container delivery. It holds lots of iron for our weather station. Bring us the container, and we'll pay you for delivery.\",TASKS\nUS_10_01_DELIVERY_HUNT_TSK,Assistance To Civilization,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Vehicles Spare Parts; 1\u00d7 Crate Large; 1\u00d7 Logs Short,600,6200,\"Living in the woods is tough. We need tools to repair the trailer, some odds and ends and some firewood.\",TASKS\nUS_10_01_DELIVERY_LIFT_TSK,Tools For The Lift,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Service Spare Parts; 1\u00d7 Service Spare Parts,400,3800,\"We need to check the equipment of the ski lifts, but we are short on tools and resources. Please help us, and we'll pay.\",TASKS\nUS_10_01_CONT_VISIT,Data For The Observatory,_CONTESTS,US_10,British Columbia,exploration,,150,2000,\"The observatory need massive weather data, so give them our HDD as soon as possible!\",CONTESTS\nUS_10_01_CONTEST_RACE,Deadly Race,_CONTESTS,US_10,British Columbia,exploration,,125,2000,\"In winter, this is a serene ski resort, and in summer the organizers set up a real test! Steep slopes, difficult terrain... so just brace yourself and show what you can do!\",CONTESTS\nUS_10_02_DELIVERY_BOAT_01_OBJ,Special Order,_CONTRACTS,US_10,British Columbia,truckDelivery,,760,9500,We hear that you're the go-to person for any cargo deliveries. We've got a special contract for you - delivering a fishing boat. Are you game for this highly responsible task?,CONTRACTS\nUS_10_02_WILDLIFE_OFFICERS_01_OILTANK_OBJ,Fuel For The Law Enforcement,_CONTRACTS,US_10,British Columbia,truckDelivery,,500,5300,\"The Wildlife Protection officers need help. Now, they vitally need fuel. Deliver the fuel trailer from the city to their place.\",CONTRACTS\nUS_10_02_DELIVERY_TRUCK_01_OBJ,Cargo For The Fishers,_CONTRACTS,US_10,British Columbia,truckDelivery,,500,5250,\"Local fishers never got important cargo for their boats. The delivery trailer got stuck on the northwest road near the plant. Get it to the docks, and the fishers will pay you.\",CONTRACTS\nUS_10_02_DELIVERY_CARGO_01_OBJ,Important Delivery From The Port,_CONTRACTS,US_10,British Columbia,cargoDelivery,2\u00d7 Container Small Special,900,9650,Our Port is the only point where heavy cargo is delivered by water. Your task is to ship two containers from us to Duncan Bay. You will get good money!,CONTRACTS\nUS_10_02_DELIVERY_TRAILER_01_TSK,Special Order For The Builders,_TASKS,US_10,British Columbia,truckDelivery,,840,8700,Force majeure has interrupted construction plans. People are short on some basic supplies. We gathered all the necessary things in two trailers. Deliver them to the construction site.,TASKS\nUS_10_02_DELIVERY_TRUCK_01_TSK,Confiscated Truck,_TASKS,US_10,British Columbia,truckDelivery,,420,4000,We need to confiscate this truck. It belonged to poachers. Fix the truck and deliver it to the service center. You'll be paid on site.,TASKS\nUS_10_02_DELIVERY_OILTANK_01_TSK,Abandoned Oil Tank,_TASKS,US_10,British Columbia,truckDelivery,,360,3150,\"Our driver got stuck in the marshes with an oil tank. The road is too rough, this calls for a pro like yourself. Help us with the delivery, and we'll be even.\",TASKS\nUS_10_02_FIX_BRIDGE_01_TSK,Road Artery,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Logs Medium,620,7750,\"The region is barely explored, and the old bridges date back to our fathers' time. The bridge has to be rebuilt. Are you up to it?\",TASKS\nUS_10_02_WAREHOUSE_01_CONSTRUCTION_TSK,Support Local Businesses!,_TASKS,US_10,British Columbia,cargoDelivery,2\u00d7 Metal Roll; 2\u00d7 Bags,780,7050,\"As a business owner, I see that this city lacks a service center. Help me with construction. I have listed the materials I need. Will you help?\",TASKS\nUS_10_02_REMOVE_STONES_01_TSK,Gotta Go Through The Marshes,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Service Spare Parts,270,2800,These rocks block the direct route. The locals will pay you handsomely if you help clear them.,TASKS\nUS_10_02_SCOUT_VISIT_01_TSK,Water Probe,_TASKS,US_10,British Columbia,exploration,,290,4950,Local geologists need a water probe from the two lakes in the North and South of North Peak Park. Paid by the piece!,TASKS\nUS_10_02_CONT_01,All For The Great View!,_CONTESTS,US_10,British Columbia,exploration,,170,3200,This terrain is tough. Local residents like to race their cars here to the top of the hill. The view there is spectacular!,CONTESTS\nUS_10_02_WILDLIFE_OFFICERS_02_VISIT_OBJ,Following The Poachers' Trail,_CONTRACTS,US_10,British Columbia,exploration,,140,2100,\"Wildlife Protection officers discovered a poacher hiding spot. Help the officers with the investigation. Study the situation, cooperation is rewarded.\",CONTRACTS\nUS_10_02_DELIVERY_TRAILER_02_SCOUT_TSK,Recyclable,_TASKS,US_10,British Columbia,truckDelivery,,500,5000,We have collected some junk the poachers left behind. We need to get rid of this trailer. Have it recycled.,TASKS\nUS_10_02_DELIVERY_TRUCK_02_TSK,Plant's Workhorse,_TASKS,US_10,British Columbia,truckDelivery,,440,4250,\"Hey, driver! I was heading for the plant but got stuck in those marshes. The truck is really good, too bad I'm a poor driver. Please help me, I'll owe you one.\",TASKS\nUS_10_02_FIX_BRIDGE_02_TSK,The Coastal Frustration,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Logs Medium; 2\u00d7 Wooden Planks,800,9700,\"The wooden bridge collapsed under the heavy trucks. Bridges collapse here quite frequently, so there are always a lot of jobs.\",TASKS\nUS_10_02_REMOVE_STONES_02_TSK,The Mountain Obstacle,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Service Spare Parts,160,1350,\"Geologists have warned us about the rockslide hazard here many times. Help us remove them, and people will thank you.\",TASKS\nUS_10_02_CONT_02,Waste Disposal,_CONTESTS,US_10,British Columbia,cargoDelivery,2\u00d7 Barrels Chemicals,600,5000,Fish farmers have a weird tradition. They compete to see who will be the first to deliver some waste barrels. Join the local games and show what you can do!,CONTESTS\nUS_10_02_WILDLIFE_OFFICERS_03_BOAT_OBJ,Helping The Investigation,_CONTRACTS,US_10,British Columbia,cargoDelivery,1\u00d7 Service Spare Parts Special,770,9650,\"The poacher's boat is stuck, and its cargo has to be delivered to the police station. Use the crane to deliver the evidence.\",CONTRACTS\nUS_10_02_DELIVERY_TRUCK_03_TSK,Long Distance Haul,_TASKS,US_10,British Columbia,truckDelivery,,490,5150,\"Buddy, help me! My truck's stuck on this bad road. I need to have it pulled out and fixed. I'd be grateful for the help.\",TASKS\nUS_10_02_CANNERY_CONSTRUCTION_OBJ,Uncle Fish Will Do This Job!,_CONTRACTS,US_10,British Columbia,cargoDelivery,2\u00d7 Bricks; 2\u00d7 Metal Planks; 2\u00d7 Crate Large; 2\u00d7 Concrete Slab,2110,18950,\"Our company is expanding. We're making the best canned fish! Plant construction is our top priority, so we'll pay well for cooperation. Are you on board?\",CONTRACTS\nUS_10_02_GARAGE_CONSTRUCTION_OBJ,Second Garage Construction,_CONTRACTS,US_10,British Columbia,cargoDelivery,2\u00d7 Wooden Planks; 2\u00d7 Bricks; 2\u00d7 Vehicles Spare Parts,1380,16600,\"Sadly, we're not done with garage construction. Accept this contract, and we will be. The garage will be your home away from home!\",CONTRACTS\nUS_10_02_TELEPHONE_TOWER_OBJ,Restoring Communication,_CONTRACTS,US_10,British Columbia,cargoDelivery,1\u00d7 Container Small; 1\u00d7 Service Spare Parts,820,8700,\"The Communication Tower is out of order, it needs to be repaired. Bring the spare parts and materials needed for restoration to the repair crew.\",CONTRACTS\nUS_10_02_FARM_TOWER_CONSTRUCTION_TSK,Tower For The Fish Farm,_TASKS,US_10,British Columbia,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Container Small,940,9550,\"We have dismantled the old water tower and we want to build a new one. Help us with the materials, and we'll pay for the delivery.\",TASKS\nUS_10_02_REMOVE_TOWERS_TSK,Tower For Scrap,_TASKS,US_10,British Columbia,cargoDelivery,2\u00d7 Service Spare Parts,450,5150,\"Local residents need instruments to remove this fallen tower. They will disassemble it, and you will get part of the money. Do we have a deal?\",TASKS\nUS_11_01_BLOCKAGE_ON_THE_ROAD_01_TASK,Eastern Blockage,_TASKS,US_11,Scandinavia,cargoDelivery,4\u00d7 Wooden Planks,440,5000,\"All those months of ignoring safety regulations while working in the mountains have finally caught up with me \u2014 the road is completely blocked by a rockslide. If you help me get those rocks out of there, I'll pay you handsomely.\",TASKS\nUS_11_01_BLOCKAGE_ON_THE_ROAD_02_TASK,Western Blockage,_TASKS,US_11,Scandinavia,cargoDelivery,2\u00d7 Wooden Planks,310,3500,We've got a problem area that always gets blocked every season. This time it's your job. But we'll pay you for your time!,TASKS\nUS_11_01_EQUIPMENT_SET_MOUNTAIN_CONTRACT,Mountain Movie Set,_CONTRACTS,US_11,Scandinavia,truckDelivery,1\u00d7 Plane,1280,13400,\"No... no... this is all wrong. We need a real plane! Hey, could you give us a hand? I ordered the prop I need a long time ago, but it got stuck somewhere on the road. I need you to deliver it to me as soon as possible. I'm counting on you!\",CONTRACTS\nUS_11_01_CABLE_CAR_CONTRACT,The First Trip,_CONTRACTS,US_11,Scandinavia,truckDelivery,2\u00d7 Bags; 2\u00d7 Metal Roll,1340,12600,\"We need to finish work on the ski lift. Deliver the trailer containing the gear and materials we need, and then we'll settle up.\",CONTRACTS\nUS_11_01_REPAIR_WAREHOUSE_CONTRACT,Warehouse Repairs,_CONTRACTS,US_11,Scandinavia,truckDelivery,2\u00d7 Metal Roll; 1\u00d7 Metal Planks,1270,11450,\"Could you repair the old warehouse? The roof caved in. It's not the end of the world, but we've still got some useful materials in there.\",CONTRACTS\nUS_11_01_FILM_STAR_MOUNTAIN_CONTRACT,A Bad Connection,_CONTRACTS,US_11,Scandinavia,truckDelivery,,700,8400,\"As if the harsh weather weren't enough, you can't even get a signal around here! Deliver some communication equipment to the movie set with the plane.\",CONTRACTS\nUS_11_01_CITY_CONSTRUCTION_CONTRACT,Construction Help,_CONTRACTS,US_11,Scandinavia,truckDelivery,,540,6000,\"The sets turned out to be bigger than we thought. We're going to need some extra equipment. We almost had it, too, but something went wrong in shipping. Could you help us out?\",CONTRACTS\nUS_11_01_REQUIRED_TOOLS_CONTRACT,The Right Tools For The Job,_CONTRACTS,US_11,Scandinavia,truckDelivery,,510,5500,\"Listen, we've got a little situation here... we left a trailer full of tools at the lower ski station. Could you go get them? We're kind of swamped over here.\",CONTRACTS\nUS_11_01_LOST_TRAILERS_CONTRACT,Lost Trailers,_CONTRACTS,US_11,Scandinavia,truckDelivery,,430,4800,\"Spring came early this year, so the river overflowed and flooded the main roads. The workers couldn't get the trailers out, and we need them ASAP. Could you haul them the rest of the way here?\",CONTRACTS\nUS_11_01_FILM_STAR_CITY_CONTRACT,The Movie Star,_CONTRACTS,US_11,Scandinavia,truckDelivery,,440,4600,\"Our company wants to make sure every star who works with us feels right at home. We need you to deliver an expensive star trailer to the movie set, and we need it to get there in one piece.\",CONTRACTS\nUS_11_01_STAR_TRAILER_MOUNTAIN_CONTRACT,Way To Success,_CONTRACTS,US_11,Scandinavia,truckDelivery,,420,3850,\"We're done filming in the city, so please drive our A-lister to the next movie set. The road might be a bit tricky, but I'm sure you can make it!\",CONTRACTS\nUS_11_01_CABIN_IN_THE_MOUNTAIN_CONTRACT,Cabins In The Mountains,_CONTRACTS,US_11,Scandinavia,cargoDelivery,3\u00d7 Wooden Planks; 1\u00d7 Logs Medium; 1\u00d7 Bags; 2\u00d7 Wooden Planks Medium,2000,21200,We've got a couple of unfinished cabins for people who like to look down at the local landscapes. We need to deliver the materials so they can finish the cabins.,CONTRACTS\nUS_11_01_HOLLYWOOD_SMILE_CONTRACT,A Taste Of Tinseltown,_CONTRACTS,US_11,Scandinavia,cargoDelivery,4\u00d7 Metal Roll; 4\u00d7 Concrete Slab,2450,21000,\"We've got big news! A famous actor is coming here soon to film a new picture. Do you know what that means? We need letters over our town, just like the Hollywood sign! Help us make it happen, okay? Do it for art!\",CONTRACTS\nUS_11_01_HOLIDAY_BEGINS_WITH_WORK_CONTRACT,Business Before Pleasure,_CONTRACTS,US_11,Scandinavia,cargoDelivery,4\u00d7 Metal Planks; 2\u00d7 Metal Roll; 2\u00d7 Wooden Planks; 2\u00d7 Forklift Caravan Container 2,2470,20850,Your job is to deliver workers' cabins to the resort. You can pick them up at the factory. But first we need a generator to start manufacturing them. Time to get to work!,CONTRACTS\nUS_11_01_CABIN_IN_THE_WOODS_CONTRACT,Cabins In The Woods,_CONTRACTS,US_11,Scandinavia,cargoDelivery,2\u00d7 Wooden Planks; 2\u00d7 Crate Large; 1\u00d7 Logs Short; 3\u00d7 Wooden Planks,1670,19350,\"We need to finish up a few tourist cabins before resort seasons gets going, but we've got a delivery issue. We need your help. Don't worry, we'll make it worth your while.\",CONTRACTS\nUS_11_01_VIEW_FROM_ABOVE_CONTRACT,A Bird's Eye View,_CONTRACTS,US_11,Scandinavia,cargoDelivery,4\u00d7 Cable Car,1680,18800,\"Alright, just one last big push, and all our construction projects will be done. All we have to do now is deliver a few cable cars, and then we can open the resort!\",CONTRACTS\nUS_11_01_TOP_VIEW_CONTRACT,The View from Above,_CONTRACTS,US_11,Scandinavia,cargoDelivery,1\u00d7 Logs Short; 3\u00d7 Crate Large,1190,12750,\"A movie studio is shooting a new film nearby. We'd like to have a view of the surrounding area in case something happens and they need our help. Get the job done, and you'll have a front-row seat for the shoot!\",CONTRACTS\nUS_11_01_FORESTER_IS_WATCHING_YOU_CONTRACT,The Ranger Is Watching,_CONTRACTS,US_11,Scandinavia,cargoDelivery,2\u00d7 Wooden Planks; 2\u00d7 Metal Roll,1200,11700,\"The forestry service is getting ready for tourist season, so we want to make the area is as safe and well-supervised as possible. The towers are almost ready, but we still need some more materials. Deliver them, and enjoy those beautiful views while you're at it.\",CONTRACTS\nUS_11_01_CLEAR_MOUNTAIN_CONTRACT,Mountains' Majesty,_CONTRACTS,US_11,Scandinavia,cargoDelivery,2\u00d7 Rubbish,930,10150,\"Like with any other construction project, we've got a bunch of leftover junk. We probably shouldn't just leave it here. There's a recycling plant nearby. Could you lug all this stuff over there?\",CONTRACTS\nUS_11_01_HELP_FOR_DROWNING_CONTRACT,Helping The Fishermen,_CONTRACTS,US_11,Scandinavia,cargoDelivery,2\u00d7 Crate Large; 1\u00d7 Crate Large; 1\u00d7 Crate Large; 2\u00d7 Crate Large,980,9150,\"Spawning season is starting soon, and our fishermen's boats are broken. Help them! We need to make sure there's enough surstr\u00f6mming for everybody!\",CONTRACTS\nUS_11_01_EQUIPMENT_SET_CITY_CONTRACT,Town Movie Set,_CONTRACTS,US_11,Scandinavia,cargoDelivery,2\u00d7 Metal Planks; 6\u00d7 Wooden Planks,890,7950,\"As you can see, we're shooting a movie here. The local countryside has some spectacular views, but, as I'm sure you understand, we still need extra set dressing in this business. Could you deliver all the supplies we need to build our sets?\",CONTRACTS\nUS_11_01_FALLING_TOOLS_CONTRACT,Dropped Cargo,_CONTRACTS,US_11,Scandinavia,cargoDelivery,3\u00d7 Service Spare Parts,520,4800,We've got a problem: some essential supplies fell off a truck and landed right on the ice! We don't have the right gear to go back and get them. Do you think you can handle it?,CONTRACTS\nUS_11_01_TERRIORY_EXPLORATION_CONTRACT,Scouting The Area,_CONTRACTS,US_11,Scandinavia,exploration,,120,2100,\"I know that you are quite busy, but perhaps you could take up one more task? We would like to check those territories for contamination, but we don't have the technical means to do it. Can you help?\",CONTRACTS\nUS_11_01_LOST_CARGO_TASK,Shoring Up Losses,_TASKS,US_11,Scandinavia,truckDelivery,2\u00d7 Service Spare Parts,750,8100,We`ve lost some essential equipment during shipping. The trailer is still by the river. Could you get it back for us?,TASKS\nUS_11_01_CORNERED_TASK,In A Tough Spot,_TASKS,US_11,Scandinavia,truckDelivery,2\u00d7 Service Spare Parts,650,5350,\"I somehow managed to get lost in the swamp in the middle of the night. The country there was so rough, I had to leave my trailer. Please help me pull it out.\",TASKS\nUS_11_01_STRUCK_NOT_BROKEN_TASK,\"Bent, But Not Broken!\",_TASKS,US_11,Scandinavia,truckDelivery,,490,5150,\"Here's the thing, I figured I'd give my beauty a little challenge and drove her down the route across the river. The engine died, but that's nothing. My poor car was swept away, and you're the only one who can help me pull it back out. I promise, I'll return the favor.\",TASKS\nUS_11_01_RECHARGING_TASK,Power Generator,_TASKS,US_11,Scandinavia,truckDelivery,,440,4700,\"Aside from the odd storm, the weather around here is pretty calm. Unfortunately, our facility's power supply got hit this time. Help us deliver the trailer with our new generator so we can get the factory up and running again!\",TASKS\nUS_11_01_TAMER_TROLLS_TASK,Troll Tamer,_TASKS,US_11,Scandinavia,truckDelivery,,450,4400,\"Hello! I took pictures of a troll! Then it chased me into a swamp, so I had to drop everything and run for my life. If you pull my vehicle out of the swamp, you can have it. All I need are the photos.\",TASKS\nUS_11_01_FRIENDLY_HELP_TASK,Friendly Help,_TASKS,US_11,Scandinavia,truckDelivery,,420,4200,\"Hi there! You look like a nice guy who could do me a favor. Can you take this trailer and help my friend to fix his truck? I'd go myself, but I'm too busy delivering orders at the moment.\",TASKS\nUS_11_01_MISTAKES_HAPPEN_TASK,Accidents Happen,_TASKS,US_11,Scandinavia,truckDelivery,,430,3800,\"What rotten luck! I had to leave my trailer on the ridge. I'd really appreciate it if you could deliver it to us. I'll pay you, of course.\",TASKS\nUS_11_01_LEFT_TASK,Left By The Ledge,_TASKS,US_11,Scandinavia,truckDelivery,,400,3600,\"You'll never believe this, bud! My vehicle fell of a rocky ledge. Could you help me get it out of there?\",TASKS\nUS_11_01_FAILED_MOVE_TASK,An Unfortunate Twist,_TASKS,US_11,Scandinavia,truckDelivery,,420,3500,\"Hey you, the pro driver! I've heard about your skills. Mine aren't looking so hot \u2014 my favorite trailer got stuck in the mud. Help me pull it out, okay?\",TASKS\nUS_11_01_WRONG_TURN_TASK,A Suspicious Excursion,_TASKS,US_11,Scandinavia,truckDelivery,,360,3200,\"I'd rather not tell you how my vehicle got stuck in the swamp, and I DEFINITELY don't want to explain it to my wife. Drag it out of there for me, would you? I'll pay you.\",TASKS\nUS_11_01_FALLEN_CAR_TASK,Look What I Can Do!,_TASKS,US_11,Scandinavia,truckDelivery,,360,2850,My vehicle is stuck! Can you help me pull it out? I need it delivered to the movie set on the double! We'll settle up when you get there.,TASKS\nUS_11_01_BRIDGE_REPAIR_TASK,Bridge Issues,_TASKS,US_11,Scandinavia,cargoDelivery,2\u00d7 Metal Planks; 1\u00d7 Blocks,1070,8850,\"The bridge is out, so we've been taking detours around it. Help us fix it \u2014 we'll make it worth your while.\",TASKS\nUS_11_01_DROWNED_ORDER_TASK,Wet Work,_TASKS,US_11,Scandinavia,cargoDelivery,3\u00d7 Cellulose,680,5900,\"We've got an urgent job, but the cargo we need to deliver is stuck by the shore. Could you please get all three loads and deliver them for us?\",TASKS\nUS_11_01_FAIL_TASK,Rocky Chasm,_TASKS,US_11,Scandinavia,cargoDelivery,2\u00d7 Barrels,500,4950,Our driver dropped a couple of barrels by the cliff while moving on a difficult route. Help him find and recover them.,TASKS\nUS_11_01_FLOODED_RACE_CONTEST,A Flooded Race,_CONTESTS,US_11,Scandinavia,exploration,,150,750,\"Welcome to our humble abode! It's our tradition to host annual springtime races for the local fishermen. The flood washed out the roads this year, but that's what makes it even more fun! Wanna join us?\",CONTESTS\nUS_11_02_MOVIE_PLACE_CONTRACT_01_02,Celebrity Hour,_CONTRACTS,US_11,Scandinavia,truckDelivery,,420,4750,\"We sure are lucky Nordic movies are at the peak of popularity. That said, great expectations demand great quality! It's time to drive our A-lister to the next movie set.\",CONTRACTS\nUS_11_02_MOVIE_PLACE_CONTRACT_01,A Grim Background,_CONTRACTS,US_11,Scandinavia,cargoDelivery,2\u00d7 Container Small; 2\u00d7 Wooden Planks,770,7500,\"We're finally resuming shooting in the new location. In order to get the set ready and start filming, we're going to require some materials and props in containers. Please deliver them to the movie set, and expect a generous compensation.\",CONTRACTS\nUS_11_02_RECYCLING_RECOVERY_CONTRACT_01,Renewing The Cycle,_CONTRACTS,US_11,Scandinavia,cargoDelivery,2\u00d7 Bags; 2\u00d7 Service Spare Parts,560,5050,\"As you may know, we're really enthusiastic about waste, or rather its recycling. On that note, the local recycling plant needs some upgrades. Deliver the gear they need to eliminate pollution!\",CONTRACTS\nUS_11_02_WATCHTOWER_CONTRACT_01,Guardian Of The Lake,_CONTRACTS,US_11,Scandinavia,cargoDelivery,2\u00d7 Logs Medium; 2\u00d7 Metal Roll,490,4600,\"If we're going to supervise the area, we'll need to build a watchtower. Deliver the materials, and you'll get your reward.\",CONTRACTS\nUS_11_02_TRACK_DELIVERY_TASK_01,A Trip Across The Water,_TASKS,US_11,Scandinavia,truckDelivery,,360,3550,\"Look, I'm in a bit of a tight spot right now, and I really need my car delivered here. Can you do that for me? I'll pay you for the trouble.\",TASKS\nUS_11_02_PATH_TASK_01,Western Landfall Site,_TASKS,US_11,Scandinavia,cargoDelivery,1\u00d7 Metal Planks,220,1750,\"The spring season brings its share of trouble. The passage is blocked, and we're going to need some tools to clear the rubble.\",TASKS\nUS_11_02_MOVIE_PLACE_CONTRACT_02_02,A Finishing Touch,_CONTRACTS,US_11,Scandinavia,truckDelivery,,350,3450,All that's left is to film the final episode at the last movie set. The crew's ready and waiting for the star of the show. Please drive the VIP trailer to the movie set.,CONTRACTS\nUS_11_02_RECYCLING_RECOVERY_CONTRACT_02,Recycling Waste!,_CONTRACTS,US_11,Scandinavia,cargoDelivery,1\u00d7 Rubbish; 1\u00d7 Rubbish,630,6500,\"Seeing that the recycling plant is working again, it's up to us to deliver waste for recycling. Naturally, we're going to need a delivery driver, so we were thinking of signing with you again. The pay will be good!\",CONTRACTS\nUS_11_02_WATCHTOWER_CONTRACT_02,Spirit Of The River,_CONTRACTS,US_11,Scandinavia,cargoDelivery,1\u00d7 Logs Long; 2\u00d7 Metal Roll,700,5550,We have to complete construction of the watchtower to secure our control over the West. It's critical that you help us here. Will you?,CONTRACTS\nUS_11_02_MOVIE_PLACE_CONTRACT_02,A Freezing Adventure,_CONTRACTS,US_11,Scandinavia,cargoDelivery,1\u00d7 Service Spare Parts; 2\u00d7 Metal Planks,560,4100,\"Thanks for your service! At first, I wasn't too sure you were the right person for the job, but now I see how wrong I was. Your name will go down in film production history! We hope to work with you again!\",CONTRACTS\nUS_11_02_TRACK_DELIVERY_TASK_02,Wasteland Movie Set,_TASKS,US_11,Scandinavia,truckDelivery,1\u00d7 Bricks; 1\u00d7 Cellulose; 2\u00d7 Bags,310,2750,\"We're in urgent need of some junk for filming. On top of that, our driver got stuck in the middle of nowhere and knocked over that trailer we really need. Bring it all here, and the reward is yours!\",TASKS\nUS_11_02_PATH_TASK_02,Eastern Landfall Site,_TASKS,US_11,Scandinavia,cargoDelivery,1\u00d7 Metal Planks,360,3050,\"If you'd like to drive down a decent road, help me clear the rubble first. I'll be generous with your pay.\",TASKS\nUS_11_02_HOTEL_HOUSE_CONTRACT,Tourist Cabin Repairs,_CONTRACTS,US_11,Scandinavia,truckDelivery,2\u00d7 Bags; 4\u00d7 Wooden Planks,690,5500,\"We're doing our best to provide great customer service. Tourist cabins are in need of renovation, so we're going to need some construction materials. Deliver some, and we'll be sure to compensate you for your efforts.\",CONTRACTS\nUS_11_02_FIXING_STATION_CONTRACT,Collecting Waste,_CONTRACTS,US_11,Scandinavia,cargoDelivery,3\u00d7 Rubbish; 2\u00d7 Metal Planks,1480,13700,\"The area is still littered with junk, mostly left by the construction workers. Collect the waste and deliver it to the recycling station. We'll be sure to pay you for the job.\",CONTRACTS\nUS_11_02_RAIL_REPAIR_CONTRACT,Disrupted Supplies,_CONTRACTS,US_11,Scandinavia,cargoDelivery,1\u00d7 Railway; 1\u00d7 Railway,840,7300,\"There has been an unfortunate railroad accident, and this area has been struggling with logistics issues ever since. Two damaged rail pieces require replacement. Please deliver and install them where they're needed.\",CONTRACTS\nUS_11_02_WATER_CAMP_CONTRACT,Severe Dehydration,_CONTRACTS,US_11,Scandinavia,cargoDelivery,450\u00d7 Water; 250\u00d7 Water,1000,6000,\"We replenish our water supply once every few weeks. The weather has been rough lately, and you're the only delivery driver we can trust. We're sure you'll live up to our expectations.\",CONTRACTS\nUS_11_02_CAMP_CONTRACT,Winter's Hasty Retreat,_CONTRACTS,US_11,Scandinavia,cargoDelivery,2\u00d7 Logs Short,410,4450,\"Winter may be over, but the nights are still freezing! We need some firewood to keep the cabins warm. Could you help us deliver it?\",CONTRACTS\nUS_11_02_RESEARCH_CONTRACT,A Cliffside Road,_CONTRACTS,US_11,Scandinavia,exploration,,150,2500,\"We need to check out the northern mountain trail. It's a difficult and dangerous route, so you're the only one who can handle the job. Of course, you'll be compensated accordingly.\",CONTRACTS\nUS_11_02_SEARCH_TASK,A Lost Car,_TASKS,US_11,Scandinavia,truckDelivery,,650,8000,Some tourists lost control of the car last night and left it on the tourist trail. We know the approximate location of the car. You just need to drive around and find it.,TASKS\nUS_11_02_ACCIDENT_TASK,A Traffic Accident,_TASKS,US_11,Scandinavia,truckDelivery,,340,3300,\"A white fox darted across the road in front of me, and I lost control of my car! I'm just glad the poor critter is safe, and repairing the car won't be a problem. Please help me recover it first.\",TASKS\nUS_11_02_NEO_FALCON_TASK,A Rare Vehicle,_TASKS,US_11,Scandinavia,truckDelivery,,210,1800,\"Well, if it isn't our friend! See that car over there in the swamp? She's been stuck there forever. Many people tried and failed to salvage her, and if you manage to get her out, she's sure to serve you well.\",TASKS\nUS_11_02_LOST_CARGO_TASK,The Element Of Water,_TASKS,US_11,Scandinavia,cargoDelivery,4\u00d7 Service Spare Parts,480,4350,\"It's a mighty river, all right! I was unlucky enough to face its current while transporting the shipment. Please help me recover the lost cargo! I promise to make it up to you.\",TASKS\nUS_11_02_WOODEN_BRIDGE_TASK,The Wooden Bridge,_TASKS,US_11,Scandinavia,cargoDelivery,1\u00d7 Logs Long; 2\u00d7 Wooden Planks,620,3850,\"This wooden bridge once served as passage for mountain climbers looking for the most challenging trails, eventually collapsed due to old age. Just deliver some wood, and the bridge will be as good as new.\",TASKS\nUS_11_02_BRIDGE_TASK,Southern Bridge,_TASKS,US_11,Scandinavia,cargoDelivery,2\u00d7 Bags; 2\u00d7 Metal Planks,500,3850,\"The bridge collapsed, failing the test of time. It requires urgent repairs, so we'll need construction materials delivered. Please help us so traffic can resume in this area.\",TASKS\nUS_11_02_REPAIR_TASK,Repair The Repair Station,_TASKS,US_11,Scandinavia,cargoDelivery,4\u00d7 Vehicles Spare Parts,330,3150,\"As ironic as it is, the repair station needs to be repaired. Bring us the construction materials so we can get back to work.\",TASKS\nUS_11_02_VISIT_TASK,Lake Paradise,_TASKS,US_11,Scandinavia,exploration,,250,4200,\"Aside from being the ultimate relaxation spots, lakes also serve as natural fresh water reservoirs. That's why we're doing our best to combat water contamination. We have our work cut out for us, so we're going to need some samples for analysis.\",TASKS\nUS_11_02_SEISMO_TASK,Thor's Thunder,_TASKS,US_11,Scandinavia,exploration,,250,1500,\"The mine in the northern part of the region is long since abandoned, but we just got our hands on fresh water samples with a curious anomaly. You can expect good pay for scanning the rocks around the mine.\",TASKS\nUS_11_02_DELIVERY,Speedy Delivery,_CONTESTS,US_11,Scandinavia,cargoDelivery,1\u00d7 Gold,150,750,\"Right, so we're really short-handed and there's this gold bullion due for delivery yesterday. Will you please deliver it as fast as you can?\",CONTESTS\nUS_11_02_DESCENT_CONTEST,Challenging Descent,_CONTESTS,US_11,Scandinavia,exploration,,75,250,\"Hiya, friend! This is urgent. We were planning to film a ski resort ad, but our driver is out sick. Can you drive around real quick and get some footage of these gorgeous local views?\",CONTESTS\nUS_12_01_POWER_LINES_CONTRACT_01,Power Lines 1,_CONTRACTS,US_12,North Carolina,truckDelivery,,600,7250,\"We've got the transformer yard and the main power grid, but certain lines still need some work. First things first, let's get this factory connected to the grid.\",CONTRACTS\nUS_12_01_WEATHER_STATION_CONTRACT_01,Solar Power,_CONTRACTS,US_12,North Carolina,cargoDelivery,2\u00d7 Solar Panel; 1\u00d7 Solar Panel; 1\u00d7 Metal Planks; 1\u00d7 Solar Panel; 1\u00d7 Metal Planks,1460,16000,\"Splitting uranium atoms isn't the only way to power things\u2014solar power is good for that too! We've been ask to deliver some solar panels to a bunch of weather towers. It's the 21st century. Who needs batteries, right?\",CONTRACTS\nUS_12_01_RESTORATION_CROSSINGS_CONTRACT_01,Eastbound Ferry,_CONTRACTS,US_12,North Carolina,cargoDelivery,2\u00d7 Concrete Slab,650,5050,\"We used to have a ferry running between the two towns, but with the docks closed down for repairs, logistics have been disrupted. The sooner we fix those docks up, the sooner you'll have an easier way to cross over.\",CONTRACTS\nUS_12_01_WOODEN_BRIDGE_TASK_01,Logical Solution 1,_TASKS,US_12,North Carolina,cargoDelivery,3\u00d7 Wooden Planks Medium,400,5050,\"The part of the road leading to the weather tower up north is really rough. We'd like to build a bridge there for the sake of convenience\u2014get us some wood, and we'll do the job.\",TASKS\nUS_12_01_VALUE_TASK_01,Rubble Trouble 1,_TASKS,US_12,North Carolina,cargoDelivery,1\u00d7 Metal Planks,290,2350,\"The earthquakes in the area have left us a couple of nasty surprises on the road. Let's clear the rubble from the direct route to the factory, okay?\",TASKS\nUS_12_01_WEATHER_STATION_CONTRACT_02,Weather Crisis,_CONTRACTS,US_12,North Carolina,cargoDelivery,3\u00d7 Service Spare Parts; 3\u00d7 Service Spare Parts,800,8550,\"As luck would have it, two of the weather towers are now down for the count. We need to get them the supplies they need for repairs right away. Looks like the guys have found something really exciting there!\",CONTRACTS\nUS_12_01_RESTORATION_CROSSINGS_CONTRACT_02,Westbound Ferry,_CONTRACTS,US_12,North Carolina,cargoDelivery,4\u00d7 Container Small,770,7000,\"Same story here\u2014the docks are undergoing maintenance, but I can see a boat on the other side. We should give her the opportunity to dock here too, don't you think?\",CONTRACTS\nUS_12_01_POWER_LINES_CONTRACT_02,Power Lines 2,_CONTRACTS,US_12,North Carolina,cargoDelivery,2\u00d7 Metal Planks,560,5200,\"Next up, there's an electric transmission line we need to finish. As you can see, they're all spic-and-span here, so be careful when you deliver the metal beams, okay?\",CONTRACTS\nUS_12_01_WOODEN_BRIDGE_TASK_02,Logical Solution 2,_TASKS,US_12,North Carolina,cargoDelivery,3\u00d7 Wooden Planks Medium,410,5200,\"We've got a logistics issue with the eastern weather tower, too. The solution is the same here\u2014we need to build a wooden bridge!\",TASKS\nUS_12_01_VALUE_TASK_02,Rubble Trouble 2,_TASKS,US_12,North Carolina,cargoDelivery,1\u00d7 Metal Planks,290,2350,We've got another road blocked by a landslide. The usual method should work here\u2014just deliver the supplies and leave the rest to us.,TASKS\nUS_12_01_POWER_LINES_CONTRACT_03,Power Lines 3,_CONTRACTS,US_12,North Carolina,truckDelivery,1\u00d7 Metal Planks,800,8000,\"Just one more ETL to go. We still need metal beams, and I hope you didn't park our trailer too far away...\",CONTRACTS\nUS_12_01_TRANSFORMER_CONTRACT,Power Grid,_CONTRACTS,US_12,North Carolina,truckDelivery,3\u00d7 Metal Planks; 4\u00d7 Barrels; 3\u00d7 Metal Planks,2960,27800,\"Now that the power plant is done, it's time to get the buildings in the area some juice. We should start by finishing the transformer substation. It's basically done\u2014we're just missing materials for a couple of units.\",CONTRACTS\nUS_12_01_BIG_BRIDGE_CONTRACT,Bridging the Gap,_CONTRACTS,US_12,North Carolina,truckDelivery,3\u00d7 Metal Planks; 4\u00d7 Bags,1650,13550,We've got an emergency! Part of a bridge has broken down\u2014we need to repair it ASAP! Some of the researchers are now stuck on that island\u2014their only way out is swimming.,CONTRACTS\nUS_12_01_SPECIAL_BRIDGE_CONTRACT,Jumbo Bridge,_CONTRACTS,US_12,North Carolina,truckDelivery,4\u00d7 Barrels,1120,12350,\"This hunk of junk broke on us. Can you believe it? We've got dozens of deliveries waiting at the port, and the easiest way to cross that wall of water just bit the dust! We need it fixed, stat.\",CONTRACTS\nUS_12_01_SCIENTIFIC_MACHINE_CONTRACT,Vice-Like Grip,_CONTRACTS,US_12,North Carolina,truckDelivery,1\u00d7 Metal Planks,950,10000,We were driving around taking soil samples when a landslide suddenly wiped out one of the roads. We had to ditch the vehicle and run for our lives. The poor thing is still stuck there. Can you help us get it out?,CONTRACTS\nUS_12_01_LOG_FOR_STORAGE_CONTRACT,Filling Space,_CONTRACTS,US_12,North Carolina,cargoDelivery,4\u00d7 Logs Medium,1400,16500,\"We had to cut down some trees to clear the plot of land for the substation. Some of the logs went to the sawmill, but not all of them. We need to get the rest of them to the town warehouse for further processing.\",CONTRACTS\nUS_12_01_TELESCOPE_CONTRACT,It's All Connected 1,_CONTRACTS,US_12,North Carolina,cargoDelivery,3\u00d7 Metal Roll; 4\u00d7 Crate Large,1070,9550,There's been another addition to our region\u2014radio telescopes. These astronomical tools aren't just for studying celestial objects; they can also help us assess our water EM radiation levels.,CONTRACTS\nUS_12_01_WATER_PROBE_CONTRACT,Water Monitoring,_CONTRACTS,US_12,North Carolina,cargoDelivery,1\u00d7 Sensory Buoy; 1\u00d7 Sensory Buoy; 1\u00d7 Sensory Buoy,570,5800,\"Finally, a one-of-a-kind job! We need to pick up some data buoys\u2014they're the latest thing in marine research tech. They need to be dropped off at the specified aquatic locations.\",CONTRACTS\nUS_12_01_SCIENTIFIC_INTELLIGENCE_CONTRACT,More Towers,_CONTRACTS,US_12,North Carolina,exploration,,350,6000,\"What were we doing in an off-road vehicle, you ask? Looking for a spot for yet another weather tower, of course! But since landslides are a concern, we should check a seismic vibrator.\",CONTRACTS\nUS_12_01_SCIENTIFIC_CONTINUED_TASK,On Our Radar,_TASKS,US_12,North Carolina,truckDelivery,,510,5950,\"That's right, we can't waste a good hill and NOT build something on top of it. This one's easy\u2014just grab a radar trailer and bring it to the specified location.\",TASKS\nUS_12_01_LEFT_LOAD_TASK,Running on Empty,_TASKS,US_12,North Carolina,truckDelivery,4\u00d7 Bags,720,5750,\"So we dispatched an order to another factory, but the truck set out with an empty tank, somehow. Needless to say, it didn't get far before it ran out of gas on the highway. Can you get the goods to their destination?\",TASKS\nUS_12_01_CAR_SWAMP_TASK,Beached,_TASKS,US_12,North Carolina,truckDelivery,,350,3800,\"Hey, man! My car died on me right when I was hauling a fully-stocked trailer. Could you tow it to the other shore for me? I'd be much obliged!\",TASKS\nUS_12_01_CAR_TRANSFORM_TASK,Safety Net,_TASKS,US_12,North Carolina,truckDelivery,,310,3800,\"Hey friend, I need some help. We forgot a bunch of spare parts\u2014could you get the trailer carrying them to the transformer yard on the other shore? It has the tools we really need, too.\",TASKS\nUS_12_01_PRIVATE_ORDER_TASK,Private Order,_TASKS,US_12,North Carolina,cargoDelivery,4\u00d7 Forklift Caravan Container 2,1300,10100,\"As you probably know, we're building a highway that's supposed to connect the two shores. We need some extra supplies and workers' cabins brought over to the other side. I heard you might be able to do the latter for us.\",TASKS\nUS_12_01_SPARE_PARTS_TASK,Spare Parts,_TASKS,US_12,North Carolina,cargoDelivery,4\u00d7 Vehicles Spare Parts,320,3550,\"I can't get a hold of my logistics guy for some reason, and there's a shipment of all kinds of car parts waiting for us at the port. Could I trouble you to do the job and get that shipment to me?\",TASKS\nUS_12_01_DELIVERY,Literally on a Timer,_CONTESTS,US_12,North Carolina,cargoDelivery,1\u00d7 Radioctive,100,550,\"One of our researchers left without this order. Please deliver it as quickly as you can. Hurrying won't be enough, you'll practically need to teleport!\",CONTESTS\nUS_12_01_TRIAL,Old Road,_CONTESTS,US_12,North Carolina,exploration,,120,700,\"Unfortunately, there's not much to clean on this route nowadays, and we aren't even sure if we even need to. With that in mind, what do you say to driving the route as fast as you can? Are you willing to accept the risks?\",CONTESTS\nUS_12_02_ENGINEERING_WORKS_CONTRACT,Electricity in the Air,_CONTRACTS,US_12,North Carolina,truckDelivery,2\u00d7 Service Spare Parts; 2\u00d7 Service Spare Parts,1390,15550,\"HELP! Our wind farm substation just croaked, along with a couple of turbines. We need to do some maintenance to get the farm running again. We're expecting some supplies and a trailer, but it's nowhere to be seen...\",CONTRACTS\nUS_12_02_STATOR_FOR_START_CONTRACT,You Spin Me Right 'Round,_CONTRACTS,US_12,North Carolina,truckDelivery,,1050,14950,\"The turbo generator stator is the heaviest piece of equipment at the power plant. They can't start the reactor without it, but the transportation requirements are really strict. Good luck out there\u2014you're gonna need it.\",CONTRACTS\nUS_12_02_DELIVERY_TOOLS_CONTRACT,Packing Nightmare,_CONTRACTS,US_12,North Carolina,truckDelivery,2\u00d7 Vehicles Spare Parts,880,9650,One of the hydrology field camps needs some extra supplies right away. Looks like a logistics error\u2014the guys forgot some things they needed.,CONTRACTS\nUS_12_02_FEMM_DELIVERY_CONTRACT,Abandoned Giant,_CONTRACTS,US_12,North Carolina,truckDelivery,,350,3500,\"One of the workers building the highway got drunk and left a vehicle idling at the construction site. It ran out of gas, and no one's been able to move it since. I say we get it out of there\u2014it just might come in handy.\",CONTRACTS\nUS_12_02_LARGE_DELIVERY_CONTRACT,Backlog,_CONTRACTS,US_12,North Carolina,cargoDelivery,2\u00d7 Logs Short; 4\u00d7 Logs Medium; 1\u00d7 Logs Long,2700,32600,\"While the sawmill was idle, our backlog got so bad, we almost went through our entire log supply. Another delivery isn't scheduled for a while yet, but you've got some free time on your hands, so I bet you can handle it.\",CONTRACTS\nUS_12_02_PUMPS_DELIVERY_CONTRACT,Static Pressure,_CONTRACTS,US_12,North Carolina,cargoDelivery,4\u00d7 Npp Pump,2590,28350,The list of special deliveries for our power plant also includes multistage pumps\u2014they're the last things we need to put our turbine hall into operation. Please be extra careful with them.,CONTRACTS\nUS_12_02_START_WORK_CONTRACT,\"There's Prep, and Then There's Prep\",_CONTRACTS,US_12,North Carolina,cargoDelivery,4\u00d7 Metal Planks; 2\u00d7 Metal Roll; 4\u00d7 Bags,2810,23900,\"And that's our nuclear power plant, the crown jewel of the energy industry! Impressive, right? We're mostly done with construction\u2014just a couple of urgent deliveries left to make. Let's start with something easy.\",CONTRACTS\nUS_12_02_HELP_SUN_CONTRACT,Soaking Up the Sun,_CONTRACTS,US_12,North Carolina,cargoDelivery,6\u00d7 Solar Panel,1730,19950,\"We take to any kind of power source like ducks to water, but when it comes to protecting the environment, solar power is hands-down the best! We've got nearly all the panels in place\u2014just one more shipment to go.\",CONTRACTS\nUS_12_02_WATER_FOR_THE_NEED_CONTRACT,Running Dry 1,_CONTRACTS,US_12,North Carolina,cargoDelivery,2700\u00d7 Water; 1900\u00d7 Water; 1800\u00d7 Water,2370,17550,\"We usually have a rainy spell this time of the year, but this time, something's gone wrong. Why don't we have an irrigation system? Good question... but let's not go there, okay? Let's just deal with this for now.\",CONTRACTS\nUS_12_02_ATOM_CONTRACT,Monumental Science,_CONTRACTS,US_12,North Carolina,cargoDelivery,2\u00d7 Bags; 2\u00d7 Metal Planks; 3\u00d7 Metal Roll,1910,16500,The authorities thought we should capitalize on this place's reputation as the nuclear power hub of the future and build a monument in the town closest to the power station. Can we count on you to deliver the supplies?,CONTRACTS\nUS_12_02_SAWMILL_REPAIR_CONTRACT,All Boarded Up,_CONTRACTS,US_12,North Carolina,cargoDelivery,2\u00d7 Service Spare Parts; 2\u00d7 Metal Planks; 4\u00d7 Metal Roll,1800,15450,\"Unfortunately, we've been shut down by a malfunction at the sawmill. Wood is an essential construction material for the plant, its units, and other things. We need you to deliver the parts to the sawmill on the double.\",CONTRACTS\nUS_12_02_RESOURCES_DELIVERY_CONTRACT,Testing the Waters,_CONTRACTS,US_12,North Carolina,cargoDelivery,2\u00d7 Wooden Planks Medium; 3\u00d7 Wooden Planks; 2\u00d7 Vehicles Spare Parts,1180,13450,\"We're doing a lot of work here, so testing our water seemed like a good idea. Our experts have set up camps in appropriate places, but now we need some additional supplies. That's where you come in.\",CONTRACTS\nUS_12_02_RESTORATION_OF_RAILWAY_CONTRACT,Derailed 2,_CONTRACTS,US_12,North Carolina,cargoDelivery,2\u00d7 Wooden Planks Medium; 1\u00d7 Railway,1140,13150,\"Railroad trouble is one of the reasons we're behind schedule with this building. One section has to be replaced, and another has a pile of rocks blocking the way. Things will speed up once you deal with these issues.\",CONTRACTS\nUS_12_02_BOILER_FOR_FARM_TASK,Boiling Hot,_TASKS,US_12,North Carolina,truckDelivery,1\u00d7 Boiler,1260,13700,\"We've got a major order, and I think you should take it now that the trains are running again. Some fat cat needs a big new boiler for the ranch he's just bought and wants to remodel.\",TASKS\nUS_12_02_DANGEROUS_BEND_TASK,A Close Miss,_TASKS,US_12,North Carolina,truckDelivery,1\u00d7 Pipe Large,1070,6550,\"Believe it or not, we almost made it to the power plant when our rig's rear wheels lost traction, the trailer ripped out of the coupling and sent the cargo flying into the river. We need everything back\u2014can you do that?\",TASKS\nUS_12_02_SAD_RIDE_TASK,Turn 14,_TASKS,US_12,North Carolina,truckDelivery,,460,4950,\"Some guys from Tennessee organized a swamp race in these parts\u2014kind of like a rally. But one of the contestants dropped out early, and his stalled car blocked a prizewinner. Let's get it out of the way, okay?\",TASKS\nUS_12_02_FALL_FROM_BRIDGE_TASK,Off-Balance,_TASKS,US_12,North Carolina,truckDelivery,,450,4400,\"I messed up when crossing a bridge so bad that my car got flipped on its side. I'm fine other than a couple of bruises, but the car... Anyway, I'd really appreciate it if you towed it back to me.\",TASKS\nUS_12_02_NIGHT_ERROR_TASK,Hopeless Romantics,_TASKS,US_12,North Carolina,truckDelivery,,410,3750,\"So, I decided to take my girlfriend out somewhere nice at night\u2014got her flowers and all that jazz. But the car got stuck along the way! All my romantic plans fell through, and I had to ditch the car, too. Fetch it for me, will you?\",TASKS\nUS_12_02_REPAIR_TRAILER_TASK,Founders Keepers,_TASKS,US_12,North Carolina,truckDelivery,,350,3050,\"So, my car broke down, and I called a local mechanic for help. He is... an interesting character, because he picked the most convoluted way to get to me and ended up stuck, naturally. Could you get his trailer out?\",TASKS\nUS_12_02_STUCK_MUD_TASK,A Little Favor,_TASKS,US_12,North Carolina,truckDelivery,,340,2850,\"So, our wind turbines broke down, but when we tried to fix them using our own resources, my trailer with all the tools got stuck. Our help's not needed anymore, but I'd still like to get my trailer back, you know. Could you retrieve it?\",TASKS\nUS_12_02_HELPING_NEIGHBORS_TASK,Helping the Neighbors,_TASKS,US_12,North Carolina,cargoDelivery,2\u00d7 Wooden Planks Medium; 3\u00d7 Wooden Planks; 1\u00d7 Wooden Planks Long; 2\u00d7 Wooden Planks,1060,11600,\"Like we said before, there's a huge backlog of wood orders. Some locals have been waiting forever\u2014could you do us a favor and deliver a bunch of planks to these locations?\",TASKS\nUS_12_02_BLOCK_ON_THE_ROAD_TASK,Another Roadblock,_TASKS,US_12,North Carolina,cargoDelivery,2\u00d7 Wooden Planks Medium,460,5600,\"As you probably know, the whole region is tectonically active, so we've got roadblocks left, right, and center here. This one in particular has cut us off from the NPP construction site. We need that road clear, stat!\",TASKS\nUS_12_02_RESCUE_WORKSHOP_TASK,Free Repairs,_TASKS,US_12,North Carolina,cargoDelivery,2\u00d7 Service Spare Parts,450,5150,We need some tools and spare parts to keep everything running like a well-oiled machine. Could you help us out? We'll owe you one!,TASKS\nUS_12_02_BEST_FOR_NPP_TASK,Lost Shipment,_TASKS,US_12,North Carolina,cargoDelivery,2\u00d7 Container Small Special,540,4500,\"We had to take a different route because of all the recent roadblocks, but we were NOT ready for it, and all the shipping containers ended up scattered. They're full of electronics for the power plant! Will you help us?\",TASKS\nUS_12_02_FALLEN_HAY_TASK,Hay Day,_TASKS,US_12,North Carolina,cargoDelivery,3\u00d7 Stack,390,2900,\"We needed to move some hay between ranches, but, of course, the road we picked was trash, so some of the hay is still decorating the hills by the river. Could you gather it back up for us?\",TASKS\nUS_12_02_MUD_TRACK_CONTEST,Swamp Rally,_CONTESTS,US_12,North Carolina,exploration,,120,1600,\"That's right, we ARE those Tennessee guys! We decided to set us up a little swamp race to see who's the best driver 'round these parts. Lots of people have already signed up\u2014it's gonna be great! So how about it? Wanna give it a try?\",CONTESTS\nUS_12_02_MOUNTAIN_RIDE_CONTEST,Secret Race,_CONTESTS,US_12,North Carolina,exploration,,140,1400,\"Psst, it's the Tennessee guys again! So we're holding another race, but there's a catch. We don't have the official go-ahead for this one! Are you ready to risk it all and zip along a crazy dangerous mountain track?\",CONTESTS\nUS_12_03_COOLING_TOWERS_01_CONTRACT,Two Towers 1,_CONTRACTS,US_12,North Carolina,cargoDelivery,2\u00d7 Blocks; 2\u00d7 Pipes Small; 1\u00d7 Container Large,1790,17450,\"Another important part of a nuclear plant is the cooling towers. They remove heat from the circulating water used in the plant. They're big in every sense of the word, so your cargo will be too. Think you can handle it?\",CONTRACTS\nUS_12_03_CONSTRUCTION_BRIDGE_01_CONTRACT,Split Loop 1,_CONTRACTS,US_12,North Carolina,cargoDelivery,1\u00d7 Metal Planks; 2\u00d7 Blocks,1020,10650,\"We've decided to revamp our cooling circuit a bit by adding a couple of bridges over the waterways. It's more of the usual\u2014just bring us some metal and concrete to work with, and we'll get the job done.\",CONTRACTS\nUS_12_03_CONSTRUCTION_WATCHPOINT_01_CONTRACT,The All-Seeing Eye,_CONTRACTS,US_12,North Carolina,cargoDelivery,1\u00d7 Wooden Planks; 1\u00d7 Service Spare Parts,620,6400,\"The old watchtower on the island is a real relic of the past. But despite the fact that it could collapse at any moment, it's kind of a local landmark. We need to fix it up ASAP!\",CONTRACTS\nUS_12_03_CONSTRUCTION_GARAGE_01_CONTRACT,Fix My Ride,_CONTRACTS,US_12,North Carolina,cargoDelivery,4\u00d7 Metal Roll,690,5950,\"In addition to the garage, we also need somewhere we can park our cars and provide all kinds of repair services. As you can see, construction is in full swing\u2014we just need you to deliver some supplies...\",CONTRACTS\nUS_12_03_SCOUT_01_TSK,Cliffhanger,_TASKS,US_12,North Carolina,truckDelivery,,700,8450,\"So there my family and I were, having a great time outdoors on our picnic, when I noticed my car rolling towards the cliff's edge. Somehow, it didn't fall, but it's hanging on by a thread. Can you get it out for me?\",TASKS\nUS_12_03_TRAILER_PARK_01_TSK,Refueling,_TASKS,US_12,North Carolina,truckDelivery,,630,7200,\"Believe it or not, some delinquents have siphoned all the gas from our cars! How the hell are we supposed to get around now?! The factory's management is willing to provide us with fuel, but we need you to get it here.\",TASKS\nUS_12_03_REPAIR_TOWER_01_TSK,Towering Issues,_TASKS,US_12,North Carolina,truckDelivery,,630,7200,\"Mayday, mayday! We've been having issues with operating electronics due to bad reception. Could you help us fix the comms tower?\",TASKS\nUS_12_03_TRAILER_01_TSK,The Essentials,_TASKS,US_12,North Carolina,truckDelivery,,560,6050,\"We were coming back from a delivery when our vehicle got stuck in some mud. They were able to tow the car, but the semi-trailer is still hanging out there. Could you please get it back for us?\",TASKS\nUS_12_03_SCOUT_TRAILER_01_TSK,Dinner is Served,_TASKS,US_12,North Carolina,truckDelivery,,540,5600,\"The local retreat ran out of some groceries\u2014right when they had updated their menu and were receiving guests, too! We got the order, you understand, and now the packages need to be delivered.\",TASKS\nUS_12_03_TRUCK_01_TSK,Somewhere on the Coast,_TASKS,US_12,North Carolina,truckDelivery,,470,4800,\"I was always told I was a natural driver. So, I always drove on our roads without thinking about what I was doing, and now I've paid the price for my hubris. Could you please help me, poor driver?\",TASKS\nUS_12_03_WATER_HOUSE_01_TSK,Living on the Outskirts 1,_TASKS,US_12,North Carolina,cargoDelivery,1300\u00d7 Water,1190,10550,\"Some people choose to build their homes really out of the way, and plumbing can pose a bit of an issue out there. Could you help us deliver some water to this location?\",TASKS\nUS_12_03_ECO_ORDER_01_TSK,Diving for Treasure,_TASKS,US_12,North Carolina,cargoDelivery,2\u00d7 Service Spare Parts Special,560,6400,The issues at the port a while ago caused one of the ships to crash after being unable to dock properly. We've learned it was carrying some pretty valuable cargo\u2014could you check if there's anything worth salvaging?,TASKS\nUS_12_03_FISHERS_01_TSK,The Fish Aren't Biting!,_TASKS,US_12,North Carolina,cargoDelivery,1\u00d7 Service Spare Parts,360,4250,\"I know this ain't no fishing festival, but we're still in need. Tools to repair this thing, more specifically. It shouldn't take long\u2014will you do it?\",TASKS\nUS_12_03_CONT_RACE_01,What is it?,_CONTESTS,US_12,North Carolina,exploration,,50,1200,\"We know the condition of routes emanating from the asphalt road in excruciating detail, but we don't know the condition of this particular road. Can you give your professional opinion?\",CONTESTS\nUS_12_03_COOLING_TOWERS_02_CONTRACT,Two Towers 2,_CONTRACTS,US_12,North Carolina,cargoDelivery,2\u00d7 Concrete Slab; 2\u00d7 Container Small; 2\u00d7 Metal Planks,2190,18950,\"Time to get the second one up and running. Is this water radioactive, you ask? Not at all! The radioactive water heated by the core runs through a separate circuit, heating and vaporizing water in the other one. Got it?\",CONTRACTS\nUS_12_03_CONSTRUCTION_BRIDGE_02_CONTRACT,Split Loop 2,_CONTRACTS,US_12,North Carolina,cargoDelivery,1\u00d7 Metal Planks; 2\u00d7 Blocks,1020,10500,\"We've got two waterways, which means we need two bridges. We could just drive around the whole thing, but why not make our lives easier if we've got the funds? Hey, did your eyes just turn into dollar signs for a second?\",CONTRACTS\nUS_12_03_CONSTRUCTION_GARAGE_02_CONTRACT,Pimp My Ride,_CONTRACTS,US_12,North Carolina,cargoDelivery,4\u00d7 Bags; 2\u00d7 Service Spare Parts,950,7950,\"Think about it. Small town\u2014check! Big construction site\u2014check! Old and new vehicles in need of maintenance\u2014check! I could go on, but you get the point. We need the whole package for our garage!\",CONTRACTS\nUS_12_03_SCOUT_TRAILER_02_TSK,Old Barn,_TASKS,US_12,North Carolina,truckDelivery,,610,7250,\"Our old barn has been completely overgrown with weeds, and as to what happens inside, well, it's better not to know. That why it's time to clear away this rubbish. That's where you come in.\",TASKS\nUS_12_03_SCOUT_02_TSK,Empty Garage,_TASKS,US_12,North Carolina,truckDelivery,,510,5150,\"Hey! I was driving this little truck along the coast and got stuck. Well, I was the one who chose the road! Could you help me tow it out?\",TASKS\nUS_12_03_WATER_HOUSE_02_TSK,Living on the Outskirts 2,_TASKS,US_12,North Carolina,cargoDelivery,1500\u00d7 Water,1250,11650,\"We've got another remote residence cut off from the water supply. Please fill up your tank and hurry over there before they file a complaint! Yeah, I know, we messed up, but we'll compensate you for the trouble.\",TASKS\nUS_12_03_CONT_RACE_02,Hardships along the Way,_CONTESTS,US_12,North Carolina,exploration,,80,1400,\"Forest rangers are having difficulty reaching observation towers. We need to perform a quality assessment of the coastal roads. So, drive along this route with your knowledge and a timer.\",CONTESTS\nUS_12_03_REPAIR_CRANE_CONTRACT,Reloading,_CONTRACTS,US_12,North Carolina,truckDelivery,,510,5400,\"As you might have heard, we had an incident at the port: it's flooded. No worries though, we've got a clean-up crew on it and we're still accepting shipments. The only real issue is one of the cranes...\",CONTRACTS\nUS_12_03_FARM_WATER_CONTRACT,Running Dry 2,_CONTRACTS,US_12,North Carolina,cargoDelivery,2700\u00d7 Water; 1500\u00d7 Water; 1500\u00d7 Water,2300,18250,\"Didn't we have a conversation about dry \u201crainy\u201d seasons before? I think you said something about irrigation systems... anyway, please fill those water tanks. The farmers are counting on you.\",CONTRACTS\nUS_12_03_TRAILER_SHOP_CONTRACT,\"Dude, Where's My Trailer?\",_CONTRACTS,US_12,North Carolina,cargoDelivery,2\u00d7 Metal Roll; 1\u00d7 Metal Planks; 2\u00d7 Vehicles Spare Parts,1030,10150,\"So you're saying the trailer parking lot needs to be right by the garage? That'd definitely be convenient, but this is a job from a corporate client, and as far as I know you'll be able to buy those trailers later on.\",CONTRACTS\nUS_12_03_REPAIR_DAM_CONTRACT,Going with the Flow,_CONTRACTS,US_12,North Carolina,cargoDelivery,1\u00d7 Container Small; 1\u00d7 Container Small,790,7700,\"We've got a problem. The dam is fine, but because of certain internal malfunctions, water is no longer being pumped into the tanks. This has to be fixed right away, since the guys in town need tons of water right now.\",CONTRACTS\nUS_12_03_BUNKER_ORDER_CONTRACT,Lost and Found,_CONTRACTS,US_12,North Carolina,cargoDelivery,2\u00d7 Rubbish,780,7500,\"They're shutting down the nearby mine until all the power plant units are built. The workers were in such a hurry to go home, they forgot to clear the mines properly. We are talking a couple of containers of waste. You know where I'm going with this?\",CONTRACTS\nUS_12_03_SEISMO_CHECK_CONTRACT,Scanning the Area,_CONTRACTS,US_12,North Carolina,exploration,,360,6200,\"Taking into account the large number of landslides and low-magnitude earthquakes, we'd like you to test some areas for seismic activity. It's especially important because of the mine and the people working in it.\",CONTRACTS\nUS_12_03_TRAILER_ENERGY_TSK,Short Circuit,_TASKS,US_12,North Carolina,truckDelivery,,520,5600,\"The flood didn't just damage the loading docks\u2014our power unit took a hit, too. We urgently need an independent power supply. Luckily, the closest logistics center has one available.\",TASKS\nUS_12_04_FESTIVAL_PREPARING_01_TASK,Fishing Festival 1,_TASKS,US_12,North Carolina,cargoDelivery,2\u00d7 Logs Short; 1\u00d7 Service Spare Parts,570,6600,\"We're almost done getting ready for the annual fishing festival, but we still need some supplies to fix the old boats. Do you think you'll be able to get them here on time?\",TASKS\nUS_12_04_FESTIVAL_PREPARING_02_TASK,Fishing Festival 2,_TASKS,US_12,North Carolina,truckDelivery,2\u00d7 Cellulose,970,11750,\"Yup, yup, put that right here. Oh, there you are! You've been a huge help, but could you do me another favor? I need my trailer and a bunch of scrap paper brought over from the observatory. Gonna make us a bonfire!\",TASKS\nUS_12_04_FACTORY_OIL_CONTRACT,Pump It Up,_CONTRACTS,US_12,North Carolina,truckDelivery,1\u00d7 Forklift Caravan Container 2; 2\u00d7 Metal Planks,2110,19750,\"Our oil well is pretty old, but it's still going strong. The issue actually lies with the refinery. We need new storage tanks and some parts to fix things up and actually ship the goods.\",CONTRACTS\nUS_12_04_FACTORY_METAL_CONTRACT,Getting Rusty,_CONTRACTS,US_12,North Carolina,truckDelivery,2\u00d7 Wooden Planks; 2\u00d7 Blocks,1180,13600,\"This old plant fell into disrepair over time, but we decided to whip it back into shape, and it's almost ready now. We just need some supplies for the repairs and a generator to get us through the initial period.\",CONTRACTS\nUS_12_04_ANTENNA_BUILDING_CONTRACT,It's All Connected 2,_CONTRACTS,US_12,North Carolina,truckDelivery,2\u00d7 Metal Roll; 1\u00d7 Metal Planks,1040,10400,\"The observatory staff needs accurate data straight from my favorite satellites, so they've decided to build additional radio telescopes closer to the facility. Are you busy right now? Any chance we could clone you?\",CONTRACTS\nUS_12_04_LOGS_DELIVERY_CONTRACT,Everybody Gets a Log,_CONTRACTS,US_12,North Carolina,cargoDelivery,2\u00d7 Logs Short; 2\u00d7 Logs Medium; 3\u00d7 Logs Short; 1\u00d7 Logs Long; 1\u00d7 Logs Short; 1\u00d7 Logs Medium; 1\u00d7 Logs Long,2360,25250,\"Since the regional renovation plan was approved, the land has been selling like hotcakes. People are eager to build houses here, so we've had a huge uptick in log orders. Good thing there's no shortage of logs...\",CONTRACTS\nUS_12_04_OIL_DELIVERY_CONTRACT,Lonely Oil Tanker,_CONTRACTS,US_12,North Carolina,cargoDelivery,4\u00d7 Barrels Oil,1170,14450,\"Now that we've started processing the freshly-pumped oil, we need to have it delivered to the main port. Be careful not to spill anything, or our hydrologist friends are gonna have our heads on a platter.\",CONTRACTS\nUS_12_04_RAILWAY_STATION_REPAIR_CONTRACT,Getting Back on Track,_CONTRACTS,US_12,North Carolina,cargoDelivery,2\u00d7 Forklift Caravan Container 2; 2\u00d7 Metal Planks; 4\u00d7 Service Spare Parts,1850,13150,Welcome to the most remote part of the region. Abandoned utility buildings and a couple of houses are the only signs of life left here. I think we should start by repairing the train station\u2014for the sake of logistics.,CONTRACTS\nUS_12_04_COOLING_TOWER_TRASH_CONTRACT,Spring Cleaning,_CONTRACTS,US_12,North Carolina,cargoDelivery,3\u00d7 Rubbish,1220,13100,\"The construction on the cooling towers is done, but big sites like that always get messy. We've sorted the debris into separate containers, so all you need to do is deliver them to the train station and see them off.\",CONTRACTS\nUS_12_04_RAILWAY_REPAIR_CONTRACT,Derailed 1,_CONTRACTS,US_12,North Carolina,cargoDelivery,2\u00d7 Service Spare Parts; 2\u00d7 Service Spare Parts,840,10250,\"There are several places where the tracks are obstructed by rubble. You're probably sick of it at this point, but could you please help us out one more time? It's the last thing we need to get the trains running here.\",CONTRACTS\nUS_12_04_OBSERVATORY_CONTRACT,Clear Picture,_CONTRACTS,US_12,North Carolina,cargoDelivery,3\u00d7 Solar Panel; 3\u00d7 Bricks,1010,9950,\"Our old astronomer friends like to keep up with advances in tech, so they've ordered some new solar panels and maintenance supplies. You help them out, and who knows? Maybe they'll name their next discovery after you...\",CONTRACTS\nUS_12_04_REMOVAL_RADIOACTIVE_CONTRACT,Toxic Waste,_CONTRACTS,US_12,North Carolina,cargoDelivery,4\u00d7 Radioctive,800,9850,Here comes the first batch of nuclear waste! We've got a train waiting at the station to haul it away. Be careful with this cargo! We really don't want to mess up and hurt the environment.,CONTRACTS\nUS_12_04_CITY_SCOUT_CONTRACT,Ghost Town,_CONTRACTS,US_12,North Carolina,exploration,,30,500,\"People tell all kinds of stories about this old abandoned town. Personally, I don't believe in paranormal stuff like ghosts and the undead, but why don't you drive around and see for yourself?\",CONTRACTS\nUS_12_04_OIL_PROBLEMS_TASK,Mixing Oil and Water,_TASKS,US_12,North Carolina,truckDelivery,,890,10500,\"There was just an accident involving a fuel truck. Some of the gas leaked into the river, and the scientists are furious now! We need to get the truck out right away and assess the level of contamination.\",TASKS\nUS_12_04_SLIPPERY_ROAD_TASK,Slippery Slope,_TASKS,US_12,North Carolina,truckDelivery,,420,5100,\"Hey friend, can you tow a truck for us? The driver decided to take a shortcut but lost control of his vehicle. Luckily, he wasn't hurt too bad, but that truck is a no-go...\",TASKS\nUS_12_04_FOREST_VECHICLE_TASK,Just a Little Bit More,_TASKS,US_12,North Carolina,truckDelivery,1\u00d7 Vehicles Spare Parts,440,4300,\"We made camp and spent a few days in this beautiful place. Then I decided to head into the city for groceries, but I took a slightly different way back. Now the car is in a ditch, but I somehow emerged unscathed. Could you help me pull it out?\",TASKS\nUS_12_04_LOGS_PROBLEM_TASK,Failed Logging,_TASKS,US_12,North Carolina,truckDelivery,,330,3450,\"Yeah, I know, the roads are awful here. We're working on restoring the region, and you've been a great help. But uh, you know... We had a trailer full of logs, and now we need it towed...\",TASKS\nUS_12_04_UNFINISHED_BUSINESS_TASK,Unfinished Business,_TASKS,US_12,North Carolina,truckDelivery,,300,3400,\"An incident happened on the job. My truck got stuck on the road. I had to abandon the trailer, and barely made it out with just the tractor unit. If you can help me with the trailer, I'll pay you handsomely.\",TASKS\nUS_12_04_USEFUL_SEMITRAILER_TASK,Essential Semi-Trailer,_TASKS,US_12,North Carolina,truckDelivery,,310,3200,\"As soon as I stepped on the gas, the semi-trailer broke free! It rolled over the edge and fell down the slop. Listen, help me get it back.\",TASKS\nUS_12_04_BAD_PARKING_PLACE_TASK,Parking Ticket,_TASKS,US_12,North Carolina,truckDelivery,,310,3050,\"I always back my jeep right up to the boats to make loading easier, but it looks like the river bank eroded this time, and my car ended up sinking. Can you help me pull it out and tow it to my house?\",TASKS\nUS_12_04_WRONG_TURN_TASK,A Wrong Turn,_TASKS,US_12,North Carolina,truckDelivery,,320,2550,\"Hey bud, could you possibly tow our car to the old town? My friends and I were planning to explore it, but we took a detour to that pretty lake, and the rest is history. We could actually use some repairs, too...\",TASKS\nUS_12_04_HIGHWAY_TASK,High Stakes,_TASKS,US_12,North Carolina,cargoDelivery,1\u00d7 Forklift Caravan Container 2; 2\u00d7 Vehicles Spare Parts,990,11450,\"We want to finish the construction project, even though the company basically hijacked our truck. We're short on time but there's a lot to do, meaning we need more manpower, workers' cabins, and spare parts.\",TASKS\nUS_12_04_SMALL_HELP_TASK,A Little Help,_TASKS,US_12,North Carolina,cargoDelivery,1\u00d7 Wooden Planks; 1\u00d7 Bricks,400,5250,\"We're planning to remodel our home. We really need construction materials. Ordinary delivery drivers can't make it up here, but you are a special driver. Could you help us?\",TASKS\nUS_12_04_TRAIL_TO_THE_TOWER_TASK,Tower Trail,_TASKS,US_12,North Carolina,cargoDelivery,1\u00d7 Logs Medium; 1\u00d7 Wooden Planks,410,4400,\"This bridge was pretty worn down as it is, and the latest tremors were the final straw. This was the shortest way to the watchtower, too. Can we count on you?\",TASKS\nUS_12_04_OBSTACLE_ON_THE_ROAD_TASK,Ruins of the Old Town,_TASKS,US_12,North Carolina,cargoDelivery,2\u00d7 Service Spare Parts,240,2750,\"Earth tremors aren't uncommon in this region, but Mother Nature outdid herself when she dumped a landslide on the old town bypass. Could you help us move the rubble?\",TASKS\nUS_12_04_COASTLINE_RACE_CONTEST,Fast as a Jet,_CONTESTS,US_12,North Carolina,exploration,,450,900,We've got another challenge for you! How fast can you travel the coastline? It's not your usual route\u2014lots of water and rocks; people almost never go there. You up for some thrills?,CONTESTS\nRU_13_01_SUPPLY_CAREER_01_CONTRACT,In the Wrong Place,_CONTRACTS,RU_13,Almaty,truckDelivery,,440,4300,\"Hey, a new worker! We need as much help as we can get right now. To make a long story short, we're still getting the new quarries sorted out, and we haven't had time to move the equipment from the old quarry to the logistics center yet. It shouldn't take more than twenty minutes or so, just a quick trip. Would you mind?\",CONTRACTS\nRU_13_01_EXPORT_SHALE_01_CONTRACT,One Rock at a Time 1,_CONTRACTS,RU_13,Almaty,cargoDelivery,4\u00d7 Stone Block,2010,17100,\"We're hard at work getting production up and running again. It's going pretty well, but we're short on slate. It just so happens that they've got a batch of slate ready to ship out over at the slate quarry. We literally just need a couple of little rocks. Would you mind?\",CONTRACTS\nRU_13_01_CONSTRUCTION_RIBBONS_01_CONTRACT,Ruptured Artery,_CONTRACTS,RU_13,Almaty,cargoDelivery,2\u00d7 Metal Planks; 2\u00d7 Service Spare Parts; 2\u00d7 Metal Planks; 2\u00d7 Service Spare Parts,1640,14350,\"Lousy piece of junk! The conveyor belt broke down in two places at the same time! We've got a big order to fill, and we need to act fast. We need spare parts and metal rods. Can you deliver the entire order to these coordinates?\",CONTRACTS\nRU_13_01_AERODROM_01_CONTRACT,Risen from the Rubble,_CONTRACTS,RU_13,Almaty,cargoDelivery,4\u00d7 Bags; 2\u00d7 Rubbish,1090,9050,\"Time may be the great equalizer, but all great things come from small beginnings. Take us, for example: we need concrete to restore the main part of the building. After that, we're going to need all the construction debris hauled away. Think you can handle it?\",CONTRACTS\nRU_13_01_SEISMO_01_CONTRACT,\"Close, Sesame!\",_CONTRACTS,RU_13,Almaty,exploration,,400,6950,\"The old system of underground mine shafts could cave in because of the work we've been doing in the new deposits, so we need to get some information about the current condition of these abandoned shafts. Feel like testing the soil for sturdiness?\",CONTRACTS\nRU_13_01_HELP_ADMINISTRATION_01,Lights Out,_TASKS,RU_13,Almaty,truckDelivery,,760,8700,\"Welcome to our town! We're having some issues with the power right now, and we're almost out of candles, but the mining company has agreed to lend us a couple of generators. Would you mind grabbing them from the logistics center?\",TASKS\nRU_13_01_SUPPLY_CAREER_02_CONTRACT,In the Right Place,_CONTRACTS,RU_13,Almaty,truckDelivery,,480,5300,\"Listen, pal... remember the equipment you hauled to the logistics center? Well, my boss wants us to move it to the new silver quarry now. Do me a solid, would ya?\",CONTRACTS\nRU_13_01_EXPORT_SHALE_02_CONTRACT,One Rock at a Time 2,_CONTRACTS,RU_13,Almaty,cargoDelivery,4\u00d7 Stone Block,2130,19250,\"To make a long story short, pal, we've got a big order, and our client doesn't mess around. We need you to haul a couple of big blocks to the airport, and we really want to see how fast you can get the job done. Alright, off you go!\",CONTRACTS\nRU_13_01_AERODROM_02_CONTRACT,Aviation Reborn,_CONTRACTS,RU_13,Almaty,cargoDelivery,4\u00d7 Metal Roll; 2\u00d7 Container Small Special,950,8500,\"Before we can get the airport running again, we need to repair the flight control deck and the unloading area. We'll need metal sheets and some containers to make it happen. We're almost to the finish line!\",CONTRACTS\nRU_13_01_CONSTRUCTION_RIBBONS_02_CONTRACT,Cloudy with a Chance of Rocks,_CONTRACTS,RU_13,Almaty,cargoDelivery,2\u00d7 Service Spare Parts; 2\u00d7 Metal Planks,780,6450,\"This conveyor belt used to work like gangbusters. And it still does, as long as it's run by people who know what they're doing... and have plenty of parts and metal rods, of course. You deliver the parts, we'll get the old girl back on her feet. Then we'll be off to the races!\",CONTRACTS\nRU_13_01_SEISMO_02_CONTRACT,Tremors,_CONTRACTS,RU_13,Almaty,exploration,,130,2150,\"To stop kids from playing in the new slate quarry, we've been telling them there's a terrifying creature living at the bottom that'll take them away. But, thanks to our safety regulations, we still need to make sure the rock stratum is stable. Would you mind checking it out?\",CONTRACTS\nRU_13_01_HELP_ADMINISTRATION_02,Brick Renovation,_TASKS,RU_13,Almaty,cargoDelivery,4\u00d7 Bricks,450,4700,\"We just got a batch of bricks for repairing unsafe apartment buildings. We need them delivered to the processing workshop in the village. It's urgent, and we don't have enough drivers or vehicles. We need your help!\",TASKS\nRU_13_01_SUPPLY_CAREER_03_CONTRACT,Fury Road,_CONTRACTS,RU_13,Almaty,truckDelivery,1\u00d7 Tank Wagon; 4\u00d7 Barrels Oil,790,9800,\"The tank turned over in the quarry! Without fuel, workers can lose their minds! Urgently, take the damaged tank to the logistics center. And the new tank with barrels needs to be picked up at the gas station and distributed to the quarries.\",CONTRACTS\nRU_13_01_SEISMO_03_CONTRACT,Silver Fever Checkpoint,_CONTRACTS,RU_13,Almaty,exploration,,130,2250,\"Silver is a valuable thing, so silver mining has to be done quickly. The new silver quarry is full of cavities that could lead to a cave-in, so we need to quickly check the rock stratum to make sure it's secure. Can you do it?\",CONTRACTS\nRU_13_01_HELP_ADMINISTRATION_03,Concrete Promises,_TASKS,RU_13,Almaty,cargoDelivery,4\u00d7 Blocks,900,9750,\"The inspector found damage to the buildings' concrete, so it's going to take more than just bricks to fix the problem. The concrete factory has sent us a couple of plates. Now we just need to have the local craftsmen saw them into blocks and deliver them to town.\",TASKS\nRU_13_01_SUPPLY_CAREER_04_CONTRACT,This is Not a Drill,_CONTRACTS,RU_13,Almaty,cargoDelivery,1\u00d7 Container Large Drilling; 5\u00d7 Service Spare Parts Special,1670,18100,\"The quarry's going full blast now! Well, almost... we're still missing a few things: a new drill and some parts for it. They've already been delivered to the airport, but we still need to have them shipped here so we can try them out.\",CONTRACTS\nRU_13_01_EXPORT_SILVER_CONTRACT,\"The Treasury is Empty, My Lord\",_CONTRACTS,RU_13,Almaty,cargoDelivery,8\u00d7 Silver,1700,15950,\"Here's the first batch of processed silver. We need to deliver it to the airport and unload it. I hope you realize how important this cargo is... so please don't lose it or wander off with it. If all goes well, we'll pay you a pretty penny. Sound good?\",CONTRACTS\nRU_13_01_DELIVERY_TROLLEY_CONTRACT,Pimp My Ride,_CONTRACTS,RU_13,Almaty,cargoDelivery,4\u00d7 Minecart,1750,15600,\"Things are running pretty smoothly now! After we got the factory running, my boss decided to replace some of the old mine carts to speed things up a bit. The only problem is that they still need to be shipped to the quarry from the plant. Think you can help us out?\",CONTRACTS\nRU_13_01_REPAIR_SILVER_CONTRACT,In the Ruins of Former Glory,_CONTRACTS,RU_13,Almaty,cargoDelivery,4\u00d7 Vehicles Spare Parts; 4\u00d7 Service Spare Parts,800,7750,\"The silver plant is all set to start running again. We just need to repair a couple of fitting assembles, but we can't do it without some special parts. Can we count on you to deliver them?\",CONTRACTS\nRU_13_01_LOGISTICS_CENTER_CONTRACT,Large Order #996745S,_CONTRACTS,RU_13,Almaty,cargoDelivery,8\u00d7 Service Spare Parts,690,7650,\"We were supposed to get some new parts from town: two filters boxes, a filters box large, a compressor with extra power, a box of batteries, and a large burner. The delivery's been delayed. Do you think you could grab it for us?\",CONTRACTS\nRU_13_01_GEO_EXPLORATION_CONTRACT,The Bloodhound Gang,_CONTRACTS,RU_13,Almaty,exploration,,310,5400,\"In addition to mining, we also need to predict the future movements of rock strata. Scanning the area with a special radar could help with this. The important thing is to find the right place to put the radar.\",CONTRACTS\nRU_13_01_NOT_A_ROLLER_COASTER,Not a Roller Coaster,_TASKS,RU_13,Almaty,truckDelivery,1\u00d7 Container Large,1420,15050,\"Of all the rotten luck! I was hoping to shorten my route to the quarry by cutting through this ditch. I calculated all the risks carefully, but I suck at math, so now I'm stuck. Since you're here, would you mind giving me a hand?\",TASKS\nRU_13_01_WATER_SHARES_STONE_TASK,The Way of Water,_TASKS,RU_13,Almaty,truckDelivery,1800\u00d7 Water; 2000\u00d7 Water,1420,14000,\"I heard you helped the locals fix the water tower. Well, we could use some help with water in the quarries too. We've already got a tank all ready for you. You just need to go to the old quarry and pick it up.\",TASKS\nRU_13_01_HELP_BORAT,A Residential Issue,_TASKS,RU_13,Almaty,truckDelivery,1\u00d7 Forklift Caravan Container 2,1060,8650,\"They're about to start a new construction project at the helicopter pad, and, as you can see, the crew trailer we ordered has been held up. I'd hate to leave the workers without a roof over their heads, but I also can't get there on my own. Could you give me a lift?\",TASKS\nRU_13_01_LOGS_FOR_HOUSE,Final Destination,_TASKS,RU_13,Almaty,truckDelivery,1\u00d7 Logs Medium,720,7200,\"My God, it's a good thing nobody got hurt. The cable barely hung on \u2014 it's a wonder it didn't snap. It sure took a pounding, though. I'm gonna need some help to get my vehicle out of here. It's stuck pretty good. Could you give me a hand?\",TASKS\nRU_13_01_SHEPHERD_TROUBLE,92 Horses and One Horse Breeder,_TASKS,RU_13,Almaty,truckDelivery,,420,5100,\"Man, this was a hell of a car back in the day! 92 horsepower, light, easy to drive... and now all she's good for is scrap. Think you could tow her to the garage for me? It'll be way easier for me to fix her up there.\",TASKS\nRU_13_01_GEO_RADAR,Big Brother is Scanning You,_TASKS,RU_13,Almaty,truckDelivery,,320,3600,\"This place is perfect for scanning! I've already submitted a request for the equipment, and the radar is waiting at the logistics center. I just need someone to deliver it, and then I can turn it on and start scanning. Well begun is half done, am I right?\",TASKS\nRU_13_01_TOURIST_LIST,The Bucket List,_TASKS,RU_13,Almaty,truckDelivery,,260,2650,My buddy and me decided to start traveling in our old age. It's a good thing we made up high \u2014 there was a landslide last night that damaged our vehicle. We ain't exactly spring chickens \u2014 we're not about to push that thing all the way to town. Feel like giving a couple of old geezers a tow?,TASKS\nRU_13_01_TRASH_EXPORT_TASK,Taking Out the Trash,_TASKS,RU_13,Almaty,cargoDelivery,3\u00d7 Rubbish,1730,24400,\"Your environment can really affect your mood, you know? Well, we've got a whole bunch of trash piled up from all the work we've been doing, and it's made working here pretty unpleasant. Cold you lug all this junk to the recycling center for us?\",TASKS\nRU_13_01_HUMANITARIAN_ASSISTANCE_FUEL,Humanitarian Aid: Fuel,_TASKS,RU_13,Almaty,cargoDelivery,1\u00d7 Barrels Oil; 1\u00d7 Barrels Oil; 1\u00d7 Barrels Oil; 1\u00d7 Barrels Oil; 1\u00d7 Barrels Oil,1310,14700,\"Thanks for stopping by, pal! Here's the thing... we really need some gas, or pretty soon we won't be able to run our generators anymore. And without them, things will start going south around here in a hurry. We'll owe you one!\",TASKS\nRU_13_01_RELIABLE_SUPPORT_TASK,A Strong Foundation,_TASKS,RU_13,Almaty,cargoDelivery,1\u00d7 Wooden Planks Medium; 2\u00d7 Wooden Planks; 2\u00d7 Metal Planks,1250,13500,\"The only true constant is change, and that goes double for support structures in mines \u2014 they're always getting swapped out. So we need new beams to make sure there isn't a cave-in in either the silver quarry or the slate quarry.\",TASKS\nRU_13_01_HUMANITARIAN_ASSISTANCE_WATER,Humanitarian Aid: Water,_TASKS,RU_13,Almaty,cargoDelivery,750\u00d7 Water; 900\u00d7 Water,1090,12500,\"It's great to see you! You'll never believe what happened \u2014 the local water truck's engine died, and now it's just sitting idle. We're running out of water, and without it, we're in big trouble. Would you help us out and bring us some water from the local water tower?\",TASKS\nRU_13_01_WATER_TOWER_REPAIR,How Dry I Am,_TASKS,RU_13,Almaty,cargoDelivery,2\u00d7 Metal Roll; 1\u00d7 Pipes Small,960,10800,\"The pipes are in pretty rough shape. Aside from this old water tower, there's nowhere else to get water. As always, the pipes burst, time flies, and the water's all gone. We gotta fix it. Don't worry about the pay \u2014 the whole village pitched in for this one.\",TASKS\nRU_13_01_HUMANITARIAN_ASSISTANCE_FOOD,Humanitarian Aid: Supplies,_TASKS,RU_13,Almaty,cargoDelivery,1\u00d7 Potato; 1\u00d7 Potato; 1\u00d7 Potato,890,9200,\"Greetings, traveler! We don't get many visitors around here. But since you're here, would you mind helping us out? The government can't give us any vehicles to deliver food to the camps, and our supplies are running out. We won't haggle about the price!\",TASKS\nRU_13_01_AVIATION_FUEL,Fuel for Aircraft,_TASKS,RU_13,Almaty,cargoDelivery,3\u00d7 Barrels Oil; 3\u00d7 Barrels Oil,920,9100,\"Come in! Airfield dispatch here. We're in urgent need of fuel! If we don't get some soon, we'll need to start canceling flights. We require assistance ASAP. Repeat, we require assistance ASAP! Awaiting delivery from the nearest fuel station. Over and out.\",TASKS\nRU_13_01_LOGS_PROBLEM,\"Cranes, Saws, and Three Smoking Logs\",_TASKS,RU_13,Almaty,cargoDelivery,1\u00d7 Logs Long,670,6900,\"These are top-notch evergreen trees \u2014 raised in Siberia, chopped down in Zherbaisk, and as long as a five-story building. But what's the point? They're just lying there rotting away while we wait for the crane operator to start his shift. Would you mind hitching them to your truck and delivering them to the village?\",TASKS\nRU_13_01_EASTERN_BRIDGE,A Bridge Too Far,_TASKS,RU_13,Almaty,cargoDelivery,1\u00d7 Logs Medium; 2\u00d7 Wooden Planks Medium,600,6850,\"This bridge served the people of this village for a long, long time. But the soil shifted due to the mining operation, and the bridge collapsed. The villagers have saved up enough to fix it, but they still need to have the necessary resources delivered.\",TASKS\nRU_13_01_CANYON_BLOCKAGE,Hard Hat Area,_TASKS,RU_13,Almaty,cargoDelivery,1\u00d7 Metal Planks; 2\u00d7 Service Spare Parts,680,6750,\"This is bad... I can't get reach the emergency relief service! There's barely any signal here anyway. Looks like we're on our own. I'll get the workers together while you bring us some parts for clearing the landslide. We'll settle up afterwards, okay?\",TASKS\nRU_13_01_GREAT_DEAL,A Great Deal,_TASKS,RU_13,Almaty,cargoDelivery,1\u00d7 Service Spare Parts,480,6700,\"Jeep for sale! For parts/not working. Doesn't start. Been outside for a long time. Needs new engine, power train, and gearbox. Reasonable offers considered.\",TASKS\nRU_13_01_QUARRY_BLOCKAGE,The Broken Spiral,_TASKS,RU_13,Almaty,cargoDelivery,1\u00d7 Metal Planks; 2\u00d7 Service Spare Parts,660,6350,\"Dammit, some stone blocks fell off the side! They're blocking the other way out of the quarry now. We need to get this sorted in a hurry. It shouldn't take more than a couple hours \u2014 you just need to bring us some metal rods and parts.\",TASKS\nRU_13_01_NORTHERN_BRIDGE,The Shortest Distance,_TASKS,RU_13,Almaty,cargoDelivery,1\u00d7 Wooden Planks Medium; 1\u00d7 Logs Short,400,4050,\"This bridge didn't make it through the last landslide, so it's out of commission at the moment. It hung on as long as it could, but nature had the last laugh. Looks like we gotta fix it. You in?\",TASKS\nRU_13_01_RIBBONS_ROAD_SCOUT_TASK,Conveyor Belt Inspection,_TASKS,RU_13,Almaty,exploration,,210,3650,Sometimes double-checking something just isn't enough! So would you mind doing me a little favor? I need you to take a drive around the conveyor belt's connecting points and make sure they look alright... just in case.,TASKS\nRU_13_01_TOWER_ROAD_SCOUT_TASK,All Roads Lead to the Tower,_TASKS,RU_13,Almaty,exploration,,90,1450,\"Our central tower has an excellent view of the area. I recommend checking it out. And while you're at it, you can evaluate the condition of the road. Is it passable for tourists? I'm not asking you to do it for free, of course!\",TASKS\nRU_13_01_SOUTH_ROAD_SCOUT_TASK,Detour,_TASKS,RU_13,Almaty,exploration,,60,1000,\"We don't use the detour that often, but the asphalt road to the plant is really worn out. The rock also isn't particularly stable thanks to all the mining. Could you check the route?\",TASKS\nRU_13_01_IN_SEARCH_OF_AIDAHAR_CONTEST,In Search of the Aidakhar,_CONTESTS,RU_13,Almaty,exploration,,450,900,\"The Aidakhar is a wily creature indeed! I've been after it for twenty years now. They say it's been spotted in the lakes here at night, but I need to be certain! The faster you can search the coastline, the more chances you'll have to catch it!\",CONTESTS\nRU_13_01_MOUNTAIN_CLIMBING_CONTEST,Gone in 240 Seconds,_CONTESTS,RU_13,Almaty,exploration,,400,800,\"Hey, traveler, we have a little contest here every year to see who can make it up and down the mountain paths the fastest. Anybody can enter, by the way. Should I sign you up?\",CONTESTS\nUS_14_01_WAREHOUSE_PROBLEMS_CONTRACT,Warehouse Problems,_CONTRACTS,US_14,Austria,truckDelivery,1\u00d7 Rubbish; 4\u00d7 Metal Planks; 3\u00d7 Metal Roll,3150,32050,Looks like our warehouse was struck by lightning! We need to have it replaced. Could you dismantle whatever's left and help us build a new one?,CONTRACTS\nUS_14_01_EMERGENCY_HELP_CONTRACT,A Double-edged Sword,_CONTRACTS,US_14,Austria,truckDelivery,3\u00d7 Metal Planks; 2\u00d7 Metal Roll; 2\u00d7 Wooden Planks Medium,2790,24100,\"The bridge is almost ready, although the guys on one end are out of supplies, and the guys on the other need special equipment. Start with whatever you want, but this bridge needs to be finished. Can we count on you to deliver both?\",CONTRACTS\nUS_14_01_PIPES_DELIVERY_CONTRACT,Let There be Water!,_CONTRACTS,US_14,Austria,truckDelivery,1\u00d7 Pipes Small; 1\u00d7 Pipes Medium; 4\u00d7 Bags,2140,21200,\"We're the only ones who have access to water extraction equipment in the area. Heavy use has caused a lot of wear and tear in the pipes, so they need to be replaced right away. Our usual driver's on sick leave, which is why we need your help delivering supplies.\",CONTRACTS\nUS_14_01_PROBLEM_IN_THE_PORT_CONTRACT,Trouble at the Port,_CONTRACTS,US_14,Austria,truckDelivery,6\u00d7 Vehicles Spare Parts,1120,11100,\"Stop right there! The port's closed. We've run out of fuel, and our ship's engine is out of whack. I guess you could help us out if you need to cross. We do have a crane...\",CONTRACTS\nUS_14_01_FARM_SOYA_CONTRACT,\"Ready, Set... Sow!\",_CONTRACTS,US_14,Austria,truckDelivery,4\u00d7 Fertilizer,1100,10200,\"When it rains, it pours... we can't get a hold of our farmhands Tucker and Dale. I sure hope those two are okay, but we can't wait around until they show up. Could you give us a hand? We'll pay for your trouble.\",CONTRACTS\nUS_14_01_STARTING_WORK_CONTRACT,Off to a Bad Start,_CONTRACTS,US_14,Austria,truckDelivery,4\u00d7 Vehicles Spare Parts; 6\u00d7 Metal Roll,1340,9850,\"We weren't expecting so much rain. Our main shop got flooded. The equipment is starting to rust, and we need to get it up and running ASAP. We're on a deadline here! David should've checked the roof properly when I told him...\",CONTRACTS\nUS_14_01_HELP_FROM_THE_PAST_CONTRACT,An Old Friend,_CONTRACTS,US_14,Austria,truckDelivery,,810,7800,\"A little while ago I noticed an old, rusted-out tractor in the marsh. I think it could serve us well if we retrieved it and fixed it up. We can't do it without some additional equipment, but Elias happens to have the right trailer for the job.\",CONTRACTS\nUS_14_01_SOY_DELIVERY_CONTRACT,You Reap What You Sow,_CONTRACTS,US_14,Austria,cargoDelivery,3\u00d7 Soyabean; 3\u00d7 Soyabean,2580,31050,\"Our farms have been getting a lot of orders for soybeans, but the latest shipment of seeds didn't arrive on time 'cuz of all the rain. Transportation issues shouldn't stop our customers from getting their goods. Could you help us out?\",CONTRACTS\nUS_14_01_CASTLE_UNDER_LOCK_CONTRACT,The Inhospitable Castle,_CONTRACTS,US_14,Austria,cargoDelivery,6\u00d7 Stack; 4\u00d7 Potato; 2\u00d7 Wooden Planks; 2\u00d7 Logs Medium,2590,26000,\"Ugh, we thought we had everything all set up \u2014 the flags, the display cases, the tables and chairs \u2014 but we forgot the most important thing: entertainment and food! Good thing the crops are ready for harvesting, and there should be enough hay to go around. We'll pay you well if you help out.\",CONTRACTS\nUS_14_01_HAY_COLLECTION_CONTRACT,An Urgent Harvest,_CONTRACTS,US_14,Austria,cargoDelivery,8\u00d7 Stack,1520,15350,\"Hey there! I'm in a bit of a bind here. My son is in Vienna for a couple of weeks, and I'm worried our hay could rot by the time he gets back thanks to all this rain. Please help me till and harvest the field. I'm too old to do it alone.\",CONTRACTS\nUS_14_01_LOST_LARGE_CARGO_CONTRACT,Secrets of the Castle,_CONTRACTS,US_14,Austria,cargoDelivery,2\u00d7 Container Small Special,940,10450,\"The owner of the castle placed a top-secret order and asked us to keep an eye on it, but then the damn flood came, and everything just washed away. Please help me find the containers before he decides to order a guillotine...\",CONTRACTS\nUS_14_01_REPAIR_AREA_CONTRACT,Necessary Repairs,_CONTRACTS,US_14,Austria,cargoDelivery,3\u00d7 Barrels; 4\u00d7 Vehicles Spare Parts,970,8350,\"Harvest season is upon us, which means we need to stay on top of tractor repairs. But with all this rain, we just couldn't get ready in time. Could you help us out with the fuel and check on a shipment for us? The delivery is running late for some reason.\",CONTRACTS\nUS_14_01_FARM_POTATO_CONTRACT,Potato Field,_CONTRACTS,US_14,Austria,exploration,,860,9400,\"Hello there. The name's Willie. Joe, Albert, and I have been planting potatoes for decades now, but we're not getting any younger, and we could use some help. Would you mind doing some field work for us?\",CONTRACTS\nUS_14_01_SCOUT_FARM_CONTRACT,Taking Stock of the Damage,_CONTRACTS,US_14,Austria,exploration,,370,6350,\"I see you're all ready to go, but first things first \u2014 we need to see what we're working with. Could you check the farms in the area for us? We need to get a sense of the extent of the damage. We'll pay you after you hand in your report.\",CONTRACTS\nUS_14_01_FORTRESS_CENTURY_TASK,Central Walls,_TASKS,US_14,Austria,truckDelivery,4\u00d7 Bricks,760,6550,\"With so little time before the construction deadline, our drivers tend to forget that the weight capacity of a trailer is limited. Because of this, some trailers get stuck along the way. I guess we'll just have to tow them... you in?\",TASKS\nUS_14_01_PROBLEM_WITH_TURNING_TASK,'Round the Bend,_TASKS,US_14,Austria,truckDelivery,,670,6350,\"Jeez, these sharp turns are driving me nuts. I'm not getting behind the wheel again \u2014 that's where you come in. Could you please deliver this tank to the farm and then get the truck to the garage?\",TASKS\nUS_14_01_REPAIR_PROBLEM_TASK,Collateral Damage,_TASKS,US_14,Austria,truckDelivery,,550,6250,\"I dunno what we were thinking when we started these repairs... We just went in and did the job, but when it was time to leave... We got stuck. Well, we managed to get out, but the construction rig is still there. Maybe you'd be able to pull it out.\",TASKS\nUS_14_01_LET_THERE_LIGHT_TASK,Let There be Light!,_TASKS,US_14,Austria,truckDelivery,2\u00d7 Vehicles Spare Parts,580,5450,\"The lighthouse keeper insists that the light went out when some bearded man in a robe was trying to use his lighter. I think the bulb's just dead. We need a replacement from the trailer, and some tools too... Could you make the delivery?\",TASKS\nUS_14_01_ELEMENTAL_POWER_TASK,The Power of the Elements,_TASKS,US_14,Austria,truckDelivery,,400,4000,\"We just wanted to see the beautiful countryside, but the power of nature proved to be too much for us. We're okay, but our trailer has been washed away to God knows where. All our worldly possessions are in there... Please help us get it back, or this'll be the end of the road for us.\",TASKS\nUS_14_01_MAKE_HASTE_SLOWLY_TASK,Haste Makes Waste,_TASKS,US_14,Austria,truckDelivery,,370,3400,\"I should've known not to take that shortcut... Hey buddy, I see you've got a powerful ride there. Could you tow my baby for me? She's all alone out there, covered in mud...\",TASKS\nUS_14_01_SHARP_TURN_TASK,Sharp Turn,_TASKS,US_14,Austria,truckDelivery,,360,3300,\"I was driving home from Aunt Amelia's when I lost control and went off the road. I managed to get out, but my car's still lying at the bottom of the lake. Could you please get it out for me? Maybe it's still salvageable.\",TASKS\nUS_14_01_FUEL_PROBLEMS_TASK,Rock Bottom,_TASKS,US_14,Austria,truckDelivery,,330,3050,\"Gas-guzzling generators are a thing of the past. We need to think of the future. Write this down: we need a fuel tank brought from the gas station, and don't forget to tell them that Gilbert sent you! What was I talking about? Oh, right. We've hit rock bottom!\",TASKS\nUS_14_01_PRODUCTION_ERROR_TASK,Manufacturing Issue,_TASKS,US_14,Austria,cargoDelivery,2\u00d7 Wooden Planks Long,620,7150,\"This is the first time someone's returned our goods under warranty. We value our reputation, so we've decided to recall the whole batch of wood. I hope they still give us a five-star review. Could you deliver the batch for us?\",TASKS\nUS_14_01_WATER_TREATMENTS_TASK,Pool Party,_TASKS,US_14,Austria,cargoDelivery,4800\u00d7 Water,610,6400,\"Our pool is very popular with the tourists when it's hot out. The thing is, they cut off our water due to pipeline maintenance. We do need to fill the pool somehow. The only solution I see is getting it delivered in a tank. Please hurry before people start complaining!\",TASKS\nUS_14_01_UNPLANNED_UNLOADING_TASK,Unscheduled Unloading,_TASKS,US_14,Austria,cargoDelivery,2\u00d7 Cellulose,500,5100,This is a disaster! I can't possibly stop the presses and leave people without our local news! The driver shouldn't have taken that mountain route. Now our cellulose is scattered all over the place and is at risk of rotting. I need it brought here. Would you help me?,TASKS\nUS_14_01_DIRT_FROM_PAST_TASK,Digging Up Dirt,_TASKS,US_14,Austria,cargoDelivery,3\u00d7 Barrels Chemicals,570,4250,Everyone has secrets... even the ground. Our geologists have discovered a small chemical dump site in an old mine. That stuff isn't safe - it's poisoning the soil. Could you take it to the garage for now? We'll have it processed in a bit.,TASKS\nUS_14_01_LOSS_IN_RIVER_TASK,Crates in the River,_TASKS,US_14,Austria,cargoDelivery,3\u00d7 Vehicles Spare Parts,390,3450,\"Whatever possessed me to take my boat out in this weather? I'd never have dropped the crates if the water wasn't so choppy. Luckily the supplies don't weigh a lot, so the crates are just floating there. Hey, could you see if some of them have washed up ashore?\",TASKS\nUS_14_01_URGENT_PHOTOS_CONTEST,Emergency Photo Shoot,_CONTESTS,US_14,Austria,exploration,,70,1400,\"You're so efficient! Here's a little something for your trouble. You can leave the rest to me. I'll have the pictures processed, do a bit of color correction, and submit them to the magazine.\",CONTESTS\nUS_14_01_CITY_RACING_CONTEST,Street Race,_CONTESTS,US_14,Austria,exploration,,50,1000,\"Hey, buddy! We've got a prize pool set up for an upcoming race. Just keep in mind that half of the track is concrete, the other \u2014 dirt. You gotta work the throttle on this one, but don't overdo it! You wouldn't want to run yourself into a ditch wheels-up.\",CONTESTS\nUS_14_02_DELIVERY_LOG_01_CONTRACT,Reading Logs,_CONTRACTS,US_14,Austria,truckDelivery,2\u00d7 Logs Short,970,10500,\"Now that we're done literally putting out fires, it's time to tackle our orders. The first client needs us to take these logs to his lonely little house. He promises to pay well if we make it in time. Oh, and you're free to take our trailer to make things faster.\",CONTRACTS\nUS_14_02_BUILD_CASTLE_01_CONTRACT,A Castle Made of Sugar,_CONTRACTS,US_14,Austria,cargoDelivery,2\u00d7 Bricks; 1\u00d7 Metal Planks; 2\u00d7 Bags; 2\u00d7 Container Small,2220,21800,A tower on one of our historical landmarks collapsed because of the rain. We're requesting help delivering construction supplies. The mayor's office will reward you handsomely.,CONTRACTS\nUS_14_02_FORREST_BURNED_01_CONTRACT,\"Ashes to Ashes, Water to Soil\",_CONTRACTS,US_14,Austria,cargoDelivery,700\u00d7 Water; 800\u00d7 Water,960,8550,You helped us investigate the source of the fire earlier. Now we need to make sure all the fires have been put out. We've got a water delivery job for you. Interested?,CONTRACTS\nUS_14_02_TRAILER_01_TASK,Loan Etiquette,_TASKS,US_14,Austria,truckDelivery,,600,6650,\"The loggers borrowed our service trailer, and now we can't keep up with all the maintenance work at the port. And, as luck would have it, all our drivers are busy! Could you do us a solid and get that trailer back?\",TASKS\nUS_14_02_SCOUT_TRAILER_01_TASK,On Top of the World,_TASKS,US_14,Austria,truckDelivery,,460,4550,\"I just wanted to spend the weekend high in the mountains, but ended up stuck in the mud instead. I've spent all this time waiting for someone to help me get my trailer back home... oh! Maybe you could do the job?\",TASKS\nUS_14_02_TRUCK_01_TASK,Obstacle on the Road,_TASKS,US_14,Austria,truckDelivery,,380,3600,\"Hi there. Name's Frank. I'm from Ireland. I took a delivery job in the area but got a bit distracted while driving. Ended up damaging my engine. The plugs are fine, the distributor connector too. But the T-belt got jacked up. Could you give me a lift to the warehouse?\",TASKS\nUS_14_02_BUILD_BRIDGE_01_TASK,East Bridge,_TASKS,US_14,Austria,cargoDelivery,2\u00d7 Metal Planks; 2\u00d7 Concrete Slab,1480,11150,\"Well, this job requires extra speed and dedication. We need to have this bridge fixed, like, yesterday. Considering the urgency, I guess performing a barrel roll on a plane would be a more reasonable demand than this. But, uh... could you help?\",TASKS\nUS_14_02_DELIVERY_HELP_01_TASK,Open Season,_TASKS,US_14,Austria,cargoDelivery,1\u00d7 Vehicles Spare Parts; 1\u00d7 Crate Large,810,8200,\"I'm a hunter. A lot of my snares are in a sorry state, and I also need some parts for my car. We're not going anywhere until we get the supplies. I'll compensate you if you help me out.\",TASKS\nUS_14_02_ROCKS_01_TASK,Northern Landslide,_TASKS,US_14,Austria,cargoDelivery,1\u00d7 Blocks; 1\u00d7 Service Spare Parts,320,2450,\"Dammit, I've just about had it with this mountain! So many landslides... a simple cleanup won't cut it anymore\u2014we need to make the road safe. Could you deliver some concrete blocks and other supplies to the site?\",TASKS\nUS_14_02_FORREST_BURNED_02_CONTRACT,Fahrenheit 572,_CONTRACTS,US_14,Austria,cargoDelivery,10\u00d7 Burnt Logs,2310,22200,\"Half the work's done, but we still need to deal with the burned logs. You'll need a special vehicle for this part. It's a decent job with decent pay.\",CONTRACTS\nUS_14_02_DELIVERY_LOG_02_CONTRACT,\"Wild, Wild Horses\",_CONTRACTS,US_14,Austria,cargoDelivery,1\u00d7 Logs Long,730,9050,\"The orders are still rolling in. This time, the guys from the stables are offering us a hefty sum to help build some stalls for their shiny new mustangs. They're gonna expand their horseback-riding operation. This is a top-priority order!\",CONTRACTS\nUS_14_02_TRAILER_02_TASK,Champing at the Bit,_TASKS,US_14,Austria,truckDelivery,4\u00d7 Stack,1270,14500,\"Our hay reserves have started to rot because of the flood, and the driver who was supposed to get us a fresh batch got stuck halfway here. We need to act fast to keep our people fed and our animals healthy. Could you give us a hand?\",TASKS\nUS_14_02_SCOUT_TRAILER_02_TASK,Barrel Instability,_TASKS,US_14,Austria,truckDelivery,,340,2850,\"Our fuel delivery driver decided to take a shortcut through the mountains, but lost his way and got his tank stuck in a crevice. We had to do an emergency evac for the guy, but his tank is still up there. Could you tow it here for us?\",TASKS\nUS_14_02_TRUCK_02_TASK,\"24,000 Pounds under the Dirt\",_TASKS,US_14,Austria,truckDelivery,,310,2050,\"Greetings, traveler! I decided to take a shortcut across these mountains, but a pile of rocks and some puddles landed my vehicle in deep water. It's a truck, not a submarine, so now I'm left waiting for someone to rescue me.\",TASKS\nUS_14_02_DELIVERY_HELP_02_TASK,Firewood for the Hermit,_TASKS,US_14,Austria,cargoDelivery,1\u00d7 Logs Short; 1\u00d7 Crate Large,1040,11750,\"Electricity costs way too much around here, so I prefer to heat with firewood. I've got a pretty big stockpile for now, but it might not be enough to get me through the winter. Could you deliver a couple of logs to me?\",TASKS\nUS_14_02_ROCKS_02_TASK,Southern Landslide,_TASKS,US_14,Austria,cargoDelivery,1\u00d7 Service Spare Parts; 1\u00d7 Bags,530,5100,\"What a nightmare! This cliff's been here for hundreds of years. Now look at all the rubble and broken pitons all over the road... it's been 127 minutes since the emergency service was called, but those guys don't seem to be in a hurry to do their job. Could you help us out?\",TASKS\nUS_14_02_DELIVERY_LOG_03_CONTRACT,Two Peas in a Pod,_CONTRACTS,US_14,Austria,cargoDelivery,1\u00d7 Logs Medium; 1\u00d7 Logs Long,950,11500,\"Tourism is flourishing, which is always a good thing. It also means more work for us. We need to deliver some logs to the two new guesthouses. Chop-chop! The sooner you do it, the sooner they can start renting out the properties.\",CONTRACTS\nUS_14_02_TRUCK_03_TASK,The Quick and the Crashed,_TASKS,US_14,Austria,truckDelivery,,390,3450,\"Wooo, that was quite a race! I'm still out of breath, to be honest. Uh, could we keep the details of this rally between us? And I'd appreciate some help with my car. As you can see, you win some, you lose some...\",TASKS\nUS_14_02_TRUCK_04_TASK,Eating Dirt,_TASKS,US_14,Austria,truckDelivery,,430,4450,\"Who could've known the road right next to the fields would get ruined like this? It's never happened before, but here we go again! Sheesh... mind helping an old farmer out of the dirt? I'll make it worth your while, I promise!\",TASKS\nUS_14_02_SERVICE_HOUSE_CONTRACT,At Capacity,_CONTRACTS,US_14,Austria,truckDelivery,3\u00d7 Vehicles Spare Parts; 1\u00d7 Wooden Planks; 1\u00d7 Barrels,1620,18500,\"We need the service center upgraded as soon as possible. Since everyone is in such a rush to get ready for tourist season, things keep breaking, and it's the closest shop qualified to keep our equipment up and running. We're counting on you!\",CONTRACTS\nUS_14_02_FORREST_HELP_CONTRACT,Seasonal Work,_CONTRACTS,US_14,Austria,truckDelivery,2\u00d7 Forklift Caravan Container 2,2030,15750,There's work to be done! We need living quarters and some equipment for our loggers. Please deliver everything on the list. We promise you a handsome reward.,CONTRACTS\nUS_14_02_DELIVERY_POTATO_CONTRACT,It's in the Bag,_CONTRACTS,US_14,Austria,cargoDelivery,1\u00d7 Potato; 1\u00d7 Potato; 1\u00d7 Potato; 1\u00d7 Potato; 1\u00d7 Potato; 1\u00d7 Potato,2140,21550,\"With great power comes great responsibility. After harvesting a bumper crop like that, we need to get the bagged potatoes delivered to our customers. People need our potatoes, so everyone's counting on you.\",CONTRACTS\nUS_14_02_HELP_FARM_CONTRACT,Busy Beavers,_CONTRACTS,US_14,Austria,cargoDelivery,3\u00d7 Bags 2; 3\u00d7 Rubbish,1860,18450,\"The beavers' reaction to this bad weather was immediate. Don't get me wrong, they're pretty cool animals, but we've got enough on our plate as it is. Please help us deal with the flood damage on the farm. We'll pay you well.\",CONTRACTS\nUS_14_02_SOLAR_PANEL_CONTRACT,Solar Wind,_CONTRACTS,US_14,Austria,cargoDelivery,1\u00d7 Service Spare Parts; 1\u00d7 Service Spare Parts; 1\u00d7 Solar Panel; 1\u00d7 Solar Panel,1510,18050,\"As meteorologists say, get the most out of good weather while it's here. Considering the rising cost of electricity and the recent sunny spell, we've decided to install solar panels in two areas. Now we just need to get them there. Do you think you're up for the job?\",CONTRACTS\nUS_14_02_FARM_POTATO_CONTRACT,All Work and No Play,_CONTRACTS,US_14,Austria,cargoDelivery,2\u00d7 Fertilizer; 1\u00d7 Wooden Planks,1100,11500,\"See that field? Grab a tractor and plow it. Get that done, and I'll give you some seeds. Or don't\u2014but then you ain't gettin' jack. You sow the seeds, you get potatoes. Bring those to me, and I'll pay you. Now move it! I haven't got all day.\",CONTRACTS\nUS_14_02_FARM_SOYA_CONTRACT,Helping around the Farm,_CONTRACTS,US_14,Austria,cargoDelivery,2\u00d7 Fertilizer,980,9500,\"You've proven to be a deft hand at potato farming. Do you think you could do the same with soybeans? The soil here is very fertile, not like the stuff on Mars. I think you'll be fine. So what do you say?\",CONTRACTS\nUS_14_02_SCOUTING_EXPLORER_CONTRACT,Fire Safety,_CONTRACTS,US_14,Austria,exploration,,500,8550,Our company has been made aware of the additional fire hazards during the stormy season. We need you to inspect the area. Your efforts will be well compensated.,CONTRACTS\nUS_14_02_SCOUTING_CASTLES_CONTRACT,Historical Tour,_CONTRACTS,US_14,Austria,exploration,,470,8150,The authorities need us to check three ancient castles in the area to determine the best routes after all this bad weather. The job's simple and pays well. Are you up for it?,CONTRACTS\nUS_14_02_WATER_TANK_TASK,Dry Spell,_TASKS,US_14,Austria,cargoDelivery,600\u00d7 Water; 600\u00d7 Water; 600\u00d7 Water,1140,9150,\"This is a disaster! Due to the lack of rain, the water levels in the region are so low, we're on the verge of a drought! How are people supposed to farm like this? To make matters worse, the pipeline broke down too. We need water right away! Please, we'll make it worth your while!\",TASKS\nUS_14_02_CAMP_RUNNER_TASK,Shock Tourism,_TASKS,US_14,Austria,cargoDelivery,1\u00d7 Vehicles Spare Parts; 1\u00d7 Barrels; 1\u00d7 Crate Large,820,7000,\"Everyone needs support in these trying times, especially the unlucky tourists. Please deliver a relief package to their camps. You'll be compensated fairly.\",TASKS\nUS_14_02_SCOUT_RIVER_TASK,Water Obstacle,_TASKS,US_14,Austria,exploration,,270,4550,The local geologists are requesting a water-level survey of our crossings. We need you to visit all the sites. You'll be compensated.,TASKS\nUS_14_02_DELIVERY_CONTEST,The Beaver Thieves Strike Again,_CONTESTS,US_14,Austria,cargoDelivery,1\u00d7 Stack,280,2600,\"Hi! I'm Daggett. Me and my brother Norbert got tasked with returning a haystack those thieving beavers stole from the forester's house. So here's the plan: I call for help, Norb watches the stack, and you help with the delivery. Sound good?\",CONTESTS\nUS_14_02_RACING_CONTEST,Hot Wheels,_CONTESTS,US_14,Austria,exploration,,220,3900,\"Hey, buddy! We hold amateur races here. The last racer was in such a rush to win, he drove off the mountain right after he finished. He did set a record, though. You up for beating it? Safely, of course.\",CONTESTS\nUS_15_01_PLATFORM_CONSTRUCTION_01_CONTRACT,Real Man's Job\u2014Preparations,_CONTRACTS,US_15,Quebec,truckDelivery,,1120,15750,\"Hey there! Name's Jim. I'm in charge of building an oil rig here, and that's no mean feat. We could really use your help. To start off, could you check why in the world the crane trailer's not here yet?\",CONTRACTS\nUS_15_01_LIGHTHOUSE_SUPPLY_01_CONTRACT,Bring Back the Light,_CONTRACTS,US_15,Quebec,truckDelivery,,420,4300,\"Hey there. I'm Clint, the lighthouse keeper. Please help a guy out! Some important cargo got stuck in a gorge on its way to the lighthouse. It's not going anywhere unless someone gets it out, and I need it to keep the lighthouse running. Could you lend me a hand?\",CONTRACTS\nUS_15_01_BUILDING_BRIDGE_01_CONTRACT,The Fellowship of the Road,_CONTRACTS,US_15,Quebec,cargoDelivery,4\u00d7 Metal Planks; 6\u00d7 Bags,2610,20950,\"I'm the deputy head engineer and the construction crew chief on this site. You can't pass through here, so you'd better go back. But, if you're actually willing to help, please bring us some cements and metal beams. We can't do anything without them.\",CONTRACTS\nUS_15_01_PIPELINE_01_CONTRACT,Steady Flow,_CONTRACTS,US_15,Quebec,cargoDelivery,1\u00d7 Pipe Water,750,5650,Hey buddy! We need your assistance ASAP. There's a damaged pipe in our area that needs replacing. Any chance you'd be willing to make the trip and get us a new pipe? We'd really appreciate it.,CONTRACTS\nUS_15_01_SCOUTING_SHORE_CONTRACT_01,Coast Guard,_CONTRACTS,US_15,Quebec,exploration,,300,5250,\"Welcome to Harbor! Our company works with the Canadian Coast Guard, and they've requested an initial inspection of the coastline. We need an expeditor who knows what they're doing. All expenses are gonna be covered. You up for the job?\",CONTRACTS\nUS_15_01_TRAILER_DELIVERY_01_TASK,Adventuring Gear,_TASKS,US_15,Quebec,truckDelivery,,530,5750,\"Hello! We had a trailer stocked with mountaineering equipment ferried over to the port, but it still needs to be delivered to our center. Which is not something we can do on our own, so that's where you come in. Help us out, eh?\",TASKS\nUS_15_01_TRACK_DELIVERY_01_TASK,Dangerous Route,_TASKS,US_15,Quebec,truckDelivery,,490,5100,\"You're not gonna believe what just happened! I decided to take a shortcut across this lake, because I was so sure the ice could take the weight of my car\u2014but it started cracking! Now I'm stuck, and I really need to get out before my ride goes under. Help me out, eh?\",TASKS\nUS_15_01_HOUSE_BUILDING_01_TASK,Housing in the Boonies,_TASKS,US_15,Quebec,cargoDelivery,4\u00d7 Wooden Planks Medium,970,13300,\"Hey, did you notice our town's development efforts yet? We're building new houses for people, and we could use your help to deliver some planks. All our contractors refuse because of weather conditions. Give us a hand, eh?\",TASKS\nUS_15_01_CARGO_PORT_01_TASK,Just a Delivery,_TASKS,US_15,Quebec,cargoDelivery,2\u00d7 Metal Planks; 4\u00d7 Metal Roll,1500,13000,\"Let's cut to the chase\u2014we need two metal beams and four metal rolls delivered to the port. We'll pay you by the click. The job's nothing special, but it still needs done. You up for it?\",TASKS\nUS_15_01_PIPES_01_TASK,Pipeline of the Future,_TASKS,US_15,Quebec,cargoDelivery,2\u00d7 Pipe Large,1560,11650,\"Whadda y'at, fella? Want a cut of that oil money? Well, all you have to do is deliver some large pipes for the oil line to our guys so that we can amp up extraction. It'll be worth your while, promise!\",TASKS\nUS_15_01_ROAD_BLOCK_01_TASK,Eastern Rockslide,_TASKS,US_15,Quebec,cargoDelivery,1\u00d7 Wooden Planks; 1\u00d7 Crate Large,840,10200,We need some consumables and wooden planks to clear a rockslide in the eastern part of the area. The only way around it lies through the Forbidden Plateau. Be careful - they say whoever sets foot there will be cursed by evil spirits.,TASKS\nUS_15_01_LOST_CARGO_01_TASK,Toxic Trail,_TASKS,US_15,Quebec,cargoDelivery,3\u00d7 Barrels Chemicals,660,8100,It's a disaster! The incident with that container carrier can lead to regrettable pollution of the nearshore zone. The vessel was transporting some special chemicals. We need to search the coastline and retrieve the cargo ASAP!,TASKS\nUS_15_01_PANTOON_BRIDGE_01_TASK,The Other Side,_TASKS,US_15,Quebec,cargoDelivery,2\u00d7 Metal Roll; 2\u00d7 Wooden Planks Medium,740,6050,\"Hey there, buddy! We've got a problem. The bridge isn't ready yet, but people still need to get to the other side somehow. A temporary crossing would do the trick\u2014just need the supplies to build it. Could you get them to us if you have the time?\",TASKS\nUS_15_01_CARGO_DELIVERY_01_TASK,Running on Empty,_TASKS,US_15,Quebec,cargoDelivery,3\u00d7 Barrels,590,5850,\"Hello! We're almost out of fuel, so I need you to get me a couple of barrels to keep our clients well-supplied. It's not something we can handle on our own, unfortunately. Will you take the job?\",TASKS\nUS_15_01_CONTEST_01,Don't Look Up,_CONTESTS,US_15,Quebec,exploration,,150,2700,\"Hello! At this point, it's almost a tradition for our lab techs to race to Everton to alert the authorities about yet another Earth-bound comet discovered by Dr. Randall. Oh, there's another one! Would you like to join the race?\",CONTESTS\nUS_15_01_PLATFORM_CONSTRUCTION_02_CONTRACT,Real Man's Job\u2014Breaking Through,_CONTRACTS,US_15,Quebec,cargoDelivery,2\u00d7 Container Large Drilling; 1\u00d7 Big Drills,2410,29900,\"The next step is moving the drill unit and some other equipment from the old quarry. Fair warning: roads are covered in ice, and a little birdie told me there are tons of cameras around. Don't get a speeding ticket.\",CONTRACTS\nUS_15_01_BUILDING_BRIDGE_02_CONTRACT,The Woes of a Construction Worker,_CONTRACTS,US_15,Quebec,cargoDelivery,2\u00d7 Container Large; 4\u00d7 Blocks; 2\u00d7 Metal Planks,2540,21950,\"Hey there, traveler! We urgently need some construction materials to finish this bridge. Could you get them for us? I hear you've got the skills, and I'm sure things will go a lot faster with your help.\",CONTRACTS\nUS_15_01_SHIPBOARD_SUPPLY_02_CONTRACT,Titanic Effort,_CONTRACTS,US_15,Quebec,cargoDelivery,8\u00d7 Service Spare Parts,1170,13450,\"Hey, friend! We need your help at the docks. Our boss, Mr. Cameron, told us to hurry up and fix a ship in the dockyard, but we're all out of spare parts. Could you give us a hand and deliver a batch of parts from the docks?\",CONTRACTS\nUS_15_01_LIGHTHOUSE_SUPPLY_02_CONTRACT,Shining in the Storm,_CONTRACTS,US_15,Quebec,cargoDelivery,8\u00d7 Service Spare Parts,780,8450,\"Hello! I'm back with another request now that the lighthouse has been patched up. I had some parts delivered to the port, and I'd appreciate it if you could get them for me. The lighthouse restoration efforts are still a work in progress.\",CONTRACTS\nUS_15_01_SCOUTING_SHORE_CONTRACT_02,St. Elmo's Fire,_CONTRACTS,US_15,Quebec,cargoDelivery,1\u00d7 Crate Large; 1\u00d7 Crate Large; 1\u00d7 Crate Large,870,7900,\"We've received an emergency call from John Cabot, the owner of one of the trapped sailing vessels. He says the ice shifted, damaged the hulls of all three boats, so they're in urgent need of some parts. Can you handle the delivery?\",CONTRACTS\nUS_15_01_PIPELINE_02_CONTRACT,Endless Flow,_CONTRACTS,US_15,Quebec,cargoDelivery,1\u00d7 Pipe Water,890,7100,\"Hey bud! We're in big trouble. One of the pipelines is badly damaged, to the point that it could leave the locals without any water unless we replace the leaky part. I've been told you're a great driver. Could you handle the delivery before it's too late?\",CONTRACTS\nUS_15_01_TRAILER_DELIVERY_02_TASK,Easy but Greasy,_TASKS,US_15,Quebec,truckDelivery,,560,6400,\"Lots of machinery at the new plant needs fuel to operate uninterrupted. It's our job to provide it, and it's yours to safely get it to the plant. Mind the greasy roads, eh? And give it your all!\",TASKS\nUS_15_01_ROAD_BLOCK_02_TASK,Northern Rockslide,_TASKS,US_15,Quebec,cargoDelivery,2\u00d7 Wooden Planks Medium,390,4550,\"To get rid of this rockslide, the workers are going to need two pallets of planks from the sawmill. You can get around the roadblock through the gorge down below, but watch out\u2014there have been a few Sasquatch sightings in that area. Maybe the rockslide was his doing?\",TASKS\nUS_15_01_CARGO_DELIVERY_02_TASK,Snowbird,_TASKS,US_15,Quebec,cargoDelivery,2\u00d7 Bags,490,4250,\"Hey there, traveler! I used to spend my winters down south, but this year, the urgent foundation repairs have kept me here. And I'm 65, I'll have you know. Hey, since you drive, could you get me some cement, eh?\",TASKS\nUS_15_01_CONTEST_02,Worth Its Weight in Gold,_CONTESTS,US_15,Quebec,cargoDelivery,1\u00d7 Silver; 1\u00d7 Gold,900,8000,\"I've got a job for you, and let me tell you upfront\u2014it's not gonna be easy. A cargo ship got into an accident nearby. We gotta to act fast, before the haul sinks to the bottom of the sea. The government requested the goods be recovered ASAP.\",CONTESTS\nUS_15_01_BUILDING_BRIDGE_03_CONTRACT,The Return of the Inspector,_CONTRACTS,US_15,Quebec,cargoDelivery,3\u00d7 Barrels; 2\u00d7 Metal Planks,1550,14950,\"The authorities have already dispatched someone to inspect our bridge and make sure it's up to code, as all key infrastructure should be. By the time he gets here, our bridge needs to be at 100%, so we don't have any time to lose! We need supplies.\",CONTRACTS\nUS_15_01_PLATFORM_CONSTRUCTION_03_CONTRACT,Real Man's Job\u2014Casting,_CONTRACTS,US_15,Quebec,cargoDelivery,4\u00d7 Container Small,1430,14400,\"Okay, we've got one last but very important thing to take care off\u2014get an offshore mooring anchor made. The new metalworks will need some special containers to complete that order, and that's where you come in. Hurry up, okay?\",CONTRACTS\nUS_15_01_ROAD_BLOCK_03_TASK,Southern Rockslide,_TASKS,US_15,Quebec,cargoDelivery,2\u00d7 Service Spare Parts,740,9650,\"Well, looks like we've got another rockslide. The team will need two crates of spare parts to clear the way. You can get there if you go straight over the mountain, but they say there are wolves in the area. Try not to lay on the horn if you don't want them to rip off your license plate.\",TASKS\nUS_15_01_PLATFORM_CONSTRUCTION_04_CONTRACT,Real Man's Job\u2014Deliverance,_CONTRACTS,US_15,Quebec,truckDelivery,,870,11350,\"We're in the home stretch now. That giant anchor needs to be moved to the port for rigging to the platform. It's a tricky job, so it'll take a hot minute. Be extra careful with this one, all right? We're counting on you.\",CONTRACTS\nUS_15_01_CABLE_TRAILER_CONTRACT,Flexible Solution,_CONTRACTS,US_15,Quebec,truckDelivery,,900,11850,\"The oil rig needs a special flexible drillstem to run, and they have it ready at the new plant. We're also short on fuel for our tower. Could you get it for us?\",CONTRACTS\nUS_15_01_CONSTRUCTION_FACTORY_CONTRACT,Plan Two-Point-Four,_CONTRACTS,US_15,Quebec,cargoDelivery,4\u00d7 Bricks; 2\u00d7 Concrete Slab,1650,15850,\"We still need two shipments of concrete slabs and four shipments of brick to finish our metalworks, according to the budget. But, once that's done, it'll be time to kick back and relax! Could you grab us something to drink while you're at it?\",CONTRACTS\nUS_15_01_ROAD_ISLAND_CONTRACT,Double-Double,_CONTRACTS,US_15,Quebec,cargoDelivery,2\u00d7 Wooden Planks Medium; 2\u00d7 Metal Roll,1250,14500,\"The only way to reach Tidal right now is via ice crossing, but it's just not safe. A pontoon would work much better. We're going to need two metal rolls and two bundles of long, wood beams.\",CONTRACTS\nUS_15_01_CONSTRUCTION_CLIMBERS_CONTRACT,The More the Merrier,_CONTRACTS,US_15,Quebec,cargoDelivery,3\u00d7 Barrels Oil; 2\u00d7 Vehicles Spare Parts,1050,7400,\"Welcome to our base! This place is gaining popularity by the day, which means we're going to need more fuel and supplies. But, as they say, the more the merrier, eh?\",CONTRACTS\nUS_15_01_RESTORE_ROUTE_CONTRACT,Reimagining the Ice Trail,_CONTRACTS,US_15,Quebec,cargoDelivery,2\u00d7 Wooden Planks Medium; 2\u00d7 Wooden Planks Medium,560,5200,\"Hello there! We have a major issue on our hands\u2014an avalanche damaged the bridges next to the mountaineering center, cutting it off from the rest of the world. We need to restore that transport link ASAP if we want any chance to get supplies and save the people out there. Help us out, eh?\",CONTRACTS\nUS_15_01_METEO_DELIVERY_TASK,Sky Watch,_TASKS,US_15,Quebec,cargoDelivery,2\u00d7 Bags; 2\u00d7 Service Spare Parts,1030,9750,\"This weather station is in dire need of maintenance. The equipment is outdated, and some things really need to be replaced. We need supplies to get the station up and running again. Could you handle the delivery?\",TASKS\nUS_15_01_SHIPYARD_SUPPLY_TASK,Oil Guys in a Bind,_TASKS,US_15,Quebec,cargoDelivery,8\u00d7 Barrels,1050,8850,\"After carefully considering the fuel consumption rate, we've come to the conclusion that the oil platform is missing eight more fuel barrel pallets. Therefore, taking into account the interests of oil workers, we need you to deliver those barrels to us.\",TASKS\nUS_15_01_CRASHED_CARGO_TASK,The Ice Road,_TASKS,US_15,Quebec,cargoDelivery,2\u00d7 Container Small Special,710,6950,\"What a disaster! Luckily, they've already evacuated the people, but our special delivery's still there. We have two undamaged containers by that shipwreck, and we need them delivered to the port despite any thin ice you might encounter. Will you take the job?\",TASKS\nUS_15_01_TRASH_TASK,Rough Day,_TASKS,US_15,Quebec,cargoDelivery,2\u00d7 Rubbish,630,5600,\"The event was a lot of fun! The clean-up, not so much. We've collected the garbage, but disposing of it without special tech doesn't seem feasible. Could you drop the containers off at the port if you've got a minute? We'd be much obliged.\",TASKS\nUS_15_02_HIGHWAY_01_CONTRACT,The Expected Journey,_CONTRACTS,US_15,Quebec,truckDelivery,2\u00d7 Metal Planks; 3\u00d7 Concrete Slab,2360,19900,\"So, we're waiting for some supplies we need to build a bridge here. You'll be paid on delivery for transportation, as well as for your driving service. Present company shall not be liable for the damages sustained by the cargo during transport. Are you up for the job?\",CONTRACTS\nUS_15_02_HELP_TRUCK_01_TASK,Hammered Road,_TASKS,US_15,Quebec,truckDelivery,,440,4300,\"Hey, bud! I wanted to capture the beauty of nature, so I took a canvas, an easel, and some paints\u2014set out to find the best spot. Thing is, the road was so hammered that I lost control of my car, and now it's stuck in the middle of nowhere. Could you tow it back to my house?\",TASKS\nUS_15_02_BLOCKAGE_01_TASK,Western Rockslide,_TASKS,US_15,Quebec,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Service Spare Parts,970,10000,\"We're in a bit of bind here. A rockslide has blocked the main road to the ferry station. We need your help getting some metal beams there to clear the way. Don't worry, we'll pay you for the trouble!\",TASKS\nUS_15_02_HIGHWAY_02_CONTRACT,Construction Can't Wait,_CONTRACTS,US_15,Quebec,cargoDelivery,2\u00d7 Container Small; 3\u00d7 Vehicles Spare Parts; 2\u00d7 Wooden Planks Medium; 3\u00d7 Bags,1650,13000,\"Hey. We're on a tight deadline here, and we need a buttload of supplies to finish the bridge in time. Tricky part is getting to the construction site, but we hear you're an expert. Think you could handle this?\",CONTRACTS\nUS_15_02_HELP_TRUCK_02_TASK,Several Clicks Over,_TASKS,US_15,Quebec,truckDelivery,,380,3650,\"Hey there, traveler! Could you help me tow my truck? It's stuck, and it refuses to start in these freezing temperatures. My destination is a few miles away\u2014or, well, the number's a bit higher if you're counting in clicks. So, give me a lift, eh?\",TASKS\nUS_15_02_BLOCKAGE_02_TASK,Eastern Rockslide,_TASKS,US_15,Quebec,cargoDelivery,1\u00d7 Vehicles Spare Parts; 2\u00d7 Crate Large,1040,11200,\"Greetings from the local timbermen! As you might've noticed, a pile of rubble is blocking the logging camp's access road. Our vehicles can't get anywhere like this! We'll need some consumables to clear the way. Could you help with that?\",TASKS\nUS_15_02_HELP_TRUCK_03_TASK,\"Snow, Trailer and Two Wood Planks\",_TASKS,US_15,Quebec,truckDelivery,1\u00d7 Wooden Planks; 1\u00d7 Wooden Planks Medium,740,6200,\"Hey there, bud! I've got a trailer stuck not far from here, and it has the tools I need to do some home repairs. We're also short a couple good planks for interior work. Do you think you could deliver the supplies to us?\",TASKS\nUS_15_02_HELP_TRUCK_04_TASK,Neighbors from Hell,_TASKS,US_15,Quebec,truckDelivery,,560,5950,\"Hey there! My buddy borrowed my trailer a while ago, but now his car's in the shop, and he can't bring my stuff back. He's willing to pay for delivery, though. One good deed deserves another, eh. I really need that trailer. Do you think you could fetch it for me? My bud's house isn't far.\",TASKS\nUS_15_02_WINTER_NEED_CONTRACT,Beauty Has Its Price,_CONTRACTS,US_15,Quebec,truckDelivery,2\u00d7 Logs Short; 4\u00d7 Vehicles Spare Parts,1840,21350,\"This season, our company has opened a new scenic overlook by the lake. It's a great tourist attraction, but we need to get ready for the influx. According to our estimates, we'll need more logs, spare parts, and fuel. Can you get all of that to us?\",CONTRACTS\nUS_15_02_UNDER_ICE_CONTRACT,Driving on Ice,_CONTRACTS,US_15,Quebec,truckDelivery,1\u00d7 Vehicles Spare Parts; 1\u00d7 Vehicles Spare Parts; 1\u00d7 Vehicles Spare Parts,1860,20600,\"According to the list of participants, three of the trailers are yet to arrive. Luckily, we were able to get in contact with their owners\u2014turns out they need a bit of help getting here. Could you handle that for us?\",CONTRACTS\nUS_15_02_TUNNEL_CONTRACT,Rocky Emergency,_CONTRACTS,US_15,Quebec,truckDelivery,1\u00d7 Container Large Drilling,1460,14200,This is dispatch! We've had the outer rock layer collapse in the northern tunnel. No casualties. We need a drilling rig and a repair van to resume work. Do you copy?,CONTRACTS\nUS_15_02_SUPPLY_PARK_CONTRACT,Across the Lake,_CONTRACTS,US_15,Quebec,cargoDelivery,4\u00d7 Wooden Planks; 2\u00d7 Logs Short; 2\u00d7 Barrels Oil; 2\u00d7 Barrels Oil,1900,21252,\"Our company has been hired to build several rentals, and the deadline is coming up. Most of them are ready, but we still need to finish two houses. Missing the deadline is not an option, because they've been booked in advance. Help us out, eh?\",CONTRACTS\nUS_15_02_QUARRY_DELIVERY_CONTRACT,Wormhole,_CONTRACTS,US_15,Quebec,cargoDelivery,2\u00d7 Service Spare Parts Special; 2\u00d7 Container Large,1790,15750,\"The mining operation is in full swing, but the deeper we drill, the more spare parts we go through. We keep finding all sorts of holes in the bedrock. We'd appreciate it if you could get us some new supplies before the ones we have run out.\",CONTRACTS\nUS_15_02_FISH_FESTIVAL_CONTRACT,Fishy Business,_CONTRACTS,US_15,Quebec,cargoDelivery,2\u00d7 Wooden Planks; 2\u00d7 Logs Medium,1260,14400,\"Hey, buddy! We're absolutely swamped. There's so much work to be done with the Fishing Fest coming up. We need more planks and logs, like, yesterday, if we don't want our guests to sit on the ground. Help us out, eh? The festival's at stake here!\",CONTRACTS\nUS_15_02_SUPPLY_PARAGLIDING_CONTRACT,Follow the Wind,_CONTRACTS,US_15,Quebec,cargoDelivery,3\u00d7 Vehicles Spare Parts; 5\u00d7 Barrels Oil,1300,13250,\"Hey there! I'm a paraglider and I love exploring the vast, open skies. But even I have to land sometimes, especially when I run out of fuel. Could you do me a solid\u2014get me some fuel and spare parts? I'll make it worth your while!\",CONTRACTS\nUS_15_02_SUPPLY_LOG_CONTRACT,A Timberman's Work is Never Done,_CONTRACTS,US_15,Quebec,cargoDelivery,2\u00d7 Service Spare Parts; 2\u00d7 Vehicles Spare Parts,1010,12050,Our camp has been working overtime lately. Everyone needs wood. That's why our equipment has been wearing out at a faster rate. We're in desperate need of spare parts. We need to keep this operation running!,CONTRACTS\nUS_15_02_PIPE_CASES_CONTRACT,The Lost Pipe,_CONTRACTS,US_15,Quebec,cargoDelivery,2\u00d7 Pipe Large,980,9500,\"We've got a non-delivery of two giant pipes, and they're yet to be found. The pipes weren't secured properly during transport, so they fell off somewhere by the dam. All you need to do is follow the river and bring our cargo back.\",CONTRACTS\nUS_15_02_WASTE_RECYCLING_CONTRACT,Better Safe Than Sorry,_CONTRACTS,US_15,Quebec,cargoDelivery,2\u00d7 Rubbish,840,7200,\"Mining always generates waste that needs to be properly disposed of, so we need to send all this trash for recycling before they slap us with a fine. Take it to the warehouse at the local port for us, eh?\",CONTRACTS\nUS_15_02_PIPELINE_CONTRACT,On the Other Side of a Pipeline,_CONTRACTS,US_15,Quebec,cargoDelivery,1\u00d7 Pipe Water,370,4850,\"You never know what you're going to find in an old water main. There's some sort of mushroom growing on the sides, and we've found a live turtle in one of the branches! It's high time to replace that pipeline. Mind delivering a pipe for us?\",CONTRACTS\nUS_15_02_BRICKS_TASK,Bricks for the Warehouse,_TASKS,US_15,Quebec,truckDelivery,2\u00d7 Bricks,960,9900,\"The plant's been shut down, and we're scrapping it for construction materials. We've got two pallets of bricks that need to be transported to the town warehouse. Feel free to use our plant's trailer\u2014though you'll have to get it from a worker's house by the river.\",TASKS\nUS_15_02_OLD_RESOURCE_TASK,In Search of Old Resources,_TASKS,US_15,Quebec,truckDelivery,,620,7050,\"Hello there! The paragliders have requested a prototype exploration unit be delivered to the northern tower to ensure their safety. It's going to improve cell network coverage, which is crucial in emergencies. Would you mind helping us with this life-saving job?\",TASKS\nUS_15_02_STUCK_ICE_TASK,Ice Fishing,_TASKS,US_15,Quebec,truckDelivery,,500,4050,\"Hey! Some help here? I drove my truck out onto the ice to do some fishing, but the weight proved to be a bit too much, so now it's stuck there. Could you tow it to the trailer park for me? I'm good for it, promise.\",TASKS\nUS_15_02_SUPPLY_HELP_TASK,Government Support,_TASKS,US_15,Quebec,cargoDelivery,1\u00d7 Crate Large; 1\u00d7 Crate Large; 1\u00d7 Crate Large,1220,12850,\"Due to recent weather conditions, our town council has issued essential supply kits to help keep people warm. The problem is, we don't have enough council-owned vehicles to distribute those kits. Our townspeople need your help. Will you take the job?\",TASKS\nUS_15_02_CHEMISTRY_TASK,Chemical Waste Disposal,_TASKS,US_15,Quebec,cargoDelivery,3\u00d7 Barrels Chemicals,900,8600,Hey there! We've got a bunch of chem waste barrels lying around at the old plant. They need to be moved to the port for processing before we're fined for endangering the environment. Care to help us with some recycling?,TASKS\nUS_15_02_CONTEST_MOUNTAIN,Mountain Trail,_CONTESTS,US_15,Quebec,exploration,,120,2400,\"Hey, bud! We've been riding this track on our snowmobiles, and I bet you won't be able to outrace us. It's a difficult course with a snowy ravine up ahead. Wanna take the dare and show us what you've got? There's money riding on this!\",CONTESTS\nUS_15_02_CONTEST_RACING,Keep Your Cool on Thin Ice,_CONTESTS,US_15,Quebec,exploration,,100,1500,\"Hey, anyone else want to join our ice race? The track isn't an easy one, so any slip-up could cost you a trip to the repair shop, once you've fished your car out of the ice, that is. But what's a little property damage for a chance to win the prize money, eh? Sing up today!\",CONTESTS\nUS_16_01_BUILD_BRIDGE_01_CONTRACT,High Way,_CONTRACTS,US_16,Washington,cargoDelivery,3\u00d7 Concrete Slab; 1\u00d7 Container Large,3030,32000,\"We need to build a bridge traversing the mountains. I know it's a big ask, but our logistical needs cannot be satisfied without it. It goes without saying that the cliffs will only make matters worse, so we have some meticulous planning ahead of us. Here's hoping you can help us out.\",CONTRACTS\nUS_16_01_SCOUT_TRAILER_01_TASK,Outpost in the Woods,_TASKS,US_16,Washington,truckDelivery,,1040,12600,\"Hey there! I had some provisions delivered, but couldn't get them all the way to the watchtower by myself. The road isn't the best, to put it mildly. Could you help me tow the supply trailer to the tower? These supplies will make my job so much more bearable.\",TASKS\nUS_16_01_DELIVERY_TRUCK_01_TASK,Deliver at Any Cost,_TASKS,US_16,Washington,truckDelivery,,910,10300,\"Hey there! We've run into a bit of trouble. One of our drivers took a wrong turn trying to get around a collapsed bridge and ended up in a swamp. The truck wasn't damaged, but it's stuck there. We need it back in the city ASAP. Can you help?\",TASKS\nUS_16_01_DELIVERY_SCOUT_01_TASK,The Way Home,_TASKS,US_16,Washington,truckDelivery,,860,9400,\"Hello! I'm kind of in a bind here: I lost control of my car, it turned over, and now I can't make it home. The car needs to be towed. Can I count on your help?\",TASKS\nUS_16_01_DELIVERY_TRAILER_01_TASK,Mystery Cargo,_TASKS,US_16,Washington,truckDelivery,,440,4600,\"Hey! We need you to get a trailer from the mine to the port for us. What's inside? That's for us to know and for you to find out. We need the trailer safe and sound, so be careful on the way. Just get it to the port, and everything will be A-okay!\",TASKS\nUS_16_01_ROCKS_01_TASK,Eastern Landslide,_TASKS,US_16,Washington,cargoDelivery,2\u00d7 Metal Planks,1130,8550,Our residents have been practically cut off from the rest of the world by this landslide. You'd really make our work easier if you could help us out.,TASKS\nUS_16_01_WOODEN_BRIDGE_01_TASK,The Bridge,_TASKS,US_16,Washington,cargoDelivery,2\u00d7 Wooden Planks,470,6000,\"Greetings! My name's Garrett and I work at the local quarry. We've run into an issue\u2014with the old bridge damaged, logistics have been compromised. That bridge was the only connecting point. We'd appreciate it if you could build us a new one!\",TASKS\nUS_16_01_DELIVERY_TRUCK_02_TASK,Backing Out of a Dead End,_TASKS,US_16,Washington,truckDelivery,,670,7950,\"You know, sometimes you just make poor calls. Take me: I decided to take a shortcut through here and ended up stuck. And now I need help to get my car out and take it to the mine. The clock is ticking!\",TASKS\nUS_16_01_DELIVERY_TRAILER_02_TASK,Heavy Trailer Makes Light Work,_TASKS,US_16,Washington,truckDelivery,,510,5550,\"Hey, so I got myself into a spot of trouble. My vehicle refused to cooperate on the way down, so I had to ditch the trailer. Do you think you can retrieve it and get it to the container warehouse?\",TASKS\nUS_16_01_DELIVERY_SCOUT_02_TASK,Off-Roading Gone Wrong,_TASKS,US_16,Washington,truckDelivery,,390,3450,\"Ah, I messed up. As you can see, I ran my car into a swamp. The wheels got stuck, and now it won't budge. I gotta admit, I can't get out of this on my own. Could you help me tow my car? My house isn't far.\",TASKS\nUS_16_01_ROCKS_02_TASK,Western Landslide,_TASKS,US_16,Washington,cargoDelivery,2\u00d7 Bags; 2\u00d7 Crate Large,1060,8550,The main traffic artery of our region has been blocked! Please help us clear the obstruction and save the isolated town.,TASKS\nUS_16_01_TUNNEL_RESTORATION_CONTRACT,Underground Path,_CONTRACTS,US_16,Washington,truckDelivery,4\u00d7 Bags; 3\u00d7 Concrete Slab,4910,56400,\"One of our most important routes has been jeopardized by an accident in the tunnel. The whole system could end up paralyzed unless we deal with this, and fast. We need your help to restore everything on time and prevent a full-blown disaster.\",CONTRACTS\nUS_16_01_FACTORY_DELIVERY_CONTRACT,Production Frontlines,_CONTRACTS,US_16,Washington,truckDelivery,2\u00d7 Container Small,1960,22450,\"The plant is this close to shutting down because we haven't received the necessary equipment. We need it here yesterday, or all the lines will have to be deactivated. Could you lend us a hand with this emergency?\",CONTRACTS\nUS_16_01_RECOVERY_GATE_CONTRACT,The Only Way,_CONTRACTS,US_16,Washington,truckDelivery,3\u00d7 Bags; 2\u00d7 Bags 2,1980,19600,\"Hey there! We have a situation and no way to deal with it on our own. We need to clear the way for our construction equipment\u2014deliver a couple of bags of cement, some sand, and an equipment trailer.\",CONTRACTS\nUS_16_01_LOST_DELIVERY_CONTRACT,Drowned Order,_CONTRACTS,US_16,Washington,truckDelivery,,1210,12600,\"Hey there! We've run into an issue with a supply run. The trailer got stuck in a river crossing, and the car itself couldn't make it out either. We need the supplies delivered to the mine, stat, to avoid significant revenue loss. Could you help?\",CONTRACTS\nUS_16_01_WOODEN_BATCH_CONTRACT,Delivery from the Woods,_CONTRACTS,US_16,Washington,cargoDelivery,1\u00d7 Logs Medium; 2\u00d7 Logs Short,3200,47550,\"Hello! We've prepared a shipment of wood to be delivered to the port, but unfortunately, we have neither the equipment nor the manpower for the job. Could you help us deliver the goods? We'd be much obliged!\",CONTRACTS\nUS_16_01_HELP_DELIVERY_CONTRACT,Express Delivery,_CONTRACTS,US_16,Washington,cargoDelivery,2\u00d7 Service Spare Parts; 1100\u00d7 Water; 3\u00d7 Crate Large,2260,25100,\"Time is money, and we're in a bind right now. We've got several important shipments waiting to be delivered. Delays can result in major loss of revenue here. We need an experienced driver we can rely on. You in?\",CONTRACTS\nUS_16_01_CITY_DELIVERY_CONTRACT,Making Inroads,_CONTRACTS,US_16,Washington,cargoDelivery,2\u00d7 Bags 2; 2\u00d7 Bricks; 2\u00d7 Wooden Planks; 1\u00d7 Pipes Small,2070,23450,\"Hello! We have several important delivery contracts with various towns, but none of our drivers are currently available. You have a reputation as a dependable contractor, so we're willing to hire you for these jobs.\",CONTRACTS\nUS_16_01_WAREHOUSE_RUIN_CONTRACT,All That Remains,_CONTRACTS,US_16,Washington,cargoDelivery,2\u00d7 Junk Open; 2\u00d7 Junk Closed,2280,23100,\"We own a burned-down warehouse that has to be demolished. All the rubble and scrap metal needs to be carted away to a recycling plant. It's a rough job, but we'll make it worth your while if you can do it.\",CONTRACTS\nUS_16_01_BUILD_HANGAR_CONTRACT,Steel Cover,_CONTRACTS,US_16,Washington,cargoDelivery,3\u00d7 Metal Roll; 4\u00d7 Wooden Planks,1640,19600,\"Hey there! We're getting ready for a big construction project, and delivering supplies for the hangars is part of our contract. We really hope you can help us out. We're on a deadline, so this is urgent!\",CONTRACTS\nUS_16_01_LIGHTHOUSE_CONTRACT,Seeking the Light,_CONTRACTS,US_16,Washington,cargoDelivery,2\u00d7 Service Spare Parts; 2\u00d7 Logs Short,1460,19450,\"We have an emergency. The Lighthouse is out of commission, and the ships are going in blind right now. We've been tasked with delivering repair supplies, but the deadline is tight. We need someone to deliver all the materials, fast.\",CONTRACTS\nUS_16_01_BUILD_HOUSE_CONTRACT,Forester's Den,_CONTRACTS,US_16,Washington,cargoDelivery,1\u00d7 Logs Medium; 2\u00d7 Wooden Planks; 1\u00d7 Container Small,1530,19100,Hey there! We need some help building a new forester's hut. This is a crucial project meant to improve our foresters' working conditions. Could you deliver the supplies for us and help with the construction?,CONTRACTS\nUS_16_01_WOODS_CONTRACT,Forest Watch,_CONTRACTS,US_16,Washington,cargoDelivery,1\u00d7 Barrels; 1\u00d7 Solar Panel; 1\u00d7 Service Spare Parts,1120,13300,\"Our watchtowers need some crucial supplies. Without them, our ability to control our territory will be severely impaired. This contract includes deliveries to several key facilities. Are you ready for this undertaking?\",CONTRACTS\nUS_16_01_SCOUTING_CONTRACT,Maps and Mud,_CONTRACTS,US_16,Washington,exploration,,900,15700,\"Hey, we need your help. Our drivers keep getting stuck in mud or wasting time taking detours. We need to know which routes are safe and which ones are a one-way ticket to disaster. Check the roads for us and find the best ones, okay?\",CONTRACTS\nUS_16_01_LITTLE_WATER_TASK,Water for Everyone,_TASKS,US_16,Washington,truckDelivery,2\u00d7 Vehicles Spare Parts,670,5550,\"Hey there! We've run out of water at the trailer park, and that really makes our folks' lives difficult. We've found a trailer full of water and other supplies, but delivering it is an issue as well. Could you help us out? I don't know what we're going to do without water!\",TASKS\nUS_16_01_FISHERS_ORDER_TASK,Fishermen Tech Support,_TASKS,US_16,Washington,cargoDelivery,2\u00d7 Crate Large; 1\u00d7 Service Spare Parts,1040,10850,\"Hello! We've run into a minor issue with our boats. They need maintenance, and we don't have the spare parts. The warehouse does, but we can't fetch them on our own. Could you do a delivery run for us?\",TASKS\nUS_16_01_DELIVERY_WOOD_TASK,The Road to Loneliness,_TASKS,US_16,Washington,cargoDelivery,1\u00d7 Barrels; 1\u00d7 Vehicles Spare Parts,720,7950,\"Hey there! I live alone in the forest, and I need fuel to last me through the winter. My house is quite isolated, so getting deliveries out here is a bit of a challenge. I won't be able to keep working without the fuel. Could you help me out?\",TASKS\nUS_16_01_DELIVERY_HELP_TASK,Boarding Time,_TASKS,US_16,Washington,cargoDelivery,1\u00d7 Wooden Planks; 1\u00d7 Wooden Planks; 1\u00d7 Wooden Planks,580,7600,\"Hiya! We've got a small problem at the holiday camp. The shipment of planks we're expecting is late, and we really need it for repair work. We'd appreciate it if you could help get the planks to us!\",TASKS\nUS_16_01_DELIVERY_ISLAND_TASK,Saving the Hermit,_TASKS,US_16,Washington,cargoDelivery,2\u00d7 Fertilizer,570,5150,\"People rarely visit this island. Truth be told, I can barely remember what it's like, seeing strangers. The soil here is depleted, but they can't get my fertilizer order to me because this place is so hard to reach. Could you help me with that?\",TASKS\nUS_16_01_EXPLORE_CONTEST,Mountain Track,_CONTESTS,US_16,Washington,exploration,,150,3000,\"As a former racer, I'm always on the lookout for challenging courses. But I like them when they're right next to my humble abode... Could you check out this course for me? Show 'em how it's done!\",CONTESTS\nUS_16_01_DRIVE_CONTEST,Neighbor on the Other Side,_CONTESTS,US_16,Washington,exploration,,150,2700,\"My neighbors and I used to enjoy fishing and bragging about our catches to each other, but now one of them has gone no-contact! Could you check on him, see if he's okay? He's old and he lives alone.\",CONTESTS\nUS_16_02_RAILWAY_BRIDGE_01_CONTRACT,The Start of a Long Journey,_CONTRACTS,US_16,Washington,truckDelivery,4\u00d7 Vehicles Spare Parts,960,11150,\"The railway bridge is a key link in our logistics chain, but it has suffered damage and we have no tools to clear the rubble. We need equipment ASAP! Do you think you can deliver a trailer with some spare parts to us? The sooner we get started, the sooner we can restore this route.\",CONTRACTS\nUS_16_02_RAILWAY_BRIDGE_02_CONTRACT,The End of a Long Journey,_CONTRACTS,US_16,Washington,cargoDelivery,2\u00d7 Junk Open; 2\u00d7 Junk Closed; 2\u00d7 Railway; 2\u00d7 Metal Planks; 2\u00d7 Bags,3880,40850,\"Clearing the rubble was just step 1. Now we need to get some containers and deliver the rubbish to a recycling plant. Once the area is all cleaned up, we'll be able to get the materials for the new bridge here. We'd better get this done fast to restore our logistics!\",CONTRACTS\nUS_16_02_WINTER_UPDATE_CONTRACT,Driving to a Hot Spot,_CONTRACTS,US_16,Washington,truckDelivery,1\u00d7 Boiler; 1600\u00d7 Water,2050,19000,\"Hello! We need you to deliver a water boiler to the hotel cluster. Both the route and the cargo are tricky, but you always come out on top of tasks like this. Could you help? We know we can trust you.\",CONTRACTS\nUS_16_02_GIANT_NEED_CONTRACT,Major Order,_CONTRACTS,US_16,Washington,truckDelivery,2\u00d7 Bags 2; 1\u00d7 Metal Planks,1530,14500,\"Hello! The hotel cluster has put in a big order of supplies they need to build a water tower. You have a knack for dealing with jobs like this with speed and quality, so we're counting on you!\",CONTRACTS\nUS_16_02_RESTORATION_SAWMILL_CONTRACT,Sawing Against the Grain,_CONTRACTS,US_16,Washington,truckDelivery,2\u00d7 Metal Planks; 4\u00d7 Metal Roll,1180,13550,\"This place once bustled with humming saws, the smell of wood, and dozens of trucks coming and going. The sawmill is still in operation, but the equipment's outdated. We want to give it a face-lift. If you've got the time, we'd be much obliged for your help!\",CONTRACTS\nUS_16_02_BROKEN_GATE_TASK,Where the Road Begins,_CONTRACTS,US_16,Washington,truckDelivery,2\u00d7 Logs Medium,610,6700,\"This gate welcomed rangers for generations, but now it's all but a tilting pile of logs. The park seems soulless like this. We need supplies to restore the park entrance. Let's give this place a fresh start!\",CONTRACTS\nUS_16_02_YEAR_AHEAD_CONTRACT,Winter is Coming,_CONTRACTS,US_16,Washington,cargoDelivery,2\u00d7 Logs Short; 1\u00d7 Logs Medium; 1\u00d7 Logs Long; 2\u00d7 Logs Medium,3100,40100,\"Winter is almost here, and we're running out of logs. We need to restock our warehouses ASAP to make sure there's enough fuel and construction materials for all our facilities. Could you get us a shipment of logs, please?\",CONTRACTS\nUS_16_02_SCOUT_CARE_CONTRACT,Seasonal Preparations,_CONTRACTS,US_16,Washington,cargoDelivery,2\u00d7 Wooden Planks Medium; 2\u00d7 Logs Medium; 2\u00d7 Container Small; 2\u00d7 Service Spare Parts,2340,27100,\"We need various supplies delivered to the scout camp. The living quarters need planks and logs, and the riverside building needs containers and various consumables. The terrain isn't difficult, but this job requires precision. Are you up for it?\",CONTRACTS\nUS_16_02_WATER_PIPES_CONTRACT,Waterline of Defense,_CONTRACTS,US_16,Washington,cargoDelivery,1\u00d7 Pipes Small; 2\u00d7 Pipes Medium; 2\u00d7 Blocks,2090,24500,\"We have a bit of a problem. Our water-pumping system is out of commission, which means we'll be in trouble if a fire breaks out. We urgently need supplies to fix the system. Could you help?\",CONTRACTS\nUS_16_02_HELPING_FOREST_SERVICE_CONTRACT,Before the First Snow,_CONTRACTS,US_16,Washington,cargoDelivery,2\u00d7 Wooden Planks; 2\u00d7 Wooden Planks Medium; 1\u00d7 Wooden Planks Long; 4\u00d7 Bags; 4\u00d7 Bags 2,2370,23700,\"We've seen time and again that you're a reliable person. Right now, we need supplies delivered to the Forest Service base. Winter is coming\u2014we need everything ready before it arrives.\",CONTRACTS\nUS_16_02_WINTER_SUPPLIES_CONTRACT,Strategic Supplies,_CONTRACTS,US_16,Washington,cargoDelivery,1600\u00d7 Water; 2\u00d7 Logs Short; 3\u00d7 Bags 2,1650,15900,\"Winter can be unpredictable, so it's best to prepare in advance. We need to stockpile a bunch of supplies so we're ready for whatever comes. Here's a list\u2014these things will help us get through the cold season without too much trouble.\",CONTRACTS\nUS_16_02_BUOY_BUOY_CONTRACT,In Search of Buoys,_CONTRACTS,US_16,Washington,cargoDelivery,1\u00d7 Sensory Buoy; 1\u00d7 Sensory Buoy; 1\u00d7 Sensory Buoy; 1\u00d7 Sensory Buoy,1370,13100,\"There was an incident during our data buoy delivery: the trailer got overturned and water carried the cargo away. We won't be able to find it without special equipment, and a hands-on search would take too much time. Could you help us get the buoys back?\",CONTRACTS\nUS_16_02_WOOD_BLOCK_CONTRACT,Unplanned Work,_CONTRACTS,US_16,Washington,cargoDelivery,1\u00d7 Service Spare Parts; 1\u00d7 Service Spare Parts; 1\u00d7 Service Spare Parts,870,9750,\"Yeah... The review is done, and the news isn't good... We've got washed-out roads, landslides, rockslides... First things first, we should deal with the fallen trees since they're blocking important traffic. And no word from Jerry, again. Will you help?\",CONTRACTS\nUS_16_02_BRIDGE_REPAIR_CONTRACT,The Way is Clear,_CONTRACTS,US_16,Washington,cargoDelivery,2\u00d7 Metal Planks; 2\u00d7 Wooden Planks,990,8600,\"This bridge has been dismantled due to some design errors, and now our logistics are a bust. We need construction materials to rebuild. If you get us everything we need, we'll be able to finish the job fast and restore normal communications!\",CONTRACTS\nUS_16_02_RAILWAY_ACCIDENTS_CONTRACT,Back in Business,_CONTRACTS,US_16,Washington,cargoDelivery,1\u00d7 Railway; 1\u00d7 Railway,960,7150,\"Railroad trouble always means loss of revenue, and at this point it's reached exorbitant proportions. We need to replace the worn-out parts of the tracks ASAP, or nothing will get delivered at all. If you have the time, we'd really appreciate your help.\",CONTRACTS\nUS_16_02_ENERGY_DELIVERY_CONTRACT,Surplus of Power,_CONTRACTS,US_16,Washington,cargoDelivery,1\u00d7 Transformer,690,6100,I've got a job for you. One of the key trading depots keeps experiencing blackouts. We've ordered some new power transformers but can't get them to the destination... You could say we're like a plumber whose house has a leak. Could you help us?,CONTRACTS\nUS_16_02_WILDERNESS_CONTRACT,On the Rain's Trail,_CONTRACTS,US_16,Washington,exploration,,680,11850,\"Hey there! We've had lots of heavy rains recently, so the forest routes need to be checked for damage. I usually ask Jerry, but he's busy with something important, again. Could you drive around the area and inspect the routes?\",CONTRACTS\nUS_16_02_CROOKED_PIT_TASK,A Difficult Run,_TASKS,US_16,Washington,truckDelivery,2\u00d7 Wooden Planks Medium,700,7800,\"Hey buddy! I got myself into a hot mess here. I thought I could get through, somehow, but the mud said no. Now my trailer's stuck, and they're waiting for this cargo at the railway station. I'd be forever grateful if you could tow it there!\",TASKS\nUS_16_02_FUEL_TANK_TASK,The Fuel Road,_TASKS,US_16,Washington,truckDelivery,,550,6200,\"Hi! I have a bit of a problem: my fuel tank got stuck in the swamp. I decided to take the main route, but it was a tall order considering the trailer, so here we are. But we're up to our ears in work to deal with the tank ourselves\u2014could you help us get it to the warehouse?\",TASKS\nUS_16_02_WATER_MILL_TASK,In Need of Repairs,_TASKS,US_16,Washington,truckDelivery,,710,6140,Our watermill is a bit wonky and in dire need of repairs. All the required supplies should be by the hotel cluster. Could you deliver them to the mill when you have the time?,TASKS\nUS_16_02_BAD_CURIOSITY_TASK,Muddy Route,_TASKS,US_16,Washington,truckDelivery,,460,4950,\"So I thought I'd check out a new route. I drive here often but figured this way might be shorter. Well, it wasn't, because now my car's stuck here as part of the landscape. If you have a powerful tow truck, could you help me get back to base?\",TASKS\nUS_16_02_SHARP_TURN_TASK,The Road Permits No Mistakes,_TASKS,US_16,Washington,truckDelivery,,360,3300,\"I was surveying the area when I lost control of the car and it went careening away. Not sure if it was the bumpy road or my own fault, but I can't get the car out on my own. I'd be much obliged if you help me out!\",TASKS\nUS_16_02_POTEST_TAMEN_TASK,Scrap Trailer,_TASKS,US_16,Washington,truckDelivery,,330,2700,\"Hey, you didn't happen to drive past an old village at the marsh, did you? I noticed an abandoned logging trailer there, and it looks like it could still be put to good use. Could you pick it up and bring it to the sawmill?\",TASKS\nUS_16_02_BEAUTIFUL_STONES_TASK,Rocky Delivery,_TASKS,US_16,Washington,cargoDelivery,2\u00d7 Stone Block,1110,8600,\"We're in a spot of trouble here. We ordered some slatestone from Kazakhstan, but the shipping company has refused to deliver it all the way to us. The cargo is waiting at a railway station. Could you pick it up and bring it here?\",TASKS\nUS_16_02_FRIENDLY_HELP_TASK,Winter Reserves,_TASKS,US_16,Washington,cargoDelivery,2\u00d7 Potato; 1\u00d7 Soyabean,790,7700,\"Greetings! This is bad. We're all out of winter reserves, and there's about to be a cold wave. One of our neighbors has agreed to share his supplies, but I can't transport them on my own. Can I count on you?\",TASKS\nUS_16_02_SCOUT_WATER_TASK,Just in Case,_TASKS,US_16,Washington,cargoDelivery,2200\u00d7 Water,820,7100,\"Hey, now that the water reservoir is fine, do you think you could fill our tanks? Just in case a fire starts somewhere...\",TASKS\nUS_16_02_SCOOTS_PAPER_TASK,Inspiration Delivery,_TASKS,US_16,Washington,cargoDelivery,4\u00d7 Cellulose,480,5550,\"The car's here, but we seem to be missing the paper we ordered. We need it at the scout camps for an important art project. The scouts are counting on this delivery. We'd be so grateful if you could help!\",TASKS\nUS_16_02_FERTILIZERS_SUPPLY_TASK,Wonder Garden,_TASKS,US_16,Washington,cargoDelivery,4\u00d7 Fertilizer,450,5100,\"Ah, hello! I've invented a marvelous fertilizer! If everything goes according to plan, my garden will be full of wonders soon! But running deliveries is a job for peons. So help me out with that, would you? For science!\",TASKS\nUS_16_02_ROCK_BLOKER_TASK,A Tricky Landslide,_TASKS,US_16,Washington,cargoDelivery,1\u00d7 Wooden Planks Medium,260,2500,There are so many landslides and rockslides in the area that we just can't keep up with them... But we're prioritizing the ones on the main roads. Could you help us out if you have the time?,TASKS\nUS_16_02_RIVER_DRIVE_CONTEST,An Impromptu Bet,_CONTESTS,US_16,Washington,exploration,,480,3900,\"An experienced kayaker is looking for someone to race, and he doesn't care how you traverse the river... Do you catch my drift?\",CONTESTS\nUS_16_02_TEST_RUNS_CONTEST,Test Drive,_CONTESTS,US_16,Washington,exploration,,180,2200,\"We're testing our cars' cross-country abilities here. It's just the kind of experience you could always use in our area. If you want to test yours too, be our guest. We'll even help you out with repairs if you do better than us!\",CONTESTS\nUS_16_03_DAM_RESTORATION_01_CONTRACT,Groundwork for a Hydro Power Plant,_CONTRACTS,US_16,Washington,truckDelivery,2\u00d7 Forklift Caravan Container 2,3320,38500,\"We're in the final stages of construction\u2014just a bit more, and the hydro power plant will be finished. But without a tower crane, we won't be able to assemble the last parts. The crane's at the port. We need it delivered to the construction site.\",CONTRACTS\nUS_16_03_HELPING_SCIENTISTS_01_CONTRACT,Weather Advisory,_CONTRACTS,US_16,Washington,cargoDelivery,1\u00d7 Wooden Planks Medium; 1\u00d7 Solar Panel; 2\u00d7 Bricks; 2\u00d7 Bags,1380,14250,\"All our weather towers are experiencing issues, but these two have it the worst. They're our priority in repairs, and we need you to deliver some materials to get them up and running again.\",CONTRACTS\nUS_16_03_TRAILER_HELP_01_TASK,The Way to the Top,_TASKS,US_16,Washington,truckDelivery,,670,7850,\"Hey friend! I'm in trouble: my scouting trailer got stuck, and I have to deliver it to the lookout point. I can't make it on my own, so if you've got a bit of time, could you pull it out and tow it to the destination?\",TASKS\nUS_16_03_TRUCK_HELP_01_TASK,Back to the Construction Site,_TASKS,US_16,Washington,truckDelivery,,410,3450,\"Hello, friend! We've got a problem: a construction trailer got stuck in mud on the way to us. We need your help getting it out and towing it to the construction site. The terrain is tricky, but I'm sure you'll be fine!\",TASKS\nUS_16_03_DAM_RESTORATION_02_CONTRACT,Preparing the Hydro Plant for Launch,_CONTRACTS,US_16,Washington,cargoDelivery,4\u00d7 Bags; 3\u00d7 Metal Planks; 3\u00d7 Concrete Slab,3360,28550,\"The construction work at the hydro power plant is almost finished, but we're still missing some construction materials. We need them delivered to the final stage construction site. Hope you can help with that.\",CONTRACTS\nUS_16_03_HELPING_SCIENTISTS_02_CONTRACT,The Finishing Touch,_CONTRACTS,US_16,Washington,cargoDelivery,2\u00d7 Bags 2; 1\u00d7 Solar Panel; 2\u00d7 Wooden Planks; 1\u00d7 Metal Planks,1410,13850,\"Time to deal with the other two weather towers. We're still working on restoring them, and we need supplies to make them operational. Please deliver the supplies as soon as you can.\",CONTRACTS\nUS_16_03_TRAILER_HELP_02_TASK,Hope Floats Eternal,_TASKS,US_16,Washington,truckDelivery,4\u00d7 Service Spare Parts Special,1770,17600,\"Hey, buddy! Seems like I underestimated how powerful the currents are when I tried to take the shortcut through the river crossing. Luckily, the trailer got stuck on the rocks, but all my cargo got carried away. Can you help me retrieve the trailer and search the banks for my lost goods?\",TASKS\nUS_16_03_TRUCK_HELP_02_TASK,The Tower Over Yonder,_TASKS,US_16,Washington,truckDelivery,,670,7952,\"Hey, friend! My car broke down and died halfway to the northeast weather tower. I don't think I'll make it there without your help. Could you get me out and give me a lift?\",TASKS\nUS_16_03_DAM_RESTORATION_03_CONTRACT,The Final Push in Construction,_CONTRACTS,US_16,Washington,cargoDelivery,2\u00d7 Bags; 2\u00d7 Bags 2; 4\u00d7 Bricks,2450,30700,\"We're this close to finishing the power plant! We just need some final materials for the main building. That building is the heart of the plant, so it's crucial to get it ready for all the plant's equipment.\",CONTRACTS\nUS_16_03_DAM_RESTORATION_04_CONTRACT,Powerful Finale,_CONTRACTS,US_16,Washington,cargoDelivery,1\u00d7 Transformer; 1\u00d7 Transformer,2200,27950,\"We're done with the power plant, except for one final step: delivering transformers to the transformer station. This is the key to connecting the plant to the grid. Please help us finish the job!\",CONTRACTS\nUS_16_03_VIEWPOINT_RESTORATION_CONTRACT,Preserving Natural Beauty,_CONTRACTS,US_16,Washington,truckDelivery,1\u00d7 Logs Long; 1\u00d7 Logs Medium; 2\u00d7 Wooden Planks Medium,2790,32450,\"This place attracts a lot of tourists and locals, but it's bound to fall into disarray without proper maintenance. We need fresh logs to patch up the buildings and a power generator to keep the lights on. Will you help us preserve this landmark?\",CONTRACTS\nUS_16_03_RESTORATION_LOGGING_BASE_CONTRACT,Strength in Spare Parts,_CONTRACTS,US_16,Washington,truckDelivery,2\u00d7 Service Spare Parts; 3\u00d7 Vehicles Spare Parts; 2\u00d7 Wooden Planks Medium,2240,28250,The technical issues at our logging site are getting in the way of our work. We need you to get us all the necessary spare parts along with a service trailer. That should help us resume operations and get back to logging faster.,CONTRACTS\nUS_16_03_RESTORATION_POWER_LINES_CONTRACT,Lifeline,_CONTRACTS,US_16,Washington,truckDelivery,2\u00d7 Junk Open; 2\u00d7 Junk Closed; 2\u00d7 Metal Planks; 2\u00d7 Vehicles Spare Parts; 2\u00d7 Service Spare Parts,2980,26850,One of our transmission towers went down because of a design error. We need supplies to rebuild it. Please help us with the delivery so we can restore the power supply and avoid any more problems.,CONTRACTS\nUS_16_03_HELIPAD_FUEL_CONTRACT,Timely Delivery,_CONTRACTS,US_16,Washington,truckDelivery,4\u00d7 Barrels,1520,19150,\"Hello! The helipad is all out of fuel, and at this rate, our helicopters will end up unable to fly. We need a tank and some oil barrels. Please deliver the order as soon as possible.\",CONTRACTS\nUS_16_03_BACK_TO_PLACE_CONTRACT,Logistics in Action,_CONTRACTS,US_16,Washington,truckDelivery,4\u00d7 Vehicles Spare Parts,1780,18950,\"We need help with an important delivery: some car spare parts should be taken to the logistics center, and several trailers currently scattered across the map should be brought back. We're sure you can help us deal with this in no time!\",CONTRACTS\nUS_16_03_RESTORATING_SLAB_FACTORY_CONTRACT,Back to Work,_CONTRACTS,US_16,Washington,truckDelivery,3\u00d7 Metal Roll; 2\u00d7 Service Spare Parts,1330,17650,\"Our plant produces concrete slabs, but right now we're in serious trouble. Our equipment has malfunctioned, and there's a power outage at the facility. We'd be able to get back to work if you could deliver us all the supplies we need along with a generator.\",CONTRACTS\nUS_16_03_BRIDGE_BLOCKAGE_CONTRACT,A Pillar of Progress,_CONTRACTS,US_16,Washington,truckDelivery,2\u00d7 Metal Planks; 2\u00d7 Blocks; 2\u00d7 Service Spare Parts,1750,16600,\"A mistake made by construction workers has made the bridge across that ravine unusable, but it's the only way to reach the power plant. Work at the plant had to be halted until the bridge gets rebuilt. We need supplies ASAP to speed up the process.\",CONTRACTS\nUS_16_03_NEW_TRUCK_CONTRACT,Patched-Up Behemoth,_CONTRACTS,US_16,Washington,truckDelivery,,1190,14300,\"Hey, friend! Our truck got stuck on the side of the road out in the boonies. We need to get it out, and fast. It's gonna stay there forever if you don't help us. Are you up for it?\",CONTRACTS\nUS_16_03_ROCK_BLOCKAGE_CONTRACT,Clearing the Way,_CONTRACTS,US_16,Washington,truckDelivery,2\u00d7 Metal Planks; 2\u00d7 Wooden Planks Medium,1280,11600,Hey there! A rockslide has blocked the road to the concrete plant. We need to clear it ASAP to restore logistics in the area. Could you help us remove the roadblock?,CONTRACTS\nUS_16_03_MAIN_POINTS_CONTRACT,Through the Eyes of a Scout,_CONTRACTS,US_16,Washington,exploration,,350,5800,\"We have a large territory under our purview, so sometimes things slip through the cracks. We need you to drive by certain spots and check on the infrastructure. Rain might've washed out the roads or wind might've damaged the buildings. Please tell us if you see anything dire.\",CONTRACTS\nUS_16_03_CONCRETE_DELIVERY_TASK,Trailing Time,_TASKS,US_16,Washington,truckDelivery,1\u00d7 Blocks; 1\u00d7 Blocks; 1\u00d7 Concrete Slab,1640,16000,\"Here's the deal: a trailer carrying concrete blocks got stuck on the way to the northwest weather station. It won't budge, and the blocks need to be delivered to several clients. Could you help me out and then bring the trailer back to the precast concrete plant?\",TASKS\nUS_16_03_TUBES_FOR_WELL_TASK,Wellhole Ready-to-Use,_TASKS,US_16,Washington,truckDelivery,1\u00d7 Pipes Small,820,8750,\"Hey, I could use some help here. I'm drilling a water well on my grounds, but the final step requires some pipes and a water pump. I'd be much obliged if you delivered me all that stuff so I could finish the job.\",TASKS\nUS_16_03_TREE_BLOCKAGE_TASK,Extra Sawing,_TASKS,US_16,Washington,truckDelivery,,570,6900,\"Oh no! One of the trees fell right on the road, and just when the loggers finally took a lunch break, too. No time to wait for their break to end, we need your help now! Could you get us a tool trailer?\",TASKS\nUS_16_03_FISHING_FARM_TASK,Minding the Fish,_TASKS,US_16,Washington,truckDelivery,2\u00d7 Barrels Chemicals,150,2500,\"Ah, good thing you happened by! Could you drive to the fish farm to the east of here and bring us the special pool cleaning equipment, as well as a couple of barrels of cleaning solution? The people there will be expecting you.\",TASKS\nUS_16_03_WATER_PUMP_TASK,Red Tape,_TASKS,US_16,Washington,cargoDelivery,1\u00d7 Npp Pump; 1\u00d7 Npp Pump,1210,11150,\"We're having some trouble with an uncertified pump. It was delivered to the helipad, and now we need to perform several tests and get it certified before it will be ready for use.\",TASKS\nUS_16_03_LOGS_FOR_HOUSE_TASK,Lakeside Cabin,_TASKS,US_16,Washington,cargoDelivery,1\u00d7 Logs Medium; 1\u00d7 Logs Short,790,9550,\"Hey friend! I'd like to rebuild my cabin, but I need logs for that. Could you help me with the delivery? I'll owe you one. It's not far, but the load is pretty heavy.\",TASKS\nUS_16_03_AIRDROP_TASK,Cargo from Heaven High,_TASKS,US_16,Washington,cargoDelivery,3\u00d7 Cellulose,600,4250,\"Our helicopter had to jettison some important cargo during a crash landing. We know where it dropped, approximately, but the landscape there is tricky. Please help us recover the cargo.\",TASKS\nUS_16_03_WEATHER_TOWER_SCOUTING_TASK,Equipment Check,_TASKS,US_16,Washington,exploration,,260,4250,Hi! We've got an important job for you. Please check all the weather towers and make sure our equipment is in working order. And please don't take too long. We're counting on you!,TASKS\nUS_16_03_SHORE_SOIL_TASK,Vibing by the River,_TASKS,US_16,Washington,exploration,,230,3850,Good afternoon! We're currently researching riverside soil stability. Please use this thumper to perform a comprehensive soil analysis. It will let us determine and prevent potential landslides.,TASKS\nUS_16_03_LOGGONG_BASE_CONTEST,Rushing Through the Forest,_CONTESTS,US_16,Washington,exploration,,120,1900,\"We have our hands full with logging, but the higher-ups at the camp are demanding an update on the state of the adjoining roads. Can you drive around real quick, check if there's a fallen tree blocking one of the roads or something?\",CONTESTS\nUS_16_03_SCOUT_CAMP_CONTEST,Conquering Mountain Trails,_CONTESTS,US_16,Washington,exploration,,90,1700,\"Hey there, traveler! We're trying to see who knows the trails around this mountain the best. The winner will be crowned The Mountain Trail Conqueror! Anyone is welcome to enter. Want to give it a shot?\",CONTESTS\nRU_17_01_DRILL_WATER_01_CONTRACT,Water under Our Feet,_CONTRACTS,RU_17,Zurdania,truckDelivery,1\u00d7 Vehicles Spare Parts,810,8400,\"Hey there! Our water extraction project is about to begin, but without the drilling trailer, we're dead in the water. We need you to deliver it to the site\u2014the sooner it gets there, the sooner we can start drilling. Can you handle it?\",CONTRACTS\nRU_17_01_TRAILER_DELIVERY_01_TASK,Dirty Dealings,_TASKS,RU_17,Zurdania,truckDelivery,,620,7000,\"Listen, we've got a bit of a situation here... One guy sold a trailer to the farm but couldn't deliver it\u2014got it buried deep in the swamp. Feel like earning a bit of cash? We need it pulled out and delivered to the address before it sinks for good.\",TASKS\nRU_17_01_TRUCK_DELIVERY_01_TASK,The Long Way Through a Shortcut,_TASKS,RU_17,Zurdania,truckDelivery,,610,6800,\"Ah, I'm in a real bind... The bridge was a no-go, so I tried to cut through the swamp, and now my car's stuck up to its axles. If you can, please tow it to my workplace\u2014there's no way I'm getting out of here on my own.\",TASKS\nRU_17_01_SCOUT_DELIVERY_01_TASK,The Wrong Turn,_TASKS,RU_17,Zurdania,truckDelivery,,530,6150,\"Well, I tried to save a couple of minutes and ended up losing several hours... I was driving back from my friend's farm, thought I could dash through a puddle, and now I'm stuck. Can you help tow me back home?\",TASKS\nRU_17_01_SCOUT_TRAILER_01_TASK,Abandoned but not Forgotten,_TASKS,RU_17,Zurdania,truckDelivery,,550,5800,\"Ah, someone's abandoned their equipment at the foot of the hill again. The scouting trailer seems to be in one piece, just stuck and left. Can you tow it to the parking lot by the garage?\",TASKS\nRU_17_01_DRILL_WATER_02_CONTRACT,Wellhole Number Two,_CONTRACTS,RU_17,Zurdania,truckDelivery,1\u00d7 Service Spare Parts,1190,13900,\"You've helped us out before, and now we're in a similar spot. The second site is ready for drilling, but the trailer still hasn't arrived. The crew is waiting, our equipment stands idle. You're our only hope\u2014please get it over there.\",CONTRACTS\nRU_17_01_TRUCK_DELIVERY_02_TASK,Not So Cinematic,_TASKS,RU_17,Zurdania,truckDelivery,,590,6500,\"What a day! I just finished a shooting session and thought I'd take a shortcut across the water to protect my suspension. Well, that didn't pan out since the car's now stuck in a swamp up to its headlights. I'd appreciate a tow before it starts sprouting moss.\",TASKS\nRU_17_01_SCOUT_DELIVERY_02_TASK,Rock Bottom,_TASKS,RU_17,Zurdania,truckDelivery,,420,3500,\"Hello. I was doing some geological exploration for the future construction of a water tower\u2014had to get a couple soil samples. Unfortunately, my vehicle was pulled downhill on a turn and now it's stuck. Could you help pull it out?\",TASKS\nRU_17_01_FIELD_CLEAR_CONTRACT,Virgin Land,_CONTRACTS,RU_17,Zurdania,truckDelivery,2\u00d7 Service Spare Parts; 2\u00d7 Crate Large; 4\u00d7 Fertilizer,3070,33300,\"Hey! The old field's overgrown with spruce trees, and the rocks are choking out the new sprouts. We need your help: please bring in the supplies, clear the land, and help us reclaim the field so it can bear fruit again!\",CONTRACTS\nRU_17_01_TUNNEL_CONTRACT,Light at the End of the Tunnel,_CONTRACTS,RU_17,Zurdania,truckDelivery,4\u00d7 Blocks; 3\u00d7 Bags,2510,26800,\"Good afternoon! There's been a collapse in the railway tunnel, and all traffic is completely stopped now. We need materials for the cleanup and reinforcing the structure. Without it, all transport will grind to a halt. Can you deliver everything we need?\",CONTRACTS\nRU_17_01_FIELDS_FERTILIZER_CONTRACT,Fertilizing the Fields,_CONTRACTS,RU_17,Zurdania,truckDelivery,6\u00d7 Fertilizer,1850,16450,\"The central farm is in a real bind\u2014the fertilizer and trailer are delayed, and all the equipment's just sitting idle. We needed someone reliable, and you were the first person we thought of. Can you get it all delivered on time? We're counting on you!\",CONTRACTS\nRU_17_01_FARM_SOLAR_CONTRACT,Farming Power,_CONTRACTS,RU_17,Zurdania,cargoDelivery,2\u00d7 Wooden Planks; 2\u00d7 Metal Planks; 4\u00d7 Fertilizer; 1\u00d7 Solar Panel; 1\u00d7 Solar Panel; 1\u00d7 Solar Panel,3620,36600,\"Hey bud! The farm's buzzing with activity, and some of the greenhouses are already up, but we still need to install the solar panels and finish the rest of the buildings. If you can help with the delivery and construction, the farm will be running at full capacity again in no time!\",CONTRACTS\nRU_17_01_DELIVERY_FOODS_CONTRACT,Supplies en Route,_CONTRACTS,RU_17,Zurdania,cargoDelivery,1\u00d7 Container Reefer; 1\u00d7 Container Small; 1\u00d7 Container Large; 1\u00d7 Container Reefer,2880,30050,\"Hey, could you do us a favor? We need a few containers and some refrigeration units delivered\u2014one to the city warehouse, the other to the retreat. It's nothing complicated, just get them to their destinations. You're the most reliable driver we've got.\",CONTRACTS\nRU_17_01_FIELD_WORK_CONTRACT,Verdant Fields,_CONTRACTS,RU_17,Zurdania,cargoDelivery,4\u00d7 Fertilizer; 2\u00d7 Crate Large; 3\u00d7 Service Spare Parts; 1\u00d7 Pipes Medium,2880,28700,\"Hey! Our plants survived the drought, the water's back on, but it's not enough\u2014the seedlings are still weak. We urgently need fertilizer to revive them and get the farm's harvest back on track. Can you help us get the supplies before it's too late?\",CONTRACTS\nRU_17_01_FACTORY_BUILD_CONTRACT,Business Support,_CONTRACTS,RU_17,Zurdania,cargoDelivery,2\u00d7 Metal Planks; 2\u00d7 Metal Roll; 6\u00d7 Bricks; 1\u00d7 Chiller,2960,28600,\"Hey there! Our food factory is almost ready, but we could use your help to meet our final deadline. We're really hoping you can deliver all the necessary supplies for us to kick-start production.\",CONTRACTS\nRU_17_01_OIL_COMPANY_CONTRACT,Running Hot,_CONTRACTS,RU_17,Zurdania,cargoDelivery,4\u00d7 Barrels; 2\u00d7 Bricks; 2\u00d7 Bags 2,2390,28200,\"The construction of our new boiler facility is in full swing, but the shipments are running late. We'll have to shut down production without those missing materials. If you can handle the delivery, you'll be saving the entire project.\",CONTRACTS\nRU_17_01_WATER_TOWERS_CONTRACT,Enough Water to Go Around,_CONTRACTS,RU_17,Zurdania,cargoDelivery,2\u00d7 Metal Planks; 2\u00d7 Metal Roll; 2\u00d7 Metal Planks; 2\u00d7 Metal Roll,2860,26950,\"All right, time to move on to the next step. We've got the water, but now we need to distribute it! We have to build two water towers\u2014one for the eastern fields and another for the western ones. If you can help us deliver the supplies, we'll get through the season without any drought for sure.\",CONTRACTS\nRU_17_01_DELIVERY_LOGS_CONTRACT,Fixing Up,_CONTRACTS,RU_17,Zurdania,cargoDelivery,1\u00d7 Logs Short; 1\u00d7 Logs Medium; 1\u00d7 Logs Short; 1\u00d7 Logs Medium,1920,23700,Those last rains really did a number on our farms\u2014some areas have been washed out and need repairs. We need you to haul some logs to rebuild the warehouses and outbuildings. You've always got a reliable rig ready to go!,CONTRACTS\nRU_17_01_OPEN_GATE_CONTRACT,The Mountain Route,_CONTRACTS,RU_17,Zurdania,cargoDelivery,2\u00d7 Blocks; 2\u00d7 Bags; 2\u00d7 Bags 2,1870,20450,\"Hey pal! We've got to get the mountain route back in shape: a stretch of the road has been damaged, and we need the repair supplies hauled right to the spot. Until it's fixed, the way to the other side of the region is blocked, and the crew's stuck waiting.\",CONTRACTS\nRU_17_01_BUILD_BRIDGE_CONTRACT,The Missing Link,_CONTRACTS,RU_17,Zurdania,cargoDelivery,1\u00d7 Container Large; 2\u00d7 Concrete Slab; 1\u00d7 Metal Planks,2260,19200,\"Hey friend! The bridge by the garage still isn't ready\u2014supplies got stuck somewhere, and without that connection, not a single truck can get through. It would be a huge help if you took on the delivery! We've just about given up hope waiting around here.\",CONTRACTS\nRU_17_01_ROAD_TROUBLE_CONTRACT,The Ravages of Time,_CONTRACTS,RU_17,Zurdania,cargoDelivery,2\u00d7 Junk Open; 2\u00d7 Crate Large; 2\u00d7 Junk Closed,1990,17900,\"Hey there! The tower by the road collapsed after the last storm\u2014there are piles of stone left that we can use to rebuild the temple. Please get us some empty containers, load up the usable blocks, and deliver them to the construction site.\",CONTRACTS\nRU_17_01_HOLLYWOOD_CONTRACT,The Filming Must Go On,_CONTRACTS,RU_17,Zurdania,cargoDelivery,2\u00d7 Crate Large; 1\u00d7 Service Spare Parts; 2\u00d7 Wooden Planks,1470,16900,\"Hey, we need your help! The movie set is almost ready, but not all the materials for the props and set dressings have arrived on time. We need you to deliver them so we can continue shooting without delays and get the movie finished on schedule.\",CONTRACTS\nRU_17_01_FOOD_WAREHOUSE_CONTRACT,Refrigerated Supplies,_CONTRACTS,RU_17,Zurdania,cargoDelivery,2\u00d7 Wooden Planks; 4\u00d7 Metal Roll; 2\u00d7 Container Reefer,1720,15700,\"Hey, we've got a bit of an emergency here. The central warehouse is almost empty, and without new supplies, everything's going to grind to a halt. We need building supplies and a couple of freezers delivered to get things moving again. Think you can handle it?\",CONTRACTS\nRU_17_01_CONTAINER_DELIVERY_CONTRACT,Delivery Duty,_CONTRACTS,RU_17,Zurdania,cargoDelivery,1\u00d7 Container Large; 2\u00d7 Container Small,1420,12350,\"Hey there! We could use your help: these equipment containers are needed at the sawmill and the old farm. It's a crucial delivery for our operations\u2014without it, everything has grinded to a halt.\",CONTRACTS\nRU_17_01_DELIVERY_WAREHOUSE_CONTRACT,East to West,_CONTRACTS,RU_17,Zurdania,cargoDelivery,3\u00d7 Logs Short; 4\u00d7 Bags 2,1320,11900,\"Things seem to be falling apart here when you're not around. The sand for the east farm still hasn't been delivered, and the logs are waiting for transport to the west. You'll have to drive around a bit, but be careful\u2014the road's no walk in the park. You in?\",CONTRACTS\nRU_17_01_SCOUTING_READY_CONTRACT,Surveying the Infrastructure,_CONTRACTS,RU_17,Zurdania,exploration,,790,13750,\"Hey there! The restoration efforts in the region are in full swing, but we need to make sure all the key facilities are operating properly. Please drive to the warehouse, the factories, and the train terminal to gather data for our progress report.\",CONTRACTS\nRU_17_01_SEISMO_CHECK_CONTRACT,Stability Check,_CONTRACTS,RU_17,Zurdania,exploration,,780,13550,Hello! Our management is collecting data on mountain slope soil. We need you to drive to four seismic zones and take measurements. It will help with planning future projects in the area.,CONTRACTS\nRU_17_01_ROCK_TOWNS_TASK,Supporting the Countryside,_TASKS,RU_17,Zurdania,cargoDelivery,2\u00d7 Stack; 1\u00d7 Wooden Planks; 1\u00d7 Crate Large,1450,17100,\"Management is asking for help in supplying several remote villages. The roads are in rough shape, but people live there year-round and depend on regular deliveries. We're counting on you.\",TASKS\nRU_17_01_WATCHTOWERS_HELP_TSK,Supporting the Outposts,_TASKS,RU_17,Zurdania,cargoDelivery,1\u00d7 Crate Large; 1\u00d7 Barrels; 1\u00d7 Crate Large,1300,14650,\"Good afternoon! Our people at the watchtowers are running low on supplies, and it'll be tough for them to work without a restock. Can you help deliver everything they need?\",TASKS\nRU_17_01_MAGAZINES_DELIVERY_TASK,Restocking the Shelves,_TASKS,RU_17,Zurdania,cargoDelivery,1\u00d7 Barrels; 800\u00d7 Water; 1\u00d7 Crate Large,1320,14050,\"Hey! I run a few small shops around here, and my storerooms are short on merchandise. I'd be really grateful if you could check this list and deliver what's needed. I just don't have the time to do it myself, and the roads aren't exactly in great shape right now.\",TASKS\nRU_17_01_RAILWAY_DELIVERY_TASK,Mobile Engineering,_TASKS,RU_17,Zurdania,cargoDelivery,2\u00d7 Service Spare Parts; 2\u00d7 Pipes Small,1280,13400,\"Hey, things at the warehouse by the train station have ground to a halt. We've got burst pipes, and the equipment's acting up like an old steam engine. We won't get anywhere without repairs. Could you deliver the pipes and spare parts? You'd be a real lifesaver!\",TASKS\nRU_17_01_METAL_CITY_TASK,Metal Time,_TASKS,RU_17,Zurdania,cargoDelivery,2\u00d7 Metal Roll; 1\u00d7 Metal Planks,1160,12250,\"Hey there! We're running low on metal at the pipe warehouse, and without it, production will grind to a halt. We're doing whatever we can with the shortage, but there's just no way around it. Could you get us some supplies? It would be a huge help!\",TASKS\nRU_17_01_HOUSE_ON_THE_ROCK_TASK,A Steep Climb,_TASKS,RU_17,Zurdania,cargoDelivery,2\u00d7 Service Spare Parts; 3\u00d7 Solar Panel,1170,11450,\"Heya! My cliffside house is built in a very scenic spot, but it's nearly impossible to get supplies up here on my own. Can you help me deliver the cargo? I'd be really grateful!\",TASKS\nRU_17_01_CITY_VIEW_TASK,Farm Deliveries,_TASKS,RU_17,Zurdania,cargoDelivery,1\u00d7 Service Spare Parts; 1\u00d7 Service Spare Parts; 1\u00d7 Crate Large,960,10750,\"Hey! My car broke down just outside the city, but the orders still need to be dealt with. Could you deliver the cargo for me? I'm really stuck here without your help!\",TASKS\nRU_17_01_FOREST_HOUSE_TASK,The Last Run Before the Cold,_TASKS,RU_17,Zurdania,cargoDelivery,1\u00d7 Stack; 1\u00d7 Stack; 2\u00d7 Barrels,1020,10150,\"Listen, we've got another shortage\u2014people are out of fuel and hay. We've got everything ready, it just needs to be delivered. The locations are hard to reach, especially that house by the cliff, but I know you can handle it.\",TASKS\nRU_17_01_GROUP_HOUSE_TASK,Working Under the Sun,_TASKS,RU_17,Zurdania,cargoDelivery,1\u00d7 Solar Panel; 1\u00d7 Service Spare Parts,780,8600,\"Hey! The village is having power outages again\u2014the old panels are worn out, and the equipment's down without spare parts. We've ordered new solar panels and components, but there's no one to deliver them. Can you help the folks out?\",TASKS\nRU_17_01_OIL_DELIVERY_TASK,A Fuel Workaround,_TASKS,RU_17,Zurdania,cargoDelivery,2\u00d7 Container Small; 1\u00d7 Container Large,1100,8350,\"Hey! There's been an accident on the tracks leading to our oil refinery, and the trains aren't running. We desperately need some fuel containers to keep the plant running. Can you help and deliver them yourself?\",TASKS\nRU_17_01_CROSS_HOUSE_TASK,Fuel and Chill,_TASKS,RU_17,Zurdania,cargoDelivery,1\u00d7 Barrels; 1\u00d7 Container Reefer,770,7350,\"Hey, I need a hand here! The fuel storage is almost empty, and the garage is asking for an urgent freezer delivery. Without it, all work will come to a halt. Hope you've got time to help out.\",TASKS\nRU_17_01_FARM_HELP_TASK,Cleaning Up the Farm,_TASKS,RU_17,Zurdania,cargoDelivery,2\u00d7 Rubbish,750,7100,\"Hey pal! I've got a farm here, and these waste containers are just getting in the way of everything. Can't manage it alone\u2014I need your help collecting them, loading them up, and hauling them far away. I'd be much obliged!\",TASKS\nRU_17_01_HOUSE_ALONE_TASK,\"Waste Not, Want Not\",_TASKS,RU_17,Zurdania,cargoDelivery,2\u00d7 Bags 2; 1\u00d7 Wooden Planks,720,6800,\"Listen, we have some leftover supplies at the construction site, and we decided to donate them to the warden nearby. Can you help transport them before someone steals everything?\",TASKS\nRU_17_01_RAILWAY_CHECK_CONTEST,A Test of Strength,_CONTESTS,RU_17,Zurdania,exploration,,30,500,\"Here's the thing... we need to check a section of track before we can send a train through. The rules say to do it on foot, but who's got the time? Take a drive down there, see if there's any warping on the rails.\",CONTESTS\nRU_17_01_CLIMB_PLATFORM_CONTEST,Rail Yard Route,_CONTESTS,RU_17,Zurdania,exploration,,20,300,\"Hey! Folks at the rail yard have come up with a challenge\u2014you need to drive across the platforms without falling off or getting stuck. Everyone keeps saying the route's impossible, but I'm sure you can prove them wrong.\",CONTESTS\nRU_17_02_TRAIN_01_CONTRACT,Towing the Tractor,_CONTRACTS,RU_17,Zurdania,truckDelivery,,330,2350,It's not every day the railway services ask the road transport services for help. But this is an exception. We need to tow an engine from the bridge to the southern platform for refueling. A relief train driver will be waiting there.,CONTRACTS\nRU_17_02_SOLAR_ENERGY_CONTRACT_01,The Gifts of the Sun: Basics,_CONTRACTS,RU_17,Zurdania,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Bags; 1\u00d7 Metal Planks; 1\u00d7 Bags; 1\u00d7 Metal Planks; 1\u00d7 Bags; 1\u00d7 Metal Planks; 1\u00d7 Bags; 1\u00d7 Service Spare Parts,3110,32850,\"Welcome to the solar farm! We're in the home stretch now\u2014just need to install 4 sections of solar panels to get this place humming. It's not back-breaking work, but the clock is ticking. We need metal beams and cement to get the ball rolling on the frames.\",CONTRACTS\nRU_17_02_VILLAGE_SUPPLY_TASK_01,Cellar Trouble,_TASKS,RU_17,Zurdania,truckDelivery,2\u00d7 Bags; 1\u00d7 Wooden Planks Medium,1350,16500,\"Welcome! We don't get many visitors in our village, especially driving rigs like yours. Since you're here\u2014make yourself at home. By the way, could you help me patch up a huge hole in my cellar? Not for free\u2014I'll pay you!\",TASKS\nRU_17_02_TRUCK_HELP_01,Flying Off the Handle,_TASKS,RU_17,Zurdania,truckDelivery,,700,9200,\"Thank heaven I just replaced the brake pads on my car. They held up fine in the emergency, but the other parts weren't so lucky. Could you tow the thing and help me with the repairs? I'll make it worth your while!\",TASKS\nRU_17_02_SOLAR_ENERGY_CONTRACT_02,The Gifts of the Sun: Theory,_CONTRACTS,RU_17,Zurdania,cargoDelivery,2\u00d7 Solar Panel; 2\u00d7 Solar Panel; 2\u00d7 Solar Panel; 2\u00d7 Solar Panel,1890,19900,\"On to the next step\u2014the solar panels. This cargo is quite fragile and expensive, so it needs careful handling. We need 8 pallets. My crew will stand by so they can start mounting them on the frame right away. We'll be waiting for your delivery!\",CONTRACTS\nRU_17_02_TRAILER_HELP_02,Fishing for Cargo,_TASKS,RU_17,Zurdania,truckDelivery,3\u00d7 Service Spare Parts Special,1390,15200,\"No one's safe from accidents like this. Thankfully, my truck made it through, but a big rock slammed into the trailer's side. You can see the result... Could you help me recover the cargo and pull the trailer out? I'll pay!\",TASKS\nRU_17_02_TRUCK_HELP_02,Oldie But Goldie,_TASKS,RU_17,Zurdania,truckDelivery,,470,4700,\"The driver of our old truck got stuck on the way to the transformer area, apparently trying to take a shortcut. Of course, the company has already ordered us a better truck, but it's best not to waste old assets. Can you bring the truck back?\",TASKS\nRU_17_02_VILLAGE_SUPPLY_TASK_02,For a Rainy Day,_TASKS,RU_17,Zurdania,cargoDelivery,1\u00d7 Barrels Oil; 2\u00d7 Bags 2,980,11950,\"Hey, pal, would you be interested in a side gig? I need to stock up on gasoline and sand. You never know when you might run out of fuel or need to build up a bank. So, can you get me the stuff? I'll pay!\",TASKS\nRU_17_02_SOLAR_ENERGY_CONTRACT_03,The Gifts of the Sun: Practice,_CONTRACTS,RU_17,Zurdania,truckDelivery,1\u00d7 Transformer,1190,12600,\"Curious about firing up the solar station, aren't you? Can't blame you\u2014this doesn't happen every day. For us, the final piece of the puzzle is getting the new transformer installed at the substation to convert the power. Can you haul it over?\",CONTRACTS\nRU_17_02_TRUCK_HELP_03,Mountain Safari,_TASKS,RU_17,Zurdania,truckDelivery,,380,4000,\"I've taken this route more than a few times to show tourists around. But today luck wasn't on our side, and we got stuck. Looks like the road's completely washed out. Can you help me and my passengers? Get us back to the hotel? I'll make it worth your while.\",TASKS\nRU_17_02_VILLAGE_SUPPLY_TASK_03,Clay and Trees,_TASKS,RU_17,Zurdania,cargoDelivery,2\u00d7 Bricks; 1\u00d7 Wooden Planks Medium; 1\u00d7 Wooden Planks,1300,16650,\"I don't get why people argue whether brick or wood makes a better house. Why not use both? I've already signed off on the blueprint for the new house\u2014just need the materials delivered. By the way, care to help with the delivery?\",TASKS\nRU_17_02_TRUCK_HELP_04,A Slip-Up,_TASKS,RU_17,Zurdania,truckDelivery,,710,7650,\"The puddles don't seem to be frozen over. Weird\u2014why did my car skid? I tried to slow down and straighten out before the turn but ended up in the ditch anyway, and there's a lake right behind it. Can you help me get out and fix up the car? I can pay!\",TASKS\nRU_17_02_RAILWAY_ACCIDENT_CONTRACT,Rocks and Rails: the Center,_CONTRACTS,RU_17,Zurdania,truckDelivery,400\u00d7 Consumable Repairs; 4\u00d7 Junk Open; 4\u00d7 Junk Closed,4050,40300,\"Our region isn't called the Valley of Falling Rocks for nothing. Just the other day, a massive rockslide came down right on a passing train. No one was hurt, thankfully, but we need to deal with the aftermath as soon as possible!\",CONTRACTS\nRU_17_02_SANATORIUM_RESTORATION_CONTRACT,All Set for Relaxation: Shelter,_CONTRACTS,RU_17,Zurdania,truckDelivery,1\u00d7 Metal Planks; 2\u00d7 Bricks; 1\u00d7 Bags; 2\u00d7 Wooden Planks; 1\u00d7 Wooden Planks Medium,3110,37500,\"Tourist season is fast approaching, and interior work still isn't finished! We need resources to get the job done ASAP. The construction crew is standing idle. Can you help us out? We'll cover all the costs!\",CONTRACTS\nRU_17_02_MONASTERY_RESTORATION_CONTRACT,Anastylosis,_CONTRACTS,RU_17,Zurdania,truckDelivery,2\u00d7 Bags 2; 2\u00d7 Bags; 2\u00d7 Bricks,2860,35250,\"I'm sure you know about this building restoration method that uses original materials. But we can't manage with just the old stones alone. Without modern materials, everything here will fall apart. Could you help deliver them?\",CONTRACTS\nRU_17_02_OLD_FACTORY_CONTRACT,Concrete Giant,_CONTRACTS,RU_17,Zurdania,truckDelivery,1\u00d7 Metal Planks; 1\u00d7 Pipes Medium; 1\u00d7 Service Spare Parts,1850,20900,\"This industrial giant once supplied concrete products to the entire district, but now its facilities are empty. Well, we won't stand for it, not on our watch! It's time to restore the plant to its former glory, and your help will be crucial in this endeavor!\",CONTRACTS\nRU_17_02_ASPHALT_BLOCKAGE_CONTRACT,A New Life for a Tower,_CONTRACTS,RU_17,Zurdania,truckDelivery,200\u00d7 Consumable Repairs; 2\u00d7 Junk Open; 2\u00d7 Junk Closed,2120,18350,\"It's a terrible shame that one of the most ancient towers here has collapsed. But every cloud has a silver lining. Pieces of the original masonry will be used to restore our temple. So, when you're clearing the rubble, make sure to separate ordinary stones from the original masonry.\",CONTRACTS\nRU_17_02_SANATORIUM_WATER_SYSTEM_CONTRACT,All Set for Relaxation: Water,_CONTRACTS,RU_17,Zurdania,cargoDelivery,1\u00d7 Pipes Small; 1\u00d7 Pipes Medium; 1\u00d7 Boiler,3090,36850,\"We've already run all the water lines to the building, but we still need the industrial boiler delivered and installed. Without hot water, what kind of reviews will we get from tourists? Can you handle the delivery for us?\",CONTRACTS\nRU_17_02_FERTILIZERS_CONTRACT,Encouraging Growth,_CONTRACTS,RU_17,Zurdania,cargoDelivery,1\u00d7 Fertilizer; 1\u00d7 Fertilizer; 1\u00d7 Fertilizer; 2\u00d7 Fertilizer,2020,22650,\"Our regional agricultural company is offering support to local producers. As part of this support program, we've allocated a batch of soil fertilizer that needs to be delivered to the recipients. Will you take the job?\",CONTRACTS\nRU_17_02_PLATFORM_RESTORATION_CONTRACT,Passenger Infrastructure,_CONTRACTS,RU_17,Zurdania,cargoDelivery,1\u00d7 Concrete Slab; 1\u00d7 Metal Planks; 1\u00d7 Concrete Slab; 2\u00d7 Bags,1980,20900,\"We have to think about passenger safety first and foremost, and we should start by restoring the old platforms. We need to deliver supplies to the two platforms so the construction crews can finish the restoration before the tourist season begins.\",CONTRACTS\nRU_17_02_HOLLY_WATER_CONTRACT,Water for the Weary,_CONTRACTS,RU_17,Zurdania,cargoDelivery,750\u00d7 Water; 350\u00d7 Water; 500\u00d7 Water,1740,17950,\"Since our chapels don't have a direct connection to the water supply, they need their fresh water reserves topped up from time to time. Usually, local drivers handle that, but no one's available right now. Can you help out?\",CONTRACTS\nRU_17_02_HYDRO_RESEARCH_CONTRACT,Water Constitution,_CONTRACTS,RU_17,Zurdania,cargoDelivery,1\u00d7 Sensory Buoy; 1\u00d7 Sensory Buoy; 1\u00d7 Sensory Buoy; 1\u00d7 Sensory Buoy,1350,14950,\"You're well aware that, to expand further, the local agricultural company needs to analyze the local reservoirs' water composition, since it directly affects crop yields in the area. It's simple\u20144 buoys for 4 lakes. Can you handle it?\",CONTRACTS\nRU_17_02_VILLAGE_AID_CONTRACT,Agricultural Grants,_CONTRACTS,RU_17,Zurdania,cargoDelivery,3\u00d7 Barrels Oil; 2\u00d7 Wooden Planks; 2\u00d7 Bricks,1230,14150,\"Great news! The local agricultural company is offering new grants to all interested farmers to boost their production volume. Many have already been approved for support\u2014now it just needs to be delivered. And we need a carrier, someone like you.\",CONTRACTS\nRU_17_02_SANATORIUM_FOOD_CONTRACT,All Set for Relaxation: Food,_CONTRACTS,RU_17,Zurdania,cargoDelivery,2\u00d7 Container Reefer; 2\u00d7 Service Spare Parts,1300,13150,\"It's also important to address the food situation. The kitchen is ready, all the appliances and furniture have been delivered, but we're short on freezers to store our food in. We need to get a couple of them delivered\u2014could you help?\",CONTRACTS\nRU_17_02_CHAPEL_RESTORATION_CONTRACT,Good Things Come in Threes,_CONTRACTS,RU_17,Zurdania,cargoDelivery,1\u00d7 Bags; 1\u00d7 Wooden Planks; 1\u00d7 Bags 2,980,10400,\"Even though the local chapels don't have many parishioners, keeping them in good condition is our responsibility. So, each one needs a little TLC. Can you get us the supplies? It's not much...\",CONTRACTS\nRU_17_02_SMALL_ACCIDENT_CONTRACT,Rocks and Rails: the North,_CONTRACTS,RU_17,Zurdania,cargoDelivery,2\u00d7 Wooden Planks; 1\u00d7 Wooden Planks Medium,640,7500,\"It's always one thing after another. And while this rockslide happened after our trains were cut off, it's still no good that it's blocking the tracks. This one's a quick fix\u2014a couple of planks should do the trick. Can you deliver some?\",CONTRACTS\nRU_17_02_RAILWAY_RESTORATION_CONTRACT,The Metal Binding Thread,_CONTRACTS,RU_17,Zurdania,cargoDelivery,1\u00d7 Railway; 1\u00d7 Railway,780,6050,\"Now that the rocks are out of the way, all we have to do is fix the rails. It's a good thing our warehouse has pre-assembled sections of track just sitting there. We need you to deliver them to each of the crash sites. That should let us restore railway traffic.\",CONTRACTS\nRU_17_02_FARM_WATER_CONTRACT,Dry Spell,_CONTRACTS,RU_17,Zurdania,cargoDelivery,3200\u00d7 Water,400,5250,\"What a disaster, our harvest is drying up! The hygrometer's readings have plummeted. The plants are starting to wilt, and our water reserves are running dangerously low. We need water. A lot of it. We'll pay well\u2014just get it here, fast!\",CONTRACTS\nRU_17_02_SEISMIC_HILLS_CONTRACT,Minerals in the Ground,_CONTRACTS,RU_17,Zurdania,exploration,,640,11050,\"Hey there, pal! The local agricultural company has big plans for the fertile plots in the region. Thing is, we need to analyze the soil in the mountainous areas\u2014check its structure and density. Feel like playing geologist for a while?\",CONTRACTS\nRU_17_02_FIRST_SCOUT_CONTRACT,Driving Through,_CONTRACTS,RU_17,Zurdania,exploration,,340,5850,\"Welcome! This region is known for its vast forests, clean air, and scenic mountainscapes. But tourists are still a rare sight around here. So, while you're at it, why not stop by these places and enjoy the view?\",CONTRACTS\nRU_17_02_SCANER_TASK,Covering the Bases,_TASKS,RU_17,Zurdania,truckDelivery,3\u00d7 Barrels Oil,710,7650,\"Ah, a new face! We're trying to patch the hotel into the communication network, but the reception's weak and the frequency keeps jumping. Think you could lend us a hand? A good folding radar would sort this coverage issue right out.\",TASKS\nRU_17_02_LUMBER_SUPPLY_TASK,Heart of the Sawmill,_TASKS,RU_17,Zurdania,truckDelivery,2\u00d7 Service Spare Parts; 2\u00d7 Barrels Oil,400,4750,\"Hey, traveler! Since you're passing through, care to lend a hand? We're running low on fuel, the portable generator's on its last legs, and we're short on spare parts, too. If you can haul this stuff over for us, we'll make it worth your while!\",TASKS\nRU_17_02_SWAMP_WOOD_TASK,Swamp Renovation,_TASKS,RU_17,Zurdania,cargoDelivery,2\u00d7 Logs Short; 1\u00d7 Logs Medium,780,8800,\"Hold up there, sonny! Care to do an old man a favor? You're the only heavy rig driver around these parts. Could you bring me some construction boards? My old house is getting a bit drafty. I'll see you right\u2014won't let kindness go cold.\",TASKS\nRU_17_02_VILLAGE_WOOD_TASK,No Food for the Wood Borers,_TASKS,RU_17,Zurdania,cargoDelivery,1\u00d7 Logs Long,600,6300,\"Ugh, if there's one thing I can't stand around here, it's these pests. The wood-boring beetles are multiplying like nobody's business, and that's bad for timber houses. If it's not too much trouble, could you bring us some solid planks for repairs?\",TASKS\nRU_17_02_SEISMIC_SWAMP_TASK,The Secrets Buried in the Soil,_TASKS,RU_17,Zurdania,exploration,,130,2200,\"Can't say why, but over the last few years the swamp's been spreading, and the water level's crept up a good half-centimeter on average. Even though the water's stagnant. Please check the geological activity for us\u2014we'll make it worth your while.\",TASKS\nRU_17_02_MOUNTAIN_SCOUT_TASK,Mountain Trail Inspector,_TASKS,RU_17,Zurdania,exploration,,120,2000,\"Hey there, traveler! I've lived here for a long time, and with the agricultural company expanding in the region, all the roads are seeing more traffic, which isn't doing them any favors. Could you drive around and check what shape the mountain trails are in?\",TASKS\nRU_17_02_NORTH_SCOUT_TASK,The Thickest Forest,_TASKS,RU_17,Zurdania,exploration,,120,1950,\"Listen, buddy, we've got a bit of a task on our hands. We need to scout around the logging base and look for quality timber. Our current spot's running dry, and we need to start planning a move.\",TASKS\nRU_17_02_SWAMP_RACING,Searching for Green Giants,_CONTESTS,RU_17,Zurdania,exploration,,150,1600,\"Hey, buddy, have you heard the local tale about the hermit who still lives all alone out in the marshes? They say you might just run into him out there. Are you up for a ride? I bet you won't be coming back.\",CONTESTS\nRU_17_02_MOUNTAIN_RACING,The Lord of the Cliffs,_CONTESTS,RU_17,Zurdania,exploration,,150,1550,\"Hey, good to see you! The locals and I occasionally set up races along the mountain trails. If you're looking for a shot of adrenaline, you've come to the right place. So, care to test your skills? But be warned\u2014it's pretty risky!\",CONTESTS\nUS_18_01_FOR_AGES_01_CONTRACT,Concrete vs. Waves,_CONTRACTS,US_18,Chiapas,cargoDelivery,4\u00d7 Container Small,1220,13850,\"After the recent storms, the waves have started eating away at the shore. The beach will disappear unless we do something. We need tetrapods to reinforce the coastline and protect the hotel grounds from the ocean's destructive force.\",CONTRACTS\nUS_18_01_BRIDGE_REPAIRS_01_CONTRACT,Bridging the Gap,_CONTRACTS,US_18,Chiapas,cargoDelivery,2\u00d7 Service Spare Parts; 3\u00d7 Wooden Planks,910,10100,\"Bad news, amigo. The river rose so much after the rainy season that the old bridge was just swept away. Now there's nothing between the banks but water and wreckage. The city council has decided to build a new bridge. You up for the delivery?\",CONTRACTS\nUS_18_01_SCOUT_DELIVERY_01_TASK,Against the Tide,_TASKS,US_18,Chiapas,truckDelivery,,370,3350,\"Honestly, I just wanted a little joyride along the shore. The water, the sunset\u2014it's so beautiful. But then the engine died, and it's refusing to start back up. Can you bring the truck to the city? I don't want the tide to be the last thing my car ever sees.\",TASKS\nUS_18_01_TRAILER_DELIVERY_01_TASK,Reclaiming Property,_TASKS,US_18,Chiapas,truckDelivery,,350,3050,\"We had a mounting bracket snap on the road just outside town, and the repair trailer got left behind by the roadside. It's got all our tools and gear for on-site jobs. Without it, we're dead in the water\u2014can't take any calls. Please track it down and haul it back to our warehouse.\",TASKS\nUS_18_01_WATER_DELIVERY_01_TASK,Archeology Caravan,_TASKS,US_18,Chiapas,cargoDelivery,800\u00d7 Water,900,12050,\"We've found something interesting under this layer of soil, but the heat is making it impossible to work properly. There's almost no water left, and without it, neither the crew nor the equipment will last much longer. We need to restock the camp at the dig site.\",TASKS\nUS_18_01_FORREST_EYES_01_TASK,No More Detours,_TASKS,US_18,Chiapas,cargoDelivery,1\u00d7 Vehicles Spare Parts,560,7850,\"If we clear out this section of the jungle, it'll make a nice straight shortcut. No pavement, no construction\u2014just a clean path across the cleared land. We need to remove the trees so trucks can get through here.\",TASKS\nUS_18_01_FOR_AGES_02_CONTRACT,Merry Lights on the Coastline,_CONTRACTS,US_18,Chiapas,cargoDelivery,4\u00d7 Container Small,1130,12200,\"Listen, we've got a problem. The festival's almost ready\u2014the stage is set, the lights are hung, but the waves have started eating into the beach. If this keeps up, part of the venue will just wash away. Can you get us some tetrapods to reinforce the coastline?\",CONTRACTS\nUS_18_01_BRIDGE_REPAIRS_02_CONTRACT,Broken Route,_CONTRACTS,US_18,Chiapas,cargoDelivery,2\u00d7 Wooden Planks; 1\u00d7 Metal Planks,1040,11050,\"The river has completely destroyed the bridge this rainy season. The route is now broken up, which makes the farmers and workers suffer. Please get us the supplies to help build a new bridge. Your experience is the only thing that can save the day!\",CONTRACTS\nUS_18_01_TRAILER_DELIVERY_02_TASK,\"When it Rains, Heavy Loads Get Stuck\",_TASKS,US_18,Chiapas,truckDelivery,,480,5300,\"Listen, we need a hand. The service trailer was heading to the sawmill, but the bad weather washed out the road, and we had to leave it right there in the swamp. The equipment's heavy\u2014we can't pull it out ourselves. Please help us finish the delivery and get the trailer to the destination.\",TASKS\nUS_18_01_SCOUT_DELIVERY_02_TASK,Tricky Descent,_TASKS,US_18,Chiapas,truckDelivery,,420,4350,\"I was cruising along the northern cliffs when I misjudged a turn. The car went off the trail and ended up at the bottom of the cliff. The wheels are intact, but there's no way it's getting out on its own. Please help me tow it back to the village\u2014I can't do it by myself.\",TASKS\nUS_18_01_WATER_DELIVERY_02_TASK,Standing Point,_TASKS,US_18,Chiapas,cargoDelivery,300\u00d7 Water; 2\u00d7 Bags,1110,13200,\"After working at the mouth of the cave for a week, we've discovered some unstable spots near the entrance. We urgently need cement and water to mortar it all over. It would be too dangerous to go further otherwise, because the ground keeps crumbling. Can you deliver everything to the camp?\",TASKS\nUS_18_01_FORREST_EYES_02_TASK,Free Passage,_TASKS,US_18,Chiapas,cargoDelivery,1\u00d7 Vehicles Spare Parts,450,5950,Having to go around this jungle every time is a real pain in the neck. But we could just chop the trees down to open up a shortcut. We don't need a road\u2014just clearing the area and making space for vehicles will be enough.,TASKS\nUS_18_01_FOR_AGES_03_CONTRACT,A Line in Concrete,_CONTRACTS,US_18,Chiapas,cargoDelivery,4\u00d7 Container Small,1220,13800,\"Hey! After the heavy swells, the port's piers have started sinking, and the water's creeping closer to our warehouses and equipment. We need tetrapods to reinforce the coastline. Without them, the harbor could end up seriously damaged.\",CONTRACTS\nUS_18_01_OLD_FRIEND_CONTRACT,The Return of a Giant,_CONTRACTS,US_18,Chiapas,truckDelivery,1\u00d7 Plane,1350,11500,\"Hey! An old plane has been found deep in the mangroves, a whole bomber that's falling apart. We need to carefully lift it and deliver it to the port to give this giant its rightful place in history.\",CONTRACTS\nUS_18_01_TREASURE_NATION_CONTRACT,Sky Cargo,_CONTRACTS,US_18,Chiapas,truckDelivery,,650,8300,We've finished the excavation phase and packed up all the valuable equipment and findings. They need to be delivered to the warehouse near the helipad for air transport. Please handle the delivery with care\u2014there are fragile tools and materials inside.,CONTRACTS\nUS_18_01_STAR_TRAILER_CONTRACT,Band Rider,_CONTRACTS,US_18,Chiapas,truckDelivery,,620,7100,\"We've got a festival coming up, so it'd be great if you could deliver a trailer for the band to spend the night in when they arrive.\",CONTRACTS\nUS_18_01_IMPROVED_COMMUNICATION_CONTRACT,Signal All Around,_CONTRACTS,US_18,Chiapas,cargoDelivery,1\u00d7 Metal Planks; 1\u00d7 Bags; 1\u00d7 Metal Planks; 1\u00d7 Metal Roll; 1\u00d7 Metal Roll; 2\u00d7 Bags,2480,23700,\"Hey buddy. Comm systems across the district have completely broken down after the rainy season. People are cut off\u2014can't make calls, can't get any news. We need supplies to reinforce comm towers, get the signal back, and reconnect everyone to the city.\",CONTRACTS\nUS_18_01_ENERGY_SUN_CONTRACT,Ray of Sunshine,_CONTRACTS,US_18,Chiapas,cargoDelivery,1\u00d7 Solar Panel; 1\u00d7 Barrels; 1\u00d7 Solar Panel; 1\u00d7 Solar Panel,1610,18250,\"Amigo, we're almost completely out of power here, but solar panels could turn things around. Can you haul them to the houses and workshops on the list so folks can have lights again and get back to work without any more blackouts?\",CONTRACTS\nUS_18_01_BANANAS_SCHEDULE_CONTRACT,Sprouts Reaching for the Sun,_CONTRACTS,US_18,Chiapas,cargoDelivery,1\u00d7 Logs Short; 3\u00d7 Fertilizer,1210,15650,\"Hey, listen! The farm's in bad shape: the crops are starting to turn yellow, and without fertilizer and fresh timber, we'll be waiting forever to get anything done. Do us a solid and bring everything to the farm\u2014let's get the land to provide us a bountiful harvest under this hot Mexican sun again!\",CONTRACTS\nUS_18_01_FIRST_RAYS_CONTRACT,Guardian of the Sea,_CONTRACTS,US_18,Chiapas,cargoDelivery,2\u00d7 Bricks; 2\u00d7 Bags 2,1180,14200,\"Hey, buddy! The old lighthouse has been sitting abandoned for ages, ever since the new one went up nearby. But after that shipwreck, they changed their minds. They want to bring the old one back. Can you haul the supplies out there so it can shine again and guide the ships to safety?\",CONTRACTS\nUS_18_01_WAREHOUSES_BUILDS_CONTRACT,Flooded Warehouses,_CONTRACTS,US_18,Chiapas,cargoDelivery,2\u00d7 Junk Open; 2\u00d7 Junk Closed,1660,14000,\"Hey there! So, part of the city ended up underwater after the rainy season, and the old warehouses are no longer usable. We need to get in there and tear them down\u2014there's still some salvageable stuff, but we have to move fast before the water carries it all away!\",CONTRACTS\nUS_18_01_VACATION_WILDERNESS_CONTRACT,The Call of the River,_CONTRACTS,US_18,Chiapas,cargoDelivery,2\u00d7 Crate Large; 4\u00d7 Service Spare Parts,910,8600,\"\u00a1Hola! The kayakers traversing the river are in a tight spot\u2014supplies are running low, the boats need gear, and the water's still churning like crazy after all the rain. Can you bring them what they need so they can keep paddling, train up, and get ready for their next runs?\",CONTRACTS\nUS_18_01_LOGS_CONTRACT,Coastal Festival,_CONTRACTS,US_18,Chiapas,cargoDelivery,2\u00d7 Logs Short,650,8600,\"\u00a1Hola! The festival preparations are in full swing down by the shore\u2014the stage is almost finished, the lights have been hung. The only thing missing is the wood for the bonfires and some wooden structures. We need the cargo delivered so the preparations can be wrapped up on time.\",CONTRACTS\nUS_18_01_DROPPED_CARGO_CONTRACT,Much Ado About Containers,_CONTRACTS,US_18,Chiapas,cargoDelivery,1\u00d7 Container Small Special; 1\u00d7 Container Reefer,780,7800,\"Listen. A ship capsized off the coast, and the cargo containers went overboard. Some sank, some washed up on the beach. We need to round them up and get them to where they're supposed to go. Without your help, that cargo's just going to be left scattered all over the place.\",CONTRACTS\nUS_18_01_ESPECIALLY_DANGEROUS_CONTRACT,Pumping the Waves,_CONTRACTS,US_18,Chiapas,cargoDelivery,1\u00d7 Npp Pump,500,4500,\"Hey, friend! That ship was carrying one of the important pumps for the fuel station, and we found it near the wreck. We need to pick it up and deliver it to the station, otherwise the fuel won't flow.\",CONTRACTS\nUS_18_01_NEW_PIPES_CONTRACT,Fuel Pipeline,_CONTRACTS,US_18,Chiapas,cargoDelivery,2\u00d7 Pipes Medium,560,3900,\"\u00a1Hola! There's trouble at the fuel plant: the station is idle without the pipes, leaving the whole town without fuel. We need them delivered ASAP. The roads are hot, the trucks are kicking up dust, but the plant can't start without your help.\",CONTRACTS\nUS_18_01_WATCHTOWERS_SUPPLY_TASK,Under the Mexican Sky,_TASKS,US_18,Chiapas,truckDelivery,1\u00d7 Crate Large; 1\u00d7 Crate Large,1070,11500,\"The sun beats down mercilessly here, and the towers are spaced far apart. There's a person manning each one, watching the horizon day and night. I'm asking you to help and get the supplies to the towers. It's important for the whole region.\",TASKS\nUS_18_01_TOWER_SUPPLY_TASK,Juicing Up,_TASKS,US_18,Chiapas,truckDelivery,2\u00d7 Service Spare Parts,860,10450,\"Our antenna is perched up high, but it's useless without a generator. All the necessary equipment\u2014the generator and a maintenance kit\u2014is at the port. Could you haul the cargo up to the antenna so we can get it running steadily again?\",TASKS\nUS_18_01_CROCODILE_FARM_TASK,Croccy Delivery,_TASKS,US_18,Chiapas,truckDelivery,2\u00d7 Bricks,840,9050,\"The crocodile farm is expanding, but the tools are stuck at the mine in a scouting trailer. We need to move it and deliver all the building supplies to the farm. Just make sure everything gets there in one piece\u2014the road can be tricky in places.\",TASKS\nUS_18_01_LIGHTHOUSE_SUPPLY_TASK,Sky High,_TASKS,US_18,Chiapas,truckDelivery,2\u00d7 Service Spare Parts,780,8500,\"The lighthouse stands high on the cliff, and every trip up there is a real trial. We need spare parts urgently\u2014they're in the trailer. You have to get it up top no matter how tough it gets. The lighthouse won't be able to function without those parts. We'll make it worth your while.\",TASKS\nUS_18_01_BEACH_SUPPLY_TASK,Coast Set Alight,_TASKS,US_18,Chiapas,truckDelivery,1\u00d7 Container Small,730,6700,\"Tonight, the coast is supposed to come alive with music, food, bonfires, and a fire show. There's an equipment container waiting at the port, and a trailer with fuel is stuck nearby. Please get everything delivered to the festival grounds.\",TASKS\nUS_18_01_HOUSE_ALONE_TASK,Supplying Vacations,_TASKS,US_18,Chiapas,truckDelivery,2\u00d7 Crate Large,660,5700,\"It's camping season now, and there are lots of people out and about. Some places are running out of water, others are short on camping gear. The roads are all unpaved and narrow, but we need the delivery to get through without any delays.\",TASKS\nUS_18_01_CHANGE_TRUCK_TASK,Fix and fuel up the truck,_TASKS,US_18,Chiapas,cargoDelivery,1\u00d7 Barrels Chemicals,980,9300,\"So here's the deal: we've got a vehicle stuck in the middle of the run, and that cargo needs to be delivered to the city yesterday. We want you to fix the truck, take the wheel, and finish the job. We'll be in trouble if that contract falls through.\",TASKS\nUS_18_01_HOTEL_SUPPLY_TASK,Heavy Load for Easy Living,_TASKS,US_18,Chiapas,cargoDelivery,1\u00d7 Container Large,820,7600,\"The thing is, our guests are already booking rooms, but the long container we need is still sitting in the port. It needs to be moved to the hotel\u2014the road gets narrow in some places, so you'll have to be careful. Without this cargo, we won't be able to finish getting things set up.\",TASKS\nUS_18_01_PONTOON_TASK,The Way of Water,_TASKS,US_18,Chiapas,cargoDelivery,3\u00d7 Wooden Planks,550,7100,\"Listen, the water levels are up after all the rain, so getting to the other side is a real problem now. We want to put together a pontoon crossing, but we don't have enough supplies. We need you to get us everything required so we can finally link the two banks.\",TASKS\nUS_18_01_FLOODED_TRASH_TASK,After the Rain,_TASKS,US_18,Chiapas,cargoDelivery,2\u00d7 Rubbish,630,6150,\"This place is our home. The water didn't spare it, but we're not giving up. We've gathered all the debris and prepared the containers for removal. This isn't just a job. It's helping people who want to get their lives back.\",TASKS\nUS_18_01_SWAMP_CONTEST,A Dash Across the Swamp,_CONTESTS,US_18,Chiapas,exploration,,150,2400,\"The mangrove swamps aren't meant for leisurely rides. We need to test how much time that route takes, as well as check the water levels and how stable the ground is. The sooner we get it done, the better for the whole team.\",CONTESTS\nUS_18_01_RACER_CONTEST,Roaring Among the Rocks,_CONTESTS,US_18,Chiapas,exploration,,60,1350,\"The route along the cliffs leaves no room for hesitation. On one side, there's a rock wall; on the other, a sharp drop. You need to cover the distance fast without losing control. This is a racing challenge at its finest\u2014all speed, precision, and nerve.\",CONTESTS\nUS_18_02_THINK_TWICE_01_CONTRACT,The Weak Link,_CONTRACTS,US_18,Chiapas,cargoDelivery,1\u00d7 Bags; 1\u00d7 Pipe Large; 1\u00d7 Crate Large,2230,23550,We could not start power generation at the station because we discovered damage to one pipe segment. The station will not be operational until we fix it. We need to deliver building materials and restore the main\u2014otherwise it cannot sustain the pressure.,CONTRACTS\nUS_18_02_BUILD_SUBSTATIONS_01_CONTRACT,Hot Stream,_CONTRACTS,US_18,Chiapas,cargoDelivery,1\u00d7 Pipes Small; 1\u00d7 Pipes Medium; 1\u00d7 Metal Roll; 1\u00d7 Bricks,1770,19100,\"The next step is to assemble the substation that will redirect the heat to the main building. The construction site is all set, but we cannot proceed without building materials. We need a professional who can help us complete this phase of the project.\",CONTRACTS\nUS_18_02_WORKHOUSE_01_CONTRACT,Cabins for the First Shift,_CONTRACTS,US_18,Chiapas,cargoDelivery,1\u00d7 Forklift Caravan Container 2; 1\u00d7 Forklift Caravan Container 4,1720,13350,\"Hola, friend! Construction is in full swing, but the crew have literally nowhere to live. They sleep in their vehicles and under makeshift tents. We need to deliver and install cabins to give our workers a place to sleep and rest after their shift.\",CONTRACTS\nUS_18_02_BUILD_SUBSTATIONS_02_CONTRACT,Heat Flow Line 2,_CONTRACTS,US_18,Chiapas,cargoDelivery,1\u00d7 Metal Roll; 2\u00d7 Bricks; 1\u00d7 Metal Planks; 1\u00d7 Pipes Small,2190,25650,\"We are starting to assemble the second substation. This is a major milestone; without it, the system cannot reach target capacity. The construction site is all set, the crew is ready, but we need materials and equipment. Will you take this contract?\",CONTRACTS\nUS_18_02_WORKHOUSE_02_CONTRACT,Cabins for the Second Shift,_CONTRACTS,US_18,Chiapas,cargoDelivery,1\u00d7 Forklift Caravan Container 2; 1\u00d7 Forklift Caravan Container 4,1810,14950,\"As I said, we need to provide accommodations for the second shift. Specialists cannot survive on coffee alone, they need a good night's sleep. Will you deliver more cabins?\",CONTRACTS\nUS_18_02_THINK_TWICE_02_CONTRACT,No Leaks for Launch,_CONTRACTS,US_18,Chiapas,cargoDelivery,1\u00d7 Pipe Large; 1\u00d7 Bags 2; 1\u00d7 Crate Large,1740,14900,We made a final check of the pipeline and discovered a deficiency. The station will not go live until this deficiency is dealt with. We need to deliver a set of materials and fix the damaged main.,CONTRACTS\nUS_18_02_BUILD_SUBSTATIONS_03_CONTRACT,Final Heat Flow Line,_CONTRACTS,US_18,Chiapas,cargoDelivery,1\u00d7 Pipes Small; 2\u00d7 Vehicles Spare Parts; 2\u00d7 Crate Large; 1\u00d7 Metal Roll,1820,19850,\"This is the final stage that concerns the transfer of heat to the main building. Two substations are already assembled. All we need to do now is to complete the circuit. The project has been greenlighted, the building site is ready, but we still need to deliver the materials.\",CONTRACTS\nUS_18_02_THINK_TWICE_03_CONTRACT,Final Check,_CONTRACTS,US_18,Chiapas,cargoDelivery,1\u00d7 Pipe Large; 1\u00d7 Metal Planks; 1\u00d7 Crate Large,2030,18150,Before the commissioning of the station we found a damaged main. This puts the launch schedule at risk. We need someone do deliver materials promptly to repair the pipeline.,CONTRACTS\nUS_18_02_5G_CONNECTION_TASK,The Local Voices,_TASKS,US_18,Chiapas,cargoDelivery,1\u00d7 Service Spare Parts; 1\u00d7 Solar Panel; 1\u00d7 Service Spare Parts; 1\u00d7 Solar Panel; 1\u00d7 Service Spare Parts; 1\u00d7 Service Spare Parts,1910,22550,Hello! There is no reception is some areas of the region. We are to install new antennas for wireless communication. We need someone to deliver all necessary supplies to the assembling site so that we can proceed promptly.,TASKS\nUS_18_02_TRANSFORMER_STATION_CONTRACT,High Voltage,_CONTRACTS,US_18,Chiapas,truckDelivery,1\u00d7 Transformer; 1\u00d7 Transformer,2030,19650,\"We started building the transformer station. This is a milestone of our project, and we need to complete it before we can move forward. We need assistance with delivering building materials and equipment to the construction site.\",CONTRACTS\nUS_18_02_ANTENNA_REPAIR_CONTRACT,A Voice from Outer Space,_CONTRACTS,US_18,Chiapas,truckDelivery,2\u00d7 Bags; 2\u00d7 Metal Roll; 2\u00d7 Bags 2,1850,17850,Good day to you. One of our radio telescopes significantly deteriorated. Its supports and some of the components need replacing. We kindly ask you to deliver these supplies to the telescope. Then we can do all required maintenance work and restore its functionality.,CONTRACTS\nUS_18_02_HELICOPTER_SUPPLY_CONTRACT,Open Skies Again,_CONTRACTS,US_18,Chiapas,truckDelivery,2\u00d7 Service Spare Parts; 2\u00d7 Barrels Oil,1480,17800,\"Hello! We need your assistance right away! Our helipad is temporarily out of service. We don't have the materials to repair it. If you deliver the supplies quicky, we'll be able to service the helipad and resume our cargo shipments.\",CONTRACTS\nUS_18_02_MAIN_CARGO_CONTRACT,\"Two Turbines, One Launch\",_CONTRACTS,US_18,Chiapas,truckDelivery,,1520,17050,\"We need new steam turbines to commission the station. One turbine is at the port, the other got stuck in transit. Your job is to pick up both turbines and deliver them to the site. Once it is done, the station can go live.\",CONTRACTS\nUS_18_02_FERTILIZERS_CONTRACT,Where the Grass Is Green,_CONTRACTS,US_18,Chiapas,truckDelivery,2600\u00d7 Water,1470,14750,\"Listen, we set up everything we need for the production of fertilizers. We installed the equipment and got our first orders. What we are still lacking are raw materials. Help us deliver the required supplies. We cannot wait to begin work in earnest.\",CONTRACTS\nUS_18_02_NEW_LAUNCH_CONTRACT,Concrete Rhythm,_CONTRACTS,US_18,Chiapas,truckDelivery,2\u00d7 Vehicles Spare Parts; 2\u00d7 Bags 2,1300,13550,\"After all the hiccups in production the plant came to a standstill. We are out of materials, some of the equipment needs replacement. We need you do deliver all the required supplies\u2014then we'll be back into production. We cannot manage without your assistance.\",CONTRACTS\nUS_18_02_UNFORTUNATE_TURN_CONTRACT,Tank You Very Much,_CONTRACTS,US_18,Chiapas,truckDelivery,,460,4600,\"The fueling station is ready to serve its first customers, but without fuel it's just an empty shell. The fuel tank is all set for delivery. This is a regular delivery, but the launch of this entire business depends on it.\",CONTRACTS\nUS_18_02_GEOTHERMAL_STATION_CONTRACT,The Heart of the Station,_CONTRACTS,US_18,Chiapas,cargoDelivery,1\u00d7 Bricks; 2\u00d7 Concrete Slab; 3\u00d7 Bags 2; 2\u00d7 Bags; 2\u00d7 Metal Planks,3220,29100,\"This is the final construction stage of the station building proper. Our job is to complete the building frame, install panels, get the maintenance rooms up and operational. The scope of work is large, but also expected for this type of projects.\",CONTRACTS\nUS_18_02_TRIUMPHAL_ARCHES_CONTRACT,Triumphal Arches,_CONTRACTS,US_18,Chiapas,cargoDelivery,1\u00d7 Pipe Large; 1\u00d7 Pipe Large; 1\u00d7 Pipe Large,3290,25900,\"We've got three breaks on the main pipeline routes, and we need the pipelines to arch over those roads so they don't block traffic. Naturally, building pipelines requires pipes. The crew's already been briefed on the job. So, ready to get started?\",CONTRACTS\nUS_18_02_DRILLING_HELP_CONTRACT,Mining Relaunch,_CONTRACTS,US_18,Chiapas,cargoDelivery,1\u00d7 Container Large Drilling; 2\u00d7 Barrels Oil,1560,17700,\"Work at the quarry came to a standstill a few days ago. The equipment is worn out, and there is almost no fuel left. We cannot resume sand mining in these conditions. We need new equipment and fuel. This issue is critical and we rely on your assistance.\",CONTRACTS\nUS_18_02_PIPE_DISTRIBUTOR_CONTRACT,Steel Arteries,_CONTRACTS,US_18,Chiapas,cargoDelivery,2\u00d7 Bags; 1\u00d7 Metal Roll; 1\u00d7 Pipes Small,1230,12650,\"Listen, compadre, we need pipes at the distribution hub, and we need them now. They are not just pieces of metal, they are the future arteries of the entire power station. Without them, the system cannot function properly. Deliver the pipes in good condition and on time.\",CONTRACTS\nUS_18_02_VILLAGE_SCOUTING_CONTRACT,A Test of Strength,_CONTRACTS,US_18,Chiapas,exploration,,690,11950,\"Local administration is checking up on villages following a series of earth tremors. Follow the route and make sure that residential buildings are intact. Even though this is a scheduled inspection and not an emergency, it is still very important.\",CONTRACTS\nUS_18_02_SEISMIC_EXPLORATION_CONTRACT,Subterranean Pulse,_CONTRACTS,US_18,Chiapas,exploration,,650,11200,\"We are researching rifts in this area, and we need special calibrated equipment for the job. It is important that you follow the route and stop at checkpoints. The reliability of our data depends on your precision.\",CONTRACTS\nUS_18_02_BEACH_TRAILER_TASK,The Trailer that Decided to Stay,_TASKS,US_18,Chiapas,truckDelivery,,550,6500,\"Hola, amigo, I have a bit of a situation. I was towing a trailer down an old road, which the rain turned into liquid mud. I managed to get the truck out, but the trailer got stuck. Do you think you could reach it and try to salvage it?\",TASKS\nUS_18_02_PORTABLE_RADAR_TASK,Beyond The Horizon,_TASKS,US_18,Chiapas,truckDelivery,,530,6250,\"We need to extend cellular network coverage in a remote area of the region. We plant to use a portable radar, which will service the current blind zone, but it needs to be delivered to its installation location. As I said, the are is remote, and the roads are quite bad. Will you accept this contract?\",TASKS\nUS_18_02_CURSED_ISLAND_TASK,Caught in the Tide,_TASKS,US_18,Chiapas,truckDelivery,,520,5700,\"Listen, I did something stupid. I drove to the island. Everything was fine. But then I didn't notice how quickly the tide came. I couldn't drive back\u2014my car was surrounded by water. I need you to tow it to the sawmill. Can you do it for me?\",TASKS\nUS_18_02_RACING_TRUCK_TASK,A Sandy Error,_TASKS,US_18,Chiapas,truckDelivery,,610,5400,\"I entered the local sand rift race\u2014but I wish I didn't. At one point my truck drifted and hit a pile of rocks. I made it out, but the car is still there. I'm asking you to tow it\u2014I cannot do it alone.\",TASKS\nUS_18_02_GARDEN_TRAILER_TASK,Deliver to Prickles,_TASKS,US_18,Chiapas,truckDelivery,,430,4900,\"Hey, amigo! We need a hand at the farm. We are expecting a scout trailer with some machinery. The road is narrow and rocky, and the sun saps all our strength. If you can, bring the trailer and make the farmers happy.\",TASKS\nUS_18_02_CAR_FISHING_TASK,A Wet Story,_TASKS,US_18,Chiapas,truckDelivery,,550,4750,\"Hi. We have an issue. The river swelled with the rain, but we thought we could cross it using the stone ford. Bad idea! Our vehicle slipped and fell into the water. No one was hurt, but we cannot recover our car. Could you pull it out and tow it to the camp?\",TASKS\nUS_18_02_POOR_LOGISTICS_TASK,A Trapped Five-Seater,_TASKS,US_18,Chiapas,truckDelivery,,390,3700,\"Hello, friend! I don't know who mapped this route, but it's not suited for hauling a trailer like this. Do you think you could tow it back to the Geothermal Station? I'm done with this job, but I wish you luck.\",TASKS\nUS_18_02_LOCAL_HOUSES_TASK,Building a Home,_TASKS,US_18,Chiapas,cargoDelivery,1\u00d7 Bags 2; 1\u00d7 Cellulose; 1\u00d7 Bags 2; 1\u00d7 Cellulose; 1\u00d7 Bags 2; 1\u00d7 Cellulose,2410,27350,\"Hello! I hate to ask, but we cannot manage on our own. We started building homes for the locals, but ran out of materials. If you can bring us supplies, we can finish construction.\",TASKS\nUS_18_02_WATER_TOWER_TASK,The Last Drop,_TASKS,US_18,Chiapas,cargoDelivery,1\u00d7 Cellulose; 1\u00d7 Npp Pump; 1\u00d7 Pipes Medium,1750,19850,\"The water tower is an poor state. The metal is rusty, mechanisms need replacement. If it is not fixed in time, the city could lose its water supply. We need building materials. Will you help us make this system operational again?\",TASKS\nUS_18_02_WATER_FOR_LOCALS_TASK,Drop after Drop,_TASKS,US_18,Chiapas,cargoDelivery,1000\u00d7 Water; 1200\u00d7 Water; 800\u00d7 Water,1390,12900,\"Hello, my friend! The monsoon made the roads impassable again. The truck that was supposed to deliver clean water to the villages never arrived. Our supplies run low. Deliver water to the villages in this area, if you can. The locals have been waiting for help for a while.\",TASKS\nUS_18_02_DANGER_CONTAMINATION_TASK,A Washed-Out Formula,_TASKS,US_18,Chiapas,cargoDelivery,3\u00d7 Barrels Chemicals,890,9750,\"Hello! Something happened. We were expecting a shipment of reagents for the lab, but the monsoon made fording the river impossible. Our truck got stuck, and some of the containers were washed away. We need you to recover them for the lab.\",TASKS\nUS_18_02_FOREST_CUTTING_TASK,Green Cut,_TASKS,US_18,Chiapas,cargoDelivery,3\u00d7 Service Spare Parts,730,9700,\"Listen, me and the boys decided to clean the forest a bit. We could carve a shortcut through one of the areas, but we cannot do it without special equipment. Deliver everything we need for tree-cutting and road-building.\",TASKS\nUS_18_02_BANANA_GROWTH_TASK,Green Lines,_TASKS,US_18,Chiapas,cargoDelivery,2\u00d7 Fertilizer; 2\u00d7 Fertilizer,890,8150,\"Will you give me a hand, amigo? The banana farm is growing, but the soil is depleted. If we don't fertilize it now, the yield will be poor. The fields are prepped\u2014all we need is the shipment of fertilizers. Can you deliver it to the farm?\",TASKS\nUS_18_02_CELLULOSE_FOR_SCIENCE_TASK,Yes to Sampling,_TASKS,US_18,Chiapas,cargoDelivery,4\u00d7 Cellulose,530,6250,\"Hello. We need a sample of cellulose for lab tests and analysis. The shipment is already in the area. We need you to do the final leg of the trip. Until the sample arrives, we cannot proceed to the final stage of our research. We rely on you for assistance.\",TASKS\nUS_18_02_NEW_TRUCK_TASK,Back to Work,_TASKS,US_18,Chiapas,exploration,,580,5650,\"We couldn't wait for this truck to arrive. New and powerful, it was supposed to make work at the station more efficient. And then it broke in transit! Just looking at it hurts. Can you repair it and bring it here, to its rightful place?\",TASKS\nUS_18_02_SANDY_ROCKS_CONTEST,Sand Fury,_CONTESTS,US_18,Chiapas,exploration,,60,900,\"Are you ready for real speed, friend? The path that winds through this rocky terrain is narrow and treacherous. Let's see how long it will take you to cross the finish line. Try to beat the time. Every second counts. Prove that you have what it takes!\",CONTESTS\nUS_18_02_INDICATORS_CHECK_START,Hot Data,_CONTESTS,US_18,Chiapas,exploration,,50,750,\"Friend, we call on your speed and diligence. We need to collect data from indicators around the hot spring. The road is rocky and winding. Every second counts.\",CONTESTS\n";

const state = {
  main: null,
  common: null,
  selectors: {
    missions: null,
    contests: null,
    regions: null,
    upgrades: null,
    watchtowers: null,
    discoveries: null,
    levels: null,
    garages: null,
  },
  gameStats: {
    distanceInputs: new Map(),
    statInputs: new Map(),
  },
  ui: {
    rankXpSyncLock: false,
    showLegacyRegionActions: false,
  },
  objectives: {
    data: null,
    selected: new Set(),
    visibleKeys: [],
    finishedInSave: new Set(),
    catalogKeys: new Set(),
    catalogNames: {},
    catalogMeta: {},
    catalogSource: "",
    catalogLoaded: false,
    catalogLoading: false,
    catalogWarmupScheduled: false,
    catalogLoadPromise: null,
  },
  rules: {
    controls: new Map(),
  },
  regions: {
    featureControls: new Map(),
  },
  folder: {
    loaded: false,
    rootName: "",
    sourceKind: "standard",
    files: new Map(),
    rawFiles: new Map(),
    wgsBlobByLogicalKey: new Map(),
  },
  improveShare: {
    uploading: false,
    lastUploadedSignature: "",
    preferredSource: "",
  },
  fog: {
    brushValue: 255,
    brushSize: 8,
    currentKey: "",
    currentName: "",
    currentBytes: null,
    currentPixels: null,
    currentWidth: 0,
    currentHeight: 0,
    currentFooter: new Uint8Array(0),
    currentZlibOffset: 0,
    currentZlibLength: 0,
    currentZlibHeader: new Uint8Array([0x78, 0x9c]),
    drawing: false,
    lastX: 0,
    lastY: 0,
    viewScale: 1,
    viewOffsetX: 0,
    viewOffsetY: 0,
    seasonChecks: new Map(),
  },
  trialChecks: new Map(),
};

const els = {
  status: document.getElementById("status"),
  mainInput: document.getElementById("main-file-input"),
  commonInput: document.getElementById("common-file-input"),
  singleInput: document.getElementById("single-file-input"),
  folderInput: document.getElementById("folder-input"),
  folderUploadBtn: document.getElementById("folder-upload-btn"),
  singleUploadBtn: document.getElementById("single-upload-btn"),
  improveShareCheckbox: document.getElementById("improve-share-checkbox"),
  improveShareMeta: document.getElementById("improve-share-meta"),
  mainMeta: document.getElementById("main-meta"),
  commonMeta: document.getElementById("common-meta"),
  folderMeta: document.getElementById("folder-meta"),
  folderMainChoices: document.getElementById("folder-main-choices"),
  downloadMainBtn: document.getElementById("download-main-btn"),
  downloadCommonBtn: document.getElementById("download-common-btn"),
  downloadFolderBtn: document.getElementById("download-folder-btn"),
  tabNav: document.getElementById("tab-nav"),
  refreshGameStatsBtn: document.getElementById("refresh-game-stats-btn"),
  saveGameStatsBtn: document.getElementById("save-game-stats-btn"),
  gameStatsInfo: document.getElementById("game-stats-info"),
  distanceList: document.getElementById("distance-list"),
  gameStatList: document.getElementById("game-stat-list"),
  moneyInput: document.getElementById("money-input"),
  rankInput: document.getElementById("rank-input"),
  xpInput: document.getElementById("xp-input"),
  rankXpHelpBtn: document.getElementById("rank-xp-help-btn"),
  rankXpTable: document.getElementById("rank-xp-table"),
  timeDayInput: document.getElementById("time-day-input"),
  timeNightInput: document.getElementById("time-night-input"),
  skipTimeInput: document.getElementById("skip-time-input"),
  applyMoneyRankBtn: document.getElementById("apply-money-rank-btn"),
  applyTimeBtn: document.getElementById("apply-time-btn"),
  applyMissionsBtn: document.getElementById("apply-missions-btn"),
  applyContestsBtn: document.getElementById("apply-contests-btn"),
  objectivesSearch: document.getElementById("objectives-search"),
  objectivesType: document.getElementById("objectives-type"),
  objectivesRegion: document.getElementById("objectives-region"),
  objectivesCategory: document.getElementById("objectives-category"),
  refreshObjectivesBtn: document.getElementById("refresh-objectives-btn"),
  selectObjectivesBtn: document.getElementById("select-objectives-btn"),
  clearObjectivesBtn: document.getElementById("clear-objectives-btn"),
  completeObjectivesBtn: document.getElementById("complete-objectives-btn"),
  objectivesInfo: document.getElementById("objectives-info"),
  objectivesList: document.getElementById("objectives-list"),
  regionsCheckAllBtn: document.getElementById("regions-check-all-btn"),
  regionsClearBtn: document.getElementById("regions-clear-btn"),
  regionsFeatureList: document.getElementById("regions-feature-list"),
  regionsInfo: document.getElementById("regions-info"),
  regionsShowLegacy: document.getElementById("regions-show-legacy"),
  applyRegionsBtn: document.getElementById("apply-regions-btn"),
  unlockWatchBtn: document.getElementById("unlock-watchtowers-btn"),
  unlockDiscoveriesBtn: document.getElementById("unlock-discoveries-btn"),
  unlockLevelsBtn: document.getElementById("unlock-levels-btn"),
  unlockGaragesBtn: document.getElementById("unlock-garages-btn"),
  unlockUpgradesBtn: document.getElementById("unlock-upgrades-btn"),
  garageUpgradeAll: document.getElementById("garage-upgrade-all"),
  rulesEditor: document.getElementById("rules-editor"),
  rulesRandom: document.getElementById("rules-random"),
  rulesCustom: document.getElementById("rules-custom"),
  applyRulesBtn: document.getElementById("apply-rules-btn"),
  trialsList: document.getElementById("trials-list"),
  trialsSelectAllBtn: document.getElementById("trials-select-all-btn"),
  trialsClearBtn: document.getElementById("trials-clear-btn"),
  saveTrialsBtn: document.getElementById("save-trials-btn"),
  prosRegistration: document.getElementById("pros-registration"),
  prosRoadcraft: document.getElementById("pros-roadcraft"),
  saveProsBtn: document.getElementById("save-pros-btn"),
  achievementsStats: document.getElementById("achievements-stats"),
  unlockAchievementsBtn: document.getElementById("unlock-achievements-btn"),
  fogFileSelect: document.getElementById("fog-file-select"),
  fogBrushSize: document.getElementById("fog-brush-size"),
  fogBrushColor: document.getElementById("fog-brush-color"),
  fogOpenBtn: document.getElementById("fog-open-btn"),
  fogSaveBtn: document.getElementById("fog-save-btn"),
  fogDownloadCurrentBtn: document.getElementById("fog-download-current-btn"),
  fogEditorInfo: document.getElementById("fog-editor-info"),
  fogCanvas: document.getElementById("fog-canvas"),
  fogSlotSelect: document.getElementById("fog-slot-select"),
  fogFilterMode: document.getElementById("fog-filter-mode"),
  fogExtraSeasons: document.getElementById("fog-extra-seasons"),
  fogRegionFilters: document.getElementById("fog-region-filters"),
  fogCoverBtn: document.getElementById("fog-cover-btn"),
  fogUncoverBtn: document.getElementById("fog-uncover-btn"),
  fogAutoInfo: document.getElementById("fog-auto-info"),
};

function init() {
  state.selectors.missions = buildSeasonMapSelector(document.getElementById("missions-selector"), "missions");
  state.selectors.contests = buildSeasonMapSelector(document.getElementById("contests-selector"), "contests");
  state.selectors.regions = buildRegionSelector(document.getElementById("regions-selector"), "regions");
  state.selectors.upgrades = buildRegionSelector(document.getElementById("upgrades-selector"), "upgrades");
  state.selectors.watchtowers = buildRegionSelector(document.getElementById("watchtowers-selector"), "watchtowers");
  state.selectors.discoveries = buildRegionSelector(document.getElementById("discoveries-selector"), "discoveries");
  state.selectors.levels = buildRegionSelector(document.getElementById("levels-selector"), "levels");
  state.selectors.garages = buildRegionSelector(document.getElementById("garages-selector"), "garages");
  buildRegionsFeatureEditor();
  onRegionsLegacyToggleChanged();
  buildRulesEditor();
  renderRankXpTable();
  scheduleObjectivesCatalogWarmup();
  renderTrialsList();
  initFogTools();
  bindUi();
  updateMainMeta();
  updateCommonMeta();
  updateImproveShareMeta();
  updateFolderMeta();
  refreshFolderMainChoices();
  updateMainSummary();
  refreshCommonTabs();
  refreshFogFileList();
  renderFogCanvas();
  checkRuntimeDependencies();
  scheduleAnalyticsInit();
  updateDownloadButtons();
}

function bindUi() {
  if (els.folderUploadBtn && els.folderInput) {
    els.folderUploadBtn.addEventListener("click", () => els.folderInput.click());
  }
  if (els.singleUploadBtn && els.singleInput) {
    els.singleUploadBtn.addEventListener("click", () => els.singleInput.click());
  }
  if (els.mainInput) {
    els.mainInput.addEventListener("change", onMainFileSelected);
  }
  if (els.commonInput) {
    els.commonInput.addEventListener("change", onCommonFileSelected);
  }
  if (els.folderInput) {
    els.folderInput.addEventListener("change", onFolderSelected);
  }
  if (els.improveShareCheckbox) {
    els.improveShareCheckbox.addEventListener("change", onImproveShareCheckboxChanged);
  }
  if (els.singleInput) {
    els.singleInput.addEventListener("change", onSingleFileSelected);
  }
  els.downloadMainBtn.addEventListener("click", downloadMainFile);
  els.downloadCommonBtn.addEventListener("click", downloadCommonFile);
  els.downloadFolderBtn.addEventListener("click", downloadFolderZip);

  els.tabNav.addEventListener("click", (event) => {
    const button = event.target.closest(".tab-btn");
    if (!button) {
      return;
    }
    activateTab(button.dataset.tab);
  });

  els.refreshGameStatsBtn.addEventListener("click", refreshGameStatsEditor);
  els.saveGameStatsBtn.addEventListener("click", saveGameStatsToMain);
  els.rankInput.addEventListener("input", syncXpFromRankInput);
  els.xpInput.addEventListener("input", syncRankFromXpInput);
  els.applyMoneyRankBtn.addEventListener("click", onApplyMoneyRank);
  els.applyTimeBtn.addEventListener("click", onApplyTime);
  els.applyMissionsBtn.addEventListener("click", onApplyMissions);
  els.applyContestsBtn.addEventListener("click", onApplyContests);
  els.unlockWatchBtn.addEventListener("click", () => onApplyRegionWithSelector(state.selectors.watchtowers, unlockWatchtowers, "Watchtowers updated."));
  els.unlockDiscoveriesBtn.addEventListener("click", () => onApplyRegionWithSelector(state.selectors.discoveries, unlockDiscoveries, "Discoveries updated."));
  els.unlockLevelsBtn.addEventListener("click", () => onApplyRegionWithSelector(state.selectors.levels, unlockLevels, "Levels updated."));
  els.unlockGaragesBtn.addEventListener("click", () => onApplyRegionWithSelector(state.selectors.garages, (text, regions) => unlockGarages(text, regions, Boolean(els.garageUpgradeAll.checked)), "Garages updated."));
  els.unlockUpgradesBtn.addEventListener("click", () => onApplyRegionWithSelector(state.selectors.upgrades, unlockUpgrades, "Upgrades updated."));
  els.refreshObjectivesBtn.addEventListener("click", refreshObjectivesFromMain);
  els.selectObjectivesBtn.addEventListener("click", selectVisibleObjectives);
  els.clearObjectivesBtn.addEventListener("click", clearObjectiveSelection);
  els.completeObjectivesBtn.addEventListener("click", completeSelectedObjectives);
  els.objectivesSearch.addEventListener("input", renderObjectivesList);
  els.objectivesType.addEventListener("change", renderObjectivesList);
  els.objectivesRegion.addEventListener("change", renderObjectivesList);
  els.objectivesCategory.addEventListener("change", renderObjectivesList);
  if (els.regionsCheckAllBtn) {
    els.regionsCheckAllBtn.addEventListener("click", () => setAllRegionsFeatures(true));
  }
  if (els.regionsClearBtn) {
    els.regionsClearBtn.addEventListener("click", () => setAllRegionsFeatures(false));
  }
  if (els.regionsShowLegacy) {
    els.regionsShowLegacy.addEventListener("change", onRegionsLegacyToggleChanged);
  }
  if (els.applyRegionsBtn) {
    els.applyRegionsBtn.addEventListener("click", onApplyRegions);
  }
  if (els.rulesRandom) {
    els.rulesRandom.addEventListener("change", onRulesRandomToggleChanged);
  }
  if (els.rulesCustom) {
    els.rulesCustom.addEventListener("change", onRulesCustomToggleChanged);
  }
  els.applyRulesBtn.addEventListener("click", onApplyRules);

  els.trialsSelectAllBtn.addEventListener("click", () => setAllTrials(true));
  els.trialsClearBtn.addEventListener("click", () => setAllTrials(false));
  els.saveTrialsBtn.addEventListener("click", onSaveTrials);
  els.saveProsBtn.addEventListener("click", onSavePros);
  els.unlockAchievementsBtn.addEventListener("click", onUnlockAchievements);

  if (els.fogBrushColor) {
    els.fogBrushColor.addEventListener("change", onFogBrushChanged);
  }
  if (els.fogBrushSize) {
    els.fogBrushSize.addEventListener("change", onFogBrushChanged);
  }
  bindAsyncUiAction(els.fogOpenBtn, "Open fog file", onOpenSelectedFogFile);
  bindAsyncUiAction(els.fogSaveBtn, "Save fog file", onSaveCurrentFogToFolder);
  bindAsyncUiAction(els.fogDownloadCurrentBtn, "Download fog file", downloadCurrentFogFile);
  bindAsyncUiAction(els.fogFileSelect, "Open fog file", onOpenSelectedFogFile, "change");
  bindAsyncUiAction(els.fogCoverBtn, "Fog automation cover", () => runFogAutomation("cover"));
  bindAsyncUiAction(els.fogUncoverBtn, "Fog automation uncover", () => runFogAutomation("uncover"));

  const fogCanvas = els.fogCanvas;
  if (fogCanvas) {
    fogCanvas.addEventListener("pointerdown", onFogPointerDown);
    fogCanvas.addEventListener("pointermove", onFogPointerMove);
    fogCanvas.addEventListener("pointerup", onFogPointerUp);
    fogCanvas.addEventListener("pointerleave", onFogPointerUp);
    fogCanvas.addEventListener("pointercancel", onFogPointerUp);
  }
  window.addEventListener("resize", renderFogCanvas);
}

function bindAsyncUiAction(element, label, handler, eventName = "click") {
  if (!element || typeof element.addEventListener !== "function") {
    return;
  }
  element.addEventListener(eventName, (event) => {
    Promise.resolve()
      .then(() => handler(event))
      .catch((error) => {
        const message = error && error.message ? error.message : String(error || "Unknown error");
        setStatus(`${label} failed: ${message}`, "error");
        if (els.fogAutoInfo && /fog/i.test(label)) {
          els.fogAutoInfo.textContent = `${label} failed: ${message}`;
        }
      });
  });
}

function checkRuntimeDependencies() {
  // Heavy libraries are loaded on demand (fog tools / folder zip).
}

function isVendorReady(vendor) {
  if (vendor === "pako") {
    return Boolean(window.pako && typeof window.pako.Inflate === "function");
  }
  if (vendor === "jszip") {
    return Boolean(window.JSZip);
  }
  return false;
}

function loadVendorScript(vendor) {
  const src = VENDOR_SCRIPT_URLS[vendor];
  if (!src) {
    return Promise.reject(new Error(`Unknown vendor: ${vendor}`));
  }
  if (isVendorReady(vendor)) {
    return Promise.resolve();
  }
  if (vendorLoadPromises[vendor]) {
    return vendorLoadPromises[vendor];
  }
  vendorLoadPromises[vendor] = new Promise((resolve, reject) => {
    const script = document.createElement("script");
    script.src = src;
    script.async = true;
    script.onload = () => {
      if (isVendorReady(vendor)) {
        resolve();
      } else {
        reject(new Error(`${vendor} loaded but is not available.`));
      }
    };
    script.onerror = () => reject(new Error(`Failed to load ${vendor} runtime.`));
    document.head.append(script);
  }).finally(() => {
    vendorLoadPromises[vendor] = null;
  });
  return vendorLoadPromises[vendor];
}

async function ensurePakoLoaded() {
  if (isVendorReady("pako")) {
    return true;
  }
  try {
    await loadVendorScript("pako");
    return isVendorReady("pako");
  } catch (_error) {
    return false;
  }
}

async function ensureJsZipLoaded() {
  if (isVendorReady("jszip")) {
    return true;
  }
  try {
    await loadVendorScript("jszip");
    return isVendorReady("jszip");
  } catch (_error) {
    return false;
  }
}

function scheduleAnalyticsInit() {
  if (!GA_MEASUREMENT_ID || typeof window === "undefined") {
    return;
  }
  const init = () => {
    if (window.__analyticsInitialized) {
      return;
    }
    window.__analyticsInitialized = true;
    window.dataLayer = window.dataLayer || [];
    window.gtag = window.gtag || function gtag() {
      window.dataLayer.push(arguments);
    };
    window.gtag("js", new Date());
    window.gtag("config", GA_MEASUREMENT_ID);
    const script = document.createElement("script");
    script.async = true;
    script.src = `https://www.googletagmanager.com/gtag/js?id=${encodeURIComponent(GA_MEASUREMENT_ID)}`;
    document.head.append(script);
  };
  if (typeof window.requestIdleCallback === "function") {
    window.requestIdleCallback(init, { timeout: 4000 });
  } else {
    window.setTimeout(init, 2500);
  }
}

async function onMainFileSelected() {
  const file = els.mainInput.files && els.mainInput.files[0];
  if (!file) {
    return;
  }
  try {
    const text = await file.text();
    setMainFromText(file.name, text, null);
    setStatus(buildMainLoadStatus(`Loaded main save: ${file.name}`), "success");
  } catch (error) {
    setStatus(`Failed to read main save: ${error.message}`, "error");
  } finally {
    els.mainInput.value = "";
  }
}

async function onCommonFileSelected() {
  const file = els.commonInput.files && els.commonInput.files[0];
  if (!file) {
    return;
  }
  try {
    const text = await file.text();
    setCommonFromText(file.name, text, null);
    setStatus(`Loaded CommonSslSave: ${file.name}`, "success");
  } catch (error) {
    setStatus(`Failed to read CommonSslSave: ${error.message}`, "error");
  } finally {
    els.commonInput.value = "";
  }
}

async function onSingleFileSelected() {
  const file = els.singleInput.files && els.singleInput.files[0];
  if (!file) {
    return;
  }
  try {
    const bytes = new Uint8Array(await file.arrayBuffer());
    const text = decodeBytesToText(bytes);
    const role = detectSingleFileRole(file.name, text);
    if (role === "common") {
      setCommonFromText(file.name, text, null);
      setStatus(`Loaded single file as CommonSslSave: ${file.name}`, "success");
    } else {
      setMainFromText(file.name, text, null);
      setStatus(buildMainLoadStatus(`Loaded single file as Main Save: ${file.name}`), "success");
    }
    state.improveShare.preferredSource = "single";
    state.improveShare.lastUploadedSignature = "";
    if (els.improveShareCheckbox && els.improveShareCheckbox.checked) {
      const standaloneEntries = getStandaloneLoadedEntriesForImproveShare();
      await maybeUploadImproveSamples(standaloneEntries.length > 0 ? standaloneEntries : [{
        key: String(file.name || "single-file").toLowerCase(),
        relPath: String(file.name || "single-file"),
        name: String(file.name || "single-file"),
        bytes,
        dirty: false,
      }]);
    }
  } catch (error) {
    setStatus(`Failed to read single file: ${error.message}`, "error");
  } finally {
    els.singleInput.value = "";
  }
}

function normalizeRelativePath(path) {
  return String(path || "").replace(/\\/g, "/").replace(/^\/+|\/+$/g, "");
}

function joinRelativePath(...parts) {
  return parts
    .map((part) => normalizeRelativePath(part))
    .filter(Boolean)
    .join("/");
}

function stripFolderRoot(relativePath, rootName) {
  const normalized = normalizeRelativePath(relativePath);
  const root = normalizeRelativePath(rootName);
  if (!root) {
    return normalized;
  }
  const lower = normalized.toLowerCase();
  const rootLower = root.toLowerCase();
  if (lower === rootLower) {
    return "";
  }
  if (lower.startsWith(`${rootLower}/`)) {
    return normalized.slice(root.length + 1);
  }
  return normalized;
}

function isSameOrChildRelative(path, parent) {
  const normalizedPath = normalizeRelativePath(path).toLowerCase();
  const normalizedParent = normalizeRelativePath(parent).toLowerCase();
  if (!normalizedParent) {
    return true;
  }
  return normalizedPath === normalizedParent || normalizedPath.startsWith(`${normalizedParent}/`);
}

function buildUploadedFolderEntries(files) {
  const relativePaths = (files || []).map((file) => String(file.webkitRelativePath || file.name || ""));
  const rootName = detectFolderRoot(relativePaths);
  return Promise.all((files || []).map(async (file) => {
    const relPath = normalizeRelativePath(file.webkitRelativePath || file.name || "");
    const bytes = new Uint8Array(await file.arrayBuffer());
    const relPathNoRoot = stripFolderRoot(relPath, rootName);
    return {
      key: String(relPathNoRoot || relPath || file.name).toLowerCase(),
      relPath,
      relPathNoRoot,
      name: getFileBasename(relPath || file.name),
      bytes,
      dirty: false,
    };
  })).then((entries) => ({ rootName, entries }));
}

function containsWgsMarkers(entries) {
  return (entries || []).some((entry) => {
    const lower = String(entry && entry.name ? entry.name : "").toLowerCase();
    return lower === "containers.index" || lower.startsWith("container.");
  });
}

function buildRawEntryLookup(entries) {
  const out = new Map();
  for (const entry of entries || []) {
    const key = String(entry && (entry.relPathNoRoot || entry.relPath || entry.name) ? (entry.relPathNoRoot || entry.relPath || entry.name) : "").toLowerCase();
    if (!key || out.has(key)) {
      continue;
    }
    out.set(key, entry);
  }
  return out;
}

function findRawEntry(entriesMap, relativePath) {
  if (!(entriesMap instanceof Map)) {
    return null;
  }
  return entriesMap.get(String(normalizeRelativePath(relativePath)).toLowerCase()) || null;
}

function discoverWgsCandidates(entries) {
  const userDirs = new Set();
  const containerDirs = new Set();
  for (const entry of entries || []) {
    const rel = normalizeRelativePath(entry && entry.relPathNoRoot ? entry.relPathNoRoot : "");
    const base = getFileBasename(rel).toLowerCase();
    const dir = normalizeRelativePath(rel.slice(0, Math.max(0, rel.length - base.length)).replace(/\/$/, ""));
    if (base === "containers.index") {
      userDirs.add(dir);
    } else if (base.startsWith("container.")) {
      containerDirs.add(dir);
    }
  }
  const userList = [...userDirs];
  if (userList.length > 0) {
    return userList.map((path) => ({ kind: "user_dir", path }));
  }
  const filteredContainers = [...containerDirs].filter((dir) => !userList.some((userDir) => isSameOrChildRelative(dir, userDir)));
  return filteredContainers.map((path) => ({ kind: "container_dir", path }));
}

function readUint32LE(view, offset) {
  return view.getUint32(offset, true);
}

function readInt32LE(view, offset) {
  return view.getInt32(offset, true);
}

function bytesToHex(bytes) {
  return Array.from(bytes || [], (value) => value.toString(16).padStart(2, "0")).join("").toUpperCase();
}

function readUtf16LeString(view, cursor, fixedChars = null) {
  let charCount = fixedChars;
  if (charCount == null) {
    if (cursor.offset + 4 > view.byteLength) {
      throw new Error("Unexpected end of WGS data while reading string length.");
    }
    charCount = readInt32LE(view, cursor.offset);
    cursor.offset += 4;
  }
  const byteLength = Math.max(0, Number(charCount) || 0) * 2;
  if (cursor.offset + byteLength > view.byteLength) {
    throw new Error("Unexpected end of WGS data while reading UTF-16 string.");
  }
  const bytes = new Uint8Array(view.buffer, view.byteOffset + cursor.offset, byteLength);
  cursor.offset += byteLength;
  return new TextDecoder("utf-16le").decode(bytes).replace(/\0+$/g, "");
}

function readFiletime(view, cursor) {
  if (cursor.offset + 8 > view.byteLength) {
    throw new Error("Unexpected end of WGS data while reading FILETIME.");
  }
  cursor.offset += 8;
}

function readGuidHexLe(view, cursor) {
  if (cursor.offset + 16 > view.byteLength) {
    throw new Error("Unexpected end of WGS data while reading GUID.");
  }
  const raw = new Uint8Array(view.buffer, view.byteOffset + cursor.offset, 16);
  cursor.offset += 16;
  const reordered = new Uint8Array(16);
  reordered.set([raw[3], raw[2], raw[1], raw[0]], 0);
  reordered.set([raw[5], raw[4]], 4);
  reordered.set([raw[7], raw[6]], 6);
  reordered.set(raw.subarray(8, 16), 8);
  return bytesToHex(reordered);
}

function pickWgsBlobRelativePath(entriesMap, containerDir, guidA, guidB) {
  const pathA = joinRelativePath(containerDir, guidA);
  const pathB = joinRelativePath(containerDir, guidB);
  const entryA = findRawEntry(entriesMap, pathA);
  const entryB = findRawEntry(entriesMap, pathB);
  if (guidA === guidB) {
    return entryA ? pathA : null;
  }
  if (entryA && !entryB) {
    return pathA;
  }
  if (entryB && !entryA) {
    return pathB;
  }
  if (entryB) {
    return pathB;
  }
  if (entryA) {
    return pathA;
  }
  return null;
}

function readWgsContainerEntries(containerEntry, containerDir, entriesMap) {
  const view = new DataView(containerEntry.bytes.buffer, containerEntry.bytes.byteOffset, containerEntry.bytes.byteLength);
  const cursor = { offset: 0 };
  if (view.byteLength < 8) {
    throw new Error(`Container file is incomplete: ${containerEntry.relPath}`);
  }
  cursor.offset += 4;
  const fileCount = readInt32LE(view, cursor.offset);
  cursor.offset += 4;
  const out = [];
  for (let i = 0; i < fileCount; i += 1) {
    const logicalName = readUtf16LeString(view, cursor, 64);
    const guidA = readGuidHexLe(view, cursor);
    const guidB = readGuidHexLe(view, cursor);
    const blobRelPath = pickWgsBlobRelativePath(entriesMap, containerDir, guidA, guidB);
    out.push({
      logicalName,
      blobRelPath,
      containerRelPath: containerEntry.relPathNoRoot || containerEntry.relPath,
    });
  }
  return out;
}

function readWgsUserDirEntries(candidatePath, entriesMap) {
  const indexEntry = findRawEntry(entriesMap, joinRelativePath(candidatePath, "containers.index"));
  if (!indexEntry) {
    throw new Error("The selected WGS user folder is missing containers.index.");
  }
  const view = new DataView(indexEntry.bytes.buffer, indexEntry.bytes.byteOffset, indexEntry.bytes.byteLength);
  const cursor = { offset: 0 };
  if (view.byteLength < 8) {
    throw new Error("The WGS containers.index file is incomplete.");
  }
  cursor.offset += 4;
  const containerCount = readInt32LE(view, cursor.offset);
  cursor.offset += 4;
  readUtf16LeString(view, cursor);
  const packageName = readUtf16LeString(view, cursor).split("!")[0];
  readFiletime(view, cursor);
  if (cursor.offset + 4 > view.byteLength) {
    throw new Error("The WGS containers.index file is incomplete.");
  }
  cursor.offset += 4;
  readUtf16LeString(view, cursor);
  if (cursor.offset + 8 > view.byteLength) {
    throw new Error("The WGS containers.index file is incomplete.");
  }
  cursor.offset += 8;

  const out = [];
  for (let i = 0; i < containerCount; i += 1) {
    readUtf16LeString(view, cursor);
    readUtf16LeString(view, cursor);
    readUtf16LeString(view, cursor);
    if (cursor.offset + 1 > view.byteLength) {
      throw new Error("The WGS containers.index file is incomplete.");
    }
    const containerNum = view.getUint8(cursor.offset);
    cursor.offset += 1;
    if (cursor.offset + 4 > view.byteLength) {
      throw new Error("The WGS containers.index file is incomplete.");
    }
    cursor.offset += 4;
    const containerGuid = readGuidHexLe(view, cursor);
    readFiletime(view, cursor);
    if (cursor.offset + 16 > view.byteLength) {
      throw new Error("The WGS containers.index file is incomplete.");
    }
    cursor.offset += 16;

    const containerDir = joinRelativePath(candidatePath, containerGuid);
    const containerRelPath = joinRelativePath(containerDir, `container.${containerNum}`);
    const containerEntry = findRawEntry(entriesMap, containerRelPath);
    if (!containerEntry) {
      continue;
    }
    out.push(...readWgsContainerEntries(containerEntry, containerDir, entriesMap));
  }

  if (packageName && !packageName.toLowerCase().includes("snowrunner")) {
    throw new Error(`The selected WGS user folder belongs to a different package: ${packageName}`);
  }
  return out;
}

function readWgsContainerDirEntries(candidatePath, entriesMap) {
  const out = [];
  for (const entry of entriesMap.values()) {
    const rel = normalizeRelativePath(entry && entry.relPathNoRoot ? entry.relPathNoRoot : "");
    const base = getFileBasename(rel).toLowerCase();
    const dir = normalizeRelativePath(rel.slice(0, Math.max(0, rel.length - base.length)).replace(/\/$/, ""));
    if (dir !== normalizeRelativePath(candidatePath) || !base.startsWith("container.")) {
      continue;
    }
    out.push(...readWgsContainerEntries(entry, candidatePath, entriesMap));
  }
  if (out.length === 0) {
    throw new Error("The selected folder does not contain any WGS container files.");
  }
  return out;
}

function wgsLogicalNameToRelativePath(logicalName) {
  const text = String(logicalName || "").replace(/\0/g, "").trim();
  if (!text) {
    return "";
  }
  const parts = [];
  for (const chunk of text.split(/[\\/]+/)) {
    const trimmed = chunk.trim();
    if (!trimmed || trimmed === "." || trimmed === "..") {
      continue;
    }
    parts.push(trimmed.replace(/[<>:"|?*]/g, "_"));
  }
  if (parts.length === 0) {
    return "";
  }
  let rel = parts.join("/");
  if (!/\.[A-Za-z0-9]+$/.test(rel)) {
    rel += ".cfg";
  }
  return rel;
}

function decodeWgsFolder(entries, rootName) {
  const candidates = discoverWgsCandidates(entries);
  if (candidates.length === 0) {
    return null;
  }
  if (candidates.length > 1) {
    const summary = candidates.map((item) => item.path || "<selected folder>").join(", ");
    throw new Error(`Multiple WGS save sources were found in the uploaded folder. Upload a single WGS user folder or container folder. Found: ${summary}`);
  }

  const candidate = candidates[0];
  const entriesMap = buildRawEntryLookup(entries);
  const logicalEntries = candidate.kind === "user_dir"
    ? readWgsUserDirEntries(candidate.path, entriesMap)
    : readWgsContainerDirEntries(candidate.path, entriesMap);

  const virtualEntries = [];
  const wgsBlobByLogicalKey = new Map();
  const seenLogical = new Set();
  for (const item of logicalEntries) {
    const relPath = wgsLogicalNameToRelativePath(item.logicalName);
    const blobRelPath = normalizeRelativePath(item.blobRelPath);
    if (!relPath || !blobRelPath) {
      continue;
    }
    const blobEntry = findRawEntry(entriesMap, blobRelPath);
    if (!blobEntry) {
      continue;
    }
    const key = relPath.toLowerCase();
    if (seenLogical.has(key)) {
      continue;
    }
    seenLogical.add(key);
    virtualEntries.push({
      key,
      relPath,
      relPathNoRoot: relPath,
      name: getFileBasename(relPath),
      bytes: new Uint8Array(blobEntry.bytes),
      dirty: false,
    });
    wgsBlobByLogicalKey.set(key, blobRelPath);
  }

  if (virtualEntries.length === 0) {
    throw new Error("The selected WGS folder was decoded, but no readable SnowRunner save files were found.");
  }

  return {
    rootName,
    sourceKind: "wgs",
    files: virtualEntries,
    rawFiles: entries,
    wgsBlobByLogicalKey,
  };
}

async function onFolderSelected() {
  const files = els.folderInput.files ? [...els.folderInput.files] : [];
  if (files.length === 0) {
    return;
  }
  try {
    const rawPayload = await buildUploadedFolderEntries(files);
    const allEntries = rawPayload.entries;
    const rootName = rawPayload.rootName;
    const hasWgsMarkers = containsWgsMarkers(allEntries);
    const wgsPayload = hasWgsMarkers ? decodeWgsFolder(allEntries, rootName) : null;

    const acceptedFiles = wgsPayload
      ? wgsPayload.files
      : allEntries.filter((entry) => isTopLevelFolderFile(entry.relPath));
    const ignoredCount = wgsPayload ? 0 : Math.max(0, files.length - acceptedFiles.length);

    state.folder.files.clear();
    state.folder.rawFiles.clear();
    state.folder.wgsBlobByLogicalKey.clear();
    for (const entry of acceptedFiles) {
      state.folder.files.set(entry.key, entry);
    }
    if (wgsPayload) {
      for (const entry of wgsPayload.rawFiles) {
        state.folder.rawFiles.set(String(entry.relPathNoRoot || entry.relPath || entry.name).toLowerCase(), entry);
      }
      for (const [logicalKey, blobRelPath] of wgsPayload.wgsBlobByLogicalKey.entries()) {
        state.folder.wgsBlobByLogicalKey.set(logicalKey, blobRelPath);
      }
      state.folder.sourceKind = "wgs";
    } else {
      state.folder.sourceKind = "standard";
    }
    state.folder.loaded = true;
    state.folder.rootName = rootName;
    state.improveShare.preferredSource = "folder";
    state.improveShare.lastUploadedSignature = "";
    updateFolderMeta();
    refreshFolderMainChoices(acceptedFiles);
    refreshFogFileList();

    const mainEntries = getMainFolderEntries(acceptedFiles);
    if (mainEntries.length === 1) {
      const onlyMain = mainEntries[0];
      const text = decodeBytesToText(onlyMain.bytes);
      setMainFromText(onlyMain.name, text, onlyMain.key);
    } else if (mainEntries.length !== 1) {
      state.main = null;
      updateMainMeta();
      updateMainSummary();
      updateDownloadButtons();
    }
    const commonEntry = pickCommonEntryFromFolder(acceptedFiles);
    if (commonEntry) {
      const text = decodeBytesToText(commonEntry.bytes);
      setCommonFromText(commonEntry.name, text, commonEntry.key);
    }
    updateDownloadButtons();
    const fogCount = getFogFolderEntries().length;
    let folderStatusMessage = "";
    let folderStatusType = "success";
    if (wgsPayload) {
      folderStatusMessage = `Loaded WGS folder: decoded ${acceptedFiles.length} logical files from ${allEntries.length} uploaded files (${fogCount} fog files detected). Download Folder will rebuild the WGS layout as a zip.`;
      folderStatusType = "success";
    } else if (mainEntries.length > 1) {
      folderStatusMessage = `Loaded folder: ${acceptedFiles.length} top-level files (${fogCount} fog files). Ignored ${ignoredCount} subfolder file(s). Pick your CompleteSave* file below Upload Save Folder or File.`;
      folderStatusType = "info";
    } else {
      folderStatusMessage = `Loaded folder: ${acceptedFiles.length} top-level files (${fogCount} fog files detected). Ignored ${ignoredCount} subfolder file(s).`;
    }
    const warningSuffix = getMainVersionWarningSuffix();
    if (warningSuffix) {
      folderStatusMessage += warningSuffix;
    }
    setStatus(folderStatusMessage, folderStatusType);
    await maybeUploadImproveSamples(acceptedFiles);
  } catch (error) {
    setStatus(`Failed to load folder: ${error.message}`, "error");
  } finally {
    els.folderInput.value = "";
  }
}

function activateTab(tab) {
  for (const btn of document.querySelectorAll(".tab-btn")) {
    btn.classList.toggle("active", btn.dataset.tab === tab);
  }
  for (const panel of document.querySelectorAll(".tab-panel")) {
    panel.classList.toggle("active", panel.id === `panel-${tab}`);
  }
  if (tab === "save") {
    refreshGameStatsEditor();
  }
  if (tab === "objectives") {
    ensureObjectivesCatalogLoaded(true);
    renderObjectivesList();
  }
  if (tab === "fog") {
    ensurePakoLoaded();
    refreshFogFileList();
    renderFogCanvas();
  }
  if (tab === "trials" || tab === "pros" || tab === "achievements") {
    refreshCommonTabs();
  }
}

function updateMainMeta() {
  if (!state.main) {
    els.mainMeta.textContent = "No file loaded.";
    return;
  }
  const size = (state.main.text.length / 1024).toFixed(1);
  const stateText = state.main.dirty ? "edited" : "original";
  els.mainMeta.textContent = `${state.main.name} • ${size} KB • ${stateText}`;
}

function updateCommonMeta() {
  if (!state.common) {
    els.commonMeta.textContent = "No file loaded.";
    return;
  }
  const size = (state.common.text.length / 1024).toFixed(1);
  const stateText = state.common.dirty ? "edited" : "original";
  els.commonMeta.textContent = `${state.common.name} • ${size} KB • ${stateText}`;
}

function updateFolderMeta() {
  if (!state.folder.loaded || state.folder.files.size === 0) {
    els.folderMeta.textContent = "No folder loaded.";
    return;
  }
  let edited = 0;
  for (const entry of state.folder.files.values()) {
    if (entry.dirty) {
      edited += 1;
    }
  }
  const fogCount = getFogFolderEntries().length;
  const root = state.folder.rootName ? `${state.folder.rootName} • ` : "";
  const sourceLabel = state.folder.sourceKind === "wgs" ? "WGS decoded • " : "";
  els.folderMeta.textContent = `${root}${sourceLabel}${state.folder.files.size} files • ${fogCount} fog files • ${edited} edited`;
}

function refreshFolderMainChoices(entriesOverride) {
  if (!els.folderMainChoices) {
    return;
  }
  const entries = getMainFolderEntries(entriesOverride);
  els.folderMainChoices.innerHTML = "";
  if (entries.length === 0) {
    const text = document.createElement("span");
    text.className = "meta";
    text.textContent = "No CompleteSave*.cfg/.dat files or decoded WGS main saves detected.";
    els.folderMainChoices.append(text);
    return;
  }
  for (const entry of entries) {
    const btn = makeButton(entry.name + (entry.dirty ? " (edited)" : ""), "btn folder-choice-btn");
    if (state.main && state.main.folderKey === entry.key) {
      btn.classList.add("active");
    }
    btn.addEventListener("click", () => loadMainFromFolderKey(entry.key));
    els.folderMainChoices.append(btn);
  }
}

function loadMainFromFolderKey(key) {
  if (!key) {
    return;
  }
  const entry = state.folder.files.get(String(key).toLowerCase());
  if (!entry) {
    setStatus("Selected CompleteSave file not found.", "error");
    return;
  }
  try {
    const text = decodeBytesToText(entry.bytes);
    setMainFromText(entry.name, text, entry.key);
    setStatus(buildMainLoadStatus(`Active main save switched to ${entry.name}`), "success");
  } catch (error) {
    setStatus(`Failed to load selected main save: ${error.message}`, "error");
  }
}

function setMainFromText(name, text, folderKey) {
  const slotIndex = extractCompleteSaveIndex(name);
  const versionDiffs = readSaveVersionDiffs(text);
  state.main = {
    name,
    text,
    dirty: false,
    folderKey: folderKey || null,
    versionDiffs,
  };
  const slotNumber = slotIndex + 1;
  if (els.fogSlotSelect && Number.isInteger(slotNumber) && slotNumber >= 1 && slotNumber <= 4) {
    els.fogSlotSelect.value = String(slotNumber);
  }
  if (!folderKey) {
    state.improveShare.preferredSource = "single";
  }
  updateMainMeta();
  updateMainSummary();
  refreshFolderMainChoices();
  updateDownloadButtons();
}

function buildMainLoadStatus(prefix) {
  return `${prefix}${getMainVersionWarningSuffix()}`;
}

function getMainVersionWarningSuffix() {
  if (!state.main || !Array.isArray(state.main.versionDiffs) || state.main.versionDiffs.length === 0) {
    return "";
  }
  return ` | Version warning: ${state.main.versionDiffs.join("; ")}`;
}

function readSaveVersionDiffs(content) {
  const diffs = [];
  for (const [key, expected] of Object.entries(SAVE_VERSION_EXPECTED)) {
    const re = new RegExp(`"${escapeRegExp(key)}"\\s*:\\s*(-?\\d+)`, "i");
    const m = re.exec(content);
    if (!m) {
      diffs.push(`${key}=missing (expected ${expected})`);
      continue;
    }
    const value = Number.parseInt(m[1], 10);
    if (!Number.isFinite(value)) {
      diffs.push(`${key}=unreadable (expected ${expected})`);
      continue;
    }
    if (value !== expected) {
      diffs.push(`${key}=${value} (expected ${expected})`);
    }
  }
  return diffs;
}

function setCommonFromText(name, text, folderKey) {
  state.common = {
    name,
    text,
    dirty: false,
    folderKey: folderKey || null,
  };
  if (!folderKey) {
    state.improveShare.preferredSource = "single";
  }
  updateCommonMeta();
  refreshCommonTabs();
  updateDownloadButtons();
}

function syncMainToFolderEntry() {
  if (!state.main || !state.main.folderKey) {
    return;
  }
  const entry = state.folder.files.get(state.main.folderKey);
  if (!entry) {
    return;
  }
  entry.bytes = encodeTextToBytes(state.main.text);
  entry.dirty = true;
  updateFolderMeta();
  refreshFolderMainChoices();
}

function syncCommonToFolderEntry() {
  if (!state.common || !state.common.folderKey) {
    return;
  }
  const entry = state.folder.files.get(state.common.folderKey);
  if (!entry) {
    return;
  }
  entry.bytes = encodeTextToBytes(state.common.text);
  entry.dirty = true;
  updateFolderMeta();
}

function updateDownloadButtons() {
  els.downloadMainBtn.disabled = !state.main;
  els.downloadCommonBtn.disabled = !state.common;
  els.downloadFolderBtn.disabled = !state.folder.loaded || state.folder.files.size === 0;
}

function updateMainSummary() {
  if (!state.main) {
    els.gameStatsInfo.textContent = "No file loaded.";
    els.distanceList.innerHTML = "";
    els.gameStatList.innerHTML = "";
    state.objectives.data = null;
    state.objectives.selected.clear();
    state.objectives.visibleKeys = [];
    renderObjectivesList();
    hydrateRulesFromMain();
    return;
  }
  try {
    const info = getFileInfo(state.main.text);

    if (info.money != null) {
      els.moneyInput.value = String(info.money);
    }
    if (info.rank != null) {
      els.rankInput.value = String(info.rank);
    }
    if (info.xp != null) {
      els.xpInput.value = String(info.xp);
    }
    if (info.day != null) {
      els.timeDayInput.value = String(info.day);
    }
    if (info.night != null) {
      els.timeNightInput.value = String(info.night);
    }
    if (info.skipTime != null) {
      els.skipTimeInput.checked = Boolean(info.skipTime);
    }
  } catch (_err) {
    // ignore parse errors for these quick fields
  }
  refreshGameStatsEditor();
  refreshObjectivesFromMain();
  hydrateRulesFromMain();
}

function refreshCommonTabs() {
  refreshTrialsFromCommon();
  refreshProsFromCommon();
  refreshAchievementsFromCommon();
}

function setStatus(message, type = "info") {
  els.status.textContent = message;
  els.status.classList.remove("info", "success", "error");
  els.status.classList.add(type);
}

function requireMain() {
  if (state.main) {
    return true;
  }
  setStatus("This action needs a Main Save file. Use Upload Save Folder or File.", "error");
  try {
    els.mainInput.click();
  } catch (_err) {
    // ignore
  }
  return false;
}

function requireCommon() {
  if (state.common) {
    return true;
  }
  setStatus("This action needs a CommonSslSave file. Use Upload Save Folder or File.", "error");
  try {
    els.commonInput.click();
  } catch (_err) {
    // ignore
  }
  return false;
}

function commitMain(newText, message) {
  if (!state.main) {
    return;
  }
  if (newText === state.main.text) {
    setStatus(message || "No changes were applied.", "info");
    return;
  }
  state.main.text = newText;
  state.main.dirty = true;
  state.main.versionDiffs = readSaveVersionDiffs(newText);
  syncMainToFolderEntry();
  updateMainMeta();
  updateMainSummary();
  updateDownloadButtons();
  setStatus(message || "Main save updated.", "success");
}

function commitCommon(newText, message) {
  if (!state.common) {
    return;
  }
  if (newText === state.common.text) {
    setStatus(message || "No changes were applied.", "info");
    return;
  }
  state.common.text = newText;
  state.common.dirty = true;
  syncCommonToFolderEntry();
  updateCommonMeta();
  refreshCommonTabs();
  updateDownloadButtons();
  setStatus(message || "CommonSslSave updated.", "success");
}

function downloadMainFile() {
  if (!requireMain()) {
    return;
  }
  const name = String(state.main.name || "MainSave.cfg");
  downloadText(name, state.main.text);
}

function downloadCommonFile() {
  if (!requireCommon()) {
    return;
  }
  const name = String(state.common.name || "CompleteSave.cfg");
  downloadText(name, state.common.text);
}

async function downloadFolderZip() {
  if (!state.folder.loaded || state.folder.files.size === 0) {
    setStatus("Upload a save folder first.", "error");
    return;
  }
  const jsZipReady = await ensureJsZipLoaded();
  if (!jsZipReady || !window.JSZip) {
    setStatus("Failed to load JSZip runtime.", "error");
    return;
  }
  try {
    const zip = new window.JSZip();
    if (state.folder.sourceKind === "wgs") {
      const blobOverrides = new Map();
      for (const entry of state.folder.files.values()) {
        const blobRelPath = state.folder.wgsBlobByLogicalKey.get(String(entry.key || "").toLowerCase());
        if (blobRelPath) {
          blobOverrides.set(String(blobRelPath).toLowerCase(), entry.bytes);
        }
      }
      for (const rawEntry of state.folder.rawFiles.values()) {
        const relPath = rawEntry.relPath || rawEntry.name;
        const override = blobOverrides.get(String(rawEntry.relPathNoRoot || rawEntry.relPath || rawEntry.name).toLowerCase());
        zip.file(relPath, override || rawEntry.bytes);
      }
    } else {
      for (const entry of state.folder.files.values()) {
        zip.file(entry.relPath || entry.name, entry.bytes);
      }
    }
    const root = state.folder.rootName || (state.folder.sourceKind === "wgs" ? "wgs-save" : "save-folder");
    const filename = `${root}.zip`;
    const blob = await zip.generateAsync({
      type: "blob",
      compression: "DEFLATE",
      compressionOptions: { level: 6 },
    });
    triggerBlobDownload(filename, blob);
  } catch (error) {
    setStatus(`Failed to build folder zip: ${error.message}`, "error");
  }
}

function downloadText(filename, text) {
  const blob = new Blob([text], { type: "text/plain;charset=utf-8" });
  triggerBlobDownload(filename, blob);
}

function downloadBinary(filename, bytes, contentType = "application/octet-stream") {
  const blob = new Blob([bytes], { type: contentType });
  triggerBlobDownload(filename, blob);
}

function triggerBlobDownload(filename, blob) {
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = filename;
  document.body.appendChild(a);
  a.click();
  a.remove();
  setTimeout(() => URL.revokeObjectURL(url), 0);
  setStatus(`Downloaded ${filename}`, "success");
}

function buildSeasonMapSelector(container, prefix) {
  container.innerHTML = "";
  const root = document.createElement("div");
  root.className = "selector-root";

  const actions = document.createElement("div");
  actions.className = "selector-actions";
  const checkAll = makeButton("Check all", "btn");
  const clearAll = makeButton("Clear all", "btn");
  actions.append(checkAll, clearAll);
  root.append(actions);

  const wrap = document.createElement("div");
  wrap.className = "selector-wrap";
  root.append(wrap);

  const seasonsSection = document.createElement("div");
  seasonsSection.className = "selector-section";
  seasonsSection.innerHTML = "<h4>Seasons</h4>";
  const seasonsGrid = document.createElement("div");
  seasonsGrid.className = "checklist-grid";
  seasonsSection.append(seasonsGrid);
  wrap.append(seasonsSection);

  const mapsSection = document.createElement("div");
  mapsSection.className = "selector-section";
  mapsSection.innerHTML = "<h4>Base Maps + Other Season</h4>";
  const mapsGrid = document.createElement("div");
  mapsGrid.className = "checklist-grid";
  mapsSection.append(mapsGrid);
  const otherLabel = document.createElement("label");
  otherLabel.textContent = "Other season number";
  const otherInput = document.createElement("input");
  otherInput.type = "text";
  otherInput.placeholder = "e.g. 18";
  otherLabel.append(otherInput);
  mapsSection.append(otherLabel);
  wrap.append(mapsSection);

  const seasonChecks = new Map();
  for (const season of sortedSeasons()) {
    const id = `${prefix}-season-${season}`;
    const label = SEASON_REGION_MAP[season].label;
    const box = makeCheckbox(id, label);
    seasonChecks.set(season, box.input);
    seasonsGrid.append(box.label);
  }

  const mapChecks = new Map();
  for (const map of BASE_MAPS) {
    const id = `${prefix}-map-${map.code.toLowerCase()}`;
    const box = makeCheckbox(id, map.name);
    mapChecks.set(map.code, box.input);
    mapsGrid.append(box.label);
  }

  checkAll.addEventListener("click", () => {
    for (const input of seasonChecks.values()) {
      input.checked = true;
    }
    for (const input of mapChecks.values()) {
      input.checked = true;
    }
  });
  clearAll.addEventListener("click", () => {
    for (const input of seasonChecks.values()) {
      input.checked = false;
    }
    for (const input of mapChecks.values()) {
      input.checked = false;
    }
    otherInput.value = "";
  });

  container.append(root);

  return {
    getSelectedSeasons() {
      const selected = [];
      for (const [season, input] of seasonChecks.entries()) {
        if (input.checked) {
          selected.push(season);
        }
      }
      const other = Number.parseInt(otherInput.value.trim(), 10);
      if (Number.isInteger(other) && other > 0) {
        selected.push(other);
      }
      return uniqueList(selected);
    },
    getSelectedMaps() {
      const selected = [];
      for (const [code, input] of mapChecks.entries()) {
        if (input.checked) {
          selected.push(code);
        }
      }
      return uniqueList(selected);
    },
  };
}

function buildRegionSelector(container, prefix) {
  container.innerHTML = "";
  const root = document.createElement("div");
  root.className = "selector-root";

  const actions = document.createElement("div");
  actions.className = "selector-actions";
  const checkAll = makeButton("Check all", "btn");
  const clearAll = makeButton("Clear all", "btn");
  actions.append(checkAll, clearAll);
  root.append(actions);

  const wrap = document.createElement("div");
  wrap.className = "selector-wrap";
  root.append(wrap);

  const seasonsSection = document.createElement("div");
  seasonsSection.className = "selector-section";
  seasonsSection.innerHTML = "<h4>Seasons</h4>";
  const seasonsGrid = document.createElement("div");
  seasonsGrid.className = "checklist-grid";
  seasonsSection.append(seasonsGrid);
  wrap.append(seasonsSection);

  const mapsSection = document.createElement("div");
  mapsSection.className = "selector-section";
  mapsSection.innerHTML = "<h4>Base Maps + Other Season</h4>";
  const mapsGrid = document.createElement("div");
  mapsGrid.className = "checklist-grid";
  mapsSection.append(mapsGrid);
  wrap.append(mapsSection);

  const mapChecks = new Map();
  for (const map of BASE_MAPS) {
    const id = `${prefix}-region-${map.code.toLowerCase()}`;
    const box = makeCheckbox(id, map.name);
    mapChecks.set(map.code, box.input);
    mapsGrid.append(box.label);
  }
  const seasonChecks = new Map();
  for (const season of sortedSeasons()) {
    const code = SEASON_REGION_MAP[season].code;
    const id = `${prefix}-region-${code.toLowerCase()}`;
    const box = makeCheckbox(id, SEASON_REGION_MAP[season].label);
    seasonChecks.set(season, box.input);
    seasonsGrid.append(box.label);
  }
  const otherLabel = document.createElement("label");
  otherLabel.textContent = "Other season number";
  const otherInput = document.createElement("input");
  otherInput.type = "text";
  otherInput.placeholder = "e.g. 18";
  otherLabel.append(otherInput);
  mapsSection.append(otherLabel);
  const note = document.createElement("p");
  note.className = "help";
  note.textContent = "Predefined seasons use names only. For new season numbers, type one above.";
  mapsSection.append(note);

  checkAll.addEventListener("click", () => {
    for (const input of mapChecks.values()) {
      input.checked = true;
    }
    for (const input of seasonChecks.values()) {
      input.checked = true;
    }
  });
  clearAll.addEventListener("click", () => {
    for (const input of mapChecks.values()) {
      input.checked = false;
    }
    for (const input of seasonChecks.values()) {
      input.checked = false;
    }
    otherInput.value = "";
  });

  container.append(root);

  return {
    getSelectedSeasons() {
      const selected = [];
      for (const [season, input] of seasonChecks.entries()) {
        if (input.checked) {
          selected.push(season);
        }
      }
      const other = Number.parseInt(otherInput.value.trim(), 10);
      if (Number.isInteger(other) && other > 0) {
        selected.push(other);
      }
      return uniqueList(selected);
    },
    getSelectedMaps() {
      const selected = [];
      for (const [code, input] of mapChecks.entries()) {
        if (input.checked) {
          selected.push(code);
        }
      }
      return uniqueList(selected);
    },
    getSelectedRegions() {
      const selected = [];
      for (const [code, input] of mapChecks.entries()) {
        if (input.checked) {
          selected.push(code);
        }
      }
      for (const [season, input] of seasonChecks.entries()) {
        if (input.checked && SEASON_REGION_MAP[season]) {
          selected.push(SEASON_REGION_MAP[season].code);
        }
      }
      const other = Number.parseInt(otherInput.value.trim(), 10);
      if (Number.isInteger(other) && other > 0) {
        selected.push(`US_${String(other).padStart(2, "0")}`);
      }
      return uniqueList(selected.map((v) => String(v).toUpperCase()));
    },
  };
}

const REGION_FEATURE_DEFINITIONS = [
  {
    key: "missions",
    label: "Missions (Legacy)",
    mode: "seasonMap",
    legacy: true,
    info: "You must accept the task or mission in the game before it can be completed.",
  },
  {
    key: "contests",
    label: "Contests (Legacy)",
    mode: "seasonMap",
    legacy: true,
    info: "You must accept or discover the contests for them to be marked as completed. This also completes unfinished tasks found on the same map.",
  },
  {
    key: "upgrades",
    label: "Upgrades",
    mode: "region",
    info: "At least one upgrade must already be marked or collected in-game for this to work. If a new season is added, you may need to collect or mark one new upgrade first.",
  },
  {
    key: "watchtowers",
    label: "Watchtowers",
    mode: "region",
    info: "This marks watchtowers as found, but it will not reveal the map. Use Fog Tool for that.",
  },
  {
    key: "discoveries",
    label: "Discoveries",
    mode: "region",
    info: "Sets discovered trucks to their max for selected regions but won't add them to the garage.",
  },
  {
    key: "levels",
    label: "Levels",
    mode: "region",
    info: "Lets you view regions you haven't visited yet.",
  },
  {
    key: "garages",
    label: "Garages",
    mode: "region",
    info: "Garages will be unlocked but may still be hidden under fog of war. Recover can stay semi-broken until you find the garage entrance, and some garage entrances are quest-gated.",
    extraKey: "upgradeAll",
    extraLabel: "Upgrade all garages",
  },
];

function buildRegionsFeatureEditor() {
  if (!els.regionsFeatureList) {
    return;
  }
  if (els.regionsShowLegacy) {
    els.regionsShowLegacy.checked = Boolean(state.ui.showLegacyRegionActions);
  }
  els.regionsFeatureList.innerHTML = "";
  state.regions.featureControls.clear();

  for (const def of REGION_FEATURE_DEFINITIONS) {
    const row = document.createElement("div");
    row.className = "regions-feature-row";
    row.dataset.featureKey = def.key;
    if (def.legacy) {
      row.dataset.legacy = "true";
      row.hidden = !state.ui.showLegacyRegionActions;
    }

    const mainLabel = document.createElement("label");
    mainLabel.className = "regions-feature-toggle";
    const input = document.createElement("input");
    input.type = "checkbox";
    input.id = `regions-feature-${def.key}`;
    const text = document.createElement("span");
    text.textContent = def.label;
    mainLabel.append(input, text);

    const infoBtn = document.createElement("button");
    infoBtn.type = "button";
    infoBtn.className = "regions-info-btn";
    infoBtn.textContent = "i";
    infoBtn.title = def.info;
    infoBtn.setAttribute("aria-label", `${def.label} info`);

    row.append(mainLabel, infoBtn);

    let extraInput = null;
    if (def.extraKey) {
      const extraLabel = document.createElement("label");
      extraLabel.className = "regions-feature-extra";
      extraInput = document.createElement("input");
      extraInput.type = "checkbox";
      extraInput.id = `regions-feature-${def.key}-${def.extraKey}`;
      const extraText = document.createElement("span");
      extraText.textContent = def.extraLabel;
      extraLabel.append(extraInput, extraText);
      row.append(extraLabel);
    }

    state.regions.featureControls.set(def.key, { input, extraInput, row, definition: def });
    els.regionsFeatureList.append(row);
  }
}

function onRegionsLegacyToggleChanged() {
  state.ui.showLegacyRegionActions = Boolean(els.regionsShowLegacy && els.regionsShowLegacy.checked);
  for (const control of state.regions.featureControls.values()) {
    if (!control || !control.definition || !control.row) {
      continue;
    }
    if (!control.definition.legacy) {
      continue;
    }
    control.row.hidden = !state.ui.showLegacyRegionActions;
    if (!state.ui.showLegacyRegionActions && control.input) {
      control.input.checked = false;
    }
  }
  if (els.regionsInfo && !state.ui.showLegacyRegionActions) {
    els.regionsInfo.textContent = "Legacy actions hidden. Enable the toggle above if you need Missions / Contests.";
  }
}

function setAllRegionsFeatures(enabled) {
  for (const control of state.regions.featureControls.values()) {
    if (control && control.input) {
      if (control.definition && control.definition.legacy && !state.ui.showLegacyRegionActions) {
        continue;
      }
      control.input.checked = Boolean(enabled);
    }
  }
  if (els.regionsInfo) {
    els.regionsInfo.textContent = enabled ? "All region features selected." : "No region features selected.";
  }
}

function getSelectedRegionsFeatures() {
  const selected = [];
  for (const def of REGION_FEATURE_DEFINITIONS) {
    const control = state.regions.featureControls.get(def.key);
    if (!control || !control.input) {
      continue;
    }
    if (def.legacy && !state.ui.showLegacyRegionActions) {
      continue;
    }
    if (control.input.checked) {
      selected.push({
        definition: def,
        extra: control.extraInput ? Boolean(control.extraInput.checked) : false,
      });
    }
  }
  return selected;
}

function sortedSeasons() {
  return Object.keys(SEASON_REGION_MAP)
    .map(Number)
    .sort((a, b) => a - b);
}

function makeButton(label, className) {
  const button = document.createElement("button");
  button.type = "button";
  button.className = className;
  button.textContent = label;
  return button;
}

function makeCheckbox(id, labelText) {
  const label = document.createElement("label");
  const input = document.createElement("input");
  input.type = "checkbox";
  input.id = id;
  label.setAttribute("for", id);
  label.append(input, document.createTextNode(labelText));
  return { label, input };
}

function initFogTools() {
  if (!els.fogRegionFilters) {
    return;
  }
  els.fogRegionFilters.innerHTML = "";
  state.fog.seasonChecks.clear();
  for (const map of BASE_MAPS) {
    const code = String(map.code || "").toUpperCase();
    const box = makeCheckbox(`fog-region-${code.toLowerCase()}`, map.name);
    state.fog.seasonChecks.set(code, box.input);
    els.fogRegionFilters.append(box.label);
  }
  for (const season of sortedSeasons()) {
    const item = SEASON_REGION_MAP[season];
    if (!item) {
      continue;
    }
    const code = String(item.code || "").toUpperCase();
    const box = makeCheckbox(`fog-region-${code.toLowerCase()}`, item.label || code);
    state.fog.seasonChecks.set(code, box.input);
    els.fogRegionFilters.append(box.label);
  }
  onFogBrushChanged();
}

function onFogBrushChanged() {
  state.fog.brushValue = Math.max(1, Math.min(255, Number.parseInt(els.fogBrushColor.value, 10) || 255));
  state.fog.brushSize = Math.max(1, Math.min(256, Number.parseInt(els.fogBrushSize.value, 10) || 8));
}

function refreshFogFileList() {
  if (!els.fogFileSelect) {
    return;
  }
  const files = getFogFolderEntries();
  const prev = state.fog.currentKey || els.fogFileSelect.value;
  els.fogFileSelect.innerHTML = "";
  if (files.length === 0) {
    const opt = document.createElement("option");
    opt.value = "";
    opt.textContent = "No fog files loaded";
    els.fogFileSelect.append(opt);
    els.fogFileSelect.disabled = true;
    els.fogOpenBtn.disabled = true;
    els.fogSaveBtn.disabled = true;
    els.fogDownloadCurrentBtn.disabled = true;
    state.fog.currentKey = "";
    state.fog.currentName = "";
    state.fog.currentBytes = null;
    state.fog.currentPixels = null;
    state.fog.currentWidth = 0;
    state.fog.currentHeight = 0;
    state.fog.currentZlibHeader = new Uint8Array([0x78, 0x9c]);
    els.fogEditorInfo.textContent = "Upload a save folder to load fog files.";
    renderFogCanvas();
    return;
  }
  els.fogFileSelect.disabled = false;
  els.fogOpenBtn.disabled = false;
  const currentStillLoaded = Boolean(state.fog.currentPixels && state.fog.currentKey && files.some((entry) => entry.key === state.fog.currentKey));
  els.fogSaveBtn.disabled = !currentStillLoaded;
  els.fogDownloadCurrentBtn.disabled = !currentStillLoaded;
  for (const entry of files) {
    const opt = document.createElement("option");
    opt.value = entry.key;
    const mark = entry.dirty ? " (edited)" : "";
    opt.textContent = `${entry.relPath}${mark}`;
    els.fogFileSelect.append(opt);
  }
  if (prev && files.some((entry) => entry.key === prev)) {
    els.fogFileSelect.value = prev;
  } else {
    els.fogFileSelect.selectedIndex = 0;
  }
  renderFogCanvas();
}

function getFogFolderEntries() {
  if (!state.folder.loaded || state.folder.files.size === 0) {
    return [];
  }
  const files = [];
  for (const entry of state.folder.files.values()) {
    if (isFogFilename(entry.name)) {
      files.push(entry);
    }
  }
  files.sort((a, b) => String(a.relPath).localeCompare(String(b.relPath), undefined, { sensitivity: "base" }));
  return files;
}

function isFogFilename(name) {
  const base = getFileBasename(name).toLowerCase();
  if (!base.endsWith(".cfg") && !base.endsWith(".dat")) {
    return false;
  }
  return /^(?:[0-3]_)?fog_level/.test(base);
}

async function onOpenSelectedFogFile() {
  const pakoReady = await ensurePakoLoaded();
  if (!pakoReady) {
    setStatus("Failed to load fog runtime (pako).", "error");
    return;
  }
  const key = String(els.fogFileSelect.value || "").toLowerCase();
  if (!key) {
    setStatus("Select a fog file first.", "error");
    return;
  }
  const entry = state.folder.files.get(key);
  if (!entry) {
    setStatus("Selected fog file was not found in loaded folder.", "error");
    return;
  }
  try {
    setStatus(`Opening fog file: ${entry.name}...`, "info");
    await openFogFromEntry(entry);
    setStatus(`Loaded fog file: ${entry.name}`, "success");
  } catch (error) {
    setStatus(`Failed to open fog file: ${error.message}`, "error");
  }
}

async function openFogFromEntry(entry, options = {}) {
  const parsed = await decodeFogFileRobust(entry.bytes);
  state.fog.currentKey = entry.key;
  state.fog.currentName = entry.name;
  state.fog.currentBytes = entry.bytes.slice();
  state.fog.currentPixels = parsed.pixels;
  state.fog.currentWidth = parsed.width;
  state.fog.currentHeight = parsed.height;
  state.fog.currentFooter = parsed.footer;
  state.fog.currentZlibOffset = parsed.zlibOffset;
  state.fog.currentZlibLength = parsed.zlibLength;
  state.fog.currentZlibHeader = parsed.zlibHeader || new Uint8Array([0x78, 0x9c]);
  state.fog.drawing = false;
  els.fogSaveBtn.disabled = false;
  els.fogDownloadCurrentBtn.disabled = false;
  els.fogEditorInfo.textContent = `${entry.name} • ${parsed.width}x${parsed.height}`;
  refreshFogFileList();
  renderFogCanvas();
  if (!options.silent) {
    updateFolderMeta();
  }
}

async function decodeFogFileRobust(fileBytes) {
  return decodeFogFile(fileBytes);
}

function decodeFogFile(fileBytes) {
  if (!window.pako || typeof window.pako.Inflate !== "function") {
    throw new Error("Pako library missing.");
  }
  const bytes = fileBytes instanceof Uint8Array ? fileBytes : new Uint8Array(fileBytes || []);
  for (let i = 0; i < bytes.length - 2; i += 1) {
    if (bytes[i] !== 0x78) {
      continue;
    }
    let inflated = null;
    try {
      inflated = inflateZlibPayload(bytes.subarray(i));
    } catch (_err) {
      inflated = null;
    }
    if (!inflated || !inflated.payload || inflated.payload.length < 8 || inflated.consumed <= 0) {
      continue;
    }
    const payload = inflated.payload;
    const view = new DataView(payload.buffer, payload.byteOffset, payload.byteLength);
    const width = view.getUint32(0, true);
    const height = view.getUint32(4, true);
    if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) {
      continue;
    }
    const pixelCount = width * height;
    const expected = 8 + pixelCount;
    if (expected > payload.length) {
      continue;
    }
    const filePixels = payload.slice(8, expected);
    const footer = payload.slice(expected);
    const pixels = flipVerticalGrayscale(filePixels, width, height);
    return {
      width,
      height,
      pixels,
      footer,
      zlibOffset: i,
      zlibLength: inflated.consumed,
      zlibHeader: bytes.slice(i, i + 2),
    };
  }
  throw new Error("Could not find a valid fog zlib stream.");
}

function inflateZlibPayload(inputBytes) {
  try {
    const inflator = new window.pako.Inflate();
    inflator.push(inputBytes, true);
    if (inflator.err) {
      throw new Error(inflator.msg || "inflate failed");
    }
    const payload = inflator.result instanceof Uint8Array
      ? inflator.result
      : (inflator.result ? new Uint8Array(inflator.result) : new Uint8Array(0));
    if (payload.length < 8) {
      throw new Error("inflate returned empty payload");
    }
    const consumed = detectZlibConsumedLength(inputBytes, payload, getPakoConsumedBytes(inflator, inputBytes.length));
    return { payload, consumed };
  } catch (zlibError) {
    // Some SnowRunner fog files decode with raw deflate when pako rejects the wrapper.
    if (!(inputBytes instanceof Uint8Array) || inputBytes.length < 6) {
      throw zlibError;
    }
    try {
      const rawPayload = window.pako.inflateRaw(inputBytes.subarray(2));
      const payload = rawPayload instanceof Uint8Array ? rawPayload : new Uint8Array(rawPayload || []);
      if (payload.length < 8) {
        throw new Error("inflateRaw returned empty payload");
      }
      const consumed = detectZlibConsumedLength(inputBytes, payload, inputBytes.length - 4);
      return { payload, consumed };
    } catch (_rawErr) {
      throw zlibError;
    }
  }
}

function getPakoConsumedBytes(inflator, inputLength) {
  if (!inflator || !inflator.strm) {
    return 0;
  }
  const strm = inflator.strm;
  if (typeof strm.total_in === "number" && Number.isFinite(strm.total_in) && strm.total_in > 0) {
    return strm.total_in;
  }
  if (typeof strm.next_in === "number" && Number.isFinite(strm.next_in) && strm.next_in > 0) {
    return strm.next_in;
  }
  if (
    typeof inputLength === "number" &&
    Number.isFinite(inputLength) &&
    typeof strm.avail_in === "number" &&
    Number.isFinite(strm.avail_in)
  ) {
    const consumed = inputLength - strm.avail_in;
    if (consumed > 0) {
      return consumed;
    }
  }
  return 0;
}

function detectZlibConsumedLength(inputBytes, payload, fallbackConsumed = 0) {
  const source = inputBytes instanceof Uint8Array ? inputBytes : new Uint8Array(inputBytes || []);
  const raw = payload instanceof Uint8Array ? payload : new Uint8Array(payload || []);
  if (fallbackConsumed > 0 && fallbackConsumed <= source.length) {
    return fallbackConsumed;
  }
  if (source.length >= 8 && raw.length > 0) {
    const checksum = adler32(raw);
    const end = source.length - 4;
    const b0 = (checksum >>> 24) & 0xff;
    const b1 = (checksum >>> 16) & 0xff;
    const b2 = (checksum >>> 8) & 0xff;
    const b3 = checksum & 0xff;
    if (source[end] === b0 && source[end + 1] === b1 && source[end + 2] === b2 && source[end + 3] === b3) {
      return source.length;
    }
    return source.length - 4;
  }
  return source.length;
}

function adler32(bytes) {
  const data = bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes || []);
  let a = 1;
  let b = 0;
  const mod = 65521;
  for (let i = 0; i < data.length; i += 1) {
    a += data[i];
    if (a >= mod) {
      a -= mod;
    }
    b += a;
    b %= mod;
  }
  return ((b << 16) | a) >>> 0;
}

function renderFogCanvas() {
  const canvas = els.fogCanvas;
  if (!canvas) {
    return;
  }
  const ctx = canvas.getContext("2d");
  const width = Math.max(1, Math.floor(canvas.clientWidth || canvas.width || 1));
  const height = Math.max(1, Math.floor(canvas.clientHeight || canvas.height || 1));
  if (canvas.width !== width || canvas.height !== height) {
    canvas.width = width;
    canvas.height = height;
  }
  ctx.fillStyle = "#161d1b";
  ctx.fillRect(0, 0, width, height);

  if (!state.fog.currentPixels || state.fog.currentWidth <= 0 || state.fog.currentHeight <= 0) {
    ctx.fillStyle = "rgba(255,255,255,0.72)";
    ctx.font = '14px "IBM Plex Mono", monospace';
    ctx.textAlign = "center";
    ctx.fillText("Open a fog file to start editing.", width / 2, height / 2);
    return;
  }

  const iw = state.fog.currentWidth;
  const ih = state.fog.currentHeight;
  const scale = Math.min(width / iw, height / ih);
  const drawW = Math.max(1, Math.floor(iw * scale));
  const drawH = Math.max(1, Math.floor(ih * scale));
  const offsetX = Math.floor((width - drawW) / 2);
  const offsetY = Math.floor((height - drawH) / 2);

  state.fog.viewScale = drawW / iw;
  state.fog.viewOffsetX = offsetX;
  state.fog.viewOffsetY = offsetY;

  const px = state.fog.currentPixels;
  const img = new ImageData(iw, ih);
  for (let i = 0, j = 0; i < px.length; i += 1, j += 4) {
    const v = px[i];
    img.data[j] = v;
    img.data[j + 1] = v;
    img.data[j + 2] = v;
    img.data[j + 3] = 255;
  }

  const offscreen = document.createElement("canvas");
  offscreen.width = iw;
  offscreen.height = ih;
  const offCtx = offscreen.getContext("2d");
  offCtx.putImageData(img, 0, 0);
  ctx.imageSmoothingEnabled = false;
  ctx.drawImage(offscreen, offsetX, offsetY, drawW, drawH);
  ctx.strokeStyle = "rgba(255,255,255,0.24)";
  ctx.lineWidth = 1;
  ctx.strokeRect(offsetX + 0.5, offsetY + 0.5, drawW - 1, drawH - 1);
}

function onFogPointerDown(event) {
  if (!state.fog.currentPixels) {
    return;
  }
  onFogBrushChanged();
  const pos = fogCanvasToImagePos(event, false);
  if (!pos) {
    return;
  }
  state.fog.drawing = true;
  state.fog.lastX = pos.x;
  state.fog.lastY = pos.y;
  paintFogStroke(pos.x, pos.y, pos.x, pos.y);
  renderFogCanvas();
  try {
    els.fogCanvas.setPointerCapture(event.pointerId);
  } catch (_err) {
    // ignore
  }
}

function onFogPointerMove(event) {
  if (!state.fog.drawing || !state.fog.currentPixels) {
    return;
  }
  const pos = fogCanvasToImagePos(event, true);
  if (!pos) {
    return;
  }
  paintFogStroke(state.fog.lastX, state.fog.lastY, pos.x, pos.y);
  state.fog.lastX = pos.x;
  state.fog.lastY = pos.y;
  renderFogCanvas();
}

function onFogPointerUp(event) {
  if (!state.fog.drawing) {
    return;
  }
  state.fog.drawing = false;
  try {
    els.fogCanvas.releasePointerCapture(event.pointerId);
  } catch (_err) {
    // ignore
  }
}

function fogCanvasToImagePos(event, clampToBounds) {
  const canvas = els.fogCanvas;
  if (!canvas || !state.fog.currentPixels) {
    return null;
  }
  const rect = canvas.getBoundingClientRect();
  const cx = event.clientX - rect.left;
  const cy = event.clientY - rect.top;
  const scale = state.fog.viewScale || 1;
  let x = Math.floor((cx - state.fog.viewOffsetX) / scale);
  let y = Math.floor((cy - state.fog.viewOffsetY) / scale);
  const iw = state.fog.currentWidth;
  const ih = state.fog.currentHeight;
  if (clampToBounds) {
    x = Math.max(0, Math.min(iw - 1, x));
    y = Math.max(0, Math.min(ih - 1, y));
    return { x, y };
  }
  if (x < 0 || y < 0 || x >= iw || y >= ih) {
    return null;
  }
  return { x, y };
}

function paintFogStroke(x0, y0, x1, y1) {
  const dx = x1 - x0;
  const dy = y1 - y0;
  const steps = Math.max(1, Math.floor(Math.hypot(dx, dy)));
  for (let i = 0; i <= steps; i += 1) {
    const t = i / steps;
    const x = Math.round(x0 + dx * t);
    const y = Math.round(y0 + dy * t);
    paintFogDot(x, y, state.fog.brushSize, state.fog.brushValue);
  }
}

function paintFogDot(cx, cy, size, value) {
  const pixels = state.fog.currentPixels;
  if (!pixels) {
    return;
  }
  const iw = state.fog.currentWidth;
  const ih = state.fog.currentHeight;
  const radius = Math.max(1, Math.floor(size / 2));
  const r2 = radius * radius;
  for (let dy = -radius; dy <= radius; dy += 1) {
    for (let dx = -radius; dx <= radius; dx += 1) {
      if ((dx * dx + dy * dy) > r2) {
        continue;
      }
      const x = cx + dx;
      const y = cy + dy;
      if (x < 0 || y < 0 || x >= iw || y >= ih) {
        continue;
      }
      pixels[y * iw + x] = value;
    }
  }
}

function onSaveCurrentFogToFolder() {
  if (!state.fog.currentPixels || !state.fog.currentKey) {
    setStatus("Open a fog file first.", "error");
    return;
  }
  const entry = state.folder.files.get(state.fog.currentKey);
  if (!entry) {
    setStatus("Current fog file is not part of the loaded folder.", "error");
    return;
  }
  try {
    setStatus(`Saving fog file: ${entry.name}...`, "info");
    const payload = buildCurrentFogPayload();
    const replaced = replaceFogPayloadInFile(
      state.fog.currentBytes,
      state.fog.currentZlibOffset,
      state.fog.currentZlibLength,
      state.fog.currentZlibHeader,
      payload,
    );
    entry.bytes = replaced.bytes;
    entry.dirty = true;
    state.fog.currentBytes = replaced.bytes;
    state.fog.currentZlibLength = replaced.zlibLength;
    updateFolderMeta();
    refreshFogFileList();
    setStatus(`Fog file saved: ${entry.name}`, "success");
  } catch (error) {
    setStatus(`Failed to save fog file: ${error.message}`, "error");
  }
}

function downloadCurrentFogFile() {
  if (!state.fog.currentBytes || !state.fog.currentName) {
    setStatus("Open a fog file first.", "error");
    return;
  }
  downloadBinary(state.fog.currentName, state.fog.currentBytes);
}

async function runFogAutomation(mode) {
  try {
    const pakoReady = await ensurePakoLoaded();
    if (!pakoReady) {
      setStatus("Failed to load fog runtime (pako).", "error");
      return;
    }
    if (!state.folder.loaded || state.folder.files.size === 0) {
      setStatus("Upload a save folder first.", "error");
      return;
    }
    const slot = Math.max(1, Math.min(4, Number.parseInt(String(els.fogSlotSelect.value || "1"), 10) || 1));
    const selectedOnly = String(els.fogFilterMode.value || "all") === "selected";
    const selectedCodes = [];
    for (const [code, input] of state.fog.seasonChecks.entries()) {
      if (input.checked) {
        selectedCodes.push(code);
      }
    }
    const extras = String(els.fogExtraSeasons.value || "")
      .split(",")
      .map((part) => part.trim())
      .filter((part) => /^\d+$/.test(part));

    const targets = [];
    for (const entry of getFogFolderEntries()) {
      if (parseFogSlot(entry.name) !== slot) {
        continue;
      }
      if (selectedOnly && !matchesFogRegionFilter(entry.name, selectedCodes, extras)) {
        continue;
      }
      targets.push(entry);
    }
    if (targets.length === 0) {
      const modeText = selectedOnly ? "selected filters" : `slot ${slot}`;
      els.fogAutoInfo.textContent = `No fog files matched ${modeText}.`;
      setStatus("No fog files matched automation filters.", "error");
      return;
    }

    const modeLabel = mode === "cover" ? "cover" : "uncover";
    els.fogAutoInfo.textContent = `Running ${modeLabel} on ${targets.length} fog file(s)...`;
    setStatus(`Fog automation started (${modeLabel}, ${targets.length} target files).`, "info");

    const fillValue = mode === "cover" ? 1 : 255;
    let processed = 0;
    let failed = 0;
    const touched = new Set();
    for (const entry of targets) {
      try {
        const parsed = await decodeFogFileRobust(entry.bytes);
        const painted = new Uint8Array(parsed.pixels.length);
        painted.fill(fillValue);
        const payload = buildFogPayload(parsed.width, parsed.height, painted, parsed.footer);
        const replaced = replaceFogPayloadInFile(entry.bytes, parsed.zlibOffset, parsed.zlibLength, parsed.zlibHeader, payload);
        entry.bytes = replaced.bytes;
        entry.dirty = true;
        processed += 1;
        touched.add(entry.key);
      } catch (_err) {
        failed += 1;
      }
    }
    updateFolderMeta();
    refreshFogFileList();

    if (state.fog.currentKey && touched.has(state.fog.currentKey)) {
      const currentEntry = state.folder.files.get(state.fog.currentKey);
      if (currentEntry) {
        try {
          await openFogFromEntry(currentEntry, { silent: true });
        } catch (_err) {
          // ignore refresh failure
        }
      }
    }

    els.fogAutoInfo.textContent = `Automation done: ${modeLabel} on ${processed} file(s)${failed ? `, failed: ${failed}` : ""}.`;
    setStatus(`Fog automation complete (${processed} updated, ${failed} failed).`, failed ? "info" : "success");
  } catch (error) {
    const message = error && error.message ? error.message : String(error || "Unknown error");
    if (els.fogAutoInfo) {
      els.fogAutoInfo.textContent = `Automation failed: ${message}`;
    }
    setStatus(`Fog automation failed: ${message}`, "error");
  }
}

function parseFogSlot(filename) {
  const base = getFileBasename(filename).toLowerCase();
  const prefixed = /^([0-3])_fog_level/.exec(base);
  if (prefixed) {
    return Number.parseInt(prefixed[1], 10) + 1;
  }
  if (/^fog_level/.test(base)) {
    return 1;
  }
  return -1;
}

function matchesFogRegionFilter(filename, selectedCodes, extraSeasonNumbers) {
  const lower = getFileBasename(filename).toLowerCase();
  for (const code of selectedCodes) {
    const token = `_${String(code).toLowerCase()}_`;
    if (lower.includes(token)) {
      return true;
    }
  }
  for (const num of extraSeasonNumbers) {
    const token = `_${num}_`;
    if (lower.includes(token)) {
      return true;
    }
  }
  return false;
}

function buildCurrentFogPayload() {
  return buildFogPayload(
    state.fog.currentWidth,
    state.fog.currentHeight,
    state.fog.currentPixels,
    state.fog.currentFooter,
  );
}

function buildFogPayload(width, height, editorPixels, footer) {
  const filePixels = flipVerticalGrayscale(editorPixels, width, height);
  const footerBytes = footer instanceof Uint8Array ? footer : new Uint8Array(footer || []);
  const out = new Uint8Array(8 + filePixels.length + footerBytes.length);
  const view = new DataView(out.buffer);
  view.setUint32(0, width >>> 0, true);
  view.setUint32(4, height >>> 0, true);
  out.set(filePixels, 8);
  out.set(footerBytes, 8 + filePixels.length);
  return out;
}

function replaceFogPayloadInFile(originalBytes, zlibOffset, zlibLength, zlibHeader, payload) {
  const source = originalBytes instanceof Uint8Array ? originalBytes : new Uint8Array(originalBytes || []);
  const compressed = buildStoredZlibStream(payload, zlibHeader);
  const tailStart = zlibOffset + zlibLength;
  if (zlibOffset < 0 || zlibLength <= 0 || tailStart > source.length) {
    throw new Error("Invalid fog stream offsets.");
  }
  const out = new Uint8Array(zlibOffset + compressed.length + (source.length - tailStart));
  out.set(source.subarray(0, zlibOffset), 0);
  out.set(compressed, zlibOffset);
  out.set(source.subarray(tailStart), zlibOffset + compressed.length);
  return { bytes: out, zlibLength: compressed.length };
}

function buildStoredZlibStream(payload, headerBytes) {
  const input = payload instanceof Uint8Array ? payload : new Uint8Array(payload || []);
  const header = headerBytes instanceof Uint8Array && headerBytes.length >= 2
    ? headerBytes
    : new Uint8Array([0x78, 0x9c]);

  const chunks = [];
  chunks.push(header[0], header[1]);

  const maxChunk = 0xffff;
  let offset = 0;
  while (offset < input.length) {
    const left = input.length - offset;
    const size = left > maxChunk ? maxChunk : left;
    const bfinal = offset + size >= input.length ? 1 : 0;
    chunks.push(bfinal);
    chunks.push(size & 0xff, (size >>> 8) & 0xff);
    const nlen = 0xffff ^ size;
    chunks.push(nlen & 0xff, (nlen >>> 8) & 0xff);
    for (let i = 0; i < size; i += 1) {
      chunks.push(input[offset + i]);
    }
    offset += size;
  }

  const checksum = adler32(input);
  chunks.push((checksum >>> 24) & 0xff);
  chunks.push((checksum >>> 16) & 0xff);
  chunks.push((checksum >>> 8) & 0xff);
  chunks.push(checksum & 0xff);
  return new Uint8Array(chunks);
}

function flipVerticalGrayscale(pixels, width, height) {
  const src = pixels instanceof Uint8Array ? pixels : new Uint8Array(pixels || []);
  const out = new Uint8Array(src.length);
  const row = width;
  for (let y = 0; y < height; y += 1) {
    const srcPos = (height - 1 - y) * row;
    const dstPos = y * row;
    out.set(src.subarray(srcPos, srcPos + row), dstPos);
  }
  return out;
}

function decodeBytesToText(bytes) {
  return new TextDecoder("utf-8").decode(bytes);
}

function encodeTextToBytes(text) {
  return new TextEncoder().encode(String(text || ""));
}

function detectSingleFileRole(name, text) {
  const lowerName = String(name || "").toLowerCase();
  if (lowerName.startsWith("commonsslsave")) {
    return "common";
  }
  if (lowerName.startsWith("completesave")) {
    return "main";
  }
  const content = String(text || "");
  if (/"CommonSslSave"\s*:\s*\{/i.test(content) || /"SslValue"\s*:/i.test(content)) {
    return "common";
  }
  return "main";
}

function getImproveUploadEndpoint() {
  return String(IMPROVE_UPLOAD_ENDPOINT || "").trim();
}

function isImproveUploadEndpointConfigured(endpointOverride) {
  const endpoint = String(endpointOverride || getImproveUploadEndpoint()).trim();
  if (!/^https:\/\/[^ ]+$/i.test(endpoint)) {
    return false;
  }
  if (/your-worker\.workers\.dev/i.test(endpoint) || /example\.workers\.dev/i.test(endpoint)) {
    return false;
  }
  return true;
}

function updateImproveShareMeta(messageOverride) {
  if (!els.improveShareMeta) {
    return;
  }
  if (messageOverride) {
    els.improveShareMeta.textContent = String(messageOverride);
    return;
  }
  if (!els.improveShareCheckbox || !els.improveShareCheckbox.checked) {
    els.improveShareMeta.textContent = "Optional upload: off.";
    return;
  }
  if (!isImproveUploadEndpointConfigured()) {
    els.improveShareMeta.textContent = "Optional upload: enabled, but worker URL is not configured.";
    return;
  }
  els.improveShareMeta.textContent = "Optional upload: on. Matching files are sent anonymously when save data is loaded.";
}

function onImproveShareCheckboxChanged() {
  updateImproveShareMeta();
  if (!els.improveShareCheckbox || !els.improveShareCheckbox.checked) {
    return;
  }
  const standaloneEntries = getStandaloneLoadedEntriesForImproveShare();
  const folderEntries = state.folder.loaded && state.folder.files.size > 0
    ? [...state.folder.files.values()]
    : [];

  let entriesToUpload = [];
  if (state.improveShare.preferredSource === "single") {
    entriesToUpload = standaloneEntries.length > 0 ? standaloneEntries : folderEntries;
  } else if (state.improveShare.preferredSource === "folder") {
    entriesToUpload = folderEntries.length > 0 ? folderEntries : standaloneEntries;
  } else {
    entriesToUpload = standaloneEntries.length > 0 ? standaloneEntries : folderEntries;
  }

  if (entriesToUpload.length === 0) {
    updateImproveShareMeta("Optional upload: on. Upload a save folder or single file to send samples.");
    return;
  }
  Promise.resolve()
    .then(() => maybeUploadImproveSamples(entriesToUpload))
    .catch((error) => {
      const message = error && error.message ? error.message : String(error || "Unknown error");
      updateImproveShareMeta(`Optional upload failed: ${message}`);
      setStatus(`Optional upload failed: ${message}`, "error");
    });
}

function getStandaloneLoadedEntriesForImproveShare() {
  const out = [];
  if (state.main && !state.main.folderKey && state.main.name) {
    out.push({
      key: String(state.main.name).toLowerCase(),
      relPath: String(state.main.name),
      name: String(state.main.name),
      bytes: encodeTextToBytes(state.main.text),
      dirty: Boolean(state.main.dirty),
    });
  }
  if (state.common && !state.common.folderKey && state.common.name) {
    out.push({
      key: String(state.common.name).toLowerCase(),
      relPath: String(state.common.name),
      name: String(state.common.name),
      bytes: encodeTextToBytes(state.common.text),
      dirty: Boolean(state.common.dirty),
    });
  }
  return out;
}

function classifyImproveShareFileName(name) {
  const lower = String(name || "").toLowerCase();
  if (/^completesave\d*\.(cfg|dat)$/.test(lower) || /^commonsslsave\d*\.(cfg|dat)$/.test(lower)) {
    return "core";
  }
  if (/^(fog|field|sts|user|video|gamev)/.test(lower) || /^\d+_(fog|sts)/.test(lower)) {
    return "excluded";
  }
  return "unexpected";
}

function getImproveShareEntries(entriesOverride) {
  const source = Array.isArray(entriesOverride) ? entriesOverride : [...state.folder.files.values()];
  const core = [];
  const unexpected = [];
  let unexpectedTotalBytes = 0;

  for (const entry of source) {
    if (!entry || !entry.name || !(entry.bytes instanceof Uint8Array)) {
      continue;
    }
    const fileType = classifyImproveShareFileName(entry.name);
    if (fileType === "excluded") {
      continue;
    }
    if (fileType === "core") {
      core.push(entry);
      continue;
    }
    if (entry.bytes.length > IMPROVE_UPLOAD_UNEXPECTED_MAX_FILE_BYTES) {
      continue;
    }
    unexpected.push(entry);
    unexpectedTotalBytes += entry.bytes.length;
  }

  let selectedUnexpected = unexpected;
  if (
    unexpectedTotalBytes > IMPROVE_UPLOAD_UNEXPECTED_TOTAL_LIMIT_BYTES &&
    unexpected.length > IMPROVE_UPLOAD_UNEXPECTED_MAX_WHEN_TOTAL_EXCEEDED
  ) {
    selectedUnexpected = unexpected.slice(0, IMPROVE_UPLOAD_UNEXPECTED_MAX_WHEN_TOTAL_EXCEEDED);
  }

  return [...core, ...selectedUnexpected];
}

function getImproveShareSignature(entriesOverride) {
  const entries = getImproveShareEntries(entriesOverride);
  if (entries.length === 0) {
    return "";
  }
  const parts = entries
    .map((entry) => `${String(entry.key || entry.name || "").toLowerCase()}:${entry.bytes instanceof Uint8Array ? entry.bytes.length : 0}`)
    .sort();
  return `${String(state.folder.rootName || "").toLowerCase()}|${parts.join("|")}`;
}

async function maybeUploadImproveSamples(entriesOverride) {
  if (!els.improveShareCheckbox || !els.improveShareCheckbox.checked) {
    updateImproveShareMeta();
    return;
  }
  const endpoint = getImproveUploadEndpoint();
  if (!isImproveUploadEndpointConfigured(endpoint)) {
    const message = "Optional upload skipped: worker URL is not configured in app.js.";
    updateImproveShareMeta(message);
    setStatus(message, "error");
    return;
  }
  const sampleEntries = getImproveShareEntries(entriesOverride);
  if (sampleEntries.length === 0) {
    const message = "Optional upload skipped: no files matched upload rules in top folder.";
    updateImproveShareMeta(message);
    setStatus(message, "info");
    return;
  }

  const signature = getImproveShareSignature(sampleEntries);
  if (signature && state.improveShare.lastUploadedSignature === signature) {
    const message = "Optional upload already sent for this loaded folder.";
    updateImproveShareMeta(message);
    setStatus(message, "info");
    return;
  }
  if (state.improveShare.uploading) {
    const message = "Optional upload already in progress.";
    updateImproveShareMeta(message);
    setStatus(message, "info");
    return;
  }

  state.improveShare.uploading = true;
  updateImproveShareMeta(`Uploading anonymous samples (${sampleEntries.length} file(s))...`);
  setStatus(`Uploading anonymous samples (${sampleEntries.length} file(s))...`, "info");
  try {
    const result = await uploadImproveSamples(sampleEntries, endpoint);
    const batchId = String((result && (result.batchId || result.id)) || "").trim();
    const uploadedCountRaw = Number(result && result.uploadedCount);
    const uploadedCount = Number.isFinite(uploadedCountRaw) ? uploadedCountRaw : sampleEntries.length;
    const label = batchId ? `Optional upload complete (${uploadedCount} file(s), ID: ${batchId}).` : `Optional upload complete (${uploadedCount} file(s)).`;
    state.improveShare.lastUploadedSignature = signature;
    updateImproveShareMeta(label);
    setStatus(label, "success");
  } catch (error) {
    const message = error && error.message ? error.message : String(error || "Unknown error");
    updateImproveShareMeta(`Optional upload failed: ${message}`);
    setStatus(`Optional upload failed: ${message}`, "error");
  } finally {
    state.improveShare.uploading = false;
  }
}

async function uploadImproveSamples(entries, endpoint) {
  const target = String(endpoint || getImproveUploadEndpoint()).trim();
  if (!isImproveUploadEndpointConfigured(target)) {
    throw new Error("Worker URL is not configured.");
  }
  const formData = new FormData();
  let attached = 0;
  let skippedTooLarge = 0;
  for (const entry of entries || []) {
    if (!entry || !entry.name || !(entry.bytes instanceof Uint8Array)) {
      continue;
    }
    if (entry.bytes.length > IMPROVE_UPLOAD_MAX_FILE_BYTES) {
      skippedTooLarge += 1;
      continue;
    }
    const blob = new Blob([entry.bytes], { type: "application/octet-stream" });
    formData.append("files", blob, entry.name);
    attached += 1;
  }
  if (attached === 0) {
    throw new Error("No files matched upload rules.");
  }
  if (skippedTooLarge > 0) {
    formData.append("clientSkippedTooLarge", String(skippedTooLarge));
  }

  formData.append("source", "snowrunner-save-editor-web");
  formData.append("folderRoot", String(state.folder.rootName || ""));
  formData.append("uploadedAt", new Date().toISOString());

  const controller = typeof AbortController === "function" ? new AbortController() : null;
  const timeoutId = controller
    ? window.setTimeout(() => controller.abort(), IMPROVE_UPLOAD_TIMEOUT_MS)
    : null;
  try {
    const response = await fetch(target, {
      method: "POST",
      body: formData,
      signal: controller ? controller.signal : undefined,
    });
    const rawBody = await response.text();
    let payload = null;
    if (rawBody) {
      try {
        payload = JSON.parse(rawBody);
      } catch (_err) {
        // keep null
      }
    }
    if (!response.ok) {
      const reason = payload && payload.error ? String(payload.error) : rawBody || `HTTP ${response.status}`;
      throw new Error(reason);
    }
    if (!payload || payload.success === false) {
      const reason = payload && payload.error ? String(payload.error) : "Worker did not return success JSON.";
      throw new Error(reason);
    }
    return payload;
  } catch (error) {
    if (error && error.name === "AbortError") {
      throw new Error("Request timed out.");
    }
    throw error;
  } finally {
    if (timeoutId !== null) {
      window.clearTimeout(timeoutId);
    }
  }
}

function getMainFolderEntries(entriesOverride) {
  const source = Array.isArray(entriesOverride) ? entriesOverride : [...state.folder.files.values()];
  const out = source.filter((entry) => {
    const lower = String(entry && entry.name ? entry.name : "").toLowerCase();
    return /^completesave\d*\.(cfg|dat)$/.test(lower);
  });
  out.sort((a, b) => {
    const ai = extractCompleteSaveIndex(a.name);
    const bi = extractCompleteSaveIndex(b.name);
    if (ai !== bi) {
      return ai - bi;
    }
    const aCfg = String(a.name || "").toLowerCase().endsWith(".cfg");
    const bCfg = String(b.name || "").toLowerCase().endsWith(".cfg");
    if (aCfg !== bCfg) {
      return aCfg ? -1 : 1;
    }
    return String(a.name || "").localeCompare(String(b.name || ""), undefined, { sensitivity: "base" });
  });
  return out;
}

function extractCompleteSaveIndex(name) {
  const m = /^completesave(\d*)\.(?:cfg|dat)$/i.exec(String(name || ""));
  if (!m) {
    return Number.MAX_SAFE_INTEGER;
  }
  if (!m[1]) {
    return 0;
  }
  const parsed = Number.parseInt(m[1], 10);
  return Number.isFinite(parsed) ? parsed : Number.MAX_SAFE_INTEGER;
}

function pickCommonEntryFromFolder(entries) {
  for (const entry of entries) {
    const lower = String(entry.name || "").toLowerCase();
    if (lower.startsWith("commonsslsave") && (lower.endsWith(".cfg") || lower.endsWith(".dat"))) {
      return entry;
    }
  }
  return null;
}

function detectFolderRoot(relativePaths) {
  const paths = (relativePaths || []).filter(Boolean).map((item) => String(item).replace(/\\/g, "/"));
  if (paths.length === 0) {
    return "";
  }
  const first = paths[0].split("/")[0];
  if (!first) {
    return "";
  }
  for (const path of paths) {
    if (!path.startsWith(`${first}/`) && path !== first) {
      return "";
    }
  }
  return first;
}

function getFileBasename(path) {
  const normalized = String(path || "").replace(/\\/g, "/");
  const parts = normalized.split("/");
  return parts[parts.length - 1] || normalized;
}

function isTopLevelFolderFile(relativePath) {
  const normalized = String(relativePath || "").replace(/\\/g, "/");
  const parts = normalized.split("/").filter(Boolean);
  if (parts.length <= 1) {
    return true;
  }
  // webkitdirectory usually yields: rootFolder/file.ext
  return parts.length === 2;
}

function renderTrialsList() {
  els.trialsList.innerHTML = "";
  state.trialChecks.clear();
  for (const [name, code] of TRIALS_LIST) {
    const id = `trial-${code.toLowerCase()}`;
    const box = makeCheckbox(id, name);
    state.trialChecks.set(code, box.input);
    els.trialsList.append(box.label);
  }
}

function setAllTrials(value) {
  for (const input of state.trialChecks.values()) {
    input.checked = value;
  }
}

function buildRulesEditor() {
  if (!els.rulesEditor) {
    return;
  }
  els.rulesEditor.innerHTML = "";
  state.rules.controls.clear();
  for (const def of RULE_DEFINITIONS) {
    const row = document.createElement("div");
    row.className = "rule-row";
    const label = document.createElement("label");
    label.textContent = def.label;

    const controlStack = document.createElement("div");
    controlStack.className = "rule-control-stack";

    const select = document.createElement("select");
    const options = def.key === "gameDifficultyMode"
      ? def.options
      : [...def.options, { label: RULE_RANDOM_LABEL, value: RULE_RANDOM_VALUE }];
    for (const opt of options) {
      const option = document.createElement("option");
      option.value = opt.label;
      option.textContent = opt.label;
      select.append(option);
    }
    select.addEventListener("change", () => onRuleDropdownChanged(def));

    let customInput = null;
    if (isCustomMultiplierRule(def)) {
      customInput = document.createElement("input");
      customInput.type = "number";
      customInput.step = "0.01";
      customInput.inputMode = "decimal";
      customInput.className = "rule-custom-input";
      customInput.setAttribute("aria-label", `${def.label} custom value`);
      customInput.value = formatRuleMultiplierValue(getRuleDefaultValue(def));
      customInput.addEventListener("input", () => onRuleCustomInputChanged(def));
      customInput.addEventListener("change", () => commitRuleCustomInput(def));
      customInput.addEventListener("blur", () => commitRuleCustomInput(def));
    }

    controlStack.append(select);
    if (customInput) {
      controlStack.append(customInput);
    }
    row.append(label, controlStack);
    els.rulesEditor.append(row);
    state.rules.controls.set(def.key, { select, customInput, definition: def });
  }
  refreshRulesCustomMode();
}

function isCustomMultiplierRule(def) {
  return Boolean(def && String(def.key || "").endsWith("Factor"));
}

function normalizeRuleMultiplierValue(rawValue, fallback = 1) {
  let value = Number.parseFloat(String(rawValue));
  if (!Number.isFinite(value)) {
    value = Number.parseFloat(String(fallback));
  }
  return Number.isFinite(value) ? value : 1;
}

function limitRuleMultiplierDecimals(rawValue, fallback = 1) {
  const value = normalizeRuleMultiplierValue(rawValue, fallback);
  return Math.trunc(value * 100) / 100;
}

function formatRuleMultiplierValue(rawValue, fallback = 1) {
  const value = limitRuleMultiplierDecimals(rawValue, fallback);
  if (Math.abs(value - Math.round(value)) <= 1e-9) {
    return String(Math.round(value));
  }
  return value.toFixed(2).replace(/0+$/, "").replace(/\.$/, "") || "0";
}

function jsonRuleMultiplierValue(rawValue, fallback = 1) {
  const value = limitRuleMultiplierDecimals(rawValue, fallback);
  if (Math.abs(value - Math.round(value)) <= 1e-9) {
    return Math.round(value);
  }
  return value;
}

function getRuleDefaultLabel(def) {
  return def && def.options && def.options[0] ? def.options[0].label : "";
}

function getRuleDefaultValue(def) {
  const option = def && def.options && def.options[0] ? def.options[0] : null;
  return option ? option.value : 1;
}

function getRuleOptionByLabel(def, label) {
  return (def && Array.isArray(def.options) ? def.options : []).find((opt) => opt.label === label) || null;
}

function ruleOptionMatchesValue(optionValue, rawValue) {
  if (optionValue === RULE_RANDOM_VALUE || rawValue === RULE_RANDOM_VALUE) {
    return false;
  }
  if (typeof optionValue === "boolean" || typeof rawValue === "boolean") {
    return Object.is(optionValue, rawValue);
  }
  if (typeof optionValue === "number" && typeof rawValue === "number") {
    return Math.abs(optionValue - rawValue) <= 1e-6;
  }
  return String(optionValue) === String(rawValue);
}

function getRuleOptionByValue(def, value) {
  return (def && Array.isArray(def.options) ? def.options : []).find((opt) => ruleOptionMatchesValue(opt.value, value)) || null;
}

function getRuleControl(key) {
  return state.rules.controls.get(key) || null;
}

function getRuleSelect(key) {
  const control = getRuleControl(key);
  return control && control.select ? control.select : null;
}

function getRuleCustomInput(key) {
  const control = getRuleControl(key);
  return control && control.customInput ? control.customInput : null;
}

function setRuleSelectLabel(key, label) {
  const select = getRuleSelect(key);
  if (!select || !label) {
    return;
  }
  if ([...select.options].some((option) => option.value === label)) {
    select.value = label;
  }
}

function setRuleCustomValue(def, value) {
  if (!isCustomMultiplierRule(def)) {
    return normalizeRuleMultiplierValue(getRuleDefaultValue(def));
  }
  const input = getRuleCustomInput(def.key);
  const normalized = normalizeRuleMultiplierValue(value, getRuleDefaultValue(def));
  if (input) {
    input.value = formatRuleMultiplierValue(normalized, getRuleDefaultValue(def));
  }
  return normalized;
}

function getRuleCustomValue(def) {
  if (!isCustomMultiplierRule(def)) {
    return normalizeRuleMultiplierValue(getRuleDefaultValue(def));
  }
  const input = getRuleCustomInput(def.key);
  return normalizeRuleMultiplierValue(input ? input.value : "", getRuleDefaultValue(def));
}

function syncCustomRuleFromDropdown(def) {
  if (!isCustomMultiplierRule(def)) {
    return;
  }
  const select = getRuleSelect(def.key);
  const option = getRuleOptionByLabel(def, select && select.value ? select.value : getRuleDefaultLabel(def));
  const value = option && option.value !== RULE_RANDOM_VALUE && typeof option.value === "number"
    ? option.value
    : getRuleDefaultValue(def);
  setRuleCustomValue(def, value);
}

function setGameDifficultyRuleLabel(label) {
  setRuleSelectLabel("gameDifficultyMode", label);
}

function resetRulesUiToDefaults() {
  for (const def of RULE_DEFINITIONS) {
    setRuleSelectLabel(def.key, getRuleDefaultLabel(def));
    if (isCustomMultiplierRule(def)) {
      setRuleCustomValue(def, getRuleDefaultValue(def));
    }
  }
}

function setAllRulesToRandom() {
  for (const def of RULE_DEFINITIONS) {
    if (def.key === "gameDifficultyMode") {
      continue;
    }
    setRuleSelectLabel(def.key, RULE_RANDOM_LABEL);
  }
  setGameDifficultyRuleLabel("New Game+");
}

function isRulesCustomModeActive() {
  return Boolean(els.rulesCustom && els.rulesCustom.checked);
}

function onRulesRandomToggleChanged() {
  if (els.rulesRandom && els.rulesRandom.checked) {
    setAllRulesToRandom();
  }
}

function onRulesCustomToggleChanged() {
  if (isRulesCustomModeActive()) {
    setGameDifficultyRuleLabel("New Game+");
  }
  refreshRulesCustomMode();
}

function refreshRulesCustomMode() {
  const active = isRulesCustomModeActive();
  for (const [key, control] of state.rules.controls.entries()) {
    const def = control && control.definition ? control.definition : RULE_DEFINITIONS.find((item) => item.key === key);
    if (!isCustomMultiplierRule(def)) {
      continue;
    }
    if (control.select) {
      control.select.hidden = active;
    }
    if (control.customInput) {
      control.customInput.hidden = !active;
    }
  }
}

function onRuleDropdownChanged(def) {
  if (!def) {
    return;
  }
  const select = getRuleSelect(def.key);
  const selectedLabel = select && select.value ? select.value : getRuleDefaultLabel(def);
  if (def.key === "gameDifficultyMode") {
    if (selectedLabel === "Normal") {
      if (els.rulesRandom) {
        els.rulesRandom.checked = false;
      }
      resetRulesUiToDefaults();
      setGameDifficultyRuleLabel("Normal");
    } else if (selectedLabel === "Hard" && els.rulesRandom) {
      els.rulesRandom.checked = false;
    }
    return;
  }
  syncCustomRuleFromDropdown(def);
  setGameDifficultyRuleLabel("New Game+");
}

function onRuleCustomInputChanged(def) {
  if (!isCustomMultiplierRule(def)) {
    return;
  }
  if (isRulesCustomModeActive()) {
    setGameDifficultyRuleLabel("New Game+");
  }
}

function commitRuleCustomInput(def) {
  if (!isCustomMultiplierRule(def)) {
    return;
  }
  setRuleCustomValue(def, getRuleCustomValue(def));
}

function loadSettingsDictionary(content, fallback) {
  const defaults = fallback && typeof fallback === "object" && !Array.isArray(fallback) ? fallback : {};
  const match = new RegExp(`"${escapeRegExp("settingsDictionaryForNGPScreen")}"\\s*:\\s*\\{`, "i").exec(content || "");
  if (!match) {
    return { ...defaults };
  }
  try {
    const block = extractBraceBlock(content, match.index);
    const parsed = JSON.parse(block.block);
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return { ...defaults };
    }
    return { ...defaults, ...parsed };
  } catch (_error) {
    return { ...defaults };
  }
}

function resolveRandomRuleLabel(def) {
  const concrete = (def.options || []).map((opt) => opt.label).filter((label) => label && label !== RULE_RANDOM_LABEL);
  if (concrete.length === 0) {
    return getRuleDefaultLabel(def);
  }
  const index = Math.floor(Math.random() * concrete.length);
  return concrete[index] || getRuleDefaultLabel(def);
}

function matchingNumericRuleLabel(def, value) {
  for (const option of def && Array.isArray(def.options) ? def.options : []) {
    if (!option || option.label === RULE_RANDOM_LABEL || option.value === RULE_RANDOM_VALUE) {
      continue;
    }
    if (typeof option.value === "boolean" || typeof option.value !== "number") {
      continue;
    }
    if (ruleOptionMatchesValue(option.value, value)) {
      return option.label;
    }
  }
  return null;
}

function customRuleNgpLabel(def, value) {
  const exactLabel = matchingNumericRuleLabel(def, value);
  if (exactLabel) {
    return exactLabel;
  }

  const meta = def ? RULE_NGP_DICT_META[def.key] : null;
  if (!meta || !meta.labelToState) {
    return getRuleDefaultLabel(def);
  }

  const candidates = [];
  for (const [label, stateValue] of Object.entries(meta.labelToState)) {
    const option = getRuleOptionByLabel(def, label);
    if (!option || option.label === RULE_RANDOM_LABEL || option.value === RULE_RANDOM_VALUE) {
      continue;
    }
    if (typeof option.value === "boolean" || typeof option.value !== "number") {
      continue;
    }
    const numericState = Number.parseInt(String(stateValue), 10);
    if (!Number.isFinite(numericState)) {
      continue;
    }
    candidates.push({ label, value: option.value, state: numericState });
  }

  if (!candidates.length) {
    return getRuleDefaultLabel(def);
  }

  const target = normalizeRuleMultiplierValue(value, getRuleDefaultValue(def));
  let chosen = candidates.reduce((best, item) => (
    Math.abs(item.value - target) < Math.abs(best.value - target) ? item : best
  ), candidates[0]);

  if (chosen.state === 0 && Math.abs(chosen.value - target) > 1e-6) {
    const nonDefault = candidates.filter((item) => item.state !== 0);
    if (nonDefault.length) {
      chosen = nonDefault.reduce((best, item) => (
        Math.abs(item.value - target) < Math.abs(best.value - target) ? item : best
      ), nonDefault[0]);
    }
  }

  return chosen.label || getRuleDefaultLabel(def);
}

function inferRuleLabelFromContent(def, content, settingsDict) {
  const defaultLabel = getRuleDefaultLabel(def);
  const meta = RULE_NGP_DICT_META[def.key];
  if (meta && settingsDict && Object.prototype.hasOwnProperty.call(settingsDict, meta.ngpKey)) {
    const stateValue = Number.parseInt(String(settingsDict[meta.ngpKey]), 10);
    if (Number.isFinite(stateValue)) {
      for (const [label, mappedState] of Object.entries(meta.labelToState || {})) {
        if (Number.parseInt(String(mappedState), 10) === stateValue && getRuleOptionByLabel(def, label)) {
          return label;
        }
      }
    }
  }

  if (!content) {
    return defaultLabel;
  }

  if (def.key === "truckAvailability") {
    const availability = readSimpleValueKey(content, "truckAvailability");
    if (availability === 2) {
      const level = readSimpleValueKey(content, "truckAvailabilityLevel");
      if (typeof level === "number" && level >= 30 && getRuleOptionByLabel(def, "store unlocks at rank 30")) {
        return "store unlocks at rank 30";
      }
      if (typeof level === "number" && level >= 20 && getRuleOptionByLabel(def, "store unlocks at rank 20")) {
        return "store unlocks at rank 20";
      }
      if (getRuleOptionByLabel(def, "store unlocks at rank 10")) {
        return "store unlocks at rank 10";
      }
    }
  }

  if (def.key === "internalAddonAvailability") {
    const addonAvailability = readSimpleValueKey(content, "internalAddonAvailability");
    if (addonAvailability === 2) {
      const amount = readSimpleValueKey(content, "internalAddonAmount");
      if (typeof amount === "number") {
        if (amount >= 10 && amount <= 50 && getRuleOptionByLabel(def, "10-50 per garage")) {
          return "10-50 per garage";
        }
        if (amount >= 30 && amount <= 100 && getRuleOptionByLabel(def, "30-100 per garage")) {
          return "30-100 per garage";
        }
        if (amount >= 50 && amount <= 150 && getRuleOptionByLabel(def, "50-150 per garage")) {
          return "50-150 per garage";
        }
        if (amount >= 0 && amount <= 100 && getRuleOptionByLabel(def, "0-100 per garage")) {
          return "0-100 per garage";
        }
      }
    }
  }

  if (def.key === "maxContestAttempts") {
    if (readSimpleValueKey(content, "isGoldFailReason") === true && getRuleOptionByLabel(def, "gold time only")) {
      return "gold time only";
    }
  }

  if (def.key === "regionRepaireMoneyFactor") {
    const moneyFactor = readSimpleValueKey(content, "regionRepaireMoneyFactor");
    const pointsFactor = readSimpleValueKey(content, "regionRepairePointsFactor");
    if (moneyFactor !== undefined && pointsFactor !== undefined) {
      for (const opt of def.options) {
        if (ruleOptionMatchesValue(opt.value, moneyFactor) && ruleOptionMatchesValue(opt.value, pointsFactor)) {
          return opt.label;
        }
      }
    }
  }

  if (def.key === "needToAddDlcTrucks") {
    const dlcRaw = readSimpleValueKey(content, "needToAddDlcTrucks");
    const fallbackRaw = dlcRaw === undefined ? readSimpleValueKey(content, "isDLCVehiclesAvailable") : dlcRaw;
    const found = getRuleOptionByValue(def, fallbackRaw);
    return found ? found.label : defaultLabel;
  }

  const current = readSimpleValueKey(content, def.key);
  const found = getRuleOptionByValue(def, current);
  return found ? found.label : defaultLabel;
}

function hydrateRulesFromMain() {
  if (!state.rules.controls.size) {
    return;
  }
  const settingsDict = state.main ? loadSettingsDictionary(state.main.text, DEFAULT_SETTINGS_DICT) : { ...DEFAULT_SETTINGS_DICT };
  for (const def of RULE_DEFINITIONS) {
    const select = getRuleSelect(def.key);
    if (!select) {
      continue;
    }
    const label = state.main ? inferRuleLabelFromContent(def, state.main.text, settingsDict) : getRuleDefaultLabel(def);
    select.value = label;
    if (isCustomMultiplierRule(def)) {
      const rawValue = state.main ? readSimpleValueKey(state.main.text, def.key) : undefined;
      const value = rawValue !== undefined && rawValue !== null && typeof rawValue !== "boolean"
        ? rawValue
        : getRuleDefaultValue(def);
      setRuleCustomValue(def, value);
    }
  }
  refreshRulesCustomMode();
}

function onApplyRules() {
  if (!requireMain()) {
    return;
  }
  try {
    if (els.rulesRandom && els.rulesRandom.checked) {
      setAllRulesToRandom();
    }
    if (isRulesCustomModeActive()) {
      setGameDifficultyRuleLabel("New Game+");
    }

    const resolvedRules = new Map();
    for (const def of RULE_DEFINITIONS) {
      if (isRulesCustomModeActive() && isCustomMultiplierRule(def)) {
        const value = jsonRuleMultiplierValue(getRuleCustomValue(def), getRuleDefaultValue(def));
        const label = customRuleNgpLabel(def, value);
        resolvedRules.set(def.key, { label: label || getRuleDefaultLabel(def), value, custom: true });
        continue;
      }

      const select = getRuleSelect(def.key);
      const rawLabel = select && select.value ? select.value : getRuleDefaultLabel(def);
      const label = rawLabel === RULE_RANDOM_LABEL ? resolveRandomRuleLabel(def) : rawLabel;
      const option = getRuleOptionByLabel(def, label) || def.options[0];
      resolvedRules.set(def.key, {
        label: label || getRuleDefaultLabel(def),
        value: option ? option.value : getRuleDefaultValue(def),
        custom: false,
      });
    }

    const difficultyLabel = (resolvedRules.get("gameDifficultyMode") || {}).label || "Normal";
    if (difficultyLabel === "Normal") {
      for (const def of RULE_DEFINITIONS) {
        if (def.key === "gameDifficultyMode") {
          continue;
        }
        const defaultOption = def.options[0];
        resolvedRules.set(def.key, {
          label: getRuleDefaultLabel(def),
          value: defaultOption ? defaultOption.value : getRuleDefaultValue(def),
          custom: false,
        });
        if (isCustomMultiplierRule(def)) {
          setRuleCustomValue(def, getRuleDefaultValue(def));
        }
      }
      if (els.rulesRandom) {
        els.rulesRandom.checked = false;
      }
    }

    for (const def of RULE_DEFINITIONS) {
      const select = getRuleSelect(def.key);
      if (select) {
        select.value = (resolvedRules.get(def.key) || {}).label || getRuleDefaultLabel(def);
      }
      if (isCustomMultiplierRule(def)) {
        const resolved = resolvedRules.get(def.key);
        setRuleCustomValue(def, resolved ? resolved.value : getRuleDefaultValue(def));
      }
    }

    let content = state.main.text;
    for (const def of RULE_DEFINITIONS) {
      const resolved = resolvedRules.get(def.key);
      const value = resolved ? resolved.value : getRuleDefaultValue(def);
      if (typeof value === "boolean") {
        content = replaceOrInsertBoolean(content, def.key, value);
      } else {
        content = replaceOrInsertNumeric(content, def.key, value);
      }
    }

    const difficultyOption = getRuleOptionByLabel(
      RULE_DEFINITIONS.find((def) => def.key === "gameDifficultyMode"),
      (resolvedRules.get("gameDifficultyMode") || {}).label || "Normal",
    );
    const gameDifficulty = Number.parseInt(String(difficultyOption ? difficultyOption.value : 0), 10);
    content = replaceOrInsertBoolean(content, "isHardMode", Number.isFinite(gameDifficulty) && gameDifficulty === 1);

    const truckAvailabilityLabel = (resolvedRules.get("truckAvailability") || {}).label;
    if (truckAvailabilityLabel === "store unlocks at rank 10") {
      content = replaceOrInsertNumeric(content, "truckAvailabilityLevel", 10);
    } else if (truckAvailabilityLabel === "store unlocks at rank 20") {
      content = replaceOrInsertNumeric(content, "truckAvailabilityLevel", 20);
    } else if (truckAvailabilityLabel === "store unlocks at rank 30") {
      content = replaceOrInsertNumeric(content, "truckAvailabilityLevel", 30);
    }

    const internalAddonLabel = (resolvedRules.get("internalAddonAvailability") || {}).label;
    if (Object.prototype.hasOwnProperty.call(INTERNAL_ADDON_AMOUNT_BY_LABEL, internalAddonLabel)) {
      content = replaceOrInsertNumeric(content, "internalAddonAmount", INTERNAL_ADDON_AMOUNT_BY_LABEL[internalAddonLabel]);
    }

    const maxContestLabel = (resolvedRules.get("maxContestAttempts") || {}).label;
    content = replaceOrInsertBoolean(content, "isGoldFailReason", maxContestLabel === "gold time only");
    if (maxContestLabel === "gold time only") {
      content = replaceOrInsertNumeric(content, "maxContestAttempts", -1);
    }

    const regionRepairResolved = resolvedRules.get("regionRepaireMoneyFactor");
    if (regionRepairResolved) {
      content = replaceOrInsertNumeric(content, "regionRepairePointsFactor", regionRepairResolved.value);
    }

    const dlcResolved = resolvedRules.get("needToAddDlcTrucks");
    if (dlcResolved) {
      content = replaceOrInsertBoolean(content, "isDLCVehiclesAvailable", Boolean(dlcResolved.value));
    }

    const resolvedLabels = new Map([...resolvedRules.entries()].map(([key, resolved]) => [key, resolved.label]));
    content = sanitizeRulesContent(content, resolvedLabels, difficultyLabel);
    commitMain(content, "Rules updated.");
  } catch (error) {
    setStatus(`Failed to apply rules: ${error.message}`, "error");
  }
}

function sanitizeRulesContent(content, resolvedLabels, difficultyLabel) {
  let out = content;
  for (const def of RULE_DEFINITIONS) {
    const safeDefault = def.options[0] ? def.options[0].value : 0;
    out = ensureKeyWithDefaultLiteral(out, def.key, safeDefault, false);
  }
  out = ensureKeyWithDefaultLiteral(out, "autoloadPrice", DEFAULT_AUTOLOAD_PRICE, true);
  out = ensureArrayKeyWithDefault(out, "recoveryPrice", DEFAULT_RECOVERY_PRICE);
  out = ensureArrayKeyWithDefault(out, "fullRepairPrice", DEFAULT_FULL_REPAIR_PRICE);
  out = ensureSettingsDictionaryForNgp(out, DEFAULT_SETTINGS_DICT);
  let settingsDict = difficultyLabel === "Normal"
    ? { ...DEFAULT_SETTINGS_DICT }
    : loadSettingsDictionary(out, DEFAULT_SETTINGS_DICT);
  for (const def of RULE_DEFINITIONS) {
    const meta = RULE_NGP_DICT_META[def.key];
    if (!meta) {
      continue;
    }
    const label = resolvedLabels instanceof Map
      ? (resolvedLabels.get(def.key) || getRuleDefaultLabel(def))
      : getRuleDefaultLabel(def);
    if (Object.prototype.hasOwnProperty.call(meta.labelToState, label)) {
      settingsDict[meta.ngpKey] = meta.labelToState[label];
    }
  }
  out = replaceOrInsertJsonLiteral(out, "settingsDictionaryForNGPScreen", JSON.stringify(settingsDict));
  out = ensureDeployPriceObject(out, DEFAULT_DEPLOY_PRICE);
  return out;
}

function ensureKeyWithDefaultLiteral(content, key, value, treatZeroAsMissing) {
  const jsonValue = JSON.stringify(value);
  const nullRe = new RegExp(`("${escapeRegExp(key)}"\\s*:\\s*)null`, "gi");
  let out = content.replace(nullRe, (_, p1) => `${p1}${jsonValue}`);
  if (treatZeroAsMissing) {
    const zeroRe = new RegExp(`("${escapeRegExp(key)}"\\s*:\\s*)0\\b`, "gi");
    out = out.replace(zeroRe, (_, p1) => `${p1}${jsonValue}`);
  }
  const existsRe = new RegExp(`"${escapeRegExp(key)}"\\s*:`, "i");
  if (!existsRe.test(out)) {
    out = insertKeyAtRoot(out, key, jsonValue);
  }
  return out;
}

function ensureArrayKeyWithDefault(content, key, defaultList) {
  const re = new RegExp(`"${escapeRegExp(key)}"\\s*:\\s*(\\[[^\\]]*\\])`, "i");
  const m = re.exec(content);
  if (!m) {
    return replaceOrInsertJsonLiteral(content, key, JSON.stringify(defaultList));
  }
  try {
    const arr = JSON.parse(m[1]);
    let invalid = !Array.isArray(arr) || arr.length < defaultList.length;
    if (!invalid) {
      for (let i = 2; i < defaultList.length; i += 1) {
        const val = arr[i];
        if (typeof val !== "number" || Number.isNaN(val) || val === 0) {
          invalid = true;
          break;
        }
      }
    }
    if (invalid) {
      return replaceOrInsertJsonLiteral(content, key, JSON.stringify(defaultList));
    }
    return content;
  } catch (_error) {
    return replaceOrInsertJsonLiteral(content, key, JSON.stringify(defaultList));
  }
}

function ensureSettingsDictionaryForNgp(content, defaultDict) {
  const key = "settingsDictionaryForNGPScreen";
  let out = content;
  const defaultText = JSON.stringify(defaultDict);
  const nullOrZeroRe = new RegExp(`"${escapeRegExp(key)}"\\s*:\\s*(null|0\\b)`, "gi");
  out = out.replace(nullOrZeroRe, `"${key}":${defaultText}`);

  const objectRe = new RegExp(`"${escapeRegExp(key)}"\\s*:\\s*(\\{[^\\}]*\\})`, "i");
  const m = objectRe.exec(out);
  if (!m) {
    return replaceOrInsertJsonLiteral(out, key, defaultText);
  }
  try {
    const parsed = JSON.parse(m[1]);
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return replaceOrInsertJsonLiteral(out, key, defaultText);
    }
    return out;
  } catch (_error) {
    return replaceOrInsertJsonLiteral(out, key, defaultText);
  }
}

function ensureDeployPriceObject(content, defaultValue) {
  const key = "deployPrice";
  const defaultText = JSON.stringify(defaultValue);
  const objectRe = new RegExp(`"${escapeRegExp(key)}"\\s*:\\s*(\\{[^\\}]*\\})`, "i");
  const m = objectRe.exec(content);
  if (!m) {
    return replaceOrInsertJsonLiteral(content, key, defaultText);
  }
  try {
    const parsed = JSON.parse(m[1]);
    if (
      !parsed ||
      typeof parsed !== "object" ||
      Array.isArray(parsed) ||
      !Object.prototype.hasOwnProperty.call(parsed, "Region") ||
      !Object.prototype.hasOwnProperty.call(parsed, "Map")
    ) {
      return replaceOrInsertJsonLiteral(content, key, defaultText);
    }
    return content;
  } catch (_error) {
    return replaceOrInsertJsonLiteral(content, key, defaultText);
  }
}

function refreshGameStatsEditor() {
  if (!state.main) {
    if (els.gameStatsInfo) {
      els.gameStatsInfo.textContent = "No file loaded.";
    }
    if (els.distanceList) {
      els.distanceList.innerHTML = "";
    }
    if (els.gameStatList) {
      els.gameStatList.innerHTML = "";
    }
    state.gameStats.distanceInputs.clear();
    state.gameStats.statInputs.clear();
    return;
  }

  try {
    const parsed = parseGameStatsData(state.main.text);
    state.gameStats.distanceInputs.clear();
    state.gameStats.statInputs.clear();
    els.distanceList.innerHTML = "";
    els.gameStatList.innerHTML = "";

    for (const [key, value] of parsed.distanceEntries) {
      const row = createStatRow(key, value, prettifyDistanceName(key));
      els.distanceList.append(row.row);
      state.gameStats.distanceInputs.set(key, row.input);
    }

    for (const [key, value] of parsed.statEntries) {
      const row = createStatRow(key, value, prettifyGameStatName(key));
      els.gameStatList.append(row.row);
      state.gameStats.statInputs.set(key, row.input);
    }

    els.gameStatsInfo.textContent = `Distance entries: ${parsed.distanceEntries.length} | Game stats entries: ${parsed.statEntries.length}`;
  } catch (error) {
    els.gameStatsInfo.textContent = `Failed to parse game stats: ${error.message}`;
    els.distanceList.innerHTML = "";
    els.gameStatList.innerHTML = "";
    state.gameStats.distanceInputs.clear();
    state.gameStats.statInputs.clear();
  }
}

function parseGameStatsData(content) {
  let gameStatObj = {};
  let gameStatBlock = null;
  const mStat = /"gameStat"\s*:\s*\{/i.exec(content);
  if (mStat) {
    gameStatBlock = extractBraceBlock(content, mStat.index);
    gameStatObj = JSON.parse(gameStatBlock.block);
    if (!gameStatObj || typeof gameStatObj !== "object" || Array.isArray(gameStatObj)) {
      gameStatObj = {};
    }
  }

  const distBlock = findBestDistanceBlock(content);
  let distanceObj = {};
  if (distBlock) {
    distanceObj = distBlock.parsed;
  }

  return {
    gameStatBlock,
    distanceBlock: distBlock,
    statObj: gameStatObj,
    distanceObj,
    statEntries: Object.entries(gameStatObj).sort((a, b) => String(a[0]).localeCompare(String(b[0]))),
    distanceEntries: Object.entries(distanceObj).sort((a, b) => compareDistanceKey(a[0], b[0])),
  };
}

function findBestDistanceBlock(content) {
  const matches = [...content.matchAll(/"distance"\s*:\s*\{/gi)];
  let best = null;
  let bestKnownCount = -1;
  let bestTotalCount = -1;
  for (const hit of matches) {
    try {
      const block = extractBraceBlock(content, hit.index);
      const parsed = JSON.parse(block.block);
      if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
        continue;
      }
      const knownCount = countKnownDistanceKeys(parsed);
      const totalCount = Object.keys(parsed).length;
      if (knownCount > bestKnownCount || (knownCount === bestKnownCount && totalCount > bestTotalCount)) {
        bestKnownCount = knownCount;
        bestTotalCount = totalCount;
        best = { ...block, parsed };
      }
    } catch (_err) {
      // skip invalid blocks
    }
  }
  return best;
}

function countKnownDistanceKeys(parsed) {
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    return 0;
  }
  let count = 0;
  for (const key of Object.keys(parsed)) {
    const up = String(key || "").toUpperCase();
    if (REGION_ORDER.includes(up) || up === "TRIALS") {
      count += 1;
    }
  }
  return count;
}

function compareDistanceKey(a, b) {
  const aUp = String(a || "").toUpperCase();
  const bUp = String(b || "").toUpperCase();
  const aIdx = REGION_ORDER.indexOf(aUp);
  const bIdx = REGION_ORDER.indexOf(bUp);
  if (aIdx >= 0 && bIdx >= 0) {
    return aIdx - bIdx;
  }
  if (aIdx >= 0) {
    return -1;
  }
  if (bIdx >= 0) {
    return 1;
  }
  return aUp.localeCompare(bUp);
}

function prettifyGameStatName(rawKey) {
  const words = String(rawKey || "")
    .split("_")
    .filter(Boolean)
    .map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase());
  const pretty = words.join(" ");
  const replacements = {
    "Truck Sold": "Trucks Sold",
    "Truck Bought": "Trucks Bought",
    "Trailer Sold": "Trailers Sold",
    "Trailer Bought": "Trailers Bought",
    "Addon Sold": "Addons Sold",
    "Addon Bought": "Addons Bought",
  };
  return replacements[pretty] || pretty || String(rawKey || "");
}

function prettifyDistanceName(rawKey) {
  const up = String(rawKey || "").toUpperCase();
  return REGION_LONG_NAME_MAP[up] || String(rawKey || "");
}

function createStatRow(key, value, labelText) {
  const row = document.createElement("div");
  row.className = "stat-row";
  const label = document.createElement("span");
  label.textContent = labelText || key;
  const input = document.createElement("input");
  input.type = "text";
  input.value = value == null ? "" : String(value);
  row.append(label, input);
  return { row, input };
}

function saveGameStatsToMain() {
  if (!requireMain()) {
    return;
  }
  try {
    const parsed = parseGameStatsData(state.main.text);
    const updates = [];

    if (parsed.gameStatBlock) {
      for (const [key, input] of state.gameStats.statInputs.entries()) {
        const oldValue = parsed.statObj[key];
        parsed.statObj[key] = parseStatInputValue(input.value, oldValue);
      }
      updates.push({
        start: parsed.gameStatBlock.start,
        end: parsed.gameStatBlock.end,
        value: JSON.stringify(parsed.statObj),
      });
    }

    if (parsed.distanceBlock) {
      for (const [key, input] of state.gameStats.distanceInputs.entries()) {
        const oldValue = parsed.distanceObj[key];
        parsed.distanceObj[key] = parseStatInputValue(input.value, oldValue);
      }
      updates.push({
        start: parsed.distanceBlock.start,
        end: parsed.distanceBlock.end,
        value: JSON.stringify(parsed.distanceObj),
      });
    }

    if (updates.length === 0) {
      setStatus("No gameStat/distance blocks found to save.", "error");
      return;
    }

    updates.sort((a, b) => b.start - a.start);
    let content = state.main.text;
    for (const item of updates) {
      content = content.slice(0, item.start) + item.value + content.slice(item.end);
    }
    commitMain(content, "Game stats saved.");
  } catch (error) {
    setStatus(`Failed to save game stats: ${error.message}`, "error");
  }
}

function parseStatInputValue(raw, oldValue) {
  const txt = String(raw == null ? "" : raw).trim();
  if (!txt) {
    return oldValue;
  }
  if (typeof oldValue === "number") {
    const num = parseStrictNumber(txt);
    return num != null ? num : oldValue;
  }
  if (typeof oldValue === "boolean") {
    if (txt.toLowerCase() === "true") {
      return true;
    }
    if (txt.toLowerCase() === "false") {
      return false;
    }
    return oldValue;
  }
  if (/^[+-]?(?:\d+\.?\d*|\.\d+)(?:e[+-]?\d+)?$/i.test(txt)) {
    const num = parseStrictNumber(txt);
    return num != null ? num : txt;
  }
  return txt;
}

function renderRankXpTable() {
  if (!els.rankXpTable) {
    return;
  }
  els.rankXpTable.innerHTML = "";
  for (const rank of Object.keys(RANK_XP_REQUIREMENTS).map(Number).sort((a, b) => a - b)) {
    const row = document.createElement("div");
    row.className = "rank-xp-row";
    const left = document.createElement("span");
    left.textContent = String(rank);
    const colon = document.createElement("span");
    colon.textContent = ":";
    const right = document.createElement("span");
    right.textContent = String(RANK_XP_REQUIREMENTS[rank]);
    row.append(left, colon, right);
    els.rankXpTable.append(row);
  }
}

function xpForRank(rank) {
  return RANK_XP_REQUIREMENTS[rank] != null ? Number(RANK_XP_REQUIREMENTS[rank]) : null;
}

function rankForXp(xp) {
  let out = 1;
  for (const rank of Object.keys(RANK_XP_REQUIREMENTS).map(Number).sort((a, b) => a - b)) {
    if (xp >= RANK_XP_REQUIREMENTS[rank]) {
      out = rank;
    }
  }
  return out;
}

function syncXpFromRankInput() {
  if (state.ui.rankXpSyncLock) {
    return;
  }
  const parsed = parseOptionalStrictInt(els.rankInput.value);
  if (parsed.error) {
    return;
  }
  const rank = parsed.value;
  if (rank == null || rank < 1 || rank > 30) {
    return;
  }
  const xp = xpForRank(rank);
  if (xp == null) {
    return;
  }
  state.ui.rankXpSyncLock = true;
  try {
    els.xpInput.value = String(xp);
  } finally {
    state.ui.rankXpSyncLock = false;
  }
}

function syncRankFromXpInput() {
  if (state.ui.rankXpSyncLock) {
    return;
  }
  const parsed = parseOptionalStrictInt(els.xpInput.value);
  if (parsed.error) {
    return;
  }
  const xp = parsed.value;
  if (xp == null || xp < 0) {
    return;
  }
  const rank = rankForXp(xp);
  state.ui.rankXpSyncLock = true;
  try {
    els.rankInput.value = String(rank);
  } finally {
    state.ui.rankXpSyncLock = false;
  }
}

function refreshObjectivesFromMain() {
  if (!state.main) {
    state.objectives.data = null;
    state.objectives.visibleKeys = [];
    state.objectives.selected.clear();
    state.objectives.finishedInSave.clear();
    renderObjectivesList();
    return;
  }
  try {
    const m = /"objectiveStates"\s*:\s*\{/i.exec(state.main.text);
    if (!m) {
      state.objectives.data = null;
      state.objectives.visibleKeys = [];
      state.objectives.selected.clear();
      state.objectives.finishedInSave.clear();
      els.objectivesInfo.textContent = "objectiveStates block not found.";
      els.objectivesList.innerHTML = "";
      return;
    }
    const block = extractBraceBlock(state.main.text, m.index);
    const parsed = JSON.parse(block.block);
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      throw new Error("objectiveStates is not an object.");
    }
    state.objectives.data = parsed;
    state.objectives.finishedInSave = collectFinishedObjectivesFromMain(state.main.text, parsed);
    state.objectives.selected.clear();
    renderObjectivesList();
  } catch (error) {
    state.objectives.data = null;
    state.objectives.visibleKeys = [];
    state.objectives.selected.clear();
    state.objectives.finishedInSave.clear();
    els.objectivesInfo.textContent = `Failed to parse objectiveStates: ${error.message}`;
    els.objectivesList.innerHTML = "";
  }
}

function collectFinishedObjectivesFromMain(content, objectiveStates) {
  const finished = new Set();

  if (objectiveStates && typeof objectiveStates === "object" && !Array.isArray(objectiveStates)) {
    for (const [key, entry] of Object.entries(objectiveStates)) {
      if (isObjectiveFinishedEntry(entry)) {
        finished.add(String(key || "").trim().toUpperCase());
      }
    }
  }

  const re = /"(CompleteSave\d*)"\s*:\s*\{/g;
  let hit = null;
  while ((hit = re.exec(content)) !== null) {
    try {
      const block = extractBraceBlock(content, hit.index);
      const valueData = JSON.parse(block.block);

      let ssl = valueData.SslValue;
      if (!ssl || typeof ssl !== "object") {
        const nested = valueData[hit[1]];
        ssl = nested && typeof nested === "object" ? nested.SslValue : null;
      }
      if (!ssl || typeof ssl !== "object") {
        continue;
      }

      const finishedRaw = ssl.finishedObjs;
      if (Array.isArray(finishedRaw)) {
        for (const key of finishedRaw) {
          if (typeof key === "string" && key) {
            finished.add(String(key).trim().toUpperCase());
          }
        }
      } else if (finishedRaw && typeof finishedRaw === "object") {
        for (const key of Object.keys(finishedRaw)) {
          if (key) {
            finished.add(String(key).trim().toUpperCase());
          }
        }
      }
    } catch (_err) {
      // ignore malformed CompleteSave blocks
    }
  }

  return finished;
}

function humanizeObjectiveKey(key) {
  const raw = String(key || "").trim();
  if (!raw) {
    return "";
  }
  let name = raw;
  if (/^[A-Z]{2}_[0-9]{2}_[0-9]{2}_/.test(name)) {
    name = name.replace(/^[A-Z]{2}_[0-9]{2}_[0-9]{2}_/, "");
  } else if (/^[A-Z]{2}_[0-9]{2}_/.test(name)) {
    name = name.replace(/^[A-Z]{2}_[0-9]{2}_/, "");
  }
  name = name.replace(/_(OBJ|TSK|CNT|TASK|CONTRACT|CONTEST)$/i, "");
  name = name.replace(/__+/g, "_").replace(/^_+|_+$/g, "");
  const words = name.split("_").filter(Boolean);
  if (!words.length) {
    return raw;
  }
  return words
    .map((word) => {
      if (/^[0-9]+$/.test(word)) {
        return word;
      }
      if (/^[IVXLCDM]+$/.test(word)) {
        return word;
      }
      return word.charAt(0).toUpperCase() + word.slice(1).toLowerCase();
    })
    .join(" ");
}

function isObjectiveTokenLikeName(name, key) {
  const text = String(name || "").trim();
  if (!text) {
    return true;
  }
  const upper = text.toUpperCase();
  if (upper === String(key || "").toUpperCase()) {
    return true;
  }
  if (isObjectiveKey(upper)) {
    return true;
  }
  if (/^(UI_|EXP_)/.test(upper)) {
    return true;
  }
  return /^[A-Z0-9_]+$/.test(text) && text.includes("_");
}

function getObjectiveDisplayName(key, entry, meta) {
  const candidates = [
    entry && typeof entry === "object" ? entry.displayName || entry.name || entry.title || entry.label : "",
    state.objectives.catalogNames[key],
    meta && typeof meta === "object" ? meta.displayName || meta.name || meta.title : "",
  ];
  for (const candidate of candidates) {
    const text = String(candidate || "").trim();
    if (!text) {
      continue;
    }
    if (isObjectiveTokenLikeName(text, key)) {
      continue;
    }
    return text;
  }
  return humanizeObjectiveKey(key) || key;
}

function getObjectiveTypeForFilter(key, meta) {
  const source = String((meta && (meta.source || meta.category)) || "").trim().toUpperCase();
  if (source.includes("TASK")) {
    return "TASK";
  }
  if (source.includes("CONTRACT")) {
    return "CONTRACT";
  }
  if (source.includes("CONTEST")) {
    return "CONTEST";
  }

  const upperKey = String(key || "").toUpperCase();
  if (/_CNT\b|_CONTEST\b/.test(upperKey)) {
    return "CONTEST";
  }
  if (/_OBJ\b|_CONTRACT\b/.test(upperKey)) {
    return "CONTRACT";
  }
  if (/_TSK\b|_TASK\b/.test(upperKey)) {
    return "TASK";
  }
  return "";
}

function getObjectiveCategoryForFilter(meta) {
  const category = String((meta && meta.type) || "").trim().toLowerCase();
  if (category === "truckdelivery" || category === "truck_delivery") {
    return "truckDelivery";
  }
  if (category === "cargodelivery" || category === "cargo_delivery") {
    return "cargoDelivery";
  }
  if (category === "exploration" || category === "explore") {
    return "exploration";
  }
  return "";
}

function getObjectiveRegionForFilter(key, meta) {
  const regionName = cleanObjectiveMetaField(meta && (meta.regionName || meta.region || ""));
  if (regionName) {
    return regionName;
  }
  const match = /^([A-Z]{2}_[0-9]{2})/.exec(String(key || "").toUpperCase());
  return match ? match[1] : "";
}

function readObjectivesFilters() {
  return {
    search: String(els.objectivesSearch.value || "").trim().toLowerCase(),
    type: String(els.objectivesType.value || "").trim().toUpperCase(),
    region: String(els.objectivesRegion.value || "").trim(),
    category: String(els.objectivesCategory.value || "").trim(),
  };
}

function refreshObjectivesRegionFilter(keys) {
  if (!els.objectivesRegion) {
    return;
  }
  const selected = String(els.objectivesRegion.value || "");
  const regions = [];
  const seen = new Set();

  for (const key of keys) {
    const meta = state.objectives.catalogMeta[key];
    const region = getObjectiveRegionForFilter(key, meta);
    if (!region || seen.has(region)) {
      continue;
    }
    seen.add(region);
    regions.push(region);
  }
  regions.sort((a, b) => a.localeCompare(b));

  els.objectivesRegion.innerHTML = "";
  const allOption = document.createElement("option");
  allOption.value = "";
  allOption.textContent = "All regions";
  els.objectivesRegion.append(allOption);

  for (const region of regions) {
    const option = document.createElement("option");
    option.value = region;
    option.textContent = region;
    els.objectivesRegion.append(option);
  }

  if (selected && seen.has(selected)) {
    els.objectivesRegion.value = selected;
  } else {
    els.objectivesRegion.value = "";
  }
}

function isObjectiveInSearch(key, displayName, meta, search) {
  if (!search) {
    return true;
  }
  const parts = [
    key,
    displayName,
    meta && meta.category,
    meta && meta.region,
    meta && meta.regionName,
    meta && meta.type,
    meta && meta.cargoNeeded,
    meta && meta.descriptionText,
  ];
  return parts.some((part) => String(part || "").toLowerCase().includes(search));
}

function matchesObjectiveFilters(key, entry, filters) {
  const meta = state.objectives.catalogMeta[key];
  const displayName = getObjectiveDisplayName(key, entry, meta);
  if (!isObjectiveInSearch(key, displayName, meta, filters.search)) {
    return false;
  }
  if (filters.type && getObjectiveTypeForFilter(key, meta) !== filters.type) {
    return false;
  }
  if (filters.region && getObjectiveRegionForFilter(key, meta) !== filters.region) {
    return false;
  }
  if (filters.category && getObjectiveCategoryForFilter(meta) !== filters.category) {
    return false;
  }
  return true;
}

function buildObjectiveMetaParts(meta, includeRewards) {
  if (!meta || typeof meta !== "object") {
    return [];
  }
  const parts = [];
  if (meta.category) {
    parts.push(String(meta.category).replace(/^_+/, ""));
  }
  if (meta.regionName || meta.region) {
    parts.push(meta.regionName || meta.region);
  }
  if (meta.type) {
    parts.push(meta.type);
  }
  if (includeRewards && (meta.experience || meta.money)) {
    const rewards = [];
    if (meta.experience) {
      rewards.push(`${meta.experience} XP`);
    }
    if (meta.money) {
      rewards.push(`${meta.money} money`);
    }
    if (rewards.length) {
      parts.push(rewards.join(" / "));
    }
  }
  if (meta.cargoNeeded) {
    parts.push(meta.cargoNeeded);
  }
  return parts;
}

function isObjectiveFinishedEntry(entry) {
  if (!entry || typeof entry !== "object") {
    return false;
  }
  return Boolean(entry.isFinished === true || entry.wasCompletedAtLeastOnce === true);
}

function renderObjectivesList() {
  els.objectivesList.innerHTML = "";

  if (!state.objectives.data) {
    if (!state.main && state.objectives.catalogKeys.size > 0) {
      const keys = [...state.objectives.catalogKeys].sort();
      refreshObjectivesRegionFilter(keys);
      const filters = readObjectivesFilters();
      const visible = keys.filter((key) => matchesObjectiveFilters(key, null, filters));
      state.objectives.visibleKeys = visible;

      for (const key of visible) {
        const meta = state.objectives.catalogMeta[key];
        const displayName = getObjectiveDisplayName(key, null, meta) || humanizeObjectiveKey(key) || key;
        const row = document.createElement("div");
        row.className = "objective-row objective-row-no-status";

        const infoWrap = document.createElement("div");
        const nameText = document.createElement("div");
        nameText.className = "objective-title";
        nameText.textContent = displayName;
        infoWrap.append(nameText);

        const metaParts = buildObjectiveMetaParts(meta, false);
        if (metaParts.length) {
          const metaText = document.createElement("div");
          metaText.className = "help";
          metaText.textContent = metaParts.join(" | ");
          infoWrap.append(metaText);
        }

        const check = document.createElement("input");
        check.type = "checkbox";
        check.checked = state.objectives.selected.has(key);
        check.addEventListener("change", () => {
          if (check.checked) {
            state.objectives.selected.add(key);
          } else {
            state.objectives.selected.delete(key);
          }
          updateObjectivesInfo({
            mode: "catalog",
            total: keys.length,
            filtered: visible.length,
            shown: visible.length,
          });
        });

        row.append(infoWrap, check);
        els.objectivesList.append(row);
      }

      updateObjectivesInfo({
        mode: "catalog",
        total: keys.length,
        filtered: visible.length,
        shown: visible.length,
      });
      return;
    }

    if (state.main) {
      els.objectivesInfo.textContent = "objectiveStates not loaded.";
    } else {
      setObjectivesNeedMainMessage();
    }
    return;
  }

  const keys = state.objectives.catalogKeys.size
    ? [...state.objectives.catalogKeys].sort()
    : Object.keys(state.objectives.data).sort();
  const regionKeys = state.objectives.catalogKeys.size ? [...state.objectives.catalogKeys] : keys;
  refreshObjectivesRegionFilter(regionKeys);
  const filters = readObjectivesFilters();
  const visible = keys.filter((key) => matchesObjectiveFilters(key, state.objectives.data[key], filters));
  state.objectives.visibleKeys = visible;

  let finishedCount = 0;
  for (const key of visible) {
    const entry = state.objectives.data[key];
    const isFinished =
      state.objectives.finishedInSave.has(String(key || "").toUpperCase()) ||
      isObjectiveFinishedEntry(entry);
    if (isFinished) {
      finishedCount += 1;
    }

    const meta = state.objectives.catalogMeta[key];
    const displayName = getObjectiveDisplayName(key, entry, meta) || humanizeObjectiveKey(key) || key;
    const row = document.createElement("div");
    row.className = "objective-row";

    const infoWrap = document.createElement("div");
    const nameText = document.createElement("div");
    nameText.className = "objective-title";
    nameText.textContent = displayName;
    infoWrap.append(nameText);

    const metaParts = buildObjectiveMetaParts(meta, true);
    if (metaParts.length) {
      const metaText = document.createElement("div");
      metaText.className = "help";
      metaText.textContent = metaParts.join(" | ");
      infoWrap.append(metaText);
    }

    const status = document.createElement("span");
    status.className = "pill";
    status.textContent = isFinished ? "finished" : "pending";

    const check = document.createElement("input");
    check.type = "checkbox";
    if (isFinished) {
      state.objectives.selected.delete(key);
    }
    check.checked = isFinished || state.objectives.selected.has(key);
    check.disabled = isFinished;
    check.addEventListener("change", () => {
      if (isFinished) {
        return;
      }
      if (check.checked) {
        state.objectives.selected.add(key);
      } else {
        state.objectives.selected.delete(key);
      }
      updateObjectivesInfo({
        mode: "main",
        total: keys.length,
        filtered: visible.length,
        shown: visible.length,
        finishedShown: finishedCount,
      });
    });

    row.append(infoWrap, status, check);
    els.objectivesList.append(row);
  }

  updateObjectivesInfo({
    mode: "main",
    total: keys.length,
    filtered: visible.length,
    shown: visible.length,
    finishedShown: finishedCount,
  });
}

function updateObjectivesInfo(summary) {
  const source = state.objectives.catalogSource ? ` | Source: ${state.objectives.catalogSource}` : "";
  if (summary.mode === "catalog") {
    els.objectivesInfo.textContent =
      `Catalog entries: ${summary.total} | Filtered: ${summary.filtered} | Showing: ${summary.shown} | Selected: ${state.objectives.selected.size} | Upload Main Save to apply changes.${source}`;
    return;
  }
  els.objectivesInfo.textContent =
    `Total: ${summary.total} | Filtered: ${summary.filtered} | Showing: ${summary.shown} | Finished shown: ${summary.finishedShown || 0} | Selected: ${state.objectives.selected.size}${source}`;
}

function setObjectivesNeedMainMessage() {
  const catalogLoaded = Math.max(Object.keys(state.objectives.catalogNames).length, state.objectives.catalogKeys.size);
  const source = state.objectives.catalogSource || "embedded";
  if (catalogLoaded > 0) {
    els.objectivesInfo.textContent = `Catalog ready (${catalogLoaded} entries, source: ${source}). Upload Main Save to load objectiveStates.`;
  } else {
    els.objectivesInfo.textContent = "Upload Main Save to load objectiveStates.";
  }
}

function selectVisibleObjectives() {
  for (const key of state.objectives.visibleKeys) {
    if (state.objectives.finishedInSave.has(String(key || "").toUpperCase())) {
      continue;
    }
    if (state.objectives.data && isObjectiveFinishedEntry(state.objectives.data[key])) {
      continue;
    }
    state.objectives.selected.add(key);
  }
  renderObjectivesList();
}

function clearObjectiveSelection() {
  state.objectives.selected.clear();
  renderObjectivesList();
}

function scheduleObjectivesCatalogWarmup() {
  if (state.objectives.catalogLoaded || state.objectives.catalogLoading || state.objectives.catalogWarmupScheduled) {
    return;
  }
  state.objectives.catalogWarmupScheduled = true;
  const run = () => {
    state.objectives.catalogWarmupScheduled = false;
    ensureObjectivesCatalogLoaded(true);
  };
  if (typeof window !== "undefined" && typeof window.requestIdleCallback === "function") {
    window.requestIdleCallback(run, { timeout: 2000 });
  } else {
    window.setTimeout(run, 550);
  }
}

function ensureObjectivesCatalogLoaded(silent = false) {
  if (state.objectives.catalogLoaded) {
    return true;
  }
  if (!state.objectives.catalogLoadPromise) {
    state.objectives.catalogLoadPromise = loadEmbeddedObjectivesCatalog(silent)
      .catch((error) => {
        if (!silent) {
          setStatus(`Objectives+ catalog failed to load: ${error.message}`, "error");
        }
      })
      .finally(() => {
        state.objectives.catalogLoadPromise = null;
      });
  }
  return false;
}

async function loadObjectivesCatalogFromScript() {
  const existing = typeof window !== "undefined" ? window.__OBJECTIVES_CATALOG_CSV__ : "";
  if (typeof existing === "string" && existing.trim()) {
    return existing;
  }
  if (!objectivesCatalogScriptPromise) {
    objectivesCatalogScriptPromise = new Promise((resolve, reject) => {
      const script = document.createElement("script");
      script.src = OBJECTIVES_CATALOG_SCRIPT_URL;
      script.async = true;
      script.onload = () => resolve();
      script.onerror = () => reject(new Error("catalog script failed"));
      document.head.append(script);
    }).finally(() => {
      objectivesCatalogScriptPromise = null;
    });
  }
  await objectivesCatalogScriptPromise;
  const loaded = typeof window !== "undefined" ? window.__OBJECTIVES_CATALOG_CSV__ : "";
  return typeof loaded === "string" ? loaded : "";
}

async function loadEmbeddedObjectivesCatalog(silent = false) {
  if (state.objectives.catalogLoaded || state.objectives.catalogLoading) {
    return;
  }
  state.objectives.catalogLoading = true;

  let raw = "";
  let sourceLabel = "embedded";

  try {
    raw = await loadObjectivesCatalogFromScript();
    if (raw) {
      sourceLabel = "embedded file";
    }
  } catch (_scriptError) {
    // Fall back to CSV fetch and final inline fallback.
  }

  try {
    if (!raw && OBJECTIVES_CATALOG_CSV_URL && typeof fetch === "function") {
      const response = await fetch(OBJECTIVES_CATALOG_CSV_URL, { cache: "force-cache" });
      if (response.ok) {
        raw = await response.text();
        sourceLabel = "local file";
      }
    }
  } catch (_error) {
    // Fall back to inline data if local catalog fetch fails.
  }

  if (!raw) {
    raw = String(EMBEDDED_OBJECTIVES_CSV || "");
    sourceLabel = "embedded";
  }

  if (!raw) {
    state.objectives.catalogLoading = false;
    if (!silent) {
      setStatus("Objectives+ catalog is missing.", "error");
    }
    return;
  }

  try {
    const count = applyObjectiveCatalog(raw, sourceLabel);
    state.objectives.catalogLoaded = count > 0;
    if (!silent) {
      setStatus(`Objectives+ catalog loaded (${count} keys).`, "success");
    }
    if (document.querySelector(".tab-btn.active")?.dataset?.tab === "objectives") {
      renderObjectivesList();
    }
  } catch (error) {
    if (!silent) {
      setStatus(`Objectives+ catalog failed to load: ${error.message}`, "error");
    }
  } finally {
    state.objectives.catalogLoading = false;
  }
}

function applyObjectiveCatalog(text, sourceLabel) {
  const parsed = parseOnlineObjectiveCatalog(text);
  if (!parsed.keys.size) {
    throw new Error("No objective keys found in embedded catalog.");
  }
  state.objectives.catalogNames = parsed.names;
  state.objectives.catalogKeys = parsed.keys;
  state.objectives.catalogMeta = parsed.meta;
  state.objectives.catalogSource = sourceLabel || "";
  return parsed.keys.size;
}

function parseOnlineObjectiveCatalog(text) {
  const trimmed = String(text || "").trim();
  const names = {};
  const keys = new Set();
  const meta = {};

  const csvParsed = parseObjectiveCsvCatalog(trimmed);
  if (csvParsed) {
    return csvParsed;
  }

  // If source is JSON, support object map or list rows.
  if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
    try {
      const data = JSON.parse(trimmed);
      if (Array.isArray(data)) {
        for (const row of data) {
          addObjectiveCatalogEntry(keys, names, meta, row, "");
        }
      } else if (data && typeof data === "object") {
        for (const [k, v] of Object.entries(data)) {
          addObjectiveCatalogEntry(keys, names, meta, v, k);
        }
      }
      if (keys.size > 0) {
        return { keys, names, meta };
      }
    } catch (_err) {
      // fall through to regex mode
    }
  }

  // Generic text fallback: extract objective-like keys.
  const pattern = /\b[A-Z]{2}_[0-9]{2}_[A-Z0-9_]{3,}\b/g;
  const all = trimmed.match(pattern) || [];
  for (const token of all) {
    const key = String(token || "").toUpperCase();
    if (isLikelyObjectiveKey(key)) {
      keys.add(key);
    }
  }

  return { keys, names, meta };
}

function parseObjectiveCsvCatalog(text) {
  const rows = parseCsvRows(text);
  if (!rows.length) {
    return null;
  }
  const header = rows[0].map((cell) => String(cell || "").replace(/^\uFEFF/, "").trim().toLowerCase());
  const keyIndex = header.findIndex((name) => name === "key" || name === "objective" || name === "id");
  if (keyIndex === -1) {
    return null;
  }

  const keys = new Set();
  const names = {};
  const meta = {};
  for (let i = 1; i < rows.length; i += 1) {
    const record = {};
    for (let j = 0; j < header.length; j += 1) {
      record[header[j]] = rows[i][j] == null ? "" : rows[i][j];
    }
    addObjectiveCatalogEntry(keys, names, meta, record, record.key || record.objective || record.id || "");
  }

  if (!keys.size) {
    return null;
  }
  return { keys, names, meta };
}

function parseCsvRows(text) {
  const rows = [];
  let row = [];
  let cell = "";
  let inQuotes = false;
  for (let i = 0; i < text.length; i += 1) {
    const ch = text[i];
    if (inQuotes) {
      if (ch === "\"") {
        if (text[i + 1] === "\"") {
          cell += "\"";
          i += 1;
        } else {
          inQuotes = false;
        }
      } else {
        cell += ch;
      }
      continue;
    }
    if (ch === "\"") {
      inQuotes = true;
      continue;
    }
    if (ch === ",") {
      row.push(cell);
      cell = "";
      continue;
    }
    if (ch === "\n") {
      row.push(cell);
      rows.push(row);
      row = [];
      cell = "";
      continue;
    }
    if (ch === "\r") {
      continue;
    }
    cell += ch;
  }
  if (cell.length || row.length) {
    row.push(cell);
    rows.push(row);
  }
  return rows;
}

function addObjectiveCatalogEntry(keys, names, meta, payload, keyHint) {
  const key =
    typeof keyHint === "string" && keyHint
      ? String(keyHint).trim().toUpperCase()
      : String(payload && (payload.key || payload.objective || payload.id || "")).trim().toUpperCase();
  if (!isObjectiveKey(key)) {
    return;
  }

  keys.add(key);

  if (typeof payload === "string") {
    const name = payload.trim();
    if (name) {
      names[key] = name;
    }
    return;
  }

  if (!payload || typeof payload !== "object") {
    return;
  }

  const name = String(payload.displayName || payload.displayname || payload.name || payload.title || payload.label || "").trim();
  if (name) {
    names[key] = name;
  }

  const entryMeta = {
    category: cleanObjectiveMetaField(payload.category || payload.group || ""),
    region: cleanObjectiveMetaField(payload.region || payload.region_code || payload.regioncode || ""),
    regionName: cleanObjectiveMetaField(payload.region_name || payload.regionname || ""),
    type: cleanObjectiveMetaField(payload.type || ""),
    cargoNeeded: cleanObjectiveMetaField(payload.cargo_needed || payload.cargoneeded || ""),
    experience: cleanObjectiveMetaField(payload.experience || payload.xp || ""),
    money: cleanObjectiveMetaField(payload.money || payload.reward_money || ""),
    descriptionText: cleanObjectiveMetaField(payload.descriptionText || payload.descriptiontext || payload.description || ""),
    source: cleanObjectiveMetaField(payload.source || payload.Source || ""),
  };

  if (Object.values(entryMeta).some((value) => value)) {
    meta[key] = entryMeta;
  }
}

function cleanObjectiveMetaField(value) {
  if (value == null) {
    return "";
  }
  return String(value).replace(/\s+/g, " ").trim();
}

function isObjectiveKey(token) {
  if (!token) {
    return false;
  }
  const s = String(token).toUpperCase();
  return /^[A-Z]{2}_[0-9]{2}_[A-Z0-9_]{3,}$/.test(s);
}

function isLikelyObjectiveKey(token) {
  if (!isObjectiveKey(token)) {
    return false;
  }
  return (
    token.includes("_OBJ") ||
    token.includes("_TSK") ||
    token.includes("_CNT") ||
    token.includes("_TASK") ||
    token.includes("_CONTRACT") ||
    token.includes("_CONTEST") ||
    token.includes("_DELIVER") ||
    token.includes("_DELIVERY")
  );
}

function completeSelectedObjectives() {
  if (!requireMain()) {
    return;
  }
  if (state.objectives.selected.size === 0) {
    setStatus("No objectives selected.", "error");
    return;
  }
  try {
    const m = /"objectiveStates"\s*:\s*\{/i.exec(state.main.text);
    if (!m) {
      throw new Error('"objectiveStates" block not found.');
    }
    const block = extractBraceBlock(state.main.text, m.index);
    const obj = JSON.parse(block.block);
    let updated = 0;
    for (const key of state.objectives.selected) {
      let entry = obj[key];
      if (!entry || typeof entry !== "object") {
        entry = {};
        obj[key] = entry;
      }
      if (entry.isFinished !== true || entry.wasCompletedAtLeastOnce !== true) {
        entry.isFinished = true;
        entry.wasCompletedAtLeastOnce = true;
        updated += 1;
      }
    }
    const newBlock = JSON.stringify(obj);
    const content = state.main.text.slice(0, block.start) + newBlock + state.main.text.slice(block.end);
    state.objectives.selected.clear();
    commitMain(content, `Objectives+ updated (${updated} completed).`);
  } catch (error) {
    setStatus(`Failed to complete objectives: ${error.message}`, "error");
  }
}

function onApplyMoneyRank() {
  if (!requireMain()) {
    return;
  }
  const moneyInput = parseOptionalMoneyInt(els.moneyInput.value);
  const rankInput = parseOptionalStrictInt(els.rankInput.value);
  const xpInput = parseOptionalStrictInt(els.xpInput.value);

  if (moneyInput.error) {
    setStatus("Money must be an integer (e.g. -100 or 12345).", "error");
    return;
  }
  if (rankInput.error) {
    setStatus("Rank must be a whole number in range 1-30.", "error");
    return;
  }
  if (xpInput.error) {
    setStatus("Experience must be a non-negative whole number.", "error");
    return;
  }

  const money = moneyInput.value;
  let rank = rankInput.value;
  let xp = xpInput.value;

  if (money == null && rank == null && xp == null) {
    setStatus("Enter at least one value.", "error");
    return;
  }
  if (rank != null && (rank < 1 || rank > 30)) {
    setStatus("Rank must be in range 1-30.", "error");
    return;
  }
  if (xp != null && xp < 0) {
    setStatus("Experience must be non-negative.", "error");
    return;
  }

  // Keep rank/experience linked like the desktop app:
  // rank change sets XP, XP change computes rank.
  if (rank != null && xp == null) {
    xp = xpForRank(rank);
  } else if (xp != null && rank == null) {
    rank = rankForXp(xp);
  } else if (rank != null && xp != null) {
    xp = xpForRank(rank);
  }

  try {
    let content = state.main.text;
    const writeMeta = [];
    if (money != null) {
      const out = replaceOrInsertNumericWithCount(content, "money", money);
      content = out.content;
      writeMeta.push({ key: "money", count: out.count });
    }
    if (rank != null) {
      const out = replaceOrInsertNumericWithCount(content, "rank", rank);
      content = out.content;
      writeMeta.push({ key: "rank", count: out.count });
    }
    if (xp != null) {
      const out = replaceOrInsertNumericWithCount(content, "experience", xp);
      content = out.content;
      writeMeta.push({ key: "experience", count: out.count });
    }
    let message = "Money/Rank/XP updated.";
    if (moneyInput.clamped) {
      message += ` Money clamped to ${money} (allowed ${MONEY_MIN} to ${MONEY_MAX}).`;
    }
    const duplicates = writeMeta.filter((entry) => entry.count > 1);
    if (duplicates.length > 0) {
      const details = duplicates.map((entry) => `${entry.key} x${entry.count}`).join(", ");
      message += ` Duplicate keys updated: ${details}.`;
    }
    commitMain(content, message);
  } catch (error) {
    setStatus(`Failed to update money/rank/xp: ${error.message}`, "error");
  }
}

function onApplyTime() {
  if (!requireMain()) {
    return;
  }
  const dayInput = parseOptionalStrictFloat(els.timeDayInput.value);
  const nightInput = parseOptionalStrictFloat(els.timeNightInput.value);
  if (dayInput.error) {
    setStatus("Day speed must be a valid number.", "error");
    return;
  }
  if (nightInput.error) {
    setStatus("Night speed must be a valid number.", "error");
    return;
  }
  let day = dayInput.value;
  let night = nightInput.value;
  const skip = Boolean(els.skipTimeInput.checked);

  const current = getFileInfo(state.main.text);
  if (day == null) {
    day = current.day;
  }
  if (night == null) {
    night = current.night;
  }
  if (day == null || night == null) {
    setStatus("Set both day and night values.", "error");
    return;
  }

  try {
    let content = state.main.text;
    content = replaceOrInsertNumeric(content, "timeSettingsDay", day);
    content = replaceOrInsertNumeric(content, "timeSettingsNight", night);
    content = replaceOrInsertBoolean(content, "isAbleToSkipTime", skip);
    commitMain(content, "Time settings updated.");
  } catch (error) {
    setStatus(`Failed to update time: ${error.message}`, "error");
  }
}

function onApplyMissions() {
  if (!requireMain()) {
    return;
  }
  const selectedSeasons = state.selectors.missions.getSelectedSeasons();
  const selectedMaps = state.selectors.missions.getSelectedMaps();
  if (selectedSeasons.length === 0 && selectedMaps.length === 0) {
    setStatus("Select at least one season or map.", "error");
    return;
  }
  try {
    const out = completeSeasonsAndMaps(state.main.text, selectedSeasons, selectedMaps);
    commitMain(out.content, out.message);
  } catch (error) {
    setStatus(`Failed to complete missions: ${error.message}`, "error");
  }
}

function onApplyContests() {
  if (!requireMain()) {
    return;
  }
  const selectedSeasons = state.selectors.contests.getSelectedSeasons();
  const selectedMaps = state.selectors.contests.getSelectedMaps();
  if (selectedSeasons.length === 0 && selectedMaps.length === 0) {
    setStatus("Select at least one season or map.", "error");
    return;
  }
  try {
    const out = markDiscoveredContestsComplete(state.main.text, selectedSeasons, selectedMaps);
    commitMain(out.content, out.message);
  } catch (error) {
    setStatus(`Failed to complete contests: ${error.message}`, "error");
  }
}

function onApplyRegions() {
  if (!requireMain()) {
    return;
  }
  const selector = state.selectors.regions;
  if (!selector) {
    setStatus("Regions+ selector unavailable.", "error");
    return;
  }

  const selectedFeatures = getSelectedRegionsFeatures();
  if (selectedFeatures.length === 0) {
    setStatus("Select at least one Regions+ feature.", "error");
    return;
  }

  const selectedSeasons = selector.getSelectedSeasons();
  const selectedMaps = selector.getSelectedMaps();
  const selectedRegions = selector.getSelectedRegions();
  if (selectedSeasons.length === 0 && selectedMaps.length === 0 && selectedRegions.length === 0) {
    setStatus("Select at least one season or map.", "error");
    return;
  }

  try {
    let content = state.main.text;
    const messages = [];
    for (const item of selectedFeatures) {
      const def = item.definition;
      let out = null;
      if (def.mode === "seasonMap") {
        out =
          def.key === "missions"
            ? completeSeasonsAndMaps(content, selectedSeasons, selectedMaps)
            : markDiscoveredContestsComplete(content, selectedSeasons, selectedMaps);
      } else if (def.key === "upgrades") {
        out = unlockUpgrades(content, selectedRegions);
      } else if (def.key === "watchtowers") {
        out = unlockWatchtowers(content, selectedRegions);
      } else if (def.key === "discoveries") {
        out = unlockDiscoveries(content, selectedRegions);
      } else if (def.key === "levels") {
        out = unlockLevels(content, selectedRegions);
      } else if (def.key === "garages") {
        out = unlockGarages(content, selectedRegions, item.extra);
      }
      if (!out || typeof out.content !== "string") {
        throw new Error(`Regions+ action failed for ${def.label}.`);
      }
      content = out.content;
      messages.push(`${def.label}: ${out.message || "applied"}`);
    }

    if (els.regionsInfo) {
      els.regionsInfo.textContent = messages.join(" ");
    }
    commitMain(
      content,
      `Regions+ updated (${selectedFeatures.length} feature${selectedFeatures.length === 1 ? "" : "s"}).`,
    );
  } catch (error) {
    setStatus(`Failed Regions+ action: ${error.message}`, "error");
  }
}

function onApplyRegionWithSelector(selector, mutator, successMessage) {
  if (!requireMain()) {
    return;
  }
  if (!selector) {
    setStatus("Region selector unavailable.", "error");
    return;
  }
  const selectedRegions = selector.getSelectedRegions();
  if (selectedRegions.length === 0) {
    setStatus("Select at least one region.", "error");
    return;
  }
  try {
    const out = mutator(state.main.text, selectedRegions);
    commitMain(out.content, out.message || successMessage);
  } catch (error) {
    setStatus(`Failed region action: ${error.message}`, "error");
  }
}

function onSaveTrials() {
  if (!requireCommon()) {
    return;
  }
  try {
    const selected = [];
    for (const [code, input] of state.trialChecks.entries()) {
      if (input.checked) {
        selected.push(code);
      }
    }
    const newText = writeFinishedTrials(state.common.text, selected);
    commitCommon(newText, "Trials updated.");
  } catch (error) {
    setStatus(`Failed to save trials: ${error.message}`, "error");
  }
}

function onSavePros() {
  if (!requireCommon()) {
    return;
  }
  try {
    const container = parseCommonContainer(state.common.text);
    const ssl = getCommonSslValue(container.parsed);
    let ent = ssl.givenProsEntitlements;
    if (!Array.isArray(ent)) {
      ent = [];
    }
    for (const [key] of PROS_ENTITLEMENTS) {
      const checked = key === "ProsRegistrationReward" ? els.prosRegistration.checked : els.prosRoadcraft.checked;
      if (checked && !ent.includes(key)) {
        ent.push(key);
      }
      if (!checked) {
        ent = ent.filter((v) => v !== key);
      }
    }
    ssl.givenProsEntitlements = ent;
    const newText = writeCommonContainer(state.common.text, container);
    commitCommon(newText, "PROS entitlements updated.");
  } catch (error) {
    setStatus(`Failed to save PROS: ${error.message}`, "error");
  }
}

function onUnlockAchievements() {
  if (!requireCommon()) {
    return;
  }
  try {
    const container = parseCommonContainer(state.common.text);
    const ssl = getCommonSslValue(container.parsed);
    const states = ssl.achievementStates;
    if (!states || typeof states !== "object" || Array.isArray(states)) {
      throw new Error("achievementStates not found.");
    }
    let updated = 0;
    for (const [key, value] of Object.entries(states)) {
      if (value && typeof value === "object" && !Array.isArray(value)) {
        if (value.isUnlocked !== true) {
          value.isUnlocked = true;
          updated += 1;
        }
      } else {
        states[key] = { isUnlocked: true };
        updated += 1;
      }
    }
    const newText = writeCommonContainer(state.common.text, container);
    commitCommon(newText, `Achievements unlocked for ${updated} entries.`);
  } catch (error) {
    setStatus(`Failed to unlock achievements: ${error.message}`, "error");
  }
}

function refreshTrialsFromCommon() {
  if (!state.common) {
    setAllTrials(false);
    return;
  }
  const finished = readFinishedTrials(state.common.text);
  for (const [code, input] of state.trialChecks.entries()) {
    input.checked = finished.includes(code);
  }
}

function refreshProsFromCommon() {
  els.prosRegistration.checked = false;
  els.prosRoadcraft.checked = false;
  if (!state.common) {
    return;
  }
  try {
    const container = parseCommonContainer(state.common.text);
    const ssl = getCommonSslValue(container.parsed);
    const ent = Array.isArray(ssl.givenProsEntitlements) ? ssl.givenProsEntitlements : [];
    els.prosRegistration.checked = ent.includes("ProsRegistrationReward");
    els.prosRoadcraft.checked = ent.includes("ProsRoadcraftReward");
  } catch (_err) {
    // keep controls clear on parse error
  }
}

function refreshAchievementsFromCommon() {
  if (!state.common) {
    els.achievementsStats.textContent = "No CommonSslSave loaded.";
    return;
  }
  try {
    const container = parseCommonContainer(state.common.text);
    const ssl = getCommonSslValue(container.parsed);
    const states = ssl.achievementStates;
    if (!states || typeof states !== "object" || Array.isArray(states)) {
      els.achievementsStats.textContent = "No achievementStates block found in CommonSslSave.";
      return;
    }
    const total = Object.keys(states).length;
    let unlocked = 0;
    for (const value of Object.values(states)) {
      if (value && typeof value === "object" && value.isUnlocked) {
        unlocked += 1;
      }
    }
    els.achievementsStats.textContent = `Achievements found: ${total}. Unlocked: ${unlocked}.`;
  } catch (error) {
    els.achievementsStats.textContent = `CommonSslSave parse issue: ${error.message}`;
  }
}

function getFileInfo(content) {
  return {
    money: readMaxIntKey(content, "money"),
    rank: readMaxIntKey(content, "rank"),
    xp: readMaxIntKey(content, "experience"),
    day: readNumericKey(content, "timeSettingsDay"),
    night: readNumericKey(content, "timeSettingsNight"),
    skipTime: readBoolKey(content, "isAbleToSkipTime"),
  };
}

function completeSeasonsAndMaps(content, selectedSeasons, selectedMaps) {
  const m = /"objectiveStates"\s*:\s*\{/i.exec(content);
  if (!m) {
    throw new Error('"objectiveStates" block not found.');
  }
  const block = extractBraceBlock(content, m.index);
  const objStates = JSON.parse(block.block);
  let modified = 0;
  for (const key of Object.keys(objStates)) {
    const entry = objStates[key];
    if (!entry || typeof entry !== "object") {
      continue;
    }
    let matched = false;
    for (const season of selectedSeasons) {
      const mapCode = SEASON_ID_MAP[season];
      if ((mapCode && key.includes(mapCode)) || key.includes(`_${String(season).padStart(2, "0")}_`)) {
        matched = true;
        break;
      }
    }
    if (!matched) {
      for (const map of selectedMaps) {
        if (key.includes(map)) {
          matched = true;
          break;
        }
      }
    }
    if (!matched) {
      continue;
    }
    if (entry.isFinished !== true || entry.wasCompletedAtLeastOnce !== true) {
      entry.isFinished = true;
      entry.wasCompletedAtLeastOnce = true;
      modified += 1;
    }
  }
  const newBlock = JSON.stringify(objStates);
  const nextContent = content.slice(0, block.start) + newBlock + content.slice(block.end);
  if (modified === 0) {
    return { content: nextContent, message: "No matching missions found." };
  }
  return { content: nextContent, message: `Selected missions marked complete (${modified} updated).` };
}

function markDiscoveredContestsComplete(content, selectedSeasons, selectedMaps) {
  const selectedRegionCodes = selectedSeasons.map((s) => SEASON_ID_MAP[s]).filter(Boolean);
  const seasonTokens = selectedSeasons.map((s) => `_${String(s).padStart(2, "0")}_`);
  const matches = [];
  const re = /"(CompleteSave\d*)"\s*:\s*\{/g;
  let m = null;
  while ((m = re.exec(content)) !== null) {
    matches.push({ key: m[1], index: m.index });
  }

  let changedBlocks = 0;
  let totalAdded = 0;
  const globalContestTimesNewEntries = {};
  for (let i = matches.length - 1; i >= 0; i -= 1) {
    const hit = matches[i];
    const valueBlock = extractBraceBlock(content, hit.index);
    let valueData = null;
    try {
      valueData = JSON.parse(valueBlock.block);
    } catch (_err) {
      continue;
    }

    let ssl = valueData.SslValue;
    if (!ssl || typeof ssl !== "object") {
      const maybeNested = valueData[hit.key];
      ssl = maybeNested && typeof maybeNested === "object" ? maybeNested.SslValue : null;
    }
    if (!ssl || typeof ssl !== "object") {
      ssl = {};
    }

    const discoveredRaw = ssl.discoveredObjectives;
    let discovered = [];
    if (Array.isArray(discoveredRaw)) {
      discovered = discoveredRaw;
    } else if (discoveredRaw && typeof discoveredRaw === "object") {
      discovered = Object.keys(discoveredRaw);
    }

    const finishedRaw = ssl.finishedObjs;
    const finishedIsDict = Boolean(finishedRaw && typeof finishedRaw === "object" && !Array.isArray(finishedRaw));
    const finishedSet = new Set(
      Array.isArray(finishedRaw)
        ? finishedRaw
        : finishedIsDict
          ? Object.keys(finishedRaw)
          : [],
    );

    let contestTimes = ssl.contestTimes;
    if (!contestTimes || typeof contestTimes !== "object" || Array.isArray(contestTimes)) {
      contestTimes = {};
    }

    const addedKeys = [];
    for (const key of discovered) {
      if (typeof key !== "string") {
        continue;
      }
      let matched = seasonTokens.some((token) => key.includes(token));
      if (!matched) {
        for (const code of [...selectedRegionCodes, ...selectedMaps]) {
          if (code && key.includes(code)) {
            matched = true;
            break;
          }
        }
      }
      if (!matched) {
        continue;
      }
      if (!finishedSet.has(key)) {
        finishedSet.add(key);
        addedKeys.push(key);
      }
      if (!(key in contestTimes)) {
        contestTimes[key] = 1;
        globalContestTimesNewEntries[key] = 1;
      }
    }

    if (addedKeys.length === 0) {
      continue;
    }

    ssl.finishedObjs = finishedIsDict
      ? Object.fromEntries([...finishedSet].map((key) => [key, true]))
      : [...finishedSet];
    ssl.contestTimes = contestTimes;
    if (Array.isArray(ssl.viewedUnactivatedObjectives)) {
      ssl.viewedUnactivatedObjectives = ssl.viewedUnactivatedObjectives.filter((v) => !addedKeys.includes(v));
    }

    valueData.SslValue = ssl;
    const newBlock = JSON.stringify(valueData);
    content = content.slice(0, valueBlock.start) + newBlock + content.slice(valueBlock.end);
    changedBlocks += 1;
    totalAdded += addedKeys.length;
  }

  if (Object.keys(globalContestTimesNewEntries).length > 0) {
    content = updateAllContestTimesBlocks(content, globalContestTimesNewEntries);
  }

  if (changedBlocks === 0) {
    return { content, message: "No discovered contest entries matched selected regions." };
  }
  return {
    content,
    message: `Updated ${changedBlocks} CompleteSave block(s). Added ${totalAdded} finished entries.`,
  };
}

function ensureWatchpointsDefaults(wpData) {
  let added = 0;
  let data = wpData.data;
  if (!data || typeof data !== "object" || Array.isArray(data)) {
    data = {};
    wpData.data = data;
  }
  for (const [mapKey, towers] of Object.entries(SAFE_DEFAULTS.watchpointsUnlocks)) {
    let existing = data[mapKey];
    if (!existing || typeof existing !== "object" || Array.isArray(existing)) {
      existing = {};
      data[mapKey] = existing;
    }
    for (const towerKey of Object.keys(towers || {})) {
      if (!Object.prototype.hasOwnProperty.call(existing, towerKey)) {
        existing[towerKey] = false;
        added += 1;
      }
    }
  }
  return added;
}

function ensureUpgradesDefaults(upgradesData) {
  let added = 0;
  for (const [mapKey, upgrades] of Object.entries(SAFE_DEFAULTS.upgradesGiverUnlocks)) {
    let existing = upgradesData[mapKey];
    if (!existing || typeof existing !== "object" || Array.isArray(existing)) {
      existing = {};
      upgradesData[mapKey] = existing;
    }
    for (const upgradeKey of Object.keys(upgrades || {})) {
      if (!Object.prototype.hasOwnProperty.call(existing, upgradeKey)) {
        existing[upgradeKey] = 0;
        added += 1;
      }
    }
  }
  return added;
}

function ensureDiscoveredTrucksDefaults(dtData) {
  let added = 0;
  let out = dtData;
  if (!out || typeof out !== "object" || Array.isArray(out)) {
    out = {};
  }
  for (const [mapKey, values] of Object.entries(SAFE_DEFAULTS.discoveredTrucksDefaults)) {
    let entry = out[mapKey];
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
      out[mapKey] = {
        current: Number.parseInt(String(values && values.current != null ? values.current : 0), 10) || 0,
        all: Number.parseInt(String(values && values.all != null ? values.all : 0), 10) || 0,
      };
      added += 1;
      continue;
    }
    if (!Object.prototype.hasOwnProperty.call(entry, "current")) {
      entry.current = Number.parseInt(String(values && values.current != null ? values.current : 0), 10) || 0;
      added += 1;
    }
    if (!Object.prototype.hasOwnProperty.call(entry, "all")) {
      entry.all = Number.parseInt(String(values && values.all != null ? values.all : 0), 10) || 0;
      added += 1;
    }
  }
  return { added, data: out };
}

function unlockWatchtowers(content, selectedRegions) {
  const m = /"watchPointsData"\s*:\s*\{/i.exec(content);
  if (!m) {
    throw new Error('"watchPointsData" not found.');
  }
  const block = extractBraceBlock(content, m.index);
  const wpData = JSON.parse(block.block);
  const addedDefaults = ensureWatchpointsDefaults(wpData);
  if (!wpData.data || typeof wpData.data !== "object" || Array.isArray(wpData.data)) {
    wpData.data = {};
  }

  let updated = 0;
  for (const [mapKey, towers] of Object.entries(wpData.data)) {
    if (!towers || typeof towers !== "object" || Array.isArray(towers)) {
      continue;
    }
    if (!selectedRegions.some((code) => mapKey.toLowerCase().includes(`level_${code.toLowerCase()}`))) {
      continue;
    }
    for (const towerKey of Object.keys(towers)) {
      if (towers[towerKey] === false) {
        towers[towerKey] = true;
        updated += 1;
      }
    }
  }

  const newBlock = JSON.stringify(wpData);
  const next = content.slice(0, block.start) + newBlock + content.slice(block.end);
  let message = `Unlocked ${updated} watchtower entries.`;
  if (addedDefaults > 0) {
    message += ` Added ${addedDefaults} missing entries.`;
  }
  return { content: next, message };
}

function unlockUpgrades(content, selectedRegions) {
  const m = /"upgradesGiverData"\s*:\s*\{/i.exec(content);
  if (!m) {
    throw new Error('"upgradesGiverData" not found.');
  }
  const block = extractBraceBlock(content, m.index);
  const data = JSON.parse(block.block);
  const addedDefaults = ensureUpgradesDefaults(data);
  let updated = 0;
  for (const [mapKey, upgrades] of Object.entries(data)) {
    if (!upgrades || typeof upgrades !== "object" || Array.isArray(upgrades)) {
      continue;
    }
    if (!selectedRegions.some((code) => mapKey.toLowerCase().includes(`level_${code.toLowerCase()}`))) {
      continue;
    }
    for (const key of Object.keys(upgrades)) {
      if (upgrades[key] === 0 || upgrades[key] === 1) {
        upgrades[key] = 2;
        updated += 1;
      }
    }
  }

  const newBlock = JSON.stringify(data);
  const next = content.slice(0, block.start) + newBlock + content.slice(block.end);
  let message = `Updated ${updated} upgrades.`;
  if (addedDefaults > 0) {
    message += ` Added ${addedDefaults} missing entries.`;
  }
  return { content: next, message };
}

function unlockDiscoveries(content, selectedRegions) {
  const m = /"persistentProfileData"\s*:\s*\{/i.exec(content);
  if (!m) {
    throw new Error('"persistentProfileData" not found.');
  }
  const ppBlock = extractBraceBlock(content, m.index);
  const pp = JSON.parse(ppBlock.block);
  let dt = pp.discoveredTrucks;
  const ensured = ensureDiscoveredTrucksDefaults(dt);
  dt = ensured.data;
  let updated = 0;
  for (const [mapKey, entry] of Object.entries(dt)) {
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
      continue;
    }
    if (!selectedRegions.some((code) => mapKey.toLowerCase().includes(code.toLowerCase()))) {
      continue;
    }
    setCurrentToAll(entry);
    updated += 1;
  }
  pp.discoveredTrucks = dt;

  const newBlock = JSON.stringify(pp);
  const next = content.slice(0, ppBlock.start) + newBlock + content.slice(ppBlock.end);
  let message = `Updated ${updated} discovery entries.`;
  if (ensured.added > 0) {
    message += ` Added ${ensured.added} missing entries.`;
  }
  return { content: next, message };
}

function unlockLevels(content, selectedRegions) {
  const m = /"persistentProfileData"\s*:\s*\{/i.exec(content);
  if (!m) {
    throw new Error('"persistentProfileData" not found.');
  }
  const ppBlock = extractBraceBlock(content, m.index);
  const pp = JSON.parse(ppBlock.block);

  let known = Array.isArray(pp.knownRegions) ? [...pp.knownRegions] : [];
  let addedKnown = 0;
  for (const code of selectedRegions) {
    const key = code.toLowerCase();
    if (!known.includes(key)) {
      known.push(key);
      addedKnown += 1;
    }
  }
  pp.knownRegions = known;

  content = content.slice(0, ppBlock.start) + JSON.stringify(pp) + content.slice(ppBlock.end);

  const visitedMatch = /"visitedLevels"\s*:\s*\[/i.exec(content);
  let visited = [];
  let visitedBlock = null;
  if (visitedMatch) {
    visitedBlock = extractBracketBlock(content, visitedMatch.index);
    visited = JSON.parse(visitedBlock.block);
    if (!Array.isArray(visited)) {
      visited = [];
    }
  }

  let addedVisited = 0;
  for (const code of selectedRegions) {
    const levels = REGION_LEVELS[code] || [];
    for (const levelId of levels) {
      if (!visited.includes(levelId)) {
        visited.push(levelId);
        addedVisited += 1;
      }
    }
  }

  if (visitedBlock) {
    content = content.slice(0, visitedBlock.start) + JSON.stringify(visited) + content.slice(visitedBlock.end);
  } else {
    content = insertKeyAtRoot(content, "visitedLevels", JSON.stringify(visited));
  }

  return {
    content,
    message: `Known regions added: ${addedKnown}. Visited levels added: ${addedVisited}.`,
  };
}

function unlockGarages(content, selectedRegions, upgradeAll) {
  const m = /"SslValue"\s*:\s*\{/i.exec(content);
  if (!m) {
    throw new Error('"SslValue" block not found.');
  }
  const sslBlock = extractBraceBlock(content, m.index);
  const ssl = JSON.parse(sslBlock.block);

  let lgData = ssl.levelGarageStatuses;
  if (!lgData || typeof lgData !== "object" || Array.isArray(lgData)) {
    lgData = {};
  }
  let addedDefaults = 0;
  for (const [levelId, status] of Object.entries(LEVEL_GARAGE_STATUSES_DEFAULTS)) {
    if (!(levelId in lgData)) {
      lgData[levelId] = status;
      addedDefaults += 1;
    }
  }

  const selectedLevels = [];
  for (const code of selectedRegions) {
    const levels = REGION_LEVELS[code] || [];
    for (const levelId of levels) {
      selectedLevels.push(levelId);
    }
  }

  let updated = 0;
  for (const levelId of selectedLevels) {
    if (lgData[levelId] === 1) {
      lgData[levelId] = 2;
      updated += 1;
    }
  }
  ssl.levelGarageStatuses = lgData;

  let garagesData = ssl.garagesData;
  if (!garagesData || typeof garagesData !== "object" || Array.isArray(garagesData)) {
    garagesData = {};
  }
  let addedGaragesData = 0;
  for (const levelId of selectedLevels) {
    if (lgData[levelId] === 2 && !garagesData[levelId]) {
      garagesData[levelId] = buildGarageDataEntry();
      addedGaragesData += 1;
    }
  }
  ssl.garagesData = garagesData;

  let upgradedEntries = 0;
  let addedUpgradable = 0;
  if (upgradeAll) {
    let ug = ssl.upgradableGarages;
    if (!ug || typeof ug !== "object" || Array.isArray(ug)) {
      ug = {};
    }
    for (const levelId of selectedLevels) {
      if (lgData[levelId] !== 2) {
        continue;
      }
      let foundKey = null;
      for (const [key, value] of Object.entries(ug)) {
        if (typeof key === "string" && key.toLowerCase().includes(levelId.toLowerCase())) {
          foundKey = key;
          break;
        }
        if (value && typeof value === "object" && typeof value.zoneGlobalId === "string" && value.zoneGlobalId.toLowerCase().includes(levelId.toLowerCase())) {
          foundKey = key;
          break;
        }
      }
      const key = foundKey || makeUpgradableGarageKey(levelId);
      let entry = ug[key];
      if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
        entry = {
          zoneGlobalId: key,
          featureStates: [false, false, false, false],
          isUpgradable: true,
        };
        ug[key] = entry;
        if (!foundKey) {
          addedUpgradable += 1;
        }
      }
      if (!entry.zoneGlobalId) {
        entry.zoneGlobalId = key;
      }
      normalizeFeatureStates(entry);
      entry.featureStates = entry.featureStates.map(() => true);
      entry.isUpgradable = true;
      ug[key] = entry;
      upgradedEntries += 1;
    }
    ssl.upgradableGarages = ug;
  }

  const next = content.slice(0, sslBlock.start) + JSON.stringify(ssl) + content.slice(sslBlock.end);
  let message = `Unlocked ${updated} garages.`;
  if (addedDefaults > 0) {
    message += ` Added ${addedDefaults} missing garage status entries.`;
  }
  if (addedGaragesData > 0) {
    message += ` Added ${addedGaragesData} garage data entries.`;
  }
  if (upgradeAll) {
    message += ` Upgraded ${upgradedEntries} garages.`;
    if (addedUpgradable > 0) {
      message += ` Added ${addedUpgradable} upgradable garage entries.`;
    }
  }
  return { content: next, message };
}

function buildGarageDataEntry() {
  return {
    slotsDatas: {
      garage_interior_slot_1: { garageSlotZoneId: "garage_interior_slot_1", truckDesc: null },
      garage_interior_slot_2: { garageSlotZoneId: "garage_interior_slot_2", truckDesc: null },
      garage_interior_slot_3: { garageSlotZoneId: "garage_interior_slot_3", truckDesc: null },
      garage_interior_slot_4: { garageSlotZoneId: "garage_interior_slot_4", truckDesc: null },
      garage_interior_slot_5: { garageSlotZoneId: "garage_interior_slot_5", truckDesc: null },
      garage_interior_slot_6: { garageSlotZoneId: "garage_interior_slot_6", truckDesc: null },
    },
    selectedSlot: "garage_interior_slot_1",
  };
}

function normalizeFeatureStates(entry) {
  let states = Array.isArray(entry.featureStates) ? [...entry.featureStates] : [];
  while (states.length < 4) {
    states.push(false);
  }
  states = states.map((v) => Boolean(v));
  entry.featureStates = states;
  if (!("isUpgradable" in entry)) {
    entry.isUpgradable = true;
  }
}

function makeUpgradableGarageKey(levelId) {
  const suffix = String(levelId).replace(/^level_/i, "").toUpperCase();
  return `${levelId} || ${suffix}_GARAGE_ENTRANCE`;
}

function setCurrentToAll(entry) {
  let allVal = entry.all;
  if (typeof allVal === "boolean") {
    allVal = Number(allVal);
  } else if (typeof allVal !== "number") {
    const parsed = Number.parseInt(String(allVal), 10);
    allVal = Number.isFinite(parsed) ? parsed : 0;
  }
  entry.all = allVal;
  entry.current = allVal;
}

function parseCommonContainer(text) {
  const m = /"CommonSslSave"\s*:\s*\{/i.exec(text);
  if (m) {
    const block = extractBraceBlock(text, m.index);
    const parsed = JSON.parse(block.block);
    return { parsed, embedded: true, start: block.start, end: block.end };
  }
  const parsed = JSON.parse(text);
  return { parsed, embedded: false, start: null, end: null };
}

function writeCommonContainer(originalText, container) {
  const serialized = JSON.stringify(container.parsed);
  if (container.embedded) {
    return originalText.slice(0, container.start) + serialized + originalText.slice(container.end);
  }
  return serialized;
}

function getCommonSslValue(parsed) {
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error("Invalid CommonSslSave JSON.");
  }
  if (parsed.SslValue && typeof parsed.SslValue === "object" && !Array.isArray(parsed.SslValue)) {
    return parsed.SslValue;
  }
  return parsed;
}

function readFinishedTrials(text) {
  const m = /"finishedTrials"\s*:\s*\[/i.exec(text);
  if (!m) {
    return [];
  }
  try {
    const block = extractBracketBlock(text, m.index);
    const arr = JSON.parse(block.block);
    return Array.isArray(arr) ? arr : [];
  } catch (_err) {
    return [];
  }
}

function writeFinishedTrials(text, finishedList) {
  const arrText = JSON.stringify(finishedList);
  const m = /"finishedTrials"\s*:\s*\[/i.exec(text);
  if (m) {
    try {
      const block = extractBracketBlock(text, m.index);
      return `${text.slice(0, block.start)}${arrText}${text.slice(block.end)}`;
    } catch (_error) {
      // fall back to insert if malformed
    }
  }
  const firstBrace = text.indexOf("{");
  if (firstBrace === -1) {
    return `${text}\n"finishedTrials":${arrText}\n`;
  }
  return `${text.slice(0, firstBrace + 1)}\n"finishedTrials":${arrText},${text.slice(firstBrace + 1)}`;
}

function replaceOrInsertNumeric(content, key, value) {
  const replaced = replaceNumericKeyAll(content, key, value);
  if (replaced.count > 0) {
    return replaced.content;
  }
  return insertKeyAtRoot(content, key, JSON.stringify(value));
}

function replaceOrInsertNumericWithCount(content, key, value) {
  const replaced = replaceNumericKeyAll(content, key, value);
  if (replaced.count > 0) {
    return replaced;
  }
  return {
    content: insertKeyAtRoot(content, key, JSON.stringify(value)),
    count: 1,
  };
}

function replaceOrInsertJsonLiteral(content, key, jsonValueText) {
  const replaced = replaceJsonKeyAll(content, key, jsonValueText);
  if (replaced.count > 0) {
    return replaced.content;
  }
  return insertKeyAtRoot(content, key, jsonValueText);
}

function updateAllContestTimesBlocks(content, newEntries) {
  const matches = [...content.matchAll(/"contestTimes"\s*:\s*\{/gi)];
  let out = content;
  for (let i = matches.length - 1; i >= 0; i -= 1) {
    const hit = matches[i];
    let block = null;
    try {
      block = extractBraceBlock(out, hit.index);
    } catch (_error) {
      continue;
    }
    let parsed = null;
    try {
      parsed = JSON.parse(block.block);
    } catch (_error) {
      continue;
    }
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      continue;
    }
    let changed = false;
    for (const [key, value] of Object.entries(newEntries)) {
      if (!(key in parsed)) {
        parsed[key] = value;
        changed = true;
      }
    }
    if (!changed) {
      continue;
    }
    const nextBlock = JSON.stringify(parsed);
    out = out.slice(0, block.start) + nextBlock + out.slice(block.end);
  }
  return out;
}

function replaceOrInsertBoolean(content, key, value) {
  const replaced = replaceBooleanKeyAll(content, key, value);
  if (replaced.count > 0) {
    return replaced.content;
  }
  return insertKeyAtRoot(content, key, JSON.stringify(Boolean(value)));
}

function replaceNumericKeyAll(content, key, value) {
  let count = 0;
  const re = new RegExp(
    `("${escapeRegExp(key)}"\\s*:\\s*)(?:"(?:[^"\\\\]|\\\\.)*"|-?\\d+(?:\\.\\d+)?(?:e[-+]?\\d+)?)`,
    "gi",
  );
  const out = content.replace(re, (_, p1) => {
    count += 1;
    return `${p1}${value}`;
  });
  return { content: out, count };
}

function replaceJsonKeyAll(content, key, jsonValueText) {
  let count = 0;
  const valuePattern = `(?:\"(?:[^\"\\\\]|\\\\.)*\"|\\[[^\\]]*\\]|\\{[^\\}]*\\}|[^,}\\n\\r]+)`;
  const re = new RegExp(`("${escapeRegExp(key)}"\\s*:\\s*)${valuePattern}`, "gi");
  const out = content.replace(re, (_, p1) => {
    count += 1;
    return `${p1}${jsonValueText}`;
  });
  return { content: out, count };
}

function replaceBooleanKeyAll(content, key, value) {
  let count = 0;
  const re = new RegExp(`("${escapeRegExp(key)}"\\s*:\\s*)(true|false)`, "gi");
  const out = content.replace(re, (_, p1) => {
    count += 1;
    return `${p1}${value ? "true" : "false"}`;
  });
  return { content: out, count };
}

function insertKeyAtRoot(content, key, jsonValueText) {
  const idx = content.indexOf("{");
  if (idx === -1) {
    return content;
  }
  return `${content.slice(0, idx + 1)}"${key}":${jsonValueText},${content.slice(idx + 1)}`;
}

function parseOptionalInt(value) {
  const trimmed = String(value || "").trim();
  if (!trimmed) {
    return null;
  }
  const parsed = Number.parseInt(trimmed, 10);
  return Number.isFinite(parsed) ? parsed : null;
}

function parseOptionalStrictInt(value) {
  const trimmed = String(value || "").trim();
  if (!trimmed) {
    return { value: null, error: null };
  }
  if (!/^[+-]?\d+$/.test(trimmed)) {
    return { value: null, error: "invalid" };
  }
  const parsed = Number.parseInt(trimmed, 10);
  if (!Number.isSafeInteger(parsed)) {
    return { value: null, error: "invalid" };
  }
  return { value: parsed, error: null };
}

function parseOptionalMoneyInt(value) {
  const trimmed = String(value || "").trim();
  if (!trimmed) {
    return { value: null, error: null, clamped: false };
  }
  if (!/^[+-]?\d+$/.test(trimmed)) {
    return { value: null, error: "invalid", clamped: false };
  }
  let raw = null;
  try {
    raw = BigInt(trimmed);
  } catch (_error) {
    return { value: null, error: "invalid", clamped: false };
  }
  const min = BigInt(MONEY_MIN);
  const max = BigInt(MONEY_MAX);
  let clampedValue = raw;
  if (clampedValue < min) {
    clampedValue = min;
  } else if (clampedValue > max) {
    clampedValue = max;
  }
  return {
    value: Number(clampedValue),
    error: null,
    clamped: clampedValue !== raw,
  };
}

function parseOptionalFloat(value) {
  const trimmed = String(value || "").trim();
  if (!trimmed) {
    return null;
  }
  return parseStrictNumber(trimmed);
}

function parseStrictNumber(value) {
  const trimmed = String(value == null ? "" : value).trim();
  if (!trimmed) {
    return null;
  }
  if (!/^[+-]?(?:\d+\.?\d*|\.\d+)(?:e[+-]?\d+)?$/i.test(trimmed)) {
    return null;
  }
  const parsed = Number(trimmed);
  return Number.isFinite(parsed) ? parsed : null;
}

function parseOptionalStrictFloat(value) {
  const trimmed = String(value || "").trim();
  if (!trimmed) {
    return { value: null, error: null };
  }
  const parsed = parseStrictNumber(trimmed);
  if (parsed == null) {
    return { value: null, error: "invalid" };
  }
  return { value: parsed, error: null };
}

function readMaxIntKey(content, key) {
  const re = new RegExp(`"${escapeRegExp(key)}"\\s*:\\s*(-?\\d+)`, "gi");
  let m = null;
  let max = null;
  while ((m = re.exec(content)) !== null) {
    const num = Number.parseInt(m[1], 10);
    if (!Number.isFinite(num)) {
      continue;
    }
    if (max == null || num > max) {
      max = num;
    }
  }
  return max;
}

function readNumericKey(content, key) {
  const re = new RegExp(`"${escapeRegExp(key)}"\\s*:\\s*(-?\\d+(?:\\.\\d+)?(?:e[-+]?\\d+)?)`, "i");
  const m = re.exec(content);
  if (!m) {
    return null;
  }
  const value = Number.parseFloat(m[1]);
  return Number.isFinite(value) ? value : null;
}

function readBoolKey(content, key) {
  const re = new RegExp(`"${escapeRegExp(key)}"\\s*:\\s*(true|false)`, "i");
  const m = re.exec(content);
  if (!m) {
    return null;
  }
  return m[1].toLowerCase() === "true";
}

function readSimpleValueKey(content, key) {
  const re = new RegExp(`"${escapeRegExp(key)}"\\s*:\\s*(true|false|-?\\d+(?:\\.\\d+)?(?:e[-+]?\\d+)?)`, "i");
  const m = re.exec(content);
  if (!m) {
    return undefined;
  }
  const raw = m[1].toLowerCase();
  if (raw === "true") {
    return true;
  }
  if (raw === "false") {
    return false;
  }
  const num = Number.parseFloat(m[1]);
  return Number.isFinite(num) ? num : undefined;
}

function escapeRegExp(text) {
  return String(text).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}

function extractBraceBlock(text, startIndex) {
  let openBraces = 0;
  let inString = false;
  let escape = false;
  let blockStart = null;
  for (let i = startIndex; i < text.length; i += 1) {
    const ch = text[i];
    if (ch === '"' && !escape) {
      inString = !inString;
    }
    if (!inString) {
      if (ch === "{") {
        if (openBraces === 0) {
          blockStart = i;
        }
        openBraces += 1;
      } else if (ch === "}") {
        openBraces -= 1;
        if (openBraces === 0 && blockStart != null) {
          return {
            block: text.slice(blockStart, i + 1),
            start: blockStart,
            end: i + 1,
          };
        }
      }
    }
    escape = ch === "\\" && !escape;
  }
  throw new Error("Matching closing brace not found.");
}

function extractBracketBlock(text, startIndex) {
  let open = 0;
  let inString = false;
  let escape = false;
  let blockStart = null;
  for (let i = startIndex; i < text.length; i += 1) {
    const ch = text[i];
    if (ch === '"' && !escape) {
      inString = !inString;
    }
    if (!inString) {
      if (ch === "[") {
        if (open === 0) {
          blockStart = i;
        }
        open += 1;
      } else if (ch === "]") {
        open -= 1;
        if (open === 0 && blockStart != null) {
          return {
            block: text.slice(blockStart, i + 1),
            start: blockStart,
            end: i + 1,
          };
        }
      }
    }
    escape = ch === "\\" && !escape;
  }
  throw new Error("Matching closing bracket not found.");
}

function uniqueList(list) {
  return [...new Set(list)];
}

init();
